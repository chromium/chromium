...
TEST_P(SequenceManagerTest, TaskQueueThrottler_FlushReadyDelayedTasks) {
  auto queue = CreateTaskQueue();
  ...
  throttler.SetNextAllowedWakeUp(
      base::sequence_manager::WakeUp{start_time + Seconds(10)});
  ...
  AdvanceMockTickClock(Seconds(2));

  // OnWakeUp should not be called when the throttler doesn't allow the wake up.
  EXPECT_CALL(throttler, OnWakeUp(_)).Times(0);
  EXPECT_CALL(throttler,
              GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + Seconds(1)));
  EXPECT_CALL(throttler,
              GetNextAllowedWakeUp_DesiredWakeUpTime(start_time + Seconds(2)));
  // Move the task into the |delayed_work_queue|.
  sequence_manager()->FlushReadyDelayedTasks();
  Mock::VerifyAndClearExpectations(&throttler);
