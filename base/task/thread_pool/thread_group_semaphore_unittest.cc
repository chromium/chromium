...
class ThreadGroupSemaphoreImplTestBase : public ThreadGroup::Delegate {
 public:
  ThreadGroupSemaphoreImplTestBase(const ThreadGroupSemaphoreImplTestBase&) =
      delete;
  ThreadGroupSemaphoreImplTestBase& operator=(
      const ThreadGroupSemaphoreImplTestBase&) = delete;

 protected:
  ThreadGroupSemaphoreImplTestBase()
  ...
  void CreateThreadGroup(ThreadType thread_type = ThreadType::kDefault) {
    ...
    thread_group_ = std::make_unique<ThreadGroupSemaphore>(
  ...
  std::unique_ptr<ThreadGroupSemaphore> thread_group_;
...
class ThreadGroupSemaphoreImplTest : public ThreadGroupSemaphoreImplTestBase,
                                     public testing::Test {
 public:
  ThreadGroupSemaphoreImplTest(const ThreadGroupSemaphoreImplTest&) = delete;
  ThreadGroupSemaphoreImplTest& operator=(const ThreadGroupSemaphoreImplTest&) =
      delete;

 protected:
  ThreadGroupSemaphoreImplTest() = default;

  void SetUp() override { CreateAndStartThreadGroup(); }

  void TearDown() override {
    ThreadGroupSemaphoreImplTestBase::CommonTearDown();
  }
};

class ThreadGroupSemaphoreImplTestParam
    : public ThreadGroupSemaphoreImplTestBase,
      public testing::TestWithParam<TaskSourceExecutionMode> {
 public:
  ThreadGroupSemaphoreImplTestParam(const ThreadGroupSemaphoreImplTestParam&) =
      delete;
  ThreadGroupSemaphoreImplTestParam& operator=(
      const ThreadGroupSemaphoreImplTestParam&) = delete;

 protected:
  ThreadGroupSemaphoreImplTestParam() = default;
...
class ThreadPostingTasksWaitIdle : public SimpleThread {
 public:
  ...
  ThreadPostingTasksWaitIdle(
      ThreadGroupSemaphore* thread_group,
      test::MockPooledTaskRunnerDelegate* mock_pooled_task_runner_delegate_,
 ...
 private:
  ...
  const raw_ptr<ThreadGroupSemaphore> thread_group_;
...
TEST_P(ThreadGroupSemaphoreImplTestParam, PostTasksWaitAllWorkersIdle) {
...
TEST_P(ThreadGroupSemaphoreImplTestParam, PostTasksWithOneAvailableWorker) {
...
TEST_P(ThreadGroupSemaphoreImplTestParam, Saturate) {
...
// tasks.
TEST_F(ThreadGroupSemaphoreImplTest, ShouldYieldFloodedUserVisible) {
...
INSTANTIATE_TEST_SUITE_P(Parallel,
                         ThreadGroupSemaphoreImplTestParam,
...
class ThreadGroupSemaphoreImplStartInBodyTest
    : public ThreadGroupSemaphoreImplTest {
 public:
  void SetUp() override {
    CreateThreadGroup();
...
// Verify that 2 tasks posted before Start() to a ThreadGroupSemaphore with
// more than 2 workers run on different workers when Start() is called.
TEST_F(ThreadGroupSemaphoreImplStartInBodyTest, PostTasksBeforeStart) {
...
TEST_F(ThreadGroupSemaphoreImplStartInBodyTest, PostManyTasks) {
...
class BackgroundThreadGroupSemaphoreTest : public ThreadGroupSemaphoreImplTest {
 public:
  void CreateAndStartThreadGroup(
      TimeDelta suggested_reclaim_time = TimeDelta::Max(),
      size_t max_tasks = kMaxTasks,
...
// shutdown state.
TEST_F(BackgroundThreadGroupSemaphoreTest, UpdatePriorityBlockingStarted) {
  if (!CanUseBackgroundThreadTypeForWorkerThread()) {
    return;
  }
...
class ThreadGroupSemaphoreStandbyPolicyTest
    : public ThreadGroupSemaphoreImplTestBase,
      public testing::Test {
 public:
  ThreadGroupSemaphoreStandbyPolicyTest() = default;
  ThreadGroupSemaphoreStandbyPolicyTest(
      const ThreadGroupSemaphoreStandbyPolicyTest&) = delete;
  ThreadGroupSemaphoreStandbyPolicyTest& operator=(
      const ThreadGroupSemaphoreStandbyPolicyTest&) = delete;
...
TEST_F(ThreadGroupSemaphoreStandbyPolicyTest, InitOne) {
...
class ThreadGroupSemaphoreBlockingTest
    : public ThreadGroupSemaphoreImplTestBase,
      public testing::TestWithParam<NestedBlockingType> {
 public:
  ThreadGroupSemaphoreBlockingTest() = default;
  ThreadGroupSemaphoreBlockingTest(const ThreadGroupSemaphoreBlockingTest&) =
      delete;
  ThreadGroupSemaphoreBlockingTest& operator=(
      const ThreadGroupSemaphoreBlockingTest&) = delete;
...
TEST_P(ThreadGroupSemaphoreBlockingTest, ThreadBlockedUnblocked) {
...
TEST_P(ThreadGroupSemaphoreBlockingTest, ThreadBlockedUnblockedBestEffort) {
  CreateAndStartThreadGroup();
...
TEST_P(ThreadGroupSemaphoreBlockingTest, TooManyBestEffortTasks) {
...
TEST_P(ThreadGroupSemaphoreBlockingTest, PostBeforeBlocking) {
  CreateAndStartThreadGroup();
...
TEST_P(ThreadGroupSemaphoreBlockingTest, WorkersIdleWhenOverCapacity) {
  CreateAndStartThreadGroup();
...
TEST_P(ThreadGroupSemaphoreBlockingTest, ThreadBlockedUnblockedShouldYield) {
  CreateAndStartThreadGroup();
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(NestedBlockingType(BlockingType::MAY_BLOCK,
                      ...
                      NestedBlockingType(BlockingType::WILL_BLOCK,
                                         OptionalBlockingType::MAY_BLOCK,
                                         BlockingType::WILL_BLOCK)),
    ThreadGroupSemaphoreBlockingTest::ParamInfoToString);
...
TEST_F(ThreadGroupSemaphoreBlockingTest, ThreadBlockUnblockPremature) {
...
TEST_F(ThreadGroupSemaphoreBlockingTest,
...
// after an increase.
TEST_F(ThreadGroupSemaphoreBlockingTest, ThreadBusyShutdown) {
  CreateAndStartThreadGroup();
  ASSERT_EQ(thread_group_->GetMaxTasksForTesting(), kMaxTasks);
...
class ThreadGroupSemaphoreOverCapacityTest
    : public ThreadGroupSemaphoreImplTestBase,
      public testing::TestWithParam<ReclaimType> {
 public:
  ThreadGroupSemaphoreOverCapacityTest() = default;
  ThreadGroupSemaphoreOverCapacityTest(
      const ThreadGroupSemaphoreOverCapacityTest&) = delete;
  ThreadGroupSemaphoreOverCapacityTest& operator=(
      const ThreadGroupSemaphoreOverCapacityTest&) = delete;
...
TEST_P(ThreadGroupSemaphoreOverCapacityTest, VerifyCleanup) {
  ...
  // This test in TGS does not post tasks intermittently because semaphores on
  // some platforms implement anti-starvation logic, and so a pool which is
  // given tasks from time to time will not always run the task on the same
  // worker, leading to many of the workers being "used" despite only one worker
  // being used at a time.

  if (GetParam() == ReclaimType::DELAYED_RECLAIM) {
    thread_group_->WaitForWorkersCleanedUpForTesting(kLocalMaxTasks - 1);
    EXPECT_GE(kLocalMaxTasks + 1, thread_group_->NumberOfWorkersForTesting());
    threads_continue.Signal();
  } else {
...
TEST_F(ThreadGroupSemaphoreBlockingTest, MaximumWorkersTest) {
  CreateAndStartThreadGroup();
...
// is honored.
TEST_F(ThreadGroupSemaphoreImplStartInBodyTest, MaxBestEffortTasks) {
...
TEST_F(ThreadGroupSemaphoreImplStartInBodyTest,
...
class ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest
    : public ThreadGroupSemaphoreImplTestBase,
      public testing::TestWithParam<BlockingType> {
 public:
  static constexpr int kMaxBestEffortTasks = kMaxTasks / 2;

  ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest() = default;
  ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest(
      const ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest&) =
      delete;
  ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest& operator=(
      const ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest&) =
      delete;
...
TEST_P(ThreadGroupSemaphoreBlockingCallAndMaxBestEffortTasksTest,
...
TEST_F(ThreadGroupSemaphoreImplStartInBodyTest, RacyCleanup) {
  ...
  // ThreadGroupSemaphore destruction : https://crbug.com/810464.
  mock_pooled_task_runner_delegate_.SetThreadGroup(nullptr);
  thread_group_.reset();
}
