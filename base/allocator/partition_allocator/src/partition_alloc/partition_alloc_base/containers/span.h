...
// `internal::StrictNumeric<size_t>` param; use `first(1u)` instead),
// but `first<1>()` is not.
//
...
//   `internal::StrictNumeric<size_type>` instead of `size_type` to
...
//   `internal::StrictNumeric<size_type>`.
...
inline constexpr bool
    std::ranges::enable_view<partition_alloc::internal::base::
                                 span<ElementType, Extent, InternalPtrType>> =
        true;
template <typename ElementType, size_t Extent, typename InternalPtrType>
inline constexpr bool std::ranges::enable_borrowed_range<
    partition_alloc::internal::base::
        span<ElementType, Extent, InternalPtrType>> = true;
...
constexpr auto as_byte_span(
  ...
  return PA_UNSAFE_BUFFERS(span<ByteType, kByteExtent>(
...
class PA_GSL_POINTER span {
 public:
  ...
  PA_UNSAFE_BUFFER_USAGE constexpr span(
      It first,
      internal::StrictNumeric<size_type> count)
      : data_(to_address(first)) {
    PA_BASE_CHECK(size_type{count} == extent);
  ...
  PA_UNSAFE_BUFFER_USAGE constexpr span(It first, End last)
      ...
      : PA_UNSAFE_BUFFERS(span(first, static_cast<size_type>(last - first))) {
    // Verify `last - first` did not underflow.
    PA_BASE_CHECK(first <= last);
  }

  // Array of size `extent`.
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr span(element_type (&arr PA_LIFETIME_BOUND)[extent]) noexcept
      // SAFETY: The type signature guarantees `arr` contains `extent` elements.
      : PA_UNSAFE_BUFFERS(span(arr, extent)) {}
  ...
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr explicit(N != extent) span(R&& range PA_LIFETIME_BOUND)
  ...
  constexpr explicit(OtherExtent == dynamic_extent)
      ...
      // at `data()`.
      : PA_UNSAFE_BUFFERS(span(other.data(), other.size())) {}
  ...
  constexpr void copy_from(span<const element_type, extent> other)
    requires(!std::is_const_v<element_type>)
  {
    // TODO(C++23): Switch to the C++23 feature once PA supports it and
    // above.
    if (std::is_constant_evaluated()) {
      ...
      if constexpr (extent > 0) {
        ...
        for (size_t i = 0; i < extent; ++i) {
          // SAFETY: `buffers` is allocated with `extent` elements, and the loop
          // body only executes if `i < extent`.
          (*this)[i] = PA_UNSAFE_BUFFERS(buffer[i]).value;
          PA_UNSAFE_BUFFERS(buffer[i]).value.~element_type();
        }
        delete[] buffer;
      }
    } else {
    ...
    requires(!std::is_const_v<element_type>)
  {
    ...
    PA_BASE_DCHECK(reinterpret_cast<uintptr_t>(to_address(end())) <=
                       reinterpret_cast<uintptr_t>(to_address(other.begin())) ||
    ...
    requires(Count <= extent)
  {
    ...
    return PA_UNSAFE_BUFFERS(span<element_type, Count>(data(), Count));
  }
  constexpr auto first(internal::StrictNumeric<size_type> count) const {
    PA_BASE_CHECK(size_type{count} <= extent);
    // SAFETY: `data()` points to at least `extent` elements, so the new data
    // scope is a strict subset of the old.
    return PA_UNSAFE_BUFFERS(span<element_type>(data(), count));
  }
    ...
    requires(Count <= extent)
  {
    // SAFETY: `data()` points to at least `extent` elements, so the new data
    // scope is a strict subset of the old.
    return PA_UNSAFE_BUFFERS(
        span<element_type, Count>(data() + (extent - Count), Count));
  }
  constexpr auto last(internal::StrictNumeric<size_type> count) const {
    PA_BASE_CHECK(size_type{count} <= extent);
    // SAFETY: `data()` points to at least `extent` elements, so the new data
    // scope is a strict subset of the old.
    return PA_UNSAFE_BUFFERS(
             ...
             (Count == dynamic_extent || Count <= extent - Offset))
  {
    if constexpr (Count == dynamic_extent) {
      ...
      return PA_UNSAFE_BUFFERS(
          span<element_type, Count>(data() + Offset, Count));
    }
  }
  constexpr auto subspan(internal::StrictNumeric<size_type> offset) const {
    PA_BASE_CHECK(size_type{offset} <= extent);
  ...
  constexpr auto subspan(internal::StrictNumeric<size_type> offset,
                         internal::StrictNumeric<size_type> count) const {
    // base does not allow dynamic_extent in two-arg subspan().
    PA_BASE_DCHECK(size_type{count} != dynamic_extent);
    // Deliberately combine tests to minimize code size.
    PA_BASE_CHECK(size_type{offset} <= size() &&
  ...
  constexpr auto split_at(internal::StrictNumeric<size_type> offset) const {
  ...
  //
  // Intentionally does not take `internal::StrictNumeric<size_t>`,
  // unlike all other APIs. There are far too many false positives on integer
  // literals (e.g. `s[0]`), and while `PA_ENABLE_IF_ATTR` can be used to work
  // around those for Clang, that would leave the gcc build broken. The
  // consequence of not upgrading this is that some errors will only be detected
  ...
  // When `idx` is outside the span, the underlying call will `CHECK()`.
  constexpr reference at(internal::StrictNumeric<size_type> idx) const
    requires(extent > 0)
  {
    return *get_at(idx);
  }
  ...
  constexpr pointer get_at(internal::StrictNumeric<size_type> idx) const
    requires(extent > 0)
  {
    PA_BASE_CHECK(size_type{idx} < extent);
    // SAFETY: `data()` points to at least `extent` elements, so `idx` must be
    // the index of a valid element.
    return PA_UNSAFE_BUFFERS(data() + size_type{idx});
  }
  ...
  constexpr iterator begin() const noexcept {
    ...
    return PA_UNSAFE_BUFFERS(iterator(
  ...
  constexpr iterator end() const noexcept {
    ...
    // constraints, so the iterator's requirement that begin <= current <= end
    // is guaranteed to be true.
    return PA_UNSAFE_BUFFERS(iterator(typename iterator::AssumeValid(
...
class PA_GSL_POINTER span<ElementType, dynamic_extent, InternalPtrType> {
 public:
  using element_type = ElementType;
  using value_type = std::remove_cv_t<element_type>;
  using size_type = size_t;
  ...
  template <size_t N>
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr span(element_type (&arr PA_LIFETIME_BOUND)[N]) noexcept
      // SAFETY: The type signature guarantees `arr` contains `N` elements.
      : PA_UNSAFE_BUFFERS(span(arr, N)) {}

  // Range.
  template <typename R>
    requires(internal::CompatibleRange<element_type, R>)
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr span(R&& range PA_LIFETIME_BOUND)
      // SAFETY: `std::ranges::size()` returns the number of elements
      // `std::ranges::data()` will point to, so accessing those elements will
      // be safe.
      : PA_UNSAFE_BUFFERS(
  ...
  // Initializer list.
  constexpr span(std::initializer_list<value_type> il PA_LIFETIME_BOUND)
    requires(std::is_const_v<element_type>)
      // SAFETY: `size()` is exactly the number of elements in the initializer
      // list, so accessing that many will be safe.
      : PA_UNSAFE_BUFFERS(span(il.begin(), il.size())) {}
  ...
  constexpr void copy_from(span<const element_type> other)
    requires(!std::is_const_v<element_type>)
  {
    PA_BASE_CHECK(size() == other.size());
    // TODO(c++23): Switch to the C++23 feature once PA supports it.
    if (std::is_constant_evaluated()) {
      // Comparing pointers to different objects at compile time yields
      // unspecified behavior, which would halt compilation. Instead,
      // unconditionally use a separate buffer in the constexpr context. This
  ...
  template <size_t Count>
  constexpr auto first() const {
    PA_BASE_CHECK(Count <= size());
    // SAFETY: `data()` points to at least `size()` elements, so the new data
    // scope is a strict subset of the old.
    return PA_UNSAFE_BUFFERS(span<element_type, Count>(data(), Count));
  }
  constexpr auto first(internal::StrictNumeric<size_t> count) const {
    PA_BASE_CHECK(size_type{count} <= size());
    // SAFETY: `data()` points to at least `size()` elements, so the new data
    // scope is a strict subset of the old.
    return PA_UNSAFE_BUFFERS(span<element_type>(data(), count));
  }
  ...
  constexpr auto subspan() const {
    ...
    PA_BASE_CHECK(Count <= remaining);
    // SAFETY: `data()` points to at least `size()` elements, so `Offset`
    // specifies a valid element index or the past-the-end index, and `Count` is
    // no larger than the number of remaining valid elements.
    return PA_UNSAFE_BUFFERS(span<element_type, Count>(data() + Offset, Count));
  }
  constexpr auto subspan(internal::StrictNumeric<size_type> offset) const {
    PA_BASE_CHECK(size_type{offset} <= size());
  ...
  constexpr auto take_first(internal::StrictNumeric<size_type> offset) {
    const auto [first, rest] = split_at(offset);
    *this = rest;
    return first;
  }
  ...
  // When `idx` is outside the span, the underlying call will `CHECK()`.
  //
  // Intentionally does not take `internal::StrictNumeric<size_type>`;
  // see comments on fixed-extent version for rationale.
  ...
  // (Not in `std::`; necessary when underlying memory is not yet initialized.)
  constexpr pointer get_at(internal::StrictNumeric<size_type> idx) const {
    PA_BASE_CHECK(size_type{idx} < size());
...
template <typename T>
constexpr auto span_from_ref(const T& t PA_LIFETIME_BOUND) {
  // SAFETY: It's safe to read the memory at `t`'s address as long as the
  // provided reference is valid.
  return PA_UNSAFE_BUFFERS(span<const T, 1>(std::addressof(t), 1u));
}
template <typename T>
constexpr auto span_from_ref(T& t PA_LIFETIME_BOUND) {
  // SAFETY: It's safe to read the memory at `t`'s address as long as the
  // provided reference is valid.
  return PA_UNSAFE_BUFFERS(span<T, 1>(std::addressof(t), 1u));
}
...
constexpr auto byte_span_from_ref(const T& t PA_LIFETIME_BOUND) {
...
constexpr auto byte_span_from_ref(allow_nonunique_obj_t,
                                  const T& t PA_LIFETIME_BOUND) {
  return as_bytes(allow_nonunique_obj, span_from_ref(t));
}
template <typename T>
  requires(internal::CanSafelyConvertToByteSpan<T>)
constexpr auto byte_span_from_ref(T& t PA_LIFETIME_BOUND) {
  return as_writable_bytes(span_from_ref(t));
}
template <typename T>
  requires(internal::CanSafelyConvertNonUniqueToByteSpan<T>)
constexpr auto byte_span_from_ref(allow_nonunique_obj_t,
                                  T& t PA_LIFETIME_BOUND) {
...
constexpr auto span_from_cstring(const CharT (&str PA_LIFETIME_BOUND)[Extent])
...
constexpr auto byte_span_from_cstring(
  ...
  // carry through the function call, so the `PA_ENABLE_IF_ATTR` will not be
  // satisfied.
...
constexpr auto byte_span_with_nul_from_cstring(
  ...
  // do not carry through the function call, so the `PA_ENABLE_IF_ATTR` will not
  // be satisfied.
...
constexpr auto as_byte_span(const T& t PA_LIFETIME_BOUND) {
...
template <int&... ExplicitArgumentBarrier, typename ElementType, size_t Extent>
  requires(internal::CanSafelyConvertToByteSpan<ElementType>)
constexpr auto as_byte_span(
    const ElementType (&arr PA_LIFETIME_BOUND)[Extent]) {
...
constexpr auto as_writable_byte_span(
    ElementType (&arr PA_LIFETIME_BOUND)[Extent]) {
...
[[nodiscard]] constexpr auto reinterpret_span(
  ...
  PA_BASE_CHECK(reinterpret_cast<uintptr_t>(s.data()) % alignof(ElementType) ==
                0u);

  // Runtime check for dynamic spans ensures size is a multiple of ElementType.
  if constexpr (Count == dynamic_extent) {
    PA_BASE_CHECK(s.size_bytes() % sizeof(ElementType) == 0u);
  }
  ...
  //
  // The proper solution is C++23's `start_lifetime_as_array`
  ...
  if constexpr (Count == dynamic_extent) {
    ...
    return PA_UNSAFE_BUFFERS(span<ElementType, dynamic_extent>(
        ptr, s.size_bytes() / sizeof(ElementType)));
  } else {
    // SAFETY: We checked for proper alignment, size, strict aliasing rules, and
    // started the lifetime of the array.
    constexpr size_t NewCount = Count / sizeof(ElementType);
    return PA_UNSAFE_BUFFERS(span<ElementType, NewCount>(ptr, NewCount));
  }
}
...
#endif  // PARTITION_ALLOC_PARTITION_ALLOC_BASE_CONTAINERS_SPAN_H_
