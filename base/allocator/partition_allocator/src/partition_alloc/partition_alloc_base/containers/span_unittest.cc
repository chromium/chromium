...
TEST(PASpanTest, Example_UnsafeBuffersPatterns) {
...

  {
    ...
    [[maybe_unused]] uint32_t v1 =
        *reinterpret_cast<const uint32_t*>(&span(array)[0u]);  // Front.
    [[maybe_unused]] uint64_t v2 =
        *reinterpret_cast<const uint64_t*>(&span(array)[6u]);  // Middle.
  }
...

  {
    ...
    [[maybe_unused]] uint32_t v1 =
        *reinterpret_cast<const uint32_t*>(&span(array)[0u]);  // Front.
    [[maybe_unused]] uint64_t v2 =
        *reinterpret_cast<const uint64_t*>(&span(array)[16u]);  // Middle.
  }
