...
TEST(RuleStreamTest, DeleteUrlRuleOrAmend) {
  const struct TestCase {
    ...
    const char* chrome_59_rule;
      ...
      {"a.com$~image", "a.com$~image,~popup,~websocket,~webtransport", "#0"},
      {"a.com$~popup", "a.com$~popup,~websocket,~webtransport", "a.com"},
      {"a.com$~image,~popup", "a.com$~image,~popup,~websocket,~webtransport",
       "#0"},
