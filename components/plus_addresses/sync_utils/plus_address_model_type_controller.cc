...
PreconditionState PlusAddressModelTypeController::GetPreconditionState() const {
...
void PlusAddressModelTypeController::OnStateChanged(syncer::SyncService*) {
...
void PlusAddressModelTypeController::RecreateManagedStatusFinder() {
  managed_status_finder_ = std::make_unique<signin::AccountManagedStatusFinder>(
      identity_manager_, sync_service_->GetAccountInfo(),
      base::BindOnce(&PlusAddressModelTypeController::OnAccountTypeDetermined,
                     base::Unretained(this)));
}

void PlusAddressModelTypeController::OnAccountTypeDetermined() {
  sync_service_->DataTypePreconditionChanged(type());
}
