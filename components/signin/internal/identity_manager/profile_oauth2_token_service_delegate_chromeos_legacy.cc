...
ProfileOAuth2TokenServiceDelegateChromeOSLegacy::CreateAccessTokenFetcher(
...
bool ProfileOAuth2TokenServiceDelegateChromeOSLegacy::RefreshTokenIsAvailable(
...
void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::UpdateAuthError(
    const CoreAccountId& account_id,
    const GoogleServiceAuthError& error) {
  UpdateAuthErrorInternal(account_id, error, /*fire_auth_error_changed=*/true);
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::UpdateAuthErrorInternal(
    const CoreAccountId& account_id,
    const GoogleServiceAuthError& error,
...
ProfileOAuth2TokenServiceDelegateChromeOSLegacy::GetAuthError(
    const CoreAccountId& account_id) const {
  auto it = errors_.find(account_id);
  if (it != errors_.end()) {
...
ProfileOAuth2TokenServiceDelegateChromeOSLegacy::GetAccounts() const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
...
void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::LoadCredentials(
  ...
  account_manager_->GetAccounts(base::BindOnce(
      &ProfileOAuth2TokenServiceDelegateChromeOSLegacy::OnGetAccounts,
      weak_factory_.GetWeakPtr()));
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::UpdateCredentials(
    const CoreAccountId& account_id,
...
ProfileOAuth2TokenServiceDelegateChromeOSLegacy::GetURLLoaderFactory() const {
  return nullptr;
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::OnGetAccounts(
...
void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::
    ContinueTokenUpsertProcessing(const CoreAccountId& account_id,
...
void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::OnTokenUpserted(
  ...
  account_manager_->HasDummyGaiaToken(
      account.key,
      base::BindOnce(&ProfileOAuth2TokenServiceDelegateChromeOSLegacy::
                         ContinueTokenUpsertProcessing,
                     weak_factory_.GetWeakPtr(), account_id));
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::OnAccountRemoved(
    const account_manager::Account& account) {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  DCHECK_EQ(
      signin::LoadCredentialsState::LOAD_CREDENTIALS_FINISHED_WITH_SUCCESS,
...
void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::RevokeCredentials(
    const CoreAccountId& account_id) {
  // Signing out of Chrome is not possible on Chrome OS.
  NOTREACHED();
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::RevokeAllCredentials() {
  // Signing out of Chrome is not possible on Chrome OS.
  NOTREACHED();
}

const net::BackoffEntry*
ProfileOAuth2TokenServiceDelegateChromeOSLegacy::BackoffEntry() const {
  return &backoff_entry_;
}

void ProfileOAuth2TokenServiceDelegateChromeOSLegacy::OnConnectionChanged(
