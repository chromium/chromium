...
class X509CertificateModelBase {
 ...
 protected:
  // Protected constructor: derived classes do their own parsing
  // and set the member variables directly.
...
// certificate policy qualifier OIDs.
// Returns the IDS_CERT_* resource ID if found, or std::nullopt if the
// OID is not in the shared map. Platforms may override specific entries
// by checking their own map before falling through to this function.
