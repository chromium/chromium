...
class CommonControllerBuilder {
 public:
  CommonControllerBuilder() = default;
  ~CommonControllerBuilder() = default;
  ...
  syncer::ModelTypeController::TypeVector Build(
...
SyncApiComponentFactoryImpl::CreateCommonModelTypeControllers(
    syncer::ModelTypeSet disabled_types,
    syncer::SyncService* sync_service) {
  CommonControllerBuilder builder;

  builder.SetIdentityManager(sync_client_->GetIdentityManager());
  builder.SetConsentAuditor(sync_client_->GetConsentAuditor());
  builder.SetDeviceInfoSyncService(sync_client_->GetDeviceInfoSyncService());
  builder.SetFaviconService(sync_client_->GetFaviconService());
  builder.SetHistoryService(sync_client_->GetHistoryService());
  builder.SetModelTypeStoreService(sync_client_->GetModelTypeStoreService());
#if !BUILDFLAG(IS_ANDROID)
  ...
  builder.SetPasswordSenderService(sync_client_->GetPasswordSenderService());
  builder.SetPrefService(sync_client_->GetPrefService());
  builder.SetPrefServiceSyncable(sync_client_->GetPrefServiceSyncable());
  builder.SetSessionSyncService(sync_client_->GetSessionSyncService());
  builder.SetReadingListModel(sync_client_->GetReadingListModel());
  builder.SetSendTabToSelfSyncService(
      sync_client_->GetSendTabToSelfSyncService());
  builder.SetUserEventService(sync_client_->GetUserEventService());
  builder.SetAutofillWebDataService(ui_thread_, db_thread_,
  ...
  builder.SetBookmarkSyncService(local_or_syncable_bookmark_sync_service_,
                                 account_bookmark_sync_service_);
  builder.SetPowerBookmarkService(power_bookmark_service_);
  builder.SetSupervisedUserSettingsService(supervised_user_settings_service_);
  builder.SetPlusAddressSettingService(plus_address_setting_service_,
                                       plus_address_webdata_service_);
  builder.SetProductSpecificationsService(product_specifications_service_);
  builder.SetDataSharingService(data_sharing_service_);

  return builder.Build(disabled_types, sync_service, channel_);
}

syncer::ModelTypeController::TypeVector CommonControllerBuilder::Build(
    syncer::ModelTypeSet disabled_types,
    syncer::SyncService* sync_service,
    version_info::Channel channel) {
