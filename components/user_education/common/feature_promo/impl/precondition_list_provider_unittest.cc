...
BASE_FEATURE(kTestFeature1, "TEST_Feature1", base::FEATURE_ENABLED_BY_DEFAULT);
BASE_FEATURE(kTestFeature2, "TEST_Feature2", base::FEATURE_ENABLED_BY_DEFAULT);
...
TEST(PreconditionListProviderTest,
  ...
  // TODO(dfried): Swap to `CreateForTest()` when that CL is landed.
  const auto spec = FeaturePromoSpecification::CreateForLegacyPromo(
...
TEST(PreconditionListProviderTest,
  ...
  const auto spec = FeaturePromoSpecification::CreateForLegacyPromo(
      &kTestFeature1, kAnchorId, IDS_OK);
  const auto spec2 = FeaturePromoSpecification::CreateForLegacyPromo(
