...
constexpr char kBrandCode[] = "fake_brand_code";
constexpr char kSerialNumber[] = "fake_serial_number";
...
TEST_F(RequestHandlerForDeviceStateRetrievalTest, HandleRequestForZTE) {
  ...
  initial_enrollment_state.management_domain = kManagementDomain;
  policy_storage()->SetInitialEnrollmentState(
      base::StrCat({kBrandCode, "_", kSerialNumber}), initial_enrollment_state);
  ...
  request->set_serial_number(kSerialNumber);
  ...
  EXPECT_EQ(response.device_state_retrieval_response().restore_mode(),
            em::DeviceStateRetrievalResponse::RESTORE_MODE_NONE);
  const auto& initial_response =
      response.device_state_retrieval_response().initial_state_response();
  EXPECT_EQ(initial_response.initial_enrollment_mode(),
            em::DeviceInitialEnrollmentStateResponse::
                INITIAL_ENROLLMENT_MODE_ZERO_TOUCH_ENFORCED);
  EXPECT_EQ(initial_response.management_domain(), kManagementDomain);
}

TEST_F(RequestHandlerForDeviceStateRetrievalTest, HandleRequestForFRE) {
