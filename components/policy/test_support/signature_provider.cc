...
crypto::keypair::PrivateKey DecodePrivateKey(const char* const encoded) {
  std::string to_decrypt;
  CHECK(base::Base64Decode(encoded, &to_decrypt));

  return *crypto::keypair::PrivateKey::FromPrivateKeyInfo(
...
SignatureProvider::SigningKey::SigningKey(
  ...
  public_key_ = base::as_string_view(private_key_.ToSubjectPublicKeyInfo());
}
...
bool SignatureProvider::SigningKey::Sign(
  ...
  std::vector<uint8_t> result =
      crypto::sign::Sign(kind, private_key_, base::as_byte_span(str));
...
bool SignatureProvider::SignVerificationData(const std::string& data,
  ...
  std::vector<uint8_t> result =
      crypto::sign::Sign(crypto::sign::SignatureKind::RSA_PKCS1_SHA256,
                         verification_key_, base::as_byte_span(data));
  signature->assign(std::string(result.begin(), result.end()));
...
std::string SignatureProvider::GetVerificationPublicKey() {
  return std::string(
      base::as_string_view(verification_key_.ToSubjectPublicKeyInfo()));
}

SignatureProvider::SignatureProvider()
    : verification_key_(*crypto::keypair::PrivateKey::FromPrivateKeyInfo(
