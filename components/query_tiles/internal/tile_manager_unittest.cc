...
class TileManagerTest : public testing::Test {
 public:
  ...
  void GetSingleTile(const std::string& id, std::optional<Tile> expected) {
    base::RunLoop loop;
    manager()->GetTile(
        id, true,
  ...
  void OnGetTile(base::RepeatingClosure closure,
                 std::optional<Tile> expected,
                 std::optional<Tile> actual) {
...
TEST_F(TileManagerTest, GetTileById) {
  ...
  GetSingleTile("id_not_exist", std::nullopt);
}
...
TEST_F(TileManagerTest, GetSingleTileWithTrendingSubTiles) {
  ...
  std::optional<Tile> parent_tile2 = std::make_optional(*parent_tile.get());
...
TEST_F(TileManagerTest, GetSingleTileAfterOnTileClicked) {
  ...
  std::optional<Tile> get_single_tile_expected =
      std::make_optional(*parent_tile.get());
