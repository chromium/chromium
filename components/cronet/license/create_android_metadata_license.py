...
# processed.
_TARGETS_TO_PROCESS_LICENSE_FOR_TRANSITIVELY = ["//components/cronet/android:cronet_package_android"]
  ...
  for directory_path in processed_build_files_path:
    ...
    readme_files.update((os.path.relpath(
        readme_file_path, repo_path) for readme_file_path in glob.glob(
            f"{os.path.join(repo_path, directory_path)}/**README.chromium",
            recursive=True)))
...
def update_license(repo_path: str = _ROOT_CRONET,
    ...
    verify_only: bool = False, reachable_through_dependencies: bool = False):
  """
...
if __name__ == '__main__':
  update_license(post_process_dict=constants.POST_PROCESS_OPERATION, reachable_through_dependencies=True)
