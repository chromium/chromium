...
TEST(ContentSettingsPatternTest, TrimTrailingDotsFromHost) {
  ...
  // Hosts that end in a numeric label (or punycode resulting in one) are
  // treated as IPv4 addresses per WHATWG URL spec. When preceding labels are
  // non-numeric, they are broken IPv4 addresses and must be rejected even when
  // multiple trailing dots are present.
  EXPECT_FALSE(Pattern("example.2").IsValid());
  EXPECT_FALSE(Pattern("example.2.").IsValid());
  EXPECT_FALSE(Pattern("example.2..").IsValid());
  EXPECT_FALSE(Pattern("9127.xn-91ԵxhԸ.0chr-Ὲ.2").IsValid());
  EXPECT_FALSE(Pattern("9127.xn-91ԵxhԸ.0chr-Ὲ.2.").IsValid());
  EXPECT_FALSE(Pattern("9127.xn-91ԵxhԸ.0chr-Ὲ.2..").IsValid());

  // Trailing dots disguised by ignorable characters (like soft hyphen U+00AD)
  // or fullwidth/ideographic dots must be handled consistently and rejected if
  // broken.
  EXPECT_FALSE(Pattern("example.2.\xc2\xad").IsValid());
  EXPECT_FALSE(Pattern("example.2\xe3\x80\x82").IsValid());
  EXPECT_FALSE(Pattern("example.2\xe3\x80\x82\xe3\x80\x82").IsValid());
  EXPECT_FALSE(
      Pattern("...........\xc2\xad....1....\xc2\xad..........\xc2\xad....")
          .IsValid());

  // Valid patterns with trailing dots or non-ASCII dots that resolve to IPv4.
  EXPECT_TRUE(Pattern("1..").IsValid());
  EXPECT_STREQ("0.0.0.1", Pattern("1..").ToString().c_str());
  EXPECT_TRUE(Pattern("1..") == Pattern("0.0.0.1"));
  EXPECT_TRUE(Pattern("1\xe3\x80\x82\xe3\x80\x82").IsValid());
  EXPECT_STREQ("0.0.0.1",
               Pattern("1\xe3\x80\x82\xe3\x80\x82").ToString().c_str());
  EXPECT_TRUE(Pattern("example.com\xe3\x80\x82\xe3\x80\x82").IsValid());
  EXPECT_STREQ(
      "example.com",
      Pattern("example.com\xe3\x80\x82\xe3\x80\x82").ToString().c_str());
}
