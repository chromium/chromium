...
class COMPONENT_EXPORT(PERSISTENT_CACHE) SqliteSandboxedVfsDelegate
 ...
 public:
  ...
  class COMPONENT_EXPORT(PERSISTENT_CACHE) UnregisterRunner {
   ...
   private:
    raw_ref<const SqliteVfsFileSet> vfs_file_set_;
  };
 ...
 private:
  ...
  // The map keeps an index of the `base::FilePath` to the `SandboxedFile`. It
  // doesn't owns them. The liveness of these objects are managed by the calls
  // to `RegisterSandboxedFiles` / `UnregisterSandboxedFiles`.
  base::flat_map<base::FilePath, raw_ptr<SandboxedFile>> sandboxed_files_map_
