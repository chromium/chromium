...
// The only hook we care about is the `free` hook. The rest are
// pass-through-only.
void FreeFn(const AllocatorDispatch* self, void* address, void* context) {
  if (UNLIKELY(sampling_state.Sample() &&
               RandomEvictionQuarantine::Get()->Add(address))) {
    return;
  }
...
size_t MallocShimSupport::NextGetSizeEstimate(void* ptr) {
  return g_allocator_dispatch.next->get_size_estimate_function(
      g_allocator_dispatch.next, ptr, nullptr);
}

void MallocShimSupport::NextFree(void* ptr) {
  g_allocator_dispatch.next->free_function(g_allocator_dispatch.next, ptr,
                                           nullptr);
}
