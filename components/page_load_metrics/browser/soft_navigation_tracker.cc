...
// calculator.
template <typename T>
void ProcessTmpl(typename T::LimitType limit,
...
void SoftNavigationTracker::Process(
    base::span<const mojom::EventTimingPtr>* event_timings,
    InteractionToNextPaintCalculator* calculator) const {
  ProcessTmpl<InteractionToNextPaintAdapter>(SoftNavigationSlicingTime(),
...
void SoftNavigationTracker::Process(
  ...
  ProcessTmpl<LayoutShiftAdapter>(SoftNavigationSlicingTime(), layout_shifts,
...
void SoftNavigationTracker::Process(
  ...
  ProcessTmpl<LargestContentfulPaintAdapter>(
