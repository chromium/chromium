...
bool AutofillPasswordsEphemeralModule::IsEnabled(int impression_count) {
  // Check if an AutofillPasswords tip variation is being forced via a Finch
  // feature param.
  ...
  if (AutofillPasswordsEphemeralModule::IsModuleLabel(force_show_param)) {
  ...
  if (AutofillPasswordsEphemeralModule::IsModuleLabel(force_hide_param)) {
