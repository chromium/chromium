...
void ModelExecutionManagerImpl::ExecuteModel(OptimizationTarget segment_id,
  ...
  if (model_handler_it == model_handlers_.end()) {
    // Manually post the callback to avoid re-entrancy for an early out.
    base::SequencedTaskRunnerHandle::Get()->PostTask(
        FROM_HERE,
        base::BindOnce(&ModelExecutionManagerImpl::RunModelExecutionCallback,
                       weak_ptr_factory_.GetWeakPtr(), std::move(callback), 0,
                       ModelExecutionStatus::HANDLER_NOT_FOUND_ERROR));
    return;
  }
  ...
  // * Fetch SegmentInfo from the metadata database.
