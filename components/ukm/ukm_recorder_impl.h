...
class COMPONENT_EXPORT(UKM_RECORDER) UkmRecorderImpl : public UkmRecorder {
 ...
 public:
  ...
  // (N==rate).
  void SetWebFeaturesSamplingForTesting(int rate);
 ...
 protected:
  ...
  std::map<SourceId, BitSet>& web_features() {
    return recordings_.web_features;
  }

  const std::map<SourceId, BitSet>& web_features() const {
    return recordings_.web_features;
  }
 ...
 private:
  ...
  FRIEND_TEST_ALL_PREFIXES(UkmRecorderImplTest, WebFeaturesConsent);
  FRIEND_TEST_ALL_PREFIXES(UkmRecorderImplTest, WebFeaturesSampling);
  ...
  int web_features_sampling_ = 1;   // by default, no downsampling
  ...
  struct Recordings {
    ...
    // Web features usage data (data captured by RecordWebFeatures()).
    std::map<SourceId, BitSet> web_features;
