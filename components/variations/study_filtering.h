...
// The name of a study generated by the variations server to monitor the health
// of newly deployed seeds. Processed first to ensure crash keys contain the
// most recent group in case the application of a subsequent runtime mutable
// study is crashy.
inline constexpr char kRuntimeMonitoringStudyName[] =
    "SeedRolloutRuntimeMutable";
...
// Reorders the returned studies so that the study with name
// `kRuntimeMonitoringStudyName` (if any) is placed at the front.
COMPONENT_EXPORT(VARIATIONS)
