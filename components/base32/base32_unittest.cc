...
TEST(Base32Test, EncodesRfcTestVectorsCorrectlyWithoutPadding) {
  ...
  constexpr uint8_t test_data[] = "foobar";
  ...
  for (size_t i = 0; i < std::size(expected); ++i) {
    auto test_subspan = base::make_span(test_data).first(i);
    auto encoded_output =
        Base32Encode(test_subspan, Base32EncodePolicy::OMIT_PADDING);
    EXPECT_EQ(expected[i], encoded_output);
    auto decoded_output = Base32Decode(encoded_output);
    EXPECT_TRUE(base::ranges::equal(test_subspan, decoded_output));
  }
}
...
TEST(Base32Test, EncodesRfcTestVectorsCorrectlyWithPadding) {
  ...
  for (size_t i = 0; i < std::size(expected); ++i) {
    auto test_subspan = base::make_span(test_data).first(i);
    std::string encoded_output = Base32Encode(test_subspan);
    EXPECT_EQ(expected[i], encoded_output);
