...
TEST_F(WaylandAuraShellServerTest, RotateFocus) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    auto* zaura_toplevel = zaura_shell_get_aura_toplevel_for_xdg_toplevel(
        client->globals().aura_shell.get(),
        data->test_surfaces_list[0].xdg_toplevel.get());
    zaura_toplevel_add_listener(zaura_toplevel, &listeners, &listener);
    zaura_toplevel_set_supports_screen_coordinates(zaura_toplevel);
  });
...
TEST_F(WaylandAuraShellServerTest, AckRotateFocus) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    zaura_toplevel_ack_rotate_focus(
        zaura_toplevel, serial++, ZAURA_TOPLEVEL_ROTATE_HANDLED_STATE_HANDLED);
  });
