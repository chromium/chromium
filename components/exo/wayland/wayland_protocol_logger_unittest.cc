...
TEST_F(WaylandProtocolLoggerTest, LogsBasicRequest) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    surface_id = ProxyId(wl_compositor_create_surface(client->compositor()));
  });
...
TEST_F(WaylandProtocolLoggerTest, LogsBasicEvent) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    callback_id = ProxyId(wl_display_sync(client->display()));
  });
...
TEST_F(WaylandProtocolLoggerTest, LogsObjects) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    surface_id = ProxyId(surface);
    xdg_surface_id =
        ProxyId(xdg_wm_base_get_xdg_surface(client->xdg_wm_base(), surface));
  });
...
TEST_F(WaylandProtocolLoggerTest, LogsStrings) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    xdg_toplevel_id = ProxyId(xdg_toplevel);
  });
...
TEST_F(WaylandProtocolLoggerTest, LogsFileDescriptors) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    data_source_id = ProxyId(data_source);
  });

  wl_resource* data_source_resource = test::server_util::LookUpResource(
      server_.get(), test::client_util::GetResourceKey(data_source));
...
TEST_F(WaylandProtocolLoggerTest, LogsFixedPointNumbers) {
  ...
  wl_pointer* pointer;
  PostToClientAndWait([&](test::TestClient* client) {
    pointer = wl_seat_get_pointer(client->seat());
    pointer_id = ProxyId(pointer);
  });

  // Act: Send a motion event
  wl_resource* pointer_resource = test::server_util::LookUpResource(
      server_.get(), test::client_util::GetResourceKey(pointer));
...
TEST_F(WaylandProtocolLoggerTest, LogsArrays) {
  ...
  PostToClientAndWait([&](test::TestClient* client) {
    ...
    xdg_toplevel_id = ProxyId(toplevel);
  });

  wl_resource* xdg_toplevel_resource = test::server_util::LookUpResource(
      server_.get(), test::client_util::GetResourceKey(toplevel));
