...
TEST_F(SharedMemoryTest, CreateBuffer) {
  ...
  const auto si_format = viz::SinglePlaneFormat::kRGBA_8888;
  ...
  uint32_t bytes_per_row = viz::ResourceSizes::CheckedWidthInBytes<uint32_t>(
      buffer_size.width(), si_format);
  std::unique_ptr<Buffer> buffer =
      shared_memory->CreateBuffer(buffer_size, si_format, 0, bytes_per_row);
  EXPECT_TRUE(buffer);
  ...
  uint32_t top_left_buffer_size_width =
      viz::ResourceSizes::CheckedWidthInBytes<uint32_t>(
          top_left_buffer_size.width(), si_format);
  std::unique_ptr<Buffer> top_left_buffer = shared_memory->CreateBuffer(
      top_left_buffer_size, si_format, 0, top_left_buffer_size_width);
  ...
  uint32_t bottom_right_buffer_size_width =
      viz::ResourceSizes::CheckedWidthInBytes<uint32_t>(
          bottom_right_buffer_size.width(), si_format);
  std::unique_ptr<Buffer> bottom_right_buffer = shared_memory->CreateBuffer(
      bottom_right_buffer_size, si_format,
