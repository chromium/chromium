...
// configs in components/domain_reliability/BUILD.gn instead.)
...
// Array of JSON-encoded Domain Reliability configurations.
// Read by DomainReliabilityMonitor::AddBakedInConfigs.
inline constexpr auto kBakedInJsonConfigs = std::to_array<std::string_view>({
"""
...
def main():
  ...
  parser.add_option("", "--output", metavar="FILE",
                    help="[Required] Name of the .h file to write.")

  # For response file reading.
  parser.add_option("", "--file-list", metavar="FILE",
                    help="[Required] File containing whitespace separated "
                         "names of the baked in configs files.")
  ...
  if not opts.file_list:
    print("--file-list argument required", file=sys.stderr)
    return 1
