...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {session, dp} =
      await testRunner.startBlank('Tests file download from second tab.');
  ...
  const {targetId} =
      (await dp.Target.createTarget({url: 'about:blank'})).result;
  const session2 = await session.attachChild(targetId);

  testRunner.log('Setting download behavior to allow...');
  await session2.protocol.Browser.setDownloadBehavior({
    behavior: 'allow',
  ...
  const frameTree =
      (await session2.protocol.Page.getFrameTree()).result.frameTree;
  const frameId = frameTree.frame.id;

  testRunner.log('Downloading file by clicking an anchor link...');
  const [willBeginEvent, completedEvent] = await Promise.all([
    session2.protocol.Browser.onceDownloadWillBegin(
        e => e.params.frameId === frameId),
    session2.protocol.Browser.onceDownloadProgress(
        e => e.params.state === 'completed'),
    session2.evaluate(`
