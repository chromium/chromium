...
class VIZ_SERVICE_EXPORT HitTestAggregator {
 ...
 private:
  ...
  const CheckedPtr<const HitTestManager> hit_test_manager_;

  const CheckedPtr<HitTestAggregatorDelegate> delegate_;

  const CheckedPtr<LatestLocalSurfaceIdLookupDelegate>
