...
class TransactionalLevelDBTransactionTest : public LevelDBScopesTestBase {
 ...
 protected:
  ...
  void Put(const std::string_view& key, const std::string& value) {
    std::string put_value = value;
    leveldb::Status s = leveldb_database_->Put(key, &put_value);
    ASSERT_TRUE(s.ok());
  }

  void Get(const std::string_view& key, std::string* value, bool* found) {
  ...
  bool KeysEqual(const std::string_view& a, const std::string_view& b) const {
