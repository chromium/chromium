...
class ActorLoginDelegateImplTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    mock_cleaning_service_ =
        std::make_unique<NiceMock<MockActorLoginPermissionCleaningService>>();
    delegate_client_ = std::make_unique<FakeActorLoginDelegateClient>(
        &prefs_, test_origin_, &mock_driver_, &client_,
        mock_cleaning_service_.get());
 ...
 protected:
  ...
  std::unique_ptr<FakeActorLoginDelegateClient> delegate_client_;
  // `raw_ptr` because the delegate client owns it.
...
TEST_F(ActorLoginDelegateImplTest, GetCredentials_NullClient) {
  ...
  delegate_ = nullptr;
  delegate_client_ = std::make_unique<FakeActorLoginDelegateClient>(
      &prefs_, test_origin_, &mock_driver_, nullptr,
      mock_cleaning_service_.get());
  auto* delegate =
      ActorLoginDelegateImpl::CreateForUserData(delegate_client_.get());
...
TEST_F(ActorLoginDelegateImplTest,
  ...
  prefs_.SetBoolean(autofill::prefs::kAutofillUsingPlatformAutofill, true);
...
TEST_F(ActorLoginDelegateImplTest, GetCredentialsLogsDomainAndLanguage) {
  ...
  EXPECT_CALL(*mqls_logger(),
              SetDomainAndLanguage(_, Eq(GURL("https://example.com/"))));
  delegate_->GetCredentials(/*has_sign_in_with_google_button=*/false,
...
TEST_F(ActorLoginDelegateImplTest, AttemptLoginLogsDomainAndLanguage) {
  ...
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(), base::DoNothing(),
      /*action_sequence_delegate=*/nullptr);
}
...
TEST_F(ActorLoginDelegateImplTest, AttemptLoginServiceBusy_FeatureOn) {
  ...
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(),
      first_future.GetCallback(), /*action_sequence_delegate=*/nullptr);
  // Immediately try to start a second request of the same type.
  base::test::TestFuture<LoginStatusResultOrError> second_future;
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(),
      second_future.GetCallback(), /*action_sequence_delegate=*/nullptr);
...
TEST_F(ActorLoginDelegateImplTest, CallbacksAreResetAfterCompletion_FeatureOn) {
  ...
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(), future3.GetCallback(),
      /*action_sequence_delegate=*/nullptr);
  ...
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(), future4.GetCallback(),
      /*action_sequence_delegate=*/nullptr);
  ...
  auto get_credentials_callback =
      base::BindLambdaForTesting([&](CredentialsOrError result) {
        ...
        delegate_->AttemptLogin(
            credential, false, mqls_logger(), base::TimeTicks::Now(),
            /*frame_filling_started_cb=*/base::DoNothing(),
            future.GetCallback(), /*action_sequence_delegate=*/nullptr);
      });
...
TEST_F(ActorLoginDelegateImplTest,
  ...
  delegate_->AttemptLogin(
      credential, false, mqls_logger(), base::TimeTicks::Now(),
      /*frame_filling_started_cb=*/base::DoNothing(),
      attempt_login_future.GetCallback(), /*action_sequence_delegate=*/nullptr);
...
TEST_F(ActorLoginDelegateImplTest,
  ...
  base::test::TestFuture<LoginStatusResultOrError> attempt_login_future;
  delegate_->AttemptLogin(
      credential, /*should_store_permission=*/true, mqls_logger(),
      base::TimeTicks::Now(), /*frame_filling_started_cb=*/base::DoNothing(),
      attempt_login_future.GetCallback(), /*action_sequence_delegate=*/nullptr);
  ...
  auto* cleaning_service = mock_cleaning_service_.get();
...
TEST_F(
  ...
  base::test::TestFuture<LoginStatusResultOrError> attempt_login_future;
  delegate_->AttemptLogin(
      credential, /*should_store_permission=*/false, mqls_logger(),
      base::TimeTicks::Now(), /*frame_filling_started_cb=*/base::DoNothing(),
      attempt_login_future.GetCallback(), /*action_sequence_delegate=*/nullptr);
...
TEST_F(ActorLoginDelegateImplTest,
  ...
  const Credential expected =
      delegate_client_->GetExpectedFederatedCredentials()[0];
  const Credential actual = future.Get().value()[0];
  EXPECT_EQ(actual.username, expected.username);
  EXPECT_EQ(actual.type, expected.type);
  EXPECT_EQ(actual.source_site_or_app, expected.source_site_or_app);
  EXPECT_EQ(actual.federation_detail, expected.federation_detail);
}
...
TEST_F(ActorLoginDelegateImplTest,
  ...
  auto* mock_cleaning_service = mock_cleaning_service_.get();
  GURL url = GURL(kTestUrl);
  url::Origin origin = url::Origin::Create(url);
  SetUpConflictingPermissions(url, kTestUsername);
