...
public final class DesktopSiteMetrics {
     ...
     * @param setting The {@link ContentSetting} of the newly added setting.
     * @param hostname The hostname of the newly added setting.
     */
    public static void recordDesktopSiteSettingsManuallyAdded(
            @SiteSettingsCategory.Type int type, @ContentSetting int setting, String hostname) {
        if (type != SiteSettingsCategory.Type.REQUEST_DESKTOP_SITE) {
            return;
        }

        boolean isDesktopSite = setting == ContentSetting.ALLOW;
    ...
    public static void recordDesktopSiteSettingsChanged(
            @SiteSettingsCategory.Type int type, @ContentSetting int setting, Website site) {
        if (type != SiteSettingsCategory.Type.REQUEST_DESKTOP_SITE) {
            return;
        }
