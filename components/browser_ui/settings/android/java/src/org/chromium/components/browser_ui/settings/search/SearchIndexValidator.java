...
public class SearchIndexValidator {
    ...
    /**
     * Determines whether a preference with the given class name should be included in the search
     * index. This logic mimics the filtering used during XML parsing in {@link PreferenceParser}.
     *
     * @param className The runtime class name of the preference (e.g.,
     *     "androidx.preference.PreferenceCategory").
     * @return True if the preference is indexable, false otherwise.
     */
    private static boolean isValidForIndex(String className) {
        return !className.endsWith("PreferenceScreen")
                && !className.equals(TEXT_MESSAGE_PREFERENCE_CLASS)
                && !className.endsWith("PreferenceCategory");
    }
    ...
    private static void collectPreferenceKeys(@Nullable PreferenceGroup group, Set<String> keys) {
        ...
        for (int i = 0; i < group.getPreferenceCount(); i++) {
            ...
            // Only visible preferences that are valid for the index should be validated.
            // This ensures parity with the PreferenceParser logic (e.g., ignoring categories).
            if (pref.isVisible()
                    && pref.getKey() != null
                    && isValidForIndex(pref.getClass().getName())) {
