...
class SupervisedUserServiceTestBase : public ::testing::Test {
 public:
  explicit SupervisedUserServiceTestBase(bool is_supervised) {
    ...
    settings_service_ = std::make_unique<SupervisedUserSettingsService>();
    ...
    service_ = std::make_unique<SupervisedUserService>(
        kids_chrome_management_client_.get(), *pref_service_.get(),
        *settings_service_.get(), *sync_service_.get(),
 ...
 protected:
  ...
  std::unique_ptr<SupervisedUserSettingsService> settings_service_;
...
TEST_F(SupervisedUserServiceTest, ManagedSiteListTypeMetricOnPrefsChange) {
  ...
  // Blocks `kExampleUrl0`.
  {
    ScopedDictPrefUpdate hosts_update(pref_service_.get(),
  ...
  // Blocks `kExampleURL1`.
  {
    ScopedDictPrefUpdate urls_update(pref_service_.get(),
