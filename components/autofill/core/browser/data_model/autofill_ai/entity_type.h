...
class EntityType final {
 public:
  ...
  // Returns whether this entity type supports restricting local storage of
  // obfuscated attributes to "masks" (e.g., the last x digits/characters).
  //
  // If this is `true`, the full entity information is stored on a server and
  // retrieved by the client only on demand. It is not persisted locally on
  // disk. However, note that even if this is `true`
  // - an `EntityInstance` of this type may still be stored locally if its
  //   `RecordType` is `kLocal`, and
  // - users may not be eligible for creating masked server entities depending
  //   on their sync settings or their locale. See `MayPerformAutofillAiAction`
  //   for the relevant permission checks.
  bool SupportsMaskedStorage() const;
