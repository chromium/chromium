 ...
 protected:
  IbanSuggestionGeneratorTest() {
    ...
    test_api(form).Append(test::CreateTestFormField(
        /*label=*/"iban", "iban", "", FormControlType::kInputText));
...
TEST_F(IbanSuggestionGeneratorTest, GeneratesIbanSuggestions) {
  ...
  std::vector<Suggestion> expected_suggestions = {
      ...
      GetSuggestionForIban(SetUpServerIban(
          ...
          kNickname_1)),
      SetUpSeparator(),
      SetUpFooterManagePaymentMethods()};
  ...
  EXPECT_CALL(suggestions_generated_callback,
              Run(testing::Pair(
                  ...
                  testing::UnorderedElementsAreArray(expected_suggestions))));
  EXPECT_CALL(*static_cast<MockIbanManager*>(
                  client().GetPaymentsAutofillClient()->GetIbanManager()),
              OnGetSingleFieldSuggestions(_, _, _, _))
      .WillOnce([&](const FormFieldData& field,
                    const AutofillField& autofill_field,
                    const AutofillClient& client,
                    SingleFieldFillRouter::OnSuggestionsReturnedCallback&
                        on_suggestions_returned) {
        std::move(on_suggestions_returned)
            .Run(field.global_id(), expected_suggestions);
        return true;
      });
  generator.GenerateSuggestions(form(), field(), client(),
                                {savedCallbackArgument},
