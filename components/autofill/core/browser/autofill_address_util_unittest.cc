...
TEST_F(AddressFormattingTest, GetAddressComponentsWithExtensions) {
  ...
  // Expect to find a line consisting solely of a state field. Lines are not
  // empty, see `GetAddressComponentsSkipsEmptyLines()`.
  auto state_line =
      base::ranges::find(lines, ::i18n::addressinput::AddressField::ADMIN_AREA,
                         [](const auto& line) { return line[0].field; });
  ASSERT_NE(state_line, lines.end());
  EXPECT_EQ(state_line->size(), 1u);
