...
## Obsolete hand-crafted `.icns` files

The Chromium project no longer hand-crafts `.icns` files. Instead, the
low-resolution `.icns` files generated by `actool` are used. Details about this
will be forthcoming, but for now, the instructions for the old tooling is
included below.
