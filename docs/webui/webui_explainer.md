...
When a Mojom file defines a factory interface with a method creating a page
handler (such as `PageHandlerFactory` with `CreatePageHandler`), the generated
TypeScript bindings file (`donuts.mojom-webui.ts`) automatically generates and
exports a `browserProxyFactory` singleton factory and a `BrowserProxy`
interface. See `ui/webui/resources/mojo/mojo_browser_proxy` for the
implementation and definition of the templated versions.

Note: WebUI pages may have 1 way (browser -> renderer) or 2 way
(browser -> renderer, renderer -> browser) communication. This is determined
by whether the `.mojom` file contains a `Page {}` interface.

- **`browserProxyFactory`**: An autogenerated singleton factory (of type
 `MojoBrowserProxyFactory` or `MojoHandlerProxyFactory`). Use
 `browserProxyFactory.getInstance()` in application code to get the active
 proxy instance. WebUIs with 2-way communication will have a `MojoBrowserProxy`
 that has a `handler` and `callbackRouter`. WebUIs with 1-way (browser->
 renderer only) communication will have a `MojoHandlerProxy` with just a
 `handler` member.
- In tests: WebUIs with 2-way communication should use
 `browserProxyFactory.createForTest` to inject a test handler, which
 automatically sets up a callbackRouter and returns an instance that can
 be passed to `setInstance()` and used to access the router in test code.
 WebUIs with 1 way communication should use `browserProxyFactory.setInstance()`
 directly with an instance of a test handler, since they do not need to set
 up a callback router.
- **`BrowserProxy`**: An autogenerated TypeScript interface containing
 `.handler` (to invoke C++ receiver methods) and, for pages using 2 way
 communication, `.callbackRouter` (to receive events from C++).
...
```ts
import {browserProxyFactory} from './donuts.mojom-webui.js';
...
function CheckNumberOfDonuts() {
  ...
  // response.
  browserProxyFactory.getInstance().handler.getNumberOfDonuts().then(
      (numDonuts: number) => {
...
function BakeDonuts(numDonuts: number) {
  ...
  browserProxyFactory.getInstance().handler.bakeDonuts(numDonuts);
}
```
