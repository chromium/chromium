...
TEST_F(TracingObserverProtoTest,
  ...
  tracing::DataSourceTester trace_data_tester(tracing_observer.get());
  ...
  trace_data_tester.BeginTrace(GetTraceConfig());
...
TEST_F(TracingObserverProtoTest, AddChromeDumpToTraceIfEnabled) {
  ...
  ASSERT_EQ(1ul, trace_data_tester.GetFinalizedPacketCount());

  const perfetto::protos::TracePacket* packet =
      trace_data_tester.GetFinalizedPacket(0);
...
TEST_F(TracingObserverProtoTest, AddOsDumpToTraceIfEnabled) {
  ...
  EXPECT_EQ(2ul, trace_data_tester.GetFinalizedPacketCount());
  ...
  const perfetto::protos::TracePacket* smaps_trace_packet =
      trace_data_tester.GetFinalizedPacket(1);
...
TEST_F(TracingObserverProtoTest, AsProtoInto) {
  ...
  std::unique_ptr<perfetto::TraceWriter> trace_writer =
      trace_data_tester.GetProducerClient()->CreateTraceWriter(
          config.target_buffer());
  write_dump(trace_writer->NewTracePacket());
#endif  // !BUILDFLAG(USE_PERFETTO_CLIENT_LIBRARY)
  trace_data_tester.EndTracing();
  EXPECT_EQ(1ul, trace_data_tester.GetFinalizedPacketCount());
