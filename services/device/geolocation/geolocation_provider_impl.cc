...
void GeolocationProviderImpl::NotifyNetworkLocationRequested(
  ...
  CHECK(main_task_runner_->BelongsToCurrentThread());
  for (auto& observer : internals_observers_) {
    std::vector<mojom::AccessPointDataPtr> request_clone;
    base::ranges::transform(request, std::back_inserter(request_clone),
                            &mojom::AccessPointDataPtr::Clone);
    observer->OnNetworkLocationRequested(std::move(request_clone));
  }
}
