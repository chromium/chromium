...
class COMPONENT_EXPORT(NETWORK_SERVICE) SharedDictionaryManager {
 public:
  ...
  // Returns a SharedDictionaryStorage for the `network_isolation_key`.
  ...
  // Called when the SharedDictionaryStorage for the `network_isolation_key` is
  // deleted.
  void OnStorageDeleted(const net::NetworkIsolationKey& network_isolation_key);

 protected:
  SharedDictionaryManager();

  // Called to create a SharedDictionaryStorage for the `network_isolation_key`.
  // This called only when there is no matching storage in `storages_`.
 ...
 private:
  std::map<net::NetworkIsolationKey, raw_ptr<SharedDictionaryStorage>>
      storages_;
