...
TEST(ClientHintsTest,
  ...
  // spec.
  result = ParseClientHintToDelegatedThirdPartiesHeader(
      "device-memory;resolution=GIB, rtt", MetaCHType::NameAcceptCH);
  EXPECT_THAT(
      result.map,
      UnorderedElementsAre(
          std::make_pair(
  ...
  // Unknown tokens are fine, since this meant to be extensible.
  result = ParseClientHintToDelegatedThirdPartiesHeader(
      "device-memory,  rtt , nosuchtokenwhywhywhy", MetaCHType::NameAcceptCH);
  EXPECT_THAT(
      result.map,
      UnorderedElementsAre(
          std::make_pair(
  ...
  // http-equiv="delegate-ch" syntax won't work.
  result = ParseClientHintToDelegatedThirdPartiesHeader(
      "device-memory https://foo.bar;  rtt https://foo.bar "
      "https://baz.qux ",
      MetaCHType::NameAcceptCH);
