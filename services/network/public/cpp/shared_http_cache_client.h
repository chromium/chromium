...
//
// It maintains a thread-safe in-memory hash set of cached URL hashes to quickly
// determine if a resource might be available without disk I/O, and reads
// response data from the isolated SQLite database directly via sqlite_vfs,
// bypassing IPC to the network service on cache hits.
