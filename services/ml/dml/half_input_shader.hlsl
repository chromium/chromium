...
RWBuffer<float> input_buffer : register(u0);
RWBuffer<half> output_buffer : register(u1);
...
void format_half_input(uint3 block_id : SV_GroupID, uint3 thread_id : SV_GroupThreadID)
{
  ...
  if (index < num * chw_length)
  {
    ...
    output_buffer[strides.x + strides.y + strides.z] = input_buffer[index];
  }
}