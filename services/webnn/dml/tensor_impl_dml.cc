...
void TensorImplDml::ExportTensorImpl() {
  ...
  representation_access_->SetReleaseFence(std::move(webnn_fence_to_wait_for));
}
...
bool TensorImplDml::ImportTensorImpl() {
  ...
  auto d3d_write_fence = representation_access_->GetAcquireFence();
  if (!d3d_write_fence) {
    return true;
  }
  ...
  return BeginAccessWebNN(d3d12_write_fence, d3d_write_fence->GetFenceValue());
}
