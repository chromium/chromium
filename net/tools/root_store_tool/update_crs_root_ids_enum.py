...
def load_certs_from_file(file_path):
  ...
  for pem in pem_blocks:
    try:
      ...
      print(
        f"Warning: Failed to load cert from PEM in {file_path}: {e}",
        file=sys.stderr,
      )
  return certs
...
def main():
  ...
  for anchor in itertools.chain(
    root_store.trust_anchors, root_store.additional_certs
  ):
  ...
  for crs_root_id, group in classical_groups.items():
    if len(group) == 1:
      ...
      if len(group_root_certs) != 1:
        print(
          f"count of root certs for {crs_root_id} is {len(group_root_certs)}",
          file=sys.stderr,
        )
        sys.exit(1)
