...
EmbeddedTestServerAndroid::GetRequestHeadersForUrl(
    JNIEnv* env,
  ...
  // Copy data from HttpRequest::HeaderMap (std::map with the custom comparator)
  // to the default std::map, so that it can be passed to Java.
