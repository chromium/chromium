...
#ifndef MEDIA_FILTERS_LEGACY_AUDIO_FILE_READER_H_
#define MEDIA_FILTERS_LEGACY_AUDIO_FILE_READER_H_
...
// LegacyAudioFileReader is the previous implementation of AudioFileReader,
// and is intended to be removed once the new implementation has baked on
// the Stable branch
class MEDIA_EXPORT LegacyAudioFileReader {
 public:
  // Audio file data will be read using the given protocol.
  // The LegacyAudioFileReader does not take ownership of |protocol| and
  // simply maintains a weak reference to it.
  explicit LegacyAudioFileReader(FFmpegURLProtocol* protocol);

  LegacyAudioFileReader(const LegacyAudioFileReader&) = delete;
  LegacyAudioFileReader& operator=(const LegacyAudioFileReader&) = delete;

  virtual ~LegacyAudioFileReader();
  ...
  // The methods below are helper methods which allow LegacyAudioFileReader to double
...
#endif  // MEDIA_FILTERS_LEGACY_AUDIO_FILE_READER_H_
