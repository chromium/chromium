...
TEST_F(MPEG1AudioStreamParserTest, MetadataParsing) {
  ...
  EXPECT_EQ(
      "", ParseData(base::as_byte_span(*buffer).subspan(offset, kId3TagSize)));
  ...
  EXPECT_EQ("", ParseData(base::as_byte_span(*buffer).subspan(
  ...
  EXPECT_EQ("NewSegment{ 0K }EndOfSegment",
            ParseData(base::as_byte_span(*buffer).subspan(
                offset, kFirstRealFrameSize)));
  EXPECT_TRUE(last_audio_config().IsValidConfig());
}
