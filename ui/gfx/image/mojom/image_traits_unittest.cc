...
TEST(ImageTraitsTest, NullImageSkiaRep_Serialize) {
  // ImageSkia doesn't contain null ImageSkiaReps and we don't serialize
  // null ImageSkiaReps separately.
  ImageSkiaRep null_rep;
  EXPECT_TRUE(null_rep.is_null());
  EXPECT_DEATH(gfx::mojom::ImageSkiaRep::SerializeAsMessage(&null_rep), "");
}
#endif

#if defined(GTEST_HAS_DEATH_TEST)
TEST(ImageTraitsTest, NullImageSkiaRep_Deserialize) {
  // ImageSkia doesn't contain null ImageSkiaReps and we don't serialize
  // null ImageSkiaReps separately.
  mojo::StructPtr<mojom::ImageSkiaRep> input =
      ConstructImageSkiaRep(SkBitmap(), 1);
  EXPECT_DEATH(gfx::mojom::ImageSkiaRep::SerializeAsMessage(&input), "");
}
#endif
