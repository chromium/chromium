...
TEST_F(EventConverterEvdevImplTest, BlockTelephonyMicMuteKeyByDefault) {
  ...
  struct input_event mock_kernel_queue[] = {
      {{0, 0}, EV_MSC, MSC_SCAN, 0xb002f}, {{0, 0}, EV_KEY, KEY_MICMUTE, 1},
      {{0, 0}, EV_SYN, SYN_REPORT, 0},

      {{0, 0}, EV_MSC, MSC_SCAN, 0xb002f}, {{0, 0}, EV_KEY, KEY_MICMUTE, 0},
      {{0, 0}, EV_SYN, SYN_REPORT, 0},
  };
...
TEST_F(DeferDeviceSetUpEventConverterEvdevImplTest,
  ...
  struct input_event mock_kernel_queue[] = {
      {{0, 0}, EV_MSC, MSC_SCAN, 0x0b002f}, {{0, 0}, EV_KEY, KEY_MICMUTE, 1},
      {{0, 0}, EV_SYN, SYN_REPORT, 0},

      {{0, 0}, EV_MSC, MSC_SCAN, 0x0b002f}, {{0, 0}, EV_KEY, KEY_MICMUTE, 0},
      {{0, 0}, EV_SYN, SYN_REPORT, 0},
  };
  ...
  event = dispatched_event(0);
  EXPECT_EQ(ui::EventType::ET_KEY_PRESSED, event->type());
  EXPECT_EQ(ui::VKEY_MICROPHONE_MUTE_TOGGLE, event->key_code());
  EXPECT_EQ(0xb002fu, event->scan_code());
  EXPECT_EQ(0, event->flags());

  event = dispatched_event(1);
  EXPECT_EQ(ui::EventType::ET_KEY_RELEASED, event->type());
  EXPECT_EQ(ui::VKEY_MICROPHONE_MUTE_TOGGLE, event->key_code());
  EXPECT_EQ(0xb002fu, event->scan_code());
  EXPECT_EQ(0, event->flags());
}
