...
void WaylandToplevelWindow::SetWindowGeometry(
  ...
  if (!state.auxiliary_data.insets_dip.IsEmpty()) {
    geometry_dip.Inset(state.auxiliary_data.insets_dip);
...
void WaylandToplevelWindow::TriggerStateChanges(
  ...
  auto latest_requested_state = GetLatestRequestedState();
  ...
  if (shell_toplevel_) {
    ...
    } else if (latest_requested_state.window_state ==
  ...
  latest_requested_state.window_state = window_state;
  RequestStateFromClient(latest_requested_state);
  connection()->Flush();
}
