...
// static
uint8_t SynthesizeSerialNumber() {
  static uint8_t synthesized_serial_num = 0;
  CHECK_LT(synthesized_serial_num, std::numeric_limits<uint8_t>::max())
...
int64_t VirtualDisplayUtilMac::AddDisplay(const DisplayParams& display_params) {
  uint8_t serial_number = SynthesizeSerialNumber();
