 ...
 * @mixinFunction
 */
export const CrContainerShadowMixin = dedupingMixin(superClass => {
  /**
   * @polymer
   * @mixinClass
   */
  class CrContainerShadowMixin extends superClass {
    constructor() {
      ...
      this.dropShadows_ = null;

      /** @private {?Map<!CrContainerShadowSide, !HTMLDivElement>} */
      this.intersectionProbes_ = null;
    ...
    connectedCallback() {
      ...
      const hasBottomShadow =
          this.$.container.hasAttribute('show-bottom-shadow');
    ...
    /**
     * @param {boolean} enable Whether to enable the behavior or disable it.
     *     This function does nothing if the behavior is already in the
     *     requested state.
     */
    enableShadowBehavior(enable) {
      ...
      // the drop shadow flashes once on startup, because of the DOM
      // modifications earlier in this function causing a relayout.
      window.setTimeout(() => {
    ...
    /**
     * Shows the shadows. The shadow behavior must be disabled before calling
     * this method, otherwise the intersection observer might show the shadows
     * again.
     */
    showDropShadows() {
...
/** @interface */
export class CrContainerShadowMixinInterface {
