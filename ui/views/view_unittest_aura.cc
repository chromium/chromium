...
//     +-- v8
//     +-- v9
TEST_F(ViewAuraTest, LegacyRecreateLayersWithWindows) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndDisableFeature(features::kNativeViewHostManagesLayers);
...
// Test that wm::RecreateLayers() recreates the layers for all child windows and
// all child views and that the z-order of the recreated layers matches that of
// the original layers, when kNativeViewHostManagesLayers is enabled.
// In this case, NativeViewHost has its own layer, and the attached window's
// layer is nested under it.
//
// Test hierarchy:
// w1
// +-- v1
...
//     +-- v3 (no layer)
//     +-- v4
// +-- nvh (NativeViewHost, has layer)
//     +-- w2 (attached window, layer is nested under nvh's layer)
//         +-- v5
//             +-- v6
// +-- v7
//     +-- v8
//     +-- v9
//
// TODO(crbug.com/434082728): Remove LegacyRecreateLayersWithWindows when
// kNativeViewHostManagesLayers is cleaned up.
TEST_F(ViewAuraTest, RecreateLayersWithWindows) {
  base::test::ScopedFeatureList feature_list;
  feature_list.InitAndEnableFeature(features::kNativeViewHostManagesLayers);
  ...
  auto* nvh =
      w1->GetRootView()->AddChildView(std::make_unique<NativeViewHost>());
  nvh->SetBounds(0, 5, 100, 105);
  ...
  nvh->Attach(w2->GetNativeView());

  ASSERT_TRUE(nvh->layer());
  nvh->layer()->SetName("nvh");
  ui::Layer* nvh_layer = nvh->layer();
  ...
  ASSERT_EQ("v1 v4 nvh v7", ui::test::ChildLayerNamesAsString(*w1_layer));

  ASSERT_EQ("w2", ui::test::ChildLayerNamesAsString(*nvh_layer));
  ui::Layer* w2_layer = nvh_layer->children()[0];

  ASSERT_EQ("v5", ui::test::ChildLayerNamesAsString(*w2_layer));
  ui::Layer* v5_layer = w2_layer->children()[0];
  ASSERT_EQ("v6", ui::test::ChildLayerNamesAsString(*v5_layer));

  View::Views w1_views_with_layers = GetViewsWithLayers(w1.get());
  ASSERT_EQ(4u, w1_views_with_layers.size());
  EXPECT_EQ(v1, w1_views_with_layers[0]);
  EXPECT_EQ(v4, w1_views_with_layers[1]);
  EXPECT_EQ(nvh, w1_views_with_layers[2]);
  EXPECT_EQ(v7, w1_views_with_layers[3]);

  {
    ...
    ASSERT_EQ("v1 v4 nvh v7", ui::test::ChildLayerNamesAsString(*w1_new_layer));
    ui::Layer* v1_new_layer = w1_new_layer->children()[0];
    ui::Layer* v4_new_layer = w1_new_layer->children()[1];
    ui::Layer* nvh_new_layer = w1_new_layer->children()[2];
    ui::Layer* v7_new_layer = w1_new_layer->children()[3];

    EXPECT_NE(v1_layer, v1_new_layer);
    EXPECT_NE(v4_layer, v4_new_layer);
    EXPECT_NE(nvh_layer, nvh_new_layer);
    EXPECT_NE(v7_layer, v7_new_layer);

    ASSERT_EQ("w2", ui::test::ChildLayerNamesAsString(*nvh_new_layer));
    ui::Layer* w2_new_layer = nvh_new_layer->children()[0];
    EXPECT_NE(w2_layer, w2_new_layer);

    ASSERT_EQ("v5", ui::test::ChildLayerNamesAsString(*w2_new_layer));
    ui::Layer* v5_new_layer = w2_new_layer->children()[0];
    EXPECT_NE(v5_layer, v5_new_layer);

    ASSERT_EQ("v6", ui::test::ChildLayerNamesAsString(*v5_new_layer));
    ui::Layer* v6_new_layer = v5_new_layer->children()[0];
    EXPECT_NE(v6_layer, v6_new_layer);

    ASSERT_EQ("v8 v9", ui::test::ChildLayerNamesAsString(*v7_new_layer));

    View::Views new_w1_views_with_layers = GetViewsWithLayers(w1.get());
    ASSERT_EQ(4u, new_w1_views_with_layers.size());
    EXPECT_EQ(v1, new_w1_views_with_layers[0]);
    EXPECT_EQ(v4, new_w1_views_with_layers[1]);
    EXPECT_EQ(nvh, new_w1_views_with_layers[2]);
    EXPECT_EQ(v7, new_w1_views_with_layers[3]);
  }
