...
void FrameViewLayoutLinux::LayoutTitlebar() {
  ...
  Label* title_label = view_->title_label();
  if (!title_label) {
    return;
  }
  ...
  // Center the label across the full window width. The label uses
  // ALIGN_CENTER so giving it the widest possible symmetric bounds
  // around the window center produces a visually centered title.
  // Constrain to the available space between the caption buttons.
  ...
  title_label->SetBoundsRect(
      gfx::Rect(label_x, title_y, label_width, title_height));
  title_label->SetVisible(available_width > 0);
}
