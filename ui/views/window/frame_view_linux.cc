...
void FrameViewLinux::InitViews() {
  ...
  if (widget_->widget_delegate()->ShouldShowWindowTitle()) {
    auto title =
        std::make_unique<Label>(widget_->widget_delegate()->GetWindowTitle());
    title->SetFontList(layout_->GetTitleFontList());
...
bool FrameViewLinux::HasWindowTitle() const {
  return title_label_ != nullptr;
}

bool FrameViewLinux::IsWindowTitleVisible() const {
  return title_label_ && title_label_->GetVisible();
}

void FrameViewLinux::UpdateWindowTitle() {
  if (title_label_) {
    title_label_->SetText(widget_->widget_delegate()->GetWindowTitle());
    InvalidateLayout();
  }
}
...
void FrameViewLinux::UpdateTitleLabelColors() {
  if (!title_label_) {
    return;
  }
  const auto* color_provider = GetColorProvider();
  ...
  title_label_->SetEnabledColor(color_provider->GetColor(
      active ? ui::kColorFrameCaptionForegroundActive
             : ui::kColorFrameCaptionForegroundInactive));
  title_label_->SetBackgroundColor(color_provider->GetColor(
      active ? ui::kColorFrameActive : ui::kColorFrameInactive));
}
