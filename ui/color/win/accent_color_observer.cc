...
using ABI::Windows::UI::ViewManagement::UIColorType_Accent;
...
void AccentColorObserver::OnDwmKeyUpdated() {
  ...
  // are getting the coerced color if necessary.
  Microsoft::WRL::ComPtr<IUISettings> ui_settings;
  ...
  if (SUCCEEDED(hr) && ui_settings) {
    Microsoft::WRL::ComPtr<IUISettings3> ui_settings3;
    hr = ui_settings.As(&ui_settings3);
    if (SUCCEEDED(hr) && ui_settings3) {
      Color accent_color;
      hr = ui_settings3->GetColorValue(UIColorType_Accent, &accent_color);
      if (SUCCEEDED(hr)) {
        accent_color_ = SkColorSetARGB(0xFF, accent_color.R, accent_color.G,
                                       accent_color.B);
      }
    }
  }
