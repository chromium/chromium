...
public class MotionEventUtils {
     ...
     */
    private static boolean isMouseEvent(int source, int toolType) {
        return (source & InputDevice.SOURCE_CLASS_POINTER) != 0
                && toolType == MotionEvent.TOOL_TYPE_MOUSE;
    }

    /** Returns true if a {@link MotionEvent} is detected to be a pointer event. */
    public static boolean isPointerEvent(MotionEvent event) {
        return isPointerEvent(event.getSource(), event.getToolType(0));
    }

    /** Returns true if a {@link MotionEvent} is detected to be a pointer event. */
    public static boolean isPointerEvent(int source, int toolType) {
        return isMouseEvent(source, toolType) || isTrackpadEvent(source, toolType);
    }
    ...
    public static boolean isTrackpadEvent(MotionEvent event) {
        return isTrackpadEvent(event.getSource(), event.getToolType(0));
    }

    private static boolean isTrackpadEvent(int source, int toolType) {
        return (source == InputDevice.SOURCE_MOUSE) && toolType == MotionEvent.TOOL_TYPE_FINGER;
    }
    ...
    /** Checks if the motion event was generated by a secondary button (right mouse button). */
