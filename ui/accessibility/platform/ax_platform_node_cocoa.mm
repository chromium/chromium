...
// Returns a "sanitized" version of `aString` by transforming it to avoid
// exposing potentially sensitive or personally identifiable information. The
// main goal is to analyze differences between two strings without needing
// access to the actual text content, focusing instead on structural
// differences (whitespace and token sequences).
//
// The method detects tokens (sequences of non-whitespace characters) and
// replaces them with placeholder values while preserving the whitespace in
// hexadecimal form. If `savingTokens` is not nil, this method stores each
// token in the array. If `expectingTokens` is not nil, this method checks
// that each token it encounters in `aString` matches the tokens in
// `expectingTokens`, signaling the start of a mismatch with "x"s.
+ (NSString*)sanitizedStringForDumpWithoutCrashing:(NSString*)aString
                                      savingTokens:(NSMutableArray<NSString*>*)
                                                       savingTokens
                                   expectingTokens:(NSMutableArray<NSString*>*)
                                                       expectingTokens {
  NSMutableString* result = [[NSMutableString alloc] init];
  NSMutableString* nextToken = [[NSMutableString alloc] init];
  NSMutableCharacterSet* tmpSet =
      [NSMutableCharacterSet whitespaceCharacterSet];
  // The whitespace character set does not include newline or return.
  [tmpSet addCharactersInString:@"\n\r"];
  // Make an immutable copy for better performance.
  NSCharacterSet* whitespaceSet = [tmpSet copy];

  BOOL buildingTheNextToken = NO;

  for (NSUInteger i = 0; i < aString.length; i++) {
    unichar nextChar = [aString characterAtIndex:i];

    if ([whitespaceSet characterIsMember:nextChar]) {
      if (buildingTheNextToken) {
        buildingTheNextToken = NO;

        NSString* paddingString;
        NSUInteger paddingLength = nextToken.length;

        if (savingTokens) {
          [savingTokens addObject:nextToken];
          paddingString = @"a";
        } else if (expectingTokens.count &&
                   [nextToken isEqualToString:[expectingTokens firstObject]]) {
          paddingString = @"b";
        } else {
          // Token doesn't match the next expected in the input.
          paddingString = @"x";
        }

        nextToken = [[NSMutableString alloc] init];

        if (expectingTokens.count) {
          [expectingTokens removeObjectAtIndex:0];
        }

        [result appendString:[@"" stringByPaddingToLength:paddingLength
                                               withString:paddingString
                                          startingAtIndex:0]];
      }

      // Save whitespace character as its hex value.
      [result appendFormat:@"%02X", nextChar];
    } else {
      buildingTheNextToken = YES;
      [nextToken appendFormat:@"%C", nextChar];
    }
  }
  ...
  return result.length > 1024 ? [result substringToIndex:1024] : result;
}
...
- (void)addTextAnnotationsIn:(const AXRange*)axRange
  ...
  for (const AXRange& leafTextRange : *axRange) {
    ...
    if (NSMaxRange(leafRange) > attributedString.length) {
      NSMutableString* totalLeafString = [[NSMutableString alloc] init];

      for (const AXRange& nextRange : *axRange) {
        std::u16string nextLeafString = nextRange.GetText(
            ui::AXTextConcatenationBehavior::kWithoutParagraphBreaks,
            ui::AXEmbeddedObjectBehavior::kSuppressCharacter);
        [totalLeafString appendString:base::SysUTF16ToNSString(nextLeafString)];
      }

      // Transform the original attributed string and leaf string into
      // sanitized representations. E.g., "This is a test" becomes:
      //   sanitizedAttributedString = aaaa20aa20a20aaaa
      //   sanitizedLeafString = bbbb20bb20b20bbbb
      // Here, the two strings are identical.
      //
      // Comparing "This is a test" to "  This   \nis   not a   test  ":
      //   sanitizedAttributedString = aaaa20aa20a20aaaa
      //   sanitizedLeafString = 2020bbbb2020200Abb202020xxx20x202020xxxx2020
      // The output shows they differ in both white space between the tokens,
      // and in the tokens themselves.
      NSMutableArray<NSString*>* tokens = [[NSMutableArray alloc] init];

      NSString* sanitizedAttributedString = [AXPlatformNodeCocoa
          sanitizedStringForDumpWithoutCrashing:attributedString.string
                                   savingTokens:tokens
                                expectingTokens:nil];
      NSString* sanitizedLeafString = [AXPlatformNodeCocoa
          sanitizedStringForDumpWithoutCrashing:totalLeafString
                                   savingTokens:nil
                                expectingTokens:tokens];

      // From previous tests, most of the time the strings are longer than
      // 1024, but not always. Worst case, if the mismatches only start in the
      // latter portion of a string, we'll at least catch them a few times.
      NSString* info =
          [NSString stringWithFormat:@"attr: %lu; leaf: %lu",
                                     attributedString.string.length,
                                     totalLeafString.length];
      SCOPED_CRASH_KEY_STRING64("AXPlatformNodeCocoa", "lengths",
                                base::SysNSStringToUTF8(info));
      SCOPED_CRASH_KEY_STRING1024(
          "AXPlatformNodeCocoa", "attributedString",
          base::SysNSStringToUTF8(sanitizedAttributedString));
      SCOPED_CRASH_KEY_STRING1024("AXPlatformNodeCocoa", "leafString",
                                  base::SysNSStringToUTF8(sanitizedLeafString));
