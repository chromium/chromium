...
staging only the files you have fully resolved by their explicit paths. For
this skill, do NOT use broad staging forms such as `git add .` or `git add -A`,
so that unrelated or unresolved changes cannot be staged. For this skill, do
not run any other git commands (for example, do not run
`git rebase --continue`).
