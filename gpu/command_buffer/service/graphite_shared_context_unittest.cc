...
std::unique_ptr<skgpu::graphite::Context> MakeGraphiteContext() {
  ...
  options.featureLevel = (backend == wgpu::BackendType::OpenGL ||
...
TEST_F(GraphiteSharedContextThreadSafeTest, IsThreadSafe) {
  ...
  // IsTheadSafe() is read back as true.
  EXPECT_TRUE(graphite_shared_context()->IsTheadSafe());
}
...
TEST_F(GraphiteSharedContextThreadSafeTest, ConcurrentAccess) {
  ...
  // We should not encounter any failures on either threads.
