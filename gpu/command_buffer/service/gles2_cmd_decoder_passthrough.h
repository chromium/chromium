...
struct PassthroughResources {
  ...
  class SharedImageData {
   public:
    ...
    base::flat_set<GLuint> DetachFromFramebuffers(
        gl::GLApi* api,
...
#undef GLES2_CMD_OP
  ...
  // Map from the framebuffer's service_id to a set of client texture id that
  // are shared image and are attached to this framebuffer.
