...
def _build_chromium(cwd: pathlib.Path, configs: list[eval_config.TestConfig]):
    ...
    if targets:
        ...
        subprocess.check_call(
            ['autoninja', '-C', 'out/Default', *targets], cwd=cwd
        )
...
def _discover_testcase_files(
    ...
    internal_prompts_path = (
        constants.CHROMIUM_SRC / 'internal' / 'agents' / 'prompts' / 'eval'
    )
...
def _determine_shard_values(
    parsed_shard_index: int | None, parsed_total_shards: int | None
) -> tuple[int, int]:
    ...
    shard_index_set = (
        parsed_shard_index is not None or env_shard_index is not None
    )
    total_shards_set = (
        parsed_total_shards is not None or env_total_shards is not None
    )
...
def _perform_chromium_setup(
    force: bool, build: bool, configs: list[eval_config.TestConfig]
) -> None:
...
def _fetch_sandbox_image(gemini_cli_cmd: list[str]) -> bool:
    ...
    try:
        ...
        logging.error(
            ...
            'sandboxing. Try running with --no-sandbox.%s',
            image,
            e,
            output,
        )
...
def _run_tests_with_retries(
    ...
    for iteration in range(retries + 1):
        if iteration != 0:
            logging.info(
                'Retrying %d failed tests (attempt %d of %d)',
...
def _run_prompt_eval_tests(args: argparse.Namespace) -> int:
    ...
    """
    configs_to_run = _get_tests_to_run(
        args.shard_index,
        args.total_shards,
        args.filter,
        args.tag_filter,
    ...
    _perform_chromium_setup(
        force=args.force, build=not args.no_build, configs=configs_to_run
    )
    ...
    if args.use_pinned_binaries:
        (gemini_cli_bin, node_bin) = (
            gemini_cli_installation.fetch_cipd_gemini_cli(args.verbose)
        )
    ...
    if args.sandbox and not _fetch_sandbox_image(gemini_cli_cmd=gemini_cli_cmd):
        return 1

    worker_options = workers.WorkerOptions(
    ...
    worker_pool = workers.WorkerPool(
        ...
        else len(configs_to_run),
        promptfoo,
        worker_options,
        result_options,
    )

    failed_test_results = _run_tests_with_retries(
        worker_pool, configs_to_run, args.retries
    )
    ...
    if failed_test_results:
        returncode = 1
        logging.warning(
            '%d tests ran successfully and %d failed after %d additional tries',
...
def _parse_args() -> argparse.Namespace:
    ...
    group.add_argument(
        ...
        '-f',
        action='store_true',
        help='Force execution, deleting existing workdirs if they exist.',
    )
    group.add_argument(
        '--no-build', action='store_true', help='Do not build out/Default.'
    )
    ...
    group.add_argument(
        ...
        action='store_true',
        help=(
            'Print test output even when a test succeeds. By default, '
    ...
    group.add_argument(
        ...
        action='store_true',
        help=(
            'Upload test metrics to the perf dashboard. This is only '
    ...
    group.add_argument(
        '--git-revision',
        help=(
            'The git revision being tested. Must be set if '
    ...
    group.add_argument(
        '--gcs-bucket',
        help=(
            'The GCS bucket to upload perf results to. Must '
    ...
    group.add_argument(
        '--build-id',
        help=(
            'The Buildbucket build ID to associate with perf '
    ...
    group.add_argument(
        '--builder',
        help=(
            'The name of the builder running these tests. '
    ...
    group.add_argument(
        '--builder-group',
        help=(
            'The name of the group the builder running these '
    ...
    group.add_argument(
        ...
        type=int,
        help=(
            'The build number of the build running these '
    ...
    group.add_argument(
        ...
        type=int,
        help=(
            f'The index of the current shard. If set, --total-shards must '
    ...
    group.add_argument(
        ...
        type=int,
        help=(
            f'The total number of shards used to run these tests. If set, '
    ...
    group.add_argument(
        '--list-tests',
        action='store_true',
        help='List all discovered test file paths and descriptions, then exit.',
    )
    ...
    group.add_argument(
        ...
        action='store_false',
        help=(
            'Use the pinned cipd version. This is to control what is under '
    ...
    group.add_argument(
        ...
        default=1,
        help=(
            'The number of parallel workers to run tests in. Changing this '
    ...
    retry_group.add_argument(
        ...
        type=int,
        help=(
            'Alias for --retries to conform to the isolated script standard.'
        ),
    )
    group.add_argument(
