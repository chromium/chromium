...
**Goal:** Your task is to fix all unsafe buffer operations starting from a
single C++ file. You will do this removing UNSAFE_TODO() markers and #pragma
allow_unsafe_buffers directives. This will produce compiler errors related to
`-WUnsafe-buffer-usage` opt-outs, which you will then fix by following
established patterns for safe buffer handling in Chromium. Replacing c-style
arrays, raw pointers, and manual size calculations with modern, safe C++
containers, primarily `base::span`.

**Core Task:** You will be given a single C++ file path.

1. Find all unsafe code (marked by `UNSAFE_TODO` or
   `#pragma allow_unsafe_buffers`).
2. Fix the code by applying the patterns below.
...
2. **Identify -WUnsafe-buffer-usage opt-outs:**
  ...
  - **If you find `UNSAFE_TODO(...)`:** Remove the macro wrapper, leaving the
    code inside.
...
4. **Fix the Code:** Apply the **Core Principles** and **Patterns & Fixes**
   below to replace unsafe buffer usage with safe alternatives. Use the compiler
   errors as a guide to iteratively fix all issues.

   - Your changes must be minimal and targeted. You are allowed to apply some
     local refactoring to improve the fix, but avoid large-scale rewrites or
     unrelated changes.
   - Do not refactor unrelated code.
   - **If you change a function signature, you MUST find and update all its call
     sites.** This is critical for success.

5. **Verify the Fix:** You must ensure your fix compiles and passes tests.
   **This step is mandatory.**
   ...
   - **Builder 1: `linux-rel`**
     ...
     - **Compile:** `autoninja -C out/linux-rel --quiet`
     - **Test:** `./tools/autotest.py --out-dir out/linux-rel --run-changed`

   - **Builder 2: `linux-win-cross-rel`**

     - **Compile:** `autoninja -C out/linux-win-cross-rel --quiet`
     - **Test:**
       `./tools/autotest.py --out-dir out/linux-win-cross-rel --run-changed`

   - **Builder 3: `android-14-x64-rel`**

     - **Compile:** `autoninja -C out/android-14-x64-rel --quiet`
     - **Test:**
       `./tools/autotest.py --out-dir out/android-14-x64-rel --run-changed`

   - **Builder 4: `mac-rel`**

     - **Compile:** `autoninja -C out/mac-rel --quiet`
     - **Test:** `./tools/autotest.py --out-dir out/mac-rel --run-changed`

   - **Builder 5: `chromeos-amd64-generic-dbg`**

     - **Compile:** `autoninja -C out/chromeos-amd64-generic-dbg --quiet`
     - **Test:**
       `./tools/autotest.py --out-dir out/chromeos-amd64-generic-dbg --run-changed`
...
6. **Format and Finalize:**
   ...
   - Apply the final, formatted changes to the C++ file.
   ...
   to fix unsafe buffer operations.
   ...
   - The header line should be 50 characters or less. You can transform
     transform the file path by removing directory components or take the
     relevant class name.
...
**DO NOT USE IT.** Your goal is to *eliminate* the need for it by creating spans
from sources that already know their size (like `std::vector` or `std::array`).

- **DON'T** use `UNSAFE_BUFFERS()`. If a safe fix is impossible (e.g., a complex
  third-party API), set the status to `UNSAFE_TODO` and stop without creating a
  commit_message.md.
...
- **Fix:** Convert the C-style array to a `std::array`.
...
- **Fix:** Replace them with their safe C++ or `base` library equivalents.
  ...
  - `memcpy`, `memmove` → `base::span::copy_from()` or
    `base::span::copy_prefix()`
  - `memset` → `std::ranges::fill()`
...
- **Look for Safe Alternatives:** If you encounter a class that returns a raw
  ...
  alternative like `obj->GetSpan()` or `obj->AsSpan()`.
