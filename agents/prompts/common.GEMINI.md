## Chromium Workflow and Guidelines:

### General Workflow:

  * **Follow User Queries:** Prioritize acting based on the user's request. Ask
    for clarification if the query is ambiguous or essential information is
    missing.
  ...
  * **Reading files from the repo:** All files in chromium’s source can be read
    using the `read_file` tool, substituting `chromium/src` or `//` for the
    current workspace (which can be determined by running `gclient root` and
    appending `/src` to the output).
  * **Output directories** From chromium's src folder, there will be a number of
    output directories in the out/ folder. If you are unsure which output
...
4.  **Build:** **ALWAYS** build relevant targets after making edits. successful
    formatting. "{USERS\_OUT\_DIR}" is the output directory the user has
    requested, and "{REQUESTED\_TARGET}" may be inferred. Typical
    "{REQUESTED\_TARGET}"s could be:
      ...
      * `chrome_public_test_apk` - has many of the instrumentation tests
      * `chrome_public_unit_test_apk` - has many of on device unittests
      * `{TOP_LEVEL_FOLDER}_junit_tests`, eg. `chrome_junit_tests`, for
        robolectric tests in the `//chrome` folder.
...
6.  **Test:** **ALWAYS** run relevant tests (e.g., `tools/autotest.py --quiet --run-all -C
    ...
    change should be tested. `autotest.py` does not require a build target, it
    figures out which target to build on its own. It just requires the source
...
@../../internal/agents/prompts/edit_fix_workflow.md

### Chromium specific knowledge

  * **GN:** Chromium uses `gn` as our build system. Please use
    `build/gn_ast/gn_editor.py` when possible to edit BUILD.gn files. Only
    perform manual edits on BUILD.gn files if this tool does not work, or does
    not have the capability you're looking for.
  * **Styleguides:** The code style guides can be found in the `//styleguide`
    folders, with `c++`, `java` and other styleguides present.

@../../internal/agents/prompts/chromium_specific_knowledge.md
...
@../../internal/agents/prompts/additional_tools.md
