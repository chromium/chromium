...
void ServiceWorkerClient::InheritControllerFrom(
  ...
  DCHECK(base::FeatureList::IsEnabled(kSharedWorkerBlobURLFix) ||
         blink::mojom::ServiceWorkerClientType::kDedicatedWorker ==
             GetClientType() ||
         ((blink::mojom::ServiceWorkerClientType::kWindow == GetClientType() &&
           (client_url.IsAboutSrcdoc() || client_url.IsAboutBlank()))));
  DCHECK(client_url.SchemeIsBlob() || client_url.IsAboutSrcdoc() ||
         client_url.IsAboutBlank());
