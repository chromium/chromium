...
IN_PROC_BROWSER_TEST_F(CrossPlatformAccessibilityBrowserTest,
  ...
  const BrowserAccessibility* root =
      GetManager()->GetBrowserAccessibilityRoot();
  ASSERT_EQ(1U, root->PlatformChildCount());
  const BrowserAccessibility* container = root->PlatformGetChild(0);
  ...
  // scroll.
  ExecuteScript("document.querySelector('#container').scrollTop = 500;");
  ASSERT_TRUE(waiter1.WaitForNotification());
  EXPECT_EQ(container->GetIntAttribute(ax::mojom::IntAttribute::kScrollY), 500);
...
IN_PROC_BROWSER_TEST_F(CrossPlatformAccessibilityBrowserTest,
  ...
  // Scroll the container to a location and expect a scroll update with new
  // scroll.
  ExecuteScript("document.querySelector('#container').scrollLeft = 500;");
  ASSERT_TRUE(waiter1.WaitForNotification());
  EXPECT_EQ(container->GetIntAttribute(ax::mojom::IntAttribute::kScrollX), 500);
