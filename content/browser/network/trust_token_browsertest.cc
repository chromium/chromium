...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest, OperationsRequireSecureContext) {
  ...
  std::string command =
      R"(fetch("/issue", {privateToken: {type: 'private-state-token',
                                       version: 1,
                                       operation: 'token-request'}})
...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest, IssuanceRequiresKeys) {
  ...
  std::string command = R"(
    fetch('/issue', {privateToken: {type: 'private-state-token',
                                  version: 1,
                                  operation: 'token-request'}})
...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest, CrossOriginIssuanceWorks) {
  ...
  EXPECT_EQ(
      ...
      EvalJs(shell(), JsReplace(R"(
            fetch($1, { privateToken: { type: 'private-state-token',
                                      version: 1,
                                      operation: 'token-request' } })
            .then(()=>'Success'); )",
...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest,
  ...
  std::string command =
      R"(fetch($1, {privateToken: {type: 'private-state-token',
                                 version: 1,
                                 operation: 'token-request'}})
           .catch(error => error.name);)";
...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest,
  ...
  command = R"(fetch($1, {privateToken: {type: 'private-state-token',
                                       version: 1,
                                       operation: 'token-redemption'}})
                 .catch(error => error.name);)";
  EXPECT_EQ(
...
IN_PROC_BROWSER_TEST_F(TrustTokenBrowsertest,
  ...
  command = R"(
    fetch("/sign", {privateToken: {type: 'private-state-token',
                                 version: 1,
                                 operation: 'send-redemption-record',
                                 issuers: []}})
    .then(() => 'Success')
