...
enum class WillCommitDecision {
  kProceedNoPrerenderFrameTreeNode,
  kProceedNoUrlMatch,
  kProceedAlreadyNavigated,
  kProceedUrlIdentical,
  kDeferToUpdatePrerenderURL,
};

const char* DecisionToReasonString(WillCommitDecision decision) {
  switch (decision) {
    case WillCommitDecision::kProceedNoPrerenderFrameTreeNode:
      return "no_prerender_frame_tree_node";
    case WillCommitDecision::kProceedNoUrlMatch:
      return "no_url_match";
    case WillCommitDecision::kProceedAlreadyNavigated:
      return "already_navigated";
    case WillCommitDecision::kProceedUrlIdentical:
      return "url_identical";
    case WillCommitDecision::kDeferToUpdatePrerenderURL:
      return "defer_to_update_prerender_url";
  }
}

WillCommitDecision GetWillCommitDecision(
    FrameTreeNodeId candidate_prerender_frame_tree_node_id,
    const GURL& navigation_url) {
  FrameTreeNode* prerender_frame_tree_node =
      GetRootPrerenderFrameTreeNode(candidate_prerender_frame_tree_node_id);
  ...
  if (!prerender_frame_tree_node) {
    return WillCommitDecision::kProceedNoPrerenderFrameTreeNode;
  }
  ...
  if (!prerender_host.IsUrlMatch(navigation_url)) {
    return WillCommitDecision::kProceedNoUrlMatch;
  }
  ...
  // need to change URL.
  if (prerender_host.GetInitialUrl() == navigation_url) {
    return WillCommitDecision::kProceedUrlIdentical;
  }
  return WillCommitDecision::kDeferToUpdatePrerenderURL;
}
...
PrerenderNoVarySearchCommitDeferringCondition::WillCommitNavigation(
  ...
  WillCommitDecision decision = GetWillCommitDecision(
      candidate_prerender_frame_tree_node_id_, GetNavigationHandle().GetURL());
  TRACE_EVENT_INSTANT(
      "navigation",
      "PrerenderNoVarySearchCommitDeferringCondition::WillCommitNavigation::"
      "Decision",
      "reason", DecisionToReasonString(decision));

  if (decision != WillCommitDecision::kDeferToUpdatePrerenderURL) {
