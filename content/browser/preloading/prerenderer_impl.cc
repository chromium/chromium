...
void PrerendererImpl::RecordReceivedPrerendersCountToMetrics() {
  ...
  for (auto trigger_type :
    ...
    //     PrerenderImpl also won't be created) so that it cannot be reached
    //     this code path at the first place).
    //  2) after CancelStartedPrerenders is called and started_prerenders_ are
    //     cleared once (so that as long as PreloadingDecider (which has the
    //     same lifetime with a document) that owns this (PrerenderImpl) lives,
    //     this function should be called via PrimaryPageChanged).
    //
