...
class SynchronousAuctionNonceManagerTest
 ...
 public:
  void SetUp() override {
    ...
    nonce_manager_ =
        std::make_unique<SynchronousAuctionNonceManager>(main_test_rfh());
  }
  ...
  SynchronousAuctionNonceManager& nonce_manager() { return *nonce_manager_; }

 private:
  std::unique_ptr<SynchronousAuctionNonceManager> nonce_manager_;
};

TEST_F(SynchronousAuctionNonceManagerTest, AuctionNonceIsAvailable) {
...
TEST_F(SynchronousAuctionNonceManagerTest, AuctionNonceCanOnlyBeClaimedOnce) {
...
TEST_F(SynchronousAuctionNonceManagerTest, AuctionNonceIsNotAvailable) {
  AuctionNonce nonce(
...
class AsynchronousAuctionNonceManagerTest
    : public RenderViewHostImplTestHarness {
 public:
  void SetUp() override {
    RenderViewHostImplTestHarness::SetUp();
    NavigateAndCommit(GURL("https://test.org"));
    nonce_manager_ =
        std::make_unique<AsynchronousAuctionNonceManager>(main_test_rfh());
  }
  ...
  AsynchronousAuctionNonceManager& nonce_manager() { return *nonce_manager_; }

 private:
  std::unique_ptr<AsynchronousAuctionNonceManager> nonce_manager_;
};
...
TEST_F(AsynchronousAuctionNonceManagerTest, AuctionNonceIsAvailable) {
...
TEST_F(AsynchronousAuctionNonceManagerTest, AuctionNonceCanOnlyBeClaimedOnce) {
  AuctionNonce nonce = nonce_manager().CreateAuctionNonce();
  VerifyAuctionNonceFormat(nonce);
  nonce_manager().ClaimAuctionNonceIfAvailable(nonce);
  EXPECT_FALSE(nonce_manager().ClaimAuctionNonceIfAvailable(nonce));
...
TEST_F(AsynchronousAuctionNonceManagerTest, AuctionNonceIsNotAvailable) {
  AuctionNonce nonce = nonce_manager().CreateAuctionNonce();
  VerifyAuctionNonceFormat(nonce);
...
TEST_F(AsynchronousAuctionNonceManagerTest, EachAuctionNonceIsUnique) {
