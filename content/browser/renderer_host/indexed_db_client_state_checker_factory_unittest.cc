...
class IndexedDBClientStateCheckerFactoryTest
 ...
 public:
  ...
  // with callback.
  void TestDisallowInactiveClient(RenderFrameHostImpl* rfh,
                                  DisallowInactiveClientReason reason,
                                  bool expected_was_active) {
    base::RunLoop run_loop;
    base::OnceClosure quit_closure = run_loop.QuitClosure();
    IndexedDBClientStateCheckerFactory::GetClientStateCheckerCallback().Run(
        rfh->GetProcess()->GetDeprecatedID(), rfh->GetDocumentToken(), reason,
        base::BindOnce(
            [](base::OnceClosure closure, bool expected_was_active,
               bool was_active, ScopedKeepActive keep_active) {
              EXPECT_EQ(was_active, expected_was_active);
              EXPECT_EQ(static_cast<bool>(keep_active), expected_was_active);
...
TEST_F(IndexedDBClientStateCheckerFactoryTest,
  ...
  TestDisallowInactiveClient(static_cast<RenderFrameHostImpl*>(rfh),
                             DisallowInactiveClientReason::kVersionChangeEvent,
                             /*expected_was_active=*/true);
  EXPECT_EQ(rfh->GetLifecycleState(),
            RenderFrameHost::LifecycleState::kPrerendering);
