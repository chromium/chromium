...
void RecursivelyGenerateFrameEntries(
  ...
  if (!entry) {
    // TODO(https://crbug.com/1399599) Implement persistence for
    // state.initiator_base_url, similar to what's done for
    ...
    entry = base::MakeRefCounted<FrameNavigationEntry>(
        ...
        state.initiator_origin, /* state.initiator_base_url= */ absl::nullopt,
        std::vector<GURL>(), blink::PageState::CreateFromEncodedData(data),
