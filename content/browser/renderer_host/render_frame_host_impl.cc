...
#include "content/browser/renderer_host/pepper/pepper_renderer_connection.h"
  ...
  class RenderFrameHostOrProxy {
   ...
   private:
    RenderFrameProxyHost* GetProxy() {
      if (auto** proxy =
              absl::get_if<RenderFrameProxyHost*>(&frame_or_proxy_)) {
        return *proxy;
      }
      return nullptr;
    }
  ...
  typedef std::unordered_map<GlobalFrameRoutingId,
      ...
      RoutingIDFrameMap;
  base::LazyInstance<RoutingIDFrameMap>::DestructorAtExit
  ...
  // know the commit will be successful.  Resetting nav_entry_id_ avoids acting
  // on any UpdateState or UpdateTitle messages after an ignored commit.
  ...
  // Cache. Documents that are in the bfcache should not be sending mojo
  ...
  class BackForwardCacheMessageFilter : public mojo::MessageFilter {
   ...
   private:
    ...
    // RenderFrame never send IPCs messages, even if there are active pages in
    // the process.
  ...
  // processed by the filters in the same order as the filters are added with
  // the Add() method. WillDispatch() might not be called for all filters or
  // might see a modified message if a filter earlier in the chain discards or
  // modifies it. Similarly a given filter instance might not receive a
  // DidDispatchOrReject() call even if WillDispatch() was called if a filter
  // further down the chain discarded it. Long story short, the order in which
  // filters are added is important!
  ...
  CreateMessageFilterForAssociatedReceiverImpl(
    ...
    // ActiveURLMessageFilter at the end of the chain as we need to make sure
    // that the debug url is reset, that is, DidDispatchOrReject() is called if
    // WillDispatch().
    filter_chain->Add(
  ...
  //
  // Since Create() could be called at any time, perhaps by a stray task being
  // run after a RenderFrameHost has been destroyed, the RemoterFactoryImpl uses
  // the process/routing IDs as a weak reference to the RenderFrameHostImpl.
    ...
    void Create(mojo::PendingRemote<media::mojom::RemotingSource> source,
      ...
      if (auto* host = RenderFrameHostImpl::FromID(process_id_, routing_id_)) {
        GetContentClient()->browser()->CreateMediaRemoter(
            host, std::move(source), std::move(receiver));
      }
    }
  ...
  void LogCanCommitOriginAndUrlFailureReason(
  ...
  // needs to have the trust-token-redemption Permissions Policy feature
  // enabled.
  ...
  // receives a request from the renderer to execute a Trust Tokens redemption
  // or signing operation in the frame.
  //
  ...
  // whether the trust-token-redemption Permissions Policy feature will be
  ...
  // more loaders after the network service crashes---it can directly consult
  // the current Permissions Policy state to determine whether the feature is
  // enabled.
  network::mojom::TrustTokenRedemptionPolicy
  ...
  // Verify that |browser_side_origin| and |renderer_side_origin| match.  See
  // also https://crbug.com/888079.
  void VerifyThatBrowserAndRendererCalculatedOriginsToCommitMatch(
    ...
    // Navigating to file://localhost/... on windows causes
    // `browser_side_origin` and `renderer_side_origin` to be different
    // (file://localhost/ vs file:///). In particular, without the following
    // block the test ContentSecurityPolicyBrowserTest.FileURLs fails.
  ...
  RenderFrameHost* RenderFrameHost::FromFrameToken(
      int process_id, const blink::LocalFrameToken& token) {
  ...
  RenderFrameHostImpl* RenderFrameHostImpl::FromFrameToken(
      int process_id, const blink::LocalFrameToken& frame_token) {
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  RenderFrameHostImpl::RenderFrameHostImpl(
      ...
      RenderFrameHostDelegate * delegate, FrameTree * frame_tree,
      FrameTreeNode * frame_tree_node, int32_t routing_id,
    ...
    if (lifecycle_state_ != LifecycleStateImpl::kSpeculative) {
      ...
      // 2. The opener for popup, when "allow-popups-to-escape-sandbox" isn't
      // set.
      //
      // Both are already computed and stored in the FrameTreeNode. So only a
      // copy is needed here.
    ...
    // Verify is_local_root() now indicates whether this frame is a local root
    // or not. It is safe to use this method anywhere beyond this point.
  ...
  RenderFrameHostImpl::~RenderFrameHostImpl() {
    // The lifetime of this object has ended, so remove it from the id map
    // before calling any delegates/observers, so that any calls to |FromID| no
    // longer return |this|.
    g_routing_id_frame_map.Get().erase(
        GlobalFrameRoutingId(GetProcess()->GetID(), routing_id_));

    // When a RenderFrameHostImpl is deleted, it may still contain children.
    // This can happen with the unload timer. It causes a RenderFrameHost to
    // delete itself even if it is still waiting for its children to complete
    ...
    //    LifecycleStateImpl::kRunningUnloadHandlers to track that this IPC is
    //    in flight.
    ...
    //   RenderFrameHost/RenderFrameProxyHost of the page. They are guaranteed
    //   not to be referenced by any other pages.
    //
    ...
    // messages actually operate on a node-by-node basis rather than detaching
    ...
    // - check |frame_tree_node_->render_manager()->speculative_frame_host()|
    // for
    ...
    // - check |IsCurrent()|, because the RenderFrames in an
    ...
    // speculative RFH! For subframes, this obviously does not matter: a
    // subframe will always pass the condition
    // |render_view_host_->GetMainFrame() != this|. However, it turns out that a
    // speculative main frame being deleted will *always* pass this condition as
    // well: a speculative RFH being deleted will *always* first be unassociated
    // from its corresponding RFHM. Thus, it follows that |GetMainFrame()| will
    // never return the speculative main frame being deleted, since it must have
    // already been unset.
    ...
    // Null out the unload timer; in crash dumps this member will be null only
    // if the dtor has run.  (It may also be null in tests.)
    ...
    // Note: The RenderWidgetHost of the main frame is owned by the
    // RenderViewHost instead. In this case the RenderViewHost is responsible
    // for shutting down its RenderViewHost.
    ...
    if (beforeunload_initiator && beforeunload_initiator != this) {
      base::TimeTicks approx_renderer_start_time =
  ...
  int RenderFrameHostImpl::GetRoutingID() { return routing_id_; }

  const blink::LocalFrameToken& RenderFrameHostImpl::GetFrameToken() {
    return frame_token_;
  }

  ui::AXTreeID RenderFrameHostImpl::GetAXTreeID() { return ax_tree_id(); }
  ...
  void RenderFrameHostImpl::StartBackForwardCacheEvictionTimer() {
    ...
    back_forward_cache_eviction_timer_.Start(
        ...
        base::BindOnce(
            &RenderFrameHostImpl::EvictFromBackForwardCacheWithReason,
            weak_ptr_factory_.GetWeakPtr(),
  ...
  void RenderFrameHostImpl::ForwardMessageFromHost(
      blink::TransferableMessage message, const url::Origin& source_origin) {
  ...
  RenderFrameHostImpl* RenderFrameHostImpl::GetParent() { return parent_; }
  ...
  bool RenderFrameHostImpl::IsDescendantOf(RenderFrameHost * ancestor) {
    if (!ancestor ||
        !static_cast<RenderFrameHostImpl*>(ancestor)->child_count())
      return false;
  ...
  RenderFrameHostImpl::GetNetworkIsolationKey() {
  ...
  bool RenderFrameHostImpl::CreateNetworkServiceDefaultFactory(
    ...
    // navigation about to commit, the params generated won't be correct. There
    // is no good way of fixing this before RenderDocumentHost (ie swapping
  ...
  std::unique_ptr<blink::PendingURLLoaderFactoryBundle> RenderFrameHostImpl::
      CreateSubresourceLoaderFactoriesForInitialEmptyDocument() {
    // This method should only be called (by
    // RenderViewHostImpl::CreateRenderView) when creating a new local main
    // frame in a Renderer process.
    DCHECK(!GetParent());
    ...
    switch (lifecycle_state()) {
      ...
      case RenderFrameHostImpl::LifecycleStateImpl::kPrerendering:
        ...
        // Therefore, after a NetworkService crash we need to push to the
        // renderer an updated factory.
  ...
  void
  RenderFrameHostImpl::MarkIsolatedWorldsAsRequiringSeparateURLLoaderFactory(
  ...
  void RenderFrameHostImpl::ExecuteJavaScriptMethod(
      const std::u16string& object_name, const std::u16string& method_name,
      base::Value arguments, JavaScriptResultCallback callback) {
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  void RenderFrameHostImpl::ExecuteJavaScript(
      const std::u16string& javascript, JavaScriptResultCallback callback) {
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
    CHECK(CanExecuteJavaScript());

    const bool wants_result = !callback.is_null();
    GetAssociatedLocalFrame()->JavaScriptExecuteRequest(
        javascript, wants_result, std::move(callback));
  }
  ...
  void RenderFrameHostImpl::ExecuteJavaScriptWithUserGestureForTests(
      const std::u16string& javascript, int32_t world_id) {
    DCHECK_CURRENTLY_ON(BrowserThread::UI);
  ...
  RenderFrameHostImpl::GetRemoteAssociatedInterfaces() {
    if (!remote_associated_interfaces_) {
      ...
      if (GetAgentSchedulingGroup().GetChannel()) {
        ...
        // The channel may not be initialized in some tests environments. In
        // this case we set up a dummy interface provider.
        ignore_result(
  ...
  PageVisibilityState RenderFrameHostImpl::GetVisibilityState() {
    ...
    GetContentClient()->browser()->OverridePageVisibilityState(
  ...
  bool RenderFrameHostImpl::OnMessageReceived(const IPC::Message& msg) {
    ...
    // Crash reports triggered by IPC messages for this frame should be
    // associated with its URL.
  ...
  void RenderFrameHostImpl::OnAssociatedInterfaceRequest(
    ...
    // `associated_registry_`. This is reset in InvalidateMojoConnection(),
    // which means we want to stop receiving messages on behalf of the frame.
    // Ignoring this request sounded like the right way to handle this.
  ...
  void RenderFrameHostImpl::AccessibilityFatalError() {
    ...
    if (accessibility_fatal_error_count_ > max_accessibility_resets_) {
      // This will both create an "Aw Snap..." and generate a second crash
      // report in addition to the DumpWithoutCrashing() for the first reset.
  ...
  RenderFrameHostImpl::AccessibilityGetNativeViewAccessibleForWindow() {
    ...
    // the frame to avoid ignoring any accessibility related events which are
    // not expected.
    if (IsInactiveAndDisallowActivation())
      return nullptr;
  ...
  void RenderFrameHostImpl::AccessibilityHitTest(
      ...
      ax::mojom::Event opt_event_to_fire, int opt_request_id,
  ...
  void RenderFrameHostImpl::InitializePolicyContainerHost(
    ...
    // The initial empty document inherits its policy container from its
    // creator. The creator is either its parent for iframes or its opener for
    // new windows.
    //
    // Note 1: For normal document created from a navigation, the policy
    // container is computed from the NavigationRequest and assigned in
  ...
  void RenderFrameHostImpl::RenderProcessExited(
      RenderProcessHost * host, const ChildProcessTerminationInfo& info) {
    ...
    // When a frame's process dies, its RenderFrame no longer exists, which
    // means that its child frames must be cleaned up as well.
    ResetChildren();

    // Reset state for the current RenderFrameHost once the FrameTreeNode has
    // been reset.
    RenderFrameDeleted();
    ...
    if (IsPendingDeletion()) {
      // If the process has died, we don't need to wait for the ACK. Complete
      // the deletion immediately.
      SetLifecycleState(LifecycleStateImpl::kReadyToBeDeleted);
      DCHECK(children_.empty());
      PendingDeletionCheckCompleted();
      // |this| is deleted. Don't add any more code at this point in the
      // function.
      return;
    }
  ...
  bool RenderFrameHostImpl::CreateRenderFrame(
      ...
      int parent_routing_id, int previous_sibling_routing_id) {
    TRACE_EVENT0("navigation", "RenderFrameHostImpl::CreateRenderFrame");
    DCHECK(!IsRenderFrameLive()) << "Creating frame twice";

    // The process may (if we're sharing a process with another host that
    // already initialized it) or may not (we have our own process or the old
    // process crashed) have been initialized. Calling Init() multiple times
    // will be ignored, so this is safe.
    ...
    // Normally, the replication state contains effective frame policy,
    // excluding sandbox flags and permissions policy attributes that were
    // updated but have not taken effect. However, a new RenderFrame should use
    // the pending frame policy, since it is being created as part of the
    // navigation that will commit it. (I.e., the RenderFrame needs to know the
    // policy to use when initializing the new document once it commits).
    ...
    // https://crbug.com/1006814. The renderer needs at least one of these IDs
    ...
    if (previous_routing_id != MSG_ROUTING_NONE) {
      ...
      // remember that.
      //
      // RenderDocument: |proxy| can be null, when |previous_routing_id| refers
      // to a RenderFrameHost instead of a RenderFrameProxy.
      if (proxy)
        proxy->SetRenderFrameProxyCreated(true);
    }

    // The renderer now has a RenderFrame for this RenderFrameHost.  Note that
    // this path is only used for out-of-process iframes.  Main frame
    // RenderFrames are created with their RenderView, and same-site iframes are
    // created at the time of OnCreateChildFrame.
    RenderFrameCreated();
  ...
  void RenderFrameHostImpl::DeleteRenderFrame(
    ...
    if (IsRenderFrameCreated()) {
      ...
      if (!frame_tree_node_->IsMainFrame() && IsCurrent()) {
        ...
        // Documents from the page in the BackForwardCache don't run their
        // unload handlers, even if they have one. As a result, this should
        ...
        // down immediately in the case where this is the last active frame in
        // the process. See https://crbug.com/852204.
  ...
  void RenderFrameHostImpl::RenderFrameCreated() {
    // In https://crbug.com/1146573 a WebContentsObserver was causing the frame
    // to be reinitialized during deletion. It is not valid to re-enter
    // navigation code like that and it led to an invalid state. This is not a
    // DCHECK because the corruption will not be visible until later, making the
    // bug very difficult to understand.
    CHECK_NE(render_frame_state_, RenderFrameState::kDeleting);
    // We should not create new RenderFrames while our delegate is being
    ...
    // Set up mojo connections to the renderer from the `frame_` connection
  ...
  void RenderFrameHostImpl::RenderFrameDeleted() {
    ...
    // to be reinitialized during deletion. It is not valid to re-enter
    // navigation code like that and it led to an invalid state. This is not a
    // DCHECK because the corruption will cause a crash but later, making the
    // bug very difficult to understand.
    CHECK_NE(render_frame_state_, RenderFrameState::kDeleting);
  ...
  void RenderFrameHostImpl::Init() {
    // This is only called on the main frame, for renderer-created windows.
    // These windows wait for the renderer to signal that we can show them and
    // begin navigations.
  ...
  void RenderFrameHostImpl::PropagateEmbeddingTokenToParentFrame() {
    ...
    // token gets set. It's safe to early return in those cases since this
    // method will get called anyway by RFHI::SetEmbeddingToken(), at which time
    // the outer delegate for the inner web contents will have been created
    // already.
    if (!embedding_token_)
      return;

    // We need to propagate the token to the parent frame if it's either remote
    // or part of an outer web contents, therefore we need to figure out the
    // right proxy to send the token to, if any. For local parents the
    // propagation occurs within the renderer process. The token is also present
    // on the main frame for generalization when the main frame in embedded in
    // another context (e.g. browser UI). The main frame is not embedded in the
    // context of the frame tree so it is not propagated here. See
    // RenderFrameHost::GetEmbeddingToken for more details.
  ...
  void RenderFrameHostImpl::DidAddMessageToConsole(
      ...
      const std::u16string& message, int32_t line_no,
    ...
    // Pass through log severity only on builtin components pages to limit
    // console spew.
  ...
  void RenderFrameHostImpl::OnCreateChildFrame(
      ...
      blink::mojom::TreeScopeType scope, const std::string& frame_name,
      const std::string& frame_unique_name, bool is_created_by_script,
      const blink::LocalFrameToken& frame_token,
    ...
    if (owner_type == blink::mojom::FrameOwnerElementType::kNone) {
      // Any child frame must have a HTMLFrameOwnerElement in its parent
      // document and therefore the corresponding type of kNone (specific to
      // main frames) is invalid.
    ...
    // a child, but by the time we get here, it's possible for its process to
    // have disconnected (maybe due to browser shutdown). Ignore such messages.
    if (!is_render_frame_created())
      return;
    ...
    if (lifecycle_state() != LifecycleStateImpl::kPrerendering) {
      // The RenderFrame corresponding to this host sent an IPC message to
      // create a child, but by the time we get here, it's possible for the
  ...
  void RenderFrameHostImpl::DidNavigate(
      const mojom::DidCommitProvisionalLoadParams& params,
      NavigationRequest* navigation_request, bool did_create_new_document) {
    ...
    // stay correct even if the render_frame_host later becomes pending
    // deletion. The URL is set regardless of whether it's for a net error or
    // not.
    ...
    // TODO(https://crbug.com/1164508): Remove this check once origin is
    // computed correctly by the browser side.
    ...
    // (see https://crbug.com/560511).  Instead, the next cross-process
    // navigation or transfer should decide whether to swap as if the net error
    // had not occurred.
    ...
    // the NavigationRequest instead. The browser process doesn't need to rely
    ...
    if (!navigation_request->IsSameDocument()) {
      ...
      // frame owns both parent's required document policy and child frame's
      // frame owner element which contains child's required document policy, so
      // there is no need to store required document policy in proxies.
    }
  }
  ...
  const url::Origin& RenderFrameHostImpl::ComputeTopFrameOrigin(
    ...
    // frame_free_->root() here in case we're in process of being deleted, as
    // the latter might point to what our ancestor is being replaced with rather
    // than the actual ancestor.
  ...
  net::IsolationInfo RenderFrameHostImpl::ComputeIsolationInfoInternal(
    ...
    for (const RenderFrameHostImpl* rfh = initial_rfh; rfh;
         rfh = rfh->parent_) {
    ...
    // Reset the SiteForCookies if the top frame origin is of a scheme that
    // should always be treated as the SiteForCookies.
    if (GetContentClient()
            ->browser()
  ...
  void RenderFrameHostImpl::SetOriginDependentStateOfNewFrame(
    ...
    // This method should only be called for *new* frames, that haven't
    ...
    if (GetContentClient()
            ->browser()
            ->ShouldAllowInsecurePrivateNetworkRequests(GetBrowserContext(),
                                                        new_frame_origin)) {
    ...
    // permissions policy's state depends on the origin, so the
    // PermissionsPolicy object could be configured incorrectly if it were
    // initialized before knowing the value of |last_committed_origin_|. More at
    // crbug.com/1112959.
    ResetPermissionsPolicy();
  }

  FrameTreeNode* RenderFrameHostImpl::AddChild(
      std::unique_ptr<FrameTreeNode> child, int process_id,
    ...
    // frames in the same SiteInstance as the current frame, and they can swap
    // to a different one if they navigate away.
    ...
    // initial empty document should inherit the origin of its parent (the
    // origin may change after the first commit). See also
  ...
  void RenderFrameHostImpl::Detach() {
    ...
    //   detached (i.e., the Frame in the renderer with a provisional_frame_
    //   field that points to `this`'s LocalFrame). We don't expect it to
    //   self-detach otherwise.
    // - a pending commit RenderFrameHost might detach itself due to unload
    // events
    ...
    // StartPendingDeletionOnSubtree() will
    // ResetNavigationsForPendingDeletion(), which deletes `this`.
    ...
    // A frame is removed while replacing this document with the new one. When
    // it happens, delete the frame and both the new and old documents. Unload
  ...
  void RenderFrameHostImpl::DidCallFocus() { delegate_->DidCallFocus(); }

  void RenderFrameHostImpl::CancelInitialHistoryLoad() {
    // A Javascript navigation interrupted the initial history load.  Check if
    // an initial subframe cross-process navigation needs to be canceled as a
    // result.
  ...
  void RenderFrameHostImpl::DidCommitBackForwardCacheNavigation(
    ...
    std::unique_ptr<NavigationRequest> owned_request =
  ...
  void RenderFrameHostImpl::DidCommitSameDocumentNavigation(
    ...
    // When the frame is pending deletion, the browser is waiting for it to
    // unload properly. In the meantime, because of race conditions, it might
    // tries to commit a same-document navigation before unloading. Similarly to
    // what is done with cross-document navigations, such navigation are
    // ignored. The browser already committed to destroying this
    // RenderFrameHost. See https://crbug.com/805705 and
    // https://crbug.com/930132.
  ...
  void RenderFrameHostImpl::Unload(RenderFrameProxyHost * proxy,
                                   bool is_loading) {
    // The end of this event is in OnUnloadACK when the RenderFrame has
    // completed the operation and sends back an IPC message.
    TRACE_EVENT_NESTABLE_ASYNC_BEGIN1(
        "navigation", "RenderFrameHostImpl::Unload", TRACE_ID_LOCAL(this),
        "frame_tree_node", frame_tree_node_->frame_tree_node_id());
    ...
    if (proxy) {
      ...
      if (IsRenderFrameCreated()) {
        GetMojomFrameInRenderer()->Unload(
            ...
            proxy->GetFrameToken());
        // Remember that a RenderFrameProxy was created as part of processing
        // the Unload message above.
        proxy->SetRenderFrameProxyCreated(true);
      }
    } else {
      // RenderDocument: After a local<->local swap, this function is called
      // with a null |proxy|.
  ...
  void RenderFrameHostImpl::SwapOuterDelegateFrame(RenderFrameProxyHost *
                                                   proxy) {
    GetMojomFrameInRenderer()->Unload(
  ...
  void RenderFrameHostImpl::ProcessBeforeUnloadCompleted(
    ...
    // If this renderer navigated while the beforeunload request was in flight,
    // we may have cleared this state in DidCommitProvisionalLoad, in which case
    // we can ignore this message. However renderer might also be swapped out
    // but we still want to proceed with navigation, otherwise it would block
    // future navigations. This can happen when pending cross-site navigation is
    // canceled by a second one just before DidCommitProvisionalLoad while
    // current RVH is waiting for commit but second navigation is started from
    // the beginning.
  ...
  void RenderFrameHostImpl::ProcessBeforeUnloadCompletedFromFrame(
      bool proceed, bool treat_as_final_completion_callback,
      RenderFrameHostImpl* frame, bool is_frame_being_destroyed,
      const base::TimeTicks& renderer_before_unload_start_time,
      const base::TimeTicks& renderer_before_unload_end_time) {
    // Check if we need to wait for more beforeunload completion callbacks. If
    // |proceed| is false, we know the navigation or window close will be
    // aborted, so we don't need to wait for beforeunload completion callbacks
    // from any other frames. |treat_as_final_completion_callback| also
    // indicates that we shouldn't wait for any other ACKs (e.g., when a
    // beforeunload timeout fires).
    ...
    if (!renderer_before_unload_start_time.is_null() &&
      ...
      if (!base::TimeTicks::IsConsistentAcrossProcesses()) {
        ...
        blink::InterProcessTimeTicksConverter converter(
            blink::LocalTimeTicks::FromTimeTicks(
                send_before_unload_start_time_),
    ...
    base::OnceClosure task = base::BindOnce(
        [](base::WeakPtr<RenderFrameHostImpl> self,
          ...
          // If the ACK is for a navigation, send it to the Navigator to have
          // the current navigation stop/proceed. Otherwise, send it to the
    ...
    // This is usually redundant with the dialog closure code in
    // WebContentsImpl's OnDialogClosed, but there may be some cases that Blink
    // returns !proceed without showing the dialog. We also update the address
    // bar here to be safe.
    if (!proceed)
  ...
  void RenderFrameHostImpl::OnUnloadACK() {
    ...
    if (frame_tree_node_->render_manager()->is_attaching_inner_delegate()) {
      ...
      // will stay around but it will no longer be associated with a
      // RenderFrame.
      RenderFrameDeleted();
      return;
    }
  ...
  void RenderFrameHostImpl::RequestSmartClipExtract(
      ExtractSmartClipDataCallback callback, gfx::Rect rect) {
  ...
  void RenderFrameHostImpl::OnSmartClipDataExtracted(
      int32_t callback_id, const std::u16string& text,
      const std::u16string& html, const gfx::Rect& clip_rect) {
  ...
  void RenderFrameHostImpl::RunModalPromptDialog(
      const std::u16string& alert_message, const std::u16string& default_value,
  ...
  void RenderFrameHostImpl::RunJavaScriptDialog(
      const std::u16string& message, const std::u16string& default_prompt,
  ...
  void RenderFrameHostImpl::RunBeforeUnloadConfirm(
      bool is_reload, RunBeforeUnloadConfirmCallback ipc_response_callback) {
    ...
    if (beforeunload_initiator) {
      ...
      // before-unload type wants to ignore it, then short-circuit the request
      // and respond as if the user decided to stay on the page, canceling the
      // unload.
  ...
  void RenderFrameHostImpl::FocusPage() { render_view_host_->OnFocus(); }
  ...
  void RenderFrameHostImpl::ShowCreatedWindow(
      ...
      bool user_gesture, ShowCreatedWindowCallback callback) {
  ...
  void RenderFrameHostImpl::DownloadURL(
    ...
    std::unique_ptr<download::DownloadUrlParameters> parameters(
        new download::DownloadUrlParameters(
            blink_parameters->url, GetProcess()->GetID(), GetRoutingID(),
            traffic_annotation));
  ...
  void RenderFrameHostImpl::ReportNoBinderForInterface(
  ...
  WebUI* RenderFrameHostImpl::GetWebUI() { return web_ui(); }
  ...
  int RenderFrameHostImpl::GetEnabledBindings() { return enabled_bindings_; }
  ...
  void RenderFrameHostImpl::SetWebUIProperty(const std::string& name,
    ...
    // This is a sanity check before telling the renderer to enable the
    // property. It could lie and send the corresponding IPC messages anyway,
    // but we will not act on them if enabled_bindings_ doesn't agree. If we get
    // here without WebUI bindings, terminate the renderer process.
    if (enabled_bindings_ & BINDINGS_POLICY_WEB_UI)
      web_ui_->SetProperty(name, value);
    else
      ReceivedBadMessage(GetProcess(),
                         bad_message::RVH_WEB_UI_BINDINGS_MISMATCH);
  }
  ...
  bool RenderFrameHostImpl::IsFeatureEnabled(
    ...
    return permissions_policy_ &&
           permissions_policy_->IsFeatureEnabledForOrigin(
               feature, GetLastCommittedOrigin());
  }

  void RenderFrameHostImpl::ViewSource() { delegate_->ViewSource(this); }
  ...
  // NetworkService-based factory to the renderer process.  It may also be
  // called when DevTools wants to send to the renderer process a fresh factory
  // bundle (e.g. after injecting DevToolsURLLoaderInterceptor) - the latter
  // scenario may happen even if `this` RenderFrameHostImpl has not pushed any
  // NetworkService factories to the renderer process (DevTools is agnostic to
  // this).
  void RenderFrameHostImpl::UpdateSubresourceLoaderFactories() {
    NavigationRequest* latest_nav_request_still_committing =
        FindLatestNavigationRequestThatIsStillCommitting();
    mojo::PendingRemote<network::mojom::URLLoaderFactory>
        default_factory_remote;
    ...
    if (recreate_default_url_loader_factory_after_network_service_crash_) {
      ...
      bypass_redirect_checks = CreateNetworkServiceDefaultFactoryAndObserve(
          ...
          ukm::SourceIdObj::FromInt64(latest_nav_request_still_committing
                                          ? latest_nav_request_still_committing
  ...
  RenderFrameHostImpl* RenderFrameHostImpl::FindAndVerifyChild(
      int32_t child_frame_routing_id, bad_message::BadMessageReason reason) {
  ...
  RenderFrameHostImpl* RenderFrameHostImpl::FindAndVerifyChildInternal(
    ...
    if (child_frame_or_proxy.GetFrameTreeNode()->frame_tree() != frame_tree()) {
      ...
      // TODO(altimin, lfg): Reconsider what the correct behaviour here should
      // be.
      return nullptr;
    }
  ...
  void RenderFrameHostImpl::UpdateEncoding(const std::string& encoding_name) {
    // This message is only sent for top-level frames. TODO(avi): when frame
    // tree mirroring works correctly, add a check here to enforce it.
  ...
  void RenderFrameHostImpl::FullscreenStateChanged(
      bool is_fullscreen, blink::mojom::FullscreenOptionsPtr options) {
    if (IsInactiveAndDisallowActivation())
      return;
    delegate_->FullscreenStateChanged(this, is_fullscreen, std::move(options));
  }

  void RenderFrameHostImpl::RegisterProtocolHandler(
      const std::string& scheme, const GURL& url, bool user_gesture) {
    delegate_->RegisterProtocolHandler(this, scheme, url, user_gesture);
  }

  void RenderFrameHostImpl::UnregisterProtocolHandler(
      const std::string& scheme, const GURL& url, bool user_gesture) {
  ...
  void RenderFrameHostImpl::NotifyVirtualKeyboardOverlayRect(
    ...
    if (!keyboard_rect.IsEmpty()) {
      ...
      root_widget_rect = gfx::Rect(
          root_widget_origin.x(), root_widget_origin.y(),
          view->GetViewBounds().width(), view->GetViewBounds().height());
    ...
    // Notify each SiteInstance a single time. Renderer will take care of
    // ensuring the event is dispatched to all relevant listeners in the
    // grouping of frames for the SiteInstance.
  ...
  void RenderFrameHostImpl::DidBlockNavigation(
      const GURL& blocked_url, const GURL& initiator_url,
  ...
  void RenderFrameHostImpl::GoToEntryAtOffset(int32_t offset,
    ...
    if (delegate_->IsAllowedToGoToEntryAtOffset(offset)) {
      if (IsSandboxed(network::mojom::WebSandboxFlags::kTopNavigation)) {
        // Keep track of whether this is a session history from a sandboxed
        // iframe with top level navigation disallowed.
  ...
  void RenderFrameHostImpl::HandleAccessibilityFindInPageTermination() {
    // Only update FindInPageTermination on active RenderFrameHost. Note that,
    // it is safe to ignore this call for BackForwardCache, as we terminate the
    // FindInPage session once the page enters BackForwardCache.
    if (lifecycle_state() != LifecycleStateImpl::kActive)
      return;
  ...
  void RenderFrameHostImpl::UpdateBrowserControlsState(
      cc::BrowserControlsState constraints, cc::BrowserControlsState current,
      bool animate) {
  ...
  bool RenderFrameHostImpl::IsInactiveAndDisallowActivation() {
    ...
    switch (lifecycle_state_) {
      ...
      case LifecycleStateImpl::kPendingCommit:
        ...
        // generate events that require an active/inactive check. Don't crash
        // the browser process in case it comes from a compromised renderer, but
        // kill the renderer to avoid further confusion.
  ...
  void RenderFrameHostImpl::EvictFromBackForwardCacheWithReasons(
    ...
    // TODO(hajimehoshi): Record the 'race condition' by JavaScript execution
    // when |in_back_forward_cache| is false.
    ...
    if (!in_back_forward_cache) {
      ...
      // https://crrev.com/c/2352815. Lets keep this old code for now just in
      // case and replace with a CHECK once we are confident that is the case.
      base::debug::DumpWithoutCrashing();
      BackForwardCacheMetrics::RecordEvictedAfterDocumentRestored(
          BackForwardCacheMetrics::EvictedAfterDocumentRestoredReason::
              kByJavaScript);

      // A document is evicted from the BackForwardCache, but it has already
      // been restored. The current document should be reloaded, because it is
      // not salvageable.
  ...
  bool RenderFrameHostImpl::HasSeenRecentXrOverlaySetup() {
    ...
    base::TimeDelta delta =
        base::TimeTicks::Now() - last_xr_overlay_setup_time_;
  ...
  void RenderFrameHostImpl::EnterFullscreen(
    ...
    // side when the renderer is compromised and the fullscreen request is
    // denied. Fullscreen can only be triggered by: a user activation, a
    // user-generated screen orientation change, or another feature-specific
    // transient allowance. CanEnterFullscreenWithoutUserActivation is only ever
    // true in tests, to allow fullscreen when mocking screen orientation
    // changes.
    ...
    // appropriate ancestor <iframe> elements, fire fullscreenchange events,
    // etc. Thus, walk through the ancestor chain of this frame and for each
    // (parent, child) pair, send a message about the pending fullscreen change
    // to the child's proxy in parent's SiteInstance. The renderer process will
    // use this to find the <iframe> element in the parent frame that will need
    // fullscreen styles. This is done at most once per SiteInstance: for
  ...
  void RenderFrameHostImpl::SuddenTerminationDisablerChanged(
      bool present, blink::mojom::SuddenTerminationDisablerType disabler_type) {
    switch (disabler_type) {
      ...
      case blink::mojom::SuddenTerminationDisablerType::kUnloadHandler:
        ...
        break;
      case blink::mojom::SuddenTerminationDisablerType::
  ...
  void RenderFrameHostImpl::FocusedElementChanged(
      bool is_editable_element, const gfx::Rect& bounds_in_frame_widget,
  ...
  void RenderFrameHostImpl::TextSelectionChanged(
      const std::u16string& text, uint32_t offset, const gfx::Range& range) {
  ...
  void RenderFrameHostImpl::MaybeIsolateForUserActivation() {
    ...
    // isolation hint based on the Cross-Origin-Opener-Policy header, isolate
    // the corresponding site for all future BrowsingInstances.  We also do this
    // for user activation on any same-origin subframe of such COOP main frames.
    //
    // Note that without user activation, COOP-triggered site isolation is
    // scoped only to the current BrowsingInstance.  This prevents malicious
    // sites from silently loading COOP sites to put them on the isolation list
    // and later querying that state.
    if (GetMainFrame()
            ->GetSiteInstance()
            ->GetSiteInfo()
      ...
      bool is_same_origin_activation =
          GetParent()
              ? GetMainFrame()->GetLastCommittedOrigin().IsSameOriginWith(
                    GetLastCommittedOrigin())
              : true;
  ...
  void RenderFrameHostImpl::UpdateUserActivationState(
    ...
    // of BackForwardCache, this is only called for tests and it is safe to
  ...
  void RenderFrameHostImpl::RenderFallbackContentInParentProcess() {
    bool is_object_type = frame_tree_node()
    ...
    // a frame or a proxy. When navigating cross-site from a frame which is
    // same- site with its parent, the frame is still local (e.g., about:blank).
  ...
  void RenderFrameHostImpl::ShowPopupMenu(
      ...
      const gfx::Rect& bounds, int32_t item_height, double font_size,
      int32_t selected_item, std::vector<blink::mojom::MenuItemPtr> menu_items,
      bool right_aligned, bool allow_multiple_selection) {
  ...
  void RenderFrameHostImpl::DidLoadResourceFromMemoryCache(
      const GURL& url, const std::string& http_method,
  ...
  void RenderFrameHostImpl::DidChangeCSPAttribute(
    ...
    child->frame_tree_node()->set_csp_attribute(
        std::move(parsed_csp_attribute));
  }

  void RenderFrameHostImpl::DidChangeFramePolicy(
      const blink::FrameToken& child_frame_token,
      const blink::FramePolicy& frame_policy) {
    // Ensure that a frame can only update sandbox flags or permissions policy
    // for its immediate children.  If this is not the case, the renderer is
    ...
    // the updated flags and policy, but these notifications are sent later in
  ...
  void RenderFrameHostImpl::CapturePaintPreviewOfSubframe(
      const gfx::Rect& clip_rect, const base::UnguessableToken& guid) {
    if (IsInactiveAndDisallowActivation())
      return;
    ...
    delegate()->CapturePaintPreviewOfCrossProcessSubframe(clip_rect, guid,
                                                          this);
  }
  ...
  void RenderFrameHostImpl::BindDomOperationControllerHostReceiver(
      mojo::PendingAssociatedReceiver<mojom::DomAutomationControllerHost>
          receiver) {
    DCHECK(receiver.is_valid());
    // DOM automation controller is reinstalled after a cross-document
    // navigation, which can reuse the frame.
  ...
  void RenderFrameHostImpl::DomOperationResponse(
  ...
  RenderFrameHostImpl::CreateCrossOriginPrefetchLoaderFactoryBundle() {
    ...
    bool bypass_redirect_checks = false;
    // Passing an empty IsolationInfo ensures the factory is not initialized
    // with a IsolationInfo. This is necessary for a cross-origin prefetch
    // factory because the factory must use the value provided by requests going
    // through it.
    bypass_redirect_checks = CreateNetworkServiceDefaultFactoryAndObserve(
        std::move(factory_params),
        // This is for renderer prefetches, so it's associated with this page,
        // not pending navigation.
        ukm::SourceIdObj::FromInt64(GetPageUkmSourceId()),
  ...
  void RenderFrameHostImpl::CreateNewWindow(
    ...
    TRACE_EVENT2("navigation", "RenderFrameHostImpl::CreateNewWindow",
                 "frame_tree_node", frame_tree_node_->frame_tree_node_id(),
                 "url", params->target_url.possibly_invalid_spec());
    ...
    if (!GetOrCreateWebPreferences().supports_multiple_windows) {
      // See crbug.com/1083819, we should ignore if the URL is javascript:
      // scheme, previously we already filtered out javascript: scheme and
      // replace the URL with |kBlockedURL|, so we check against |kBlockedURL|
      // here.
    ...
    // The initial empty document in the popup inherits the COEP of its opener
    // (if any).
    ...
    // initial empty document should inherit the origin of its opener (the
    // origin may change after the first commit). See also
    // https://crbug.com/932067.
    //
    ...
    // If inheriting coop (checking this via |opener_suppressed|) and the
    // original coop page has a reporter we make sure the the newly created
    // popup also has a reporter.
    ...
    // because RenderView is already created on the renderer side. Thus we need
    ...
    main_frame->render_view_host()->BindPageBroadcast(
    ...
    // this is the first message received by the remote frame (instead of
    // plumbing it with the CreateNewWindow IPC).
  ...
  void RenderFrameHostImpl::CreatePortal(
    ...
    // Note that we don't check |GetLastCommittedOrigin|, since that is
    // inherited by the initial empty document of a new frame.
    ...
    // Since the portal is newly created and has yet to commit a navigation,
  ...
  void RenderFrameHostImpl::CreateNewPopupWidget(
    ...
    // The renderer-owned widget was created before sending the IPC received
    // here.
  ...
  void RenderFrameHostImpl::BeginNavigation(
    ...
    if (frame_tree_node_->render_manager()->is_attaching_inner_delegate()) {
      // Avoid starting any new navigations since this frame is in the process
      // of attaching an inner delegate.
      return;
    }
    ...
    // blob URL, reset the blob_url_token to prevent hitting the
    // ReportBadMessage below, and to make sure we don't incorrectly try to use
    // the blob_url_token.
    ...
    // If the request was for a blob URL, but no blob_url_token was passed in
    // this is not necessarily an error. Renderer initiated reloads for example
    // are one situation where a renderer currently doesn't have an easy way of
    // resolving the blob URL. For those situations resolve the blob URL here,
    // as we don't care about ordering with other blob URL manipulation anyway.
    ...
    // |initiator_policy_container_host_keep_alive_handle|. This is just needed
    // to ensure that the PolicyContainerHost of the initiator RenderFrameHost
  ...
  void RenderFrameHostImpl::ResourceLoadComplete(
    ...
    const bool is_frame_request = blink::IsRequestDestinationFrame(
  ...
  void RenderFrameHostImpl::HandleAXEvents(
      ...
      const std::vector<ui::AXEvent>& events, int32_t reset_token,
      HandleAXEventsCallback callback) {
    // Don't process this IPC if either we're waiting on a reset and this IPC
    // doesn't have the matching token ID, or if we're not waiting on a reset
    // but this message includes a reset token.
  ...
  void RenderFrameHostImpl::ResetWaitingState() {
    ...
    // Whenever we reset the RFH state, we should not be waiting for
    // beforeunload or close acks.  We clear them here to be safe, since they
    // can cause navigations to be ignored in DidCommitProvisionalLoad.
  ...
  CanCommitStatus RenderFrameHostImpl::CanCommitOriginAndUrl(
    ...
    // URL which is not allowed in a WebUI process. As we are at the commit
    // stage, set |origin_isolation_request| to kNone.
    ...
    // (e.g. "http://localhost"). In such cases, don't verify the URL, but
    ...
    if (origin_tuple_or_precursor_tuple.IsValid()) {
      ...
      // some opaque origin precursors, like chrome-extension schemes, can
      // trigger the commit to fail. These need to be investigated.
  ...
  void RenderFrameHostImpl::DispatchBeforeUnload(BeforeUnloadType type,
    ...
    if (!for_navigation) {
      ...
      // event from wiping out the is_waiting_for_beforeunload_completion_
      // state.
    ...
    if (!ShouldDispatchBeforeUnload(check_subframes_only)) {
      ...
      // is checked earlier and we would only get here if it had already
      // returned true.
    ...
    // This may be called more than once (if the user clicks the tab close
    // button several times, or if they click the tab close button then the
    // browser close button), and we only send the message once.
    if (is_waiting_for_beforeunload_completion_) {
      ...
      // (if there was a cross-site "close" request pending when the user
      // clicked the close button). We want to keep the "for cross site" flag
      // only if both the old and the new ones are also for cross site.
      unload_ack_is_for_navigation_ =
      ...
      if (render_view_host_->GetDelegate()->IsJavaScriptDialogShowing()) {
        ...
        // sure to reply with |proceed = false|, because the presence of a
        // dialog indicates that the page can't be discarded.
  ...
  bool RenderFrameHostImpl::CheckOrDispatchBeforeUnloadForSubtree(
      bool subframes_only, bool send_ipc, bool is_reload) {
    ...
    for (FrameTreeNode* node : frame_tree()->SubtreeNodes(frame_tree_node_)) {
      ...
      // this|. In the future, this condition should be removed, and
      // beforeunload should only be sent when a handler is registered.  For
      // subframes of a navigating/closing frame, this assumption was never
      // present, so subframes are included only if they have registered a
      ...
      // handler, ask its local root to run it. If we've already sent the
      // message to that local root, skip this frame. For example, in A1(A2,A3),
      // if A2 and A3 contain beforeunload handlers, and all three frames are
  ...
  void RenderFrameHostImpl::StartPendingDeletionOnSubtree() {
    ...
    for (std::unique_ptr<FrameTreeNode>& child_frame : children_) {
      for (FrameTreeNode* node :
           frame_tree()->SubtreeNodes(child_frame.get())) {
        ...
        if (local_ancestor != child) {
          // In case of BackForwardCache, page is evicted directly from the
          // cache and deleted immediately, without waiting for unload handlers.
          bool wait_for_unload_handlers =
  ...
  void RenderFrameHostImpl::JavaScriptDialogClosed(
      JavaScriptDialogCallback dialog_closed_callback, bool success,
    ...
    // timers stopped in this frame or a frame up in the frame hierarchy.
    // Restart any timers that were stopped in OnRunBeforeUnloadConfirm().
    for (RenderFrameHostImpl* frame = this; frame; frame = frame->GetParent()) {
      if (frame->is_waiting_for_beforeunload_completion_ &&
  ...
  bool
  RenderFrameHostImpl::ShouldDispatchPagehideAndVisibilitychangeDuringCommit(
      RenderFrameHostImpl * old_frame_host, const UrlInfo& dest_url_info) {
  ...
  void RenderFrameHostImpl::CommitNavigation(
    ...
    if (is_srcdoc) {
      ...
      // a navigation attempt is made. It shouldn't reach CommitNavigation.
    ...
    // If this is an attempt to commit a URL in an incompatible process, capture
    // a crash dump to diagnose why it is occurring.
    ...
    if (!process_lock.is_error_page() && common_params->url.IsStandard() &&
      ...
      base::debug::SetCrashKeyString(
          base::debug::AllocateCrashKeyString(
              "commit_origin", base::debug::CrashKeySize::Size64),
    ...
    if (!is_same_document) {
      ...
      // existing one should be kept around so that the subresource requests
      // keep being served from the WebBundleURLLoaderFactory held by the
      // handle.
      web_bundle_handle_ = std::move(web_bundle_handle);

      // Similarly, reset |subresource_web_bundle_navigation_info_| to the new
      // one if this is NOT for same-document navigation. For same-document
    ...
    // browser provides an empty response instead. See the DCHECK in the
    // beginning of this method for the edge cases where a response isn't
    // provided.
    ...
    if (navigation_request->appcache_handle()) {
      // AppCache may create a subresource URLLoaderFactory later, so make sure
      // it has the correct origin to use when calling
    ...
    ukm::SourceIdObj next_page_ukm_source_id = ukm::SourceIdObj::FromInt64(
    ...
    if ((!is_same_document || is_first_navigation) && !is_srcdoc) {
      ...
      if (subresource_loader_params &&
        ...
        mojo::Remote<network::mojom::URLLoaderFactory> appcache_remote(
            std::move(
                subresource_loader_params->pending_appcache_loader_factory));
      ...
      // Only file resources and about:blank with an initiator that can load
      // files can load file subresources.
      //
      ...
      if ((common_params->url.SchemeIsFile() ||
        ...
        // given by the doc comment for USER_BLOCKING: Loading and rendering a
        // web page after the user clicks a link.
      ...
      non_network_factories.emplace(
          ...
          CreateFileSystemURLLoaderFactory(GetProcess()->GetID(),
                                           GetFrameTreeNodeId(),
      ...
      for (auto& factory : non_network_factories) {
        ...
        subresource_loader_factories->pending_scheme_specific_factories()
            .emplace(factory.first, std::move(pending_factory_proxy));
      }
    ...
    if (is_same_document) {
      ...
      // about:srcdoc "inherits" loaders from its parent in the renderer
      // process, There are no need to provide new ones here.
      ...
      // TODO(crbug.com/1126305): Once the Prerender2 moves to use the MPArch,
      // we need to check the relevant FrameTree to know the precise
      // prerendering state to update commit_params.is_prerendering here.
      // Current design doesn't capture the cases NavigationRequest is created
      // via CreateRendererInitiated or CreateForCommit.
      ...
      // |remote_object| is an associated interface ptr, so calls can't be made
      // on it until its request endpoint is sent. Now that the request endpoint
      // was sent, it can be used, so add it to ServiceWorkerObjectHost.
  ...
  void RenderFrameHostImpl::FailedNavigation(
      ...
      const mojom::CommitNavigationParams& commit_params,
      bool has_stale_copy_in_cache, int error_code, int extended_error_code,
  ...
  void RenderFrameHostImpl::HandleRendererDebugURL(const GURL& url) {
    ...
    // renderer debug URL, since this process is now unsafe to be reused by
    // sites that require a dedicated process.  Usually this happens at
    // ready-to-commit (NavigationRequest::OnResponseStarted) time for regular
    // navigations, but renderer debug URLs don't go through that path.  This
    // matters for initial navigations to renderer debug URLs.  See
    // https://crbug.com/1074108.
  ...
  void RenderFrameHostImpl::SetUpMojoIfNeeded() {
    ...
    associated_registry_->AddInterface(base::BindRepeating(
        [](RenderFrameHostImpl* impl,
          ...
          impl->back_forward_cache_controller_host_associated_receiver_.Bind(
              std::move(receiver));
          impl->back_forward_cache_controller_host_associated_receiver_
              .SetFilter(CreateMessageFilterForAssociatedReceiverImpl(
    ...
    // Called to bind the receiver for this interface to the local frame. We
    // need to eagarly bind here because binding happens at normal priority on
    // the main thread and future calls to this interface need to be high
    // priority.
  ...
  void RenderFrameHostImpl::InvalidateMojoConnection() {
    ...
    // permission requests so they must be reset before the routing_id mapping
    // is removed.
  ...
  bool RenderFrameHostImpl::CreateWebUI(const GURL& dest_url,
    ...
    // If |web_ui_| already exists, there is no need to create a new one.
    // However, it is useful to verify that its type hasn't changed. Site
    // isolation guarantees that RenderFrameHostImpl will be changed if the
    // WebUI type differs.
    if (web_ui_) {
  ...
  bool RenderFrameHostImpl::IsSameSiteInstance(RenderFrameHostImpl *
                                               other_render_frame_host) {
    // As a sanity check, make sure the frame belongs to the same
    // BrowserContext.
  ...
  void RenderFrameHostImpl::RequestAXTreeSnapshot(
      AXTreeSnapshotCallback callback, const ui::AXMode& ax_mode,
      bool exclude_offscreen, size_t max_nodes,
      const base::TimeDelta& timeout) {
  ...
  RenderFrameHostImpl::GetLifecycleStateFromImpl(LifecycleStateImpl state) {
    switch (state) {
      case LifecycleStateImpl::kSpeculative:
  ...
  bool RenderFrameHostImpl::HasSelection() { return has_selection_; }
  ...
  void RenderFrameHostImpl::UpdatePermissionsForNavigation(
    ...
    // schemes, so do not grant them the ability to commit additional URLs.
    if (!GetProcess()->IsForGuestsOnly()) {
      ...
      if (NavigationRequest::IsLoadDataWithBaseURL(common_params)) {
        ...
        // schemes to be accepted at commit time and during CORS checks (e.g.,
        // for font requests).
        ChildProcessSecurityPolicyImpl::GetInstance()->GrantCommitURL(
  ...
  void RenderFrameHostImpl::NavigationRequestCancelled(NavigationRequest *
                                                       navigation_request) {
    // Remove the requests from the list of NavigationRequests waiting to
    // commit. RenderDocument should obsolete the need for this, as always
    // swapping RFHs means that it won't be necessary to clean up the list of
    // navigation requests when the renderer aborts a navigation--instead, we'll
    // always just throw away the entire speculative RFH.
  ...
  RenderFrameHostImpl::FindLatestNavigationRequestThatIsStillCommitting() {
    ...
    // possibly change the origin from |last_committed_origin_| to another
    // origin.
  ...
  RenderFrameHostImpl::CreateURLLoaderFactoryParamsForMainWorld(
      NavigationRequest * navigation_request, base::StringPiece debug_tag) {
    url::Origin main_world_origin;
    net::IsolationInfo isolation_info;
    network::mojom::ClientSecurityStatePtr client_security_state;
    ...
    return URLLoaderFactoryParamsHelper::CreateForFrame(
        this, main_world_origin, isolation_info,
        std::move(client_security_state), std::move(coep_reporter_remote),
        GetProcess(), trust_token_redemption_policy, debug_tag);
  }
  ...
  bool RenderFrameHostImpl::CreateNetworkServiceDefaultFactoryAndObserve(
    ...
    if (IsOutOfProcessNetworkService() &&
      ...
      // should only be used to monitor for Network Service crashes).  Below is
      // an attempt to enforce that the factory cannot be used in practice.
  ...
  void RenderFrameHostImpl::WillCreateURLLoaderFactory(
      ...
      mojo::PendingRemote<network::mojom::TrustedURLLoaderHeaderClient>*
          header_client,
      bool* bypass_redirect_checks, bool* disable_secure_dns,
  ...
  void RenderFrameHostImpl::AccessibilityHitTestCallback(
      int request_id, ax::mojom::Event event_to_fire,
      base::OnceCallback<void(BrowserAccessibilityManager * hit_manager,
                              int hit_node_id)> opt_callback,
    ...
    hit_frame->AccessibilityHitTest(
        hit_test_response->hit_frame_transformed_point, event_to_fire,
        request_id, std::move(opt_callback));
  }
  ...
  void RenderFrameHostImpl::DeleteWebBluetoothService(WebBluetoothServiceImpl *
  ...
  void RenderFrameHostImpl::CreateWebUsbService(
    ...
    GetContentClient()->browser()->CreateWebUsbService(this,
                                                       std::move(receiver));
  }
  ...
  void RenderFrameHostImpl::BindPrerenderProcessor(
    ...
    prerender_processor_receivers_.Add(
        std::make_unique<PrerenderProcessor>(*this),
  ...
  void RenderFrameHostImpl::CancelPrerendering(
    ...
    DCHECK(blink::features::IsPrerender2Enabled());
    // This function is called from MojoBinderPolicyApplier, which should only
    // be active during prerendering. It would be an error to call this while
    // not prerendering, as it could mean an interface request is never resolved
    // for an active page.
    ...
    // TODO(https://crbug.com/1126305): Pass a FinalStatus to
    // CancelPrerendering() method when MojoInterface control, or
    // IsInactiveAndDisallowActivation are called.
  ...
  void RenderFrameHostImpl::ActivateForPrerendering() {
    ...
    // TODO(https://crbug.com/1186796): Loosen the policies of the mojo
    // capability control during dispatching the prerenderingchange event in the
    // Blink.
  ...
  void RenderFrameHostImpl::CreateDedicatedWorkerHostFactory(
      mojo::PendingReceiver<blink::mojom::DedicatedWorkerHostFactory>
          receiver) {
    ...
    // When a dedicated worker is created from the frame script, the frame is
    // both the creator and the ancestor.
    mojo::MakeSelfOwnedReceiver(
  ...
  void RenderFrameHostImpl::BindSerialService(
    ...
    SerialService::GetOrCreateForCurrentDocument(this)->Bind(
        std::move(receiver));
  }
  ...
  void RenderFrameHostImpl::GetSpeechSynthesis(
    ...
    // Blocklist SpeechSynthesis for BackForwardCache, because currently we do
    // not handle speech synthesis after placing the page in BackForwardCache.
    // TODO(sreejakshetty): Make SpeechSynthesis compatible with
    // BackForwardCache.
    OnSchedulerTrackedFeatureUsed(
  ...
  void RenderFrameHostImpl::BindCacheStorage(
    ...
    GetProcess()->BindCacheStorage(
        cross_origin_embedder_policy_, std::move(coep_reporter_remote),
  ...
  void RenderFrameHostImpl::BindRestrictedCookieManager(
    ...
    static_cast<StoragePartitionImpl*>(GetProcess()->GetStoragePartition())
        ->CreateRestrictedCookieManager(
            ...
            /* is_service_worker = */ false, GetProcess()->GetID(),
            routing_id(), std::move(receiver), CreateCookieAccessObserver());
  }
  ...
  void RenderFrameHostImpl::BindHasTrustTokensAnswerer(
    ...
    // This is enforced in benign renderers by the RuntimeEnabled=TrustTokens
    // IDL attribute (the base::Feature's value is tied to the
  ...
  void RenderFrameHostImpl::CreatePermissionService(
    ...
    PermissionServiceContext::GetOrCreateForCurrentDocument(this)
  ...
  RenderFrameHostImpl::CreateNavigationRequestForCommit(
      ...
      blink::mojom::ReferrerPtr referrer, const ui::PageTransition& transition,
      bool should_replace_current_entry, const NavigationGesture& gesture,
      const std::vector<GURL>& redirects, const GURL& original_request_url,
      bool is_same_document, bool is_same_document_history_api_navigation) {
    // This function must only be called when there are no NavigationRequests
    // for a navigation can be found at DidCommit time, which can only happen in
    // two cases: 1) This was a renderer-initiated navigation to the initial
    // empty document. 2) This was a renderer-initiated same-document
    // navigation.
    DCHECK(IsInitialEmptyCommit(url,
    ...
    if (is_same_document && subresource_web_bundle_navigation_info_) {
      // Propagate |subresource_web_bundle_navigation_info_| to
      // NavigationRequest for same-document navigation. This will be passed to
    ...
    // HTTP status code:
    // - For same-document navigations, we should retain the HTTP status code
    // from the last committed navigation.
    ...
    bool is_overriding_user_agent =
        is_same_document && is_overriding_user_agent_;
  ...
  void RenderFrameHostImpl::SetLastCommittedSiteInfo(const GURL& url) {
    // Since |url| has already committed, |origin_isolation_request| below
  ...
  bool RenderFrameHostImpl::ShouldBypassSecurityChecksForErrorPage(
      NavigationRequest * navigation_request, bool* should_commit_error_page) {
    ...
    if (SiteIsolationPolicy::IsErrorPageIsolationEnabled(is_main_frame())) {
      if (GetSiteInstance()->GetSiteInfo().is_error_page()) {
        ...
        // With error page isolation, any URL can commit in an error page
        // process.
        return true;
      }
    } else {
  ...
  RenderFrameHostImpl::BuildClientSecurityState() const {
    ...
    //
    // Due to a race condition, CreateCrossOriginPrefetchLoaderFactoryBundle()
    // is sometimes called on the previous document, before the new document is
  ...
  bool RenderFrameHostImpl::ValidateDidCommitParams(
    ...
    // Error pages may sometimes commit a URL in the wrong process, which
    // requires an exception for the CanCommitOriginAndUrl() checks.  This is ok
    // as long as the origin is opaque.
    ...
    // checks. We should also allow same-document navigations within pages
    // loaded with loadDataWithBaseURL. Since renderer-initiated same-document
    // navigations won't have a NavigationRequest at this point, we need to
    // check |is_loaded_from_load_data_with_base_url_|.
    ...
    if (!bypass_checks_for_error_page && !bypass_checks_for_file_scheme &&
      ...
      // Attempts to commit certain off-limits URL should be caught more
      // strictly than our FilterURL checks.  If a renderer violates this
      // policy, it should be killed.
    ...
    // activating an existing document do not require new embedding tokens as
    ...
    if (!is_page_activation) {
      if (!is_same_document_navigation &&
        ...
        bad_message::ReceivedBadMessage(
            process, bad_message::RFH_MISSING_EMBEDDING_TOKEN);
        return false;
  ...
  bool RenderFrameHostImpl::DidCommitNavigationInternal(
    ...
    if (!navigation_request) {
      ...
      // Fill the redirect chain for the NavigationRequest. Since this is only
      // for initial empty commits or same-document navigation, we should just
      // push the client-redirect URL (if it is a client redirect) and the final
      // URL.
      std::vector<GURL> redirects;
      if (is_same_document_navigation &&
          same_document_params->is_client_redirect) {
        // If it is a same-document navigation, it might be a client redirect,
        // in which case we should put the previous URL at the front of the
        // redirect chain.
    ...
    // Update the page transition. For subframe navigations, the renderer
    // process only gives the correct page transition at commit time.
    ...
    // Navigations in the same document, or navigations that activate an
    // existing bfcached or prerendered document, do not create a new document.
    ...
    if (navigated_to_new_document) {
      TRACE_EVENT1("content",
                   "DidCommitProvisionalLoad_StateResetForNewDocument",
    ...
    // prerender navigation doesn't seem right. This should likely be executed
    // in DidCommitNewDocument().
  ...
  // TODO(arthursonzogni): Below, many NavigationRequest's objects are passed
  // from the navigation to the new document. Consider grouping them in a single
  // struct.
  void RenderFrameHostImpl::TakeNewDocumentPropertiesFromNavigation(
    ...
    // Set the state whether this navigation is to an MHTML document, since
    // there are certain security checks that we cannot apply to subframes in
    // MHTML documents. Do not trust renderer data when determining that, rather
    // use the |navigation_request|, which was generated and stays browser side.
    ...
    is_loaded_from_load_data_with_base_url_and_unreachable_url_ =
        NavigationRequest::IsLoadDataWithBaseURLAndUnreachableURL(
            frame_tree_node_->IsMainFrame(),
            navigation_request->common_params(), data_url_as_string);

    // If we still have a PeakGpuMemoryTracker, then the loading it was
    // observing never completed. Cancel it's callback so that we don't report
    // partial loads to UMA.
    if (loading_mem_tracker_) {
  ...
  void RenderFrameHostImpl::OnSameDocumentCommitProcessed(
      const base::UnguessableToken& navigation_token,
      bool should_replace_current_entry, blink::mojom::CommitResult result) {
    auto request = same_document_navigation_requests_.find(navigation_token);
    if (request == same_document_navigation_requests_.end()) {
      // OnSameDocumentCommitProcessed will be called after DidCommitNavigation
      // on successfull same-document commits, so |request| should already be
      // deleted by the time we got here.
  ...
  void RenderFrameHostImpl::MaybeGenerateCrashReport(
      base::TerminationStatus status, int exit_code) {
    ...
    GetProcess()->GetStoragePartition()->GetNetworkContext()->QueueReport(
        ...
        isolation_info_.network_isolation_key(), base::nullopt,
        std::move(body));
  }
  ...
  void RenderFrameHostImpl::SendCommitFailedNavigation(
      ...
      mojom::CommonNavigationParamsPtr common_params,
      mojom::CommitNavigationParamsPtr commit_params,
      bool has_stale_copy_in_cache, int32_t error_code,
  ...
  void RenderFrameHostImpl::DidCommitNavigation(
    ...
    // initiated by Blink. In all other cases it should be non-null and present
    // in the map of NavigationRequests.
    ...
    // The commit IPC should be associated with the URL being committed (not
    // with the *last* committed URL that most other IPCs are associated with).
    ScopedActiveURL scoped_active_url(params->url,
                                      frame_tree()->root()->current_origin());
    ...
    // blink::mojom::LocalFrame::StopLoading() message. Treat this as an
    // implicit beforeunload completion callback to allow the pending navigation
    // to continue.
    if (is_waiting_for_beforeunload_completion_ &&
    ...
    // When a frame enters pending deletion, it waits for itself and its
    // children to properly unload. Receiving DidCommitProvisionalLoad() here
    // while the frame is not active means it comes from a navigation that
    // reached the ReadyToCommit stage just before the frame entered pending
    // deletion.
    //
    ...
    if (interface_params) {
      ...
      // If there had already been a real load committed in the frame, and this
      // is not a same-document navigation, then both the active document as
      // well as the global object was replaced in this browsing context. The
      // RenderFrame should have rebound its BrowserInterfaceBroker to a new
      // pipe, but failed to do so. Kill the renderer, and reset the old
      // receiver to ensure that any pending interface requests originating from
      // the previous document, hence possibly from a different security origin,
      // will no longer be dispatched.
      ...
      // Otherwise, it is the first real load committed, for which the
      // RenderFrame is allowed to, and will re-use the existing
      // BrowserInterfaceBroker connection if the new document is same-origin
      // with the initial empty document, and therefore the global object is not
      // replaced.
    }
  ...
  RenderFrameHostImpl::BuildCommitNavigationCallback(NavigationRequest *
                                                     navigation_request) {
    DCHECK(navigation_request);
  ...
  RenderFrameHostImpl::BuildCommitFailedNavigationCallback(NavigationRequest *
                                                           navigation_request) {
    DCHECK(navigation_request);
    return base::BindOnce(&RenderFrameHostImpl::DidCommitNavigation,
                          base::Unretained(this), navigation_request);
  ...
  void RenderFrameHostImpl::SendBeforeUnload(
      bool is_reload, base::WeakPtr<RenderFrameHostImpl> rfh) {
  ...
  void RenderFrameHostImpl::AddServiceWorkerContainerHost(
    ...
    if (IsInBackForwardCache()) {
      ...
      // ServiceWorkerContainerHost. In this case, evict the entry from the
      // cache.
      EvictFromBackForwardCacheWithReason(
          BackForwardCacheMetrics::NotRestoredReason::
  ...
  void RenderFrameHostImpl::PostMessageEvent(
      ...
      const std::u16string& source_origin, const std::u16string& target_origin,
  ...
  bool RenderFrameHostImpl::IsTestRenderFrameHost() const { return false; }
  ...
  void RenderFrameHostImpl::LogCannotCommitUrlCrashKeys(
      const GURL& url, bool is_same_document_navigation,
      NavigationRequest* navigation_request) {
    ...
    if (!GetSiteInstance()->IsDefaultSiteInstance()) {
      base::debug::SetCrashKeyString(
          base::debug::AllocateCrashKeyString(
              "original_url_origin", base::debug::CrashKeySize::Size256),
    ...
    if (navigation_request && navigation_request->IsNavigationStarted()) {
      ...
      base::debug::SetCrashKeyString(
          base::debug::AllocateCrashKeyString(
              "starting_site_instance", base::debug::CrashKeySize::Size256),
  ...
  int64_t CalculatePostID(
      ...
      int64_t last_post_id, bool is_same_document) {
    if (method != "POST")
      return -1;
  ...
  const std::string CalculateMethod(
      ...
      const std::string& last_http_method, bool is_same_document,
    ...
    // Otherwise, this is a cross-document navigation. Use the method specified
    // in the navigation request.
    return nav_request_method;
  }

  bool CalculateURLIsUnreachable(
      NavigationRequest * request, bool is_error_page,
    ...
    if (is_error_page)
      return true;
    // 2) This is a main frame navigation to a data: URL document with a
    // base_url (either an initial load or a same-document navigation). For
    // same-document navigations, we can know this by checking the value of
    // |is_loaded_from_load_data_with_base_url_and_unreachable_url|, which was
  ...
  int CalculateHTTPStatusCode(NavigationRequest * request,
    ...
    // Same-document navigations should retain the HTTP status code from the
    // last committed navigation.
    if (request->IsSameDocument())
    ...
    // The HTTP status code is not set if we never received any HTTP response
    // for the navigation.
    const int request_response_code =
        request->commit_params().http_response_code;
  ...
  bool CalculateShouldReplaceCurrentEntry(
      ...
      const mojom::DidCommitSameDocumentNavigationParamsPtr
          same_document_params,
      FrameTreeNode* node, const GURL& last_committed_url,
    ...
    // should_replace_current_entry. We use the value from
    // CommonNavigationParams to start off. This is what the browser sent the
    // renderer at commit time, but the actual result that came back from the
    // renderer after commit might be because of various decisions made by the
    // renderer, which are considered below.
    bool result = request->common_params().should_replace_current_entry;

    // -- Prepare the values needed for prediction.

    // For URLs loaded with base URL, use the base URL instead of the last
    // commit URL when doing comparisons.
    GURL previous_url = previous_commit_is_loaded_from_load_data_with_base_url
                            ? last_base_url
                            : last_committed_url;
    const bool is_restore = NavigationTypeUtils::IsRestore(
        request->common_params().navigation_type);
    const bool is_history = NavigationTypeUtils::IsHistory(
        request->common_params().navigation_type);
    ...
    if (is_error_page) {
      ...
      // navigations and restores. See RenderFrameImpl's
      ...
      // will classify a navigation as kBackForward if it's a history
      // navigation, or if it's a restore navigation with valid PageState.
      will_be_classified_as_back_forward_navigation =
          is_history || (is_restore && has_valid_page_state);
    }

    // -- Now we have all the information we need to determine the final value
    // of should_replace_current_entry.
    if (same_document_params) {
      ...
      // bit to true for anything that's not WebFrameLoadType::kStandard. This
      // includes navigations classified as kBackForward and
      // kReplaceCurrentItem:
      ...
      if (is_error_page) {
        ...
        // will result in replacement if the navigation doesn't have a valid
        // PageState. See RenderFrameImpl::CommitFailedNavigation().
        result |=
            !has_valid_page_state && (is_reload || is_history ||
                                      last_committed_url == request->GetURL());
      }
    ...
    // on a main frame when the history length is 0. If the navigation is not
    // the to an empty URL on a newly opened window, it will be turned into a
    // standard non-replacement commit.
    if (node->IsMainFrame() &&
    ...
    // commits (which excludes back/forward navigations and replacements) will
    // be converted to reloads, and the reloads will have
    // should_replace_current_entry == false, same as standard commits that
    // don't get converted.
    return result;
  }
  ...
  void RenderFrameHostImpl::
    ...
    SCOPED_CRASH_KEY_BOOL("VerifyDidCommit", "browser_gesture",
                          browser_gesture);
    ...
    DCHECK_EQ(browser_is_overriding_user_agent,
              params.is_overriding_user_agent);
    ...
    // Log histograms to trigger Chrometto slow reports, allowing us to see
    // traces to analyze what happened in these navigations.
  ...
  void RenderFrameHostImpl::LogCannotCommitOriginCrashKeys(
      bool is_same_document_navigation, NavigationRequest* navigation_request) {
    LogRendererKillCrashKeys(GetSiteInstance()->GetSiteInfo());
  ...
  void RenderFrameHostImpl::EnableMojoJsBindings() {
    // This method should only be called on RenderFrameHost which is for a
    // WebUI.
  ...
  bool RenderFrameHostImpl::IsDOMContentLoaded() { return dom_content_loaded_; }
  ...
  void RenderFrameHostImpl::SetLifecycleStateToPendingCommit() {
    ...
    // RenderFrameHost is associated with the navigation for the first time and
    // is not considered speculative anymore.
    DCHECK(children_.empty());
  ...
  void RenderFrameHostImpl::SetLifecycleStateToActive() {
    ...
    // prerender activation, update states of all the children to kActive. This
    // is called from RenderFrameHostManager::SetRenderFrameHost which happens
    // after commit.
    if (IsInBackForwardCache() ||
        lifecycle_state() == LifecycleStateImpl::kPrerendering) {
      for (auto& child : children_) {
  ...
  void RenderFrameHostImpl::SetLifecycleStateToPrerendering() {
    ...
    // when a speculative RenderFrameHost is created for navigation is in
    // pending commit state inside prerendered frame tree. This should happen
    // before activation.
    DCHECK(frame_tree()->is_prerendering());
    DCHECK(children_.empty());
  ...
  void RenderFrameHostImpl::SetLifecycleState(LifecycleStateImpl state) {
    ...
    static const base::NoDestructor<StateTransitions<LifecycleStateImpl>>
        allowed_transitions(
            ...
            // RenderFrameHost is only set speculative during its creation and
            // no transitions happen to this state during its lifetime.
  ...
  void RenderFrameHostImpl::RecordDocumentCreatedUkmEvent(
      const url::Origin& origin, const ukm::SourceId document_ukm_source_id,
    ...
    // have |is_cross_origin_frame| set to false, even though this frame is
    // cross- origin from its parent frame B. This value is only used in manual
    // analysis.
  ...
  RenderFrameHostImpl::GetPeerConnectionTrackerHost() {
  ...
  void RenderFrameHostImpl::BindHungDetectorHost(
      ...
      int32_t plugin_child_id, const base::FilePath& path) {
    pepper_hung_detectors_.Add(this, std::move(hung_host),
                               {plugin_child_id, path});
  }

  void RenderFrameHostImpl::GetPluginInfo(
      const GURL& url, const url::Origin& main_frame_origin,
      const std::string& mime_type, GetPluginInfoCallback callback) {
  ...
  void RenderFrameHostImpl::DidCreateInProcessInstance(
      int32_t instance, int32_t render_frame_id, const GURL& document_url,
  ...
  void RenderFrameHostImpl::DidCreateOutOfProcessPepperInstance(
      int32_t plugin_child_id, int32_t pp_instance, bool is_external,
      int32_t render_frame_id, const GURL& document_url, const GURL& plugin_url,
  ...
  void RenderFrameHostImpl::OpenChannelToPepperPlugin(
      const url::Origin& embedder_origin, const base::FilePath& path,
  ...
  void RenderFrameHostImpl::PluginHung(bool is_hung) {
    const HungDetectorContext& context =
        pepper_hung_detectors_.current_context();
  ...
  void RenderFrameHostImpl::PepperSetVolume(int32_t instance_id,
                                            double volume) {
  ...
  void RenderFrameHostImpl::SetEmbeddingToken(
    ...
    ui::AXActionHandlerRegistry::GetInstance()->SetFrameIDForAXTreeID(
        ui::AXActionHandlerRegistry::FrameID(GetProcess()->GetID(),
                                             routing_id_),
        ax_tree_id);
    ...
    // the WebView needs to update the ID tracking its child accessibility
    // tree).
    if (is_main_frame())
      delegate_->AXTreeIDForMainFrameHasChanged();

    // Propagate the embedding token to the RenderFrameProxyHost representing
    // the parent frame if needed, that is, if either this is a cross-process
    // subframe or the main frame of an inner web contents (i.e. would need to
    // send it to the RenderFrameProxyHost for the outer web contents owning the
    // inner one).
  ...
  void RenderFrameHostImpl::OnDidRunInsecureContent(const GURL& security_origin,
    ...
    OPTIONAL_TRACE_EVENT2(
        ...
        "security_origin", security_origin, "target_url", target_url);

    // TODO(nick, estark): Should we call FilterURL using this frame's process
    // on these parameters? |target_url| seems unused, except for a log message.
    // And |security_origin| might be replaceable with the origin of the main
    // frame.
  ...
  void RenderFrameHostImpl::OnDidRunContentWithCertificateErrors() {
    ...
    //
    // We still process this message for pending-commit RenderFrameHosts. This
    // can happen when a subframe's main resource has a certificate error. The
    ...
    // gets marked as insecure and that applies to any navigation entry using
    // the same renderer process with that same top-level origin.
    //
