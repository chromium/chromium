...
class EmailVerifier {
 public:
  ...
  EmailVerifier() = default;
  virtual ~EmailVerifier() = default;

  // Takes an `email`, a challenge `nonce` generated by an RP and
  // the RP's origin and returns, optionally, a signed verification
  // token.
  ...
  // must outlive it.
  CONTENT_EXPORT static std::unique_ptr<EmailVerifier> Create(
