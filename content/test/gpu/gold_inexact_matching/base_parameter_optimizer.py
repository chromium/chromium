...
GOLDCTL_PATHS = [
  ...
  os.path.join(CHROMIUM_SRC_DIR, 'tools', 'skia_goldctl', 'win', 'goldctl.exe'),
]
...
class BaseParameterOptimizer:
  ...
  def _DownloadData(self) -> None:
    ...
    if self._args.group_images_by_resolution:
      ...
      # does.
      json_str = json.dumps(
        grouping_dict, sort_keys=True, separators=(',', ':')
      )
      md5 = hashlib.md5()
      md5.update(json_str.encode('utf-8'))
      self._DownloadExpectations(
        f'{self._gold_url}/json/v1/positivedigestsbygrouping/{md5.hexdigest()}'
      )
  ...
  def _DownloadImagesForResolutionGrouping(self) -> None:
    ...
    test_expectations = self._expectations.get('primary', {}).get(
      self._test_name, {}
    )
    positive_digests = [
      digest
      for digest, status in test_expectations.items()
      if status == 'positive'
    ]
    ...
    for trace in self._expectations['traces']:
      ...
      if not digests:
        logging.warning(
          ...
          'This is likely due to the trace being old.',
          self._test_name,
          trace_id,
        )
  ...
  def _RunComparisonForParameters(
    ...
    for resolution, digest_list in self._images.items():
      logging.debug(
        'Resolution/trace: %s, digests: %s', resolution, digest_list
      )
      cmds = [
    ...
    logging.debug(
      'target success percent: %s', self._args.target_success_percent
    )
    successful = success_percent >= self._args.target_success_percent
    logging.debug(
      'Successful: %s, Max different pixels: %d, Max per-channel delta sum: %d',
      successful,
      max_num_pixels,
      max_max_delta,
    )
...
def RunCommandAndExtractData(cmd: list[str]) -> tuple[bool, int, int]:
  ...
  logging.debug(
    'Result for %r: success: %s, num_pixels: %d, max_delta: %d',
    cmd,
    success,
    num_pixels,
    max_delta,
