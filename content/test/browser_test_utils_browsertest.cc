...
IN_PROC_BROWSER_TEST_F(EvalJsBrowserTest, EvalJsDeletedPromise) {
  ASSERT_TRUE(embedded_test_server()->Start());
  EXPECT_TRUE(
      NavigateToURL(shell(), embedded_test_server()->GetURL("/title2.html")));

  // Return a promise that never resolves. It will get collected which should
  // be detected in the renderer.
  auto result = EvalJs(shell(), "new Promise(() => {})");

  EXPECT_TRUE(result.value.is_none());
  EXPECT_THAT(result.error,
              ::testing::EndsWith(
                  "This is usually caused by Promise resolution functions "
                  "getting destroyed without being called.\"\n"));
}
