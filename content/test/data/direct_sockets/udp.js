...
async function joinGroupSSMSameGroupDifferentSources(
  ...
  try {
    ...
    const sources =
        multicastController.joinedGroups.map(m => m.sourceAddress).sort();
...
async function joinGroupSSMTwiceWithSameSource(options, sourceIP) {
  ...
  try {
    ...
    if (e.name !== 'InvalidStateError') {
      return 'joinGroupSSMTwiceWithSameSource failed: unexpected error: ' + e;
    }
...
async function joinGroupSSMInvalidSource(options) {
  ...
  try {
    ...
    if (e.name !== 'TypeError') {
      return 'joinGroupSSMInvalidSource failed: unexpected error: ' + e;
    }
