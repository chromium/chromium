...
class RenderMediaClient : public media::MediaClient {
 ...
 private:
  ...
  void ResetConnectionForSupportedProfilesQuery()
      EXCLUSIVE_LOCKS_REQUIRED(supported_video_decoder_profiles_lock_);
  void ResetInterfaceFactoryForSupportedProfilesQueryOnMainThread()
  ...
  [[maybe_unused]] bool supported_video_decoder_profiles_are_known_
      GUARDED_BY(supported_video_decoder_profiles_lock_) = false;
  ...
  [[maybe_unused]] mojo::SharedRemote<media::mojom::VideoDecoder>
      video_decoder_for_supported_profiles_
          GUARDED_BY(supported_video_decoder_profiles_lock_);
};
