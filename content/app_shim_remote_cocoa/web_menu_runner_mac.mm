...
- (void)runMenuInView:(NSView*)view
  ...
  // If this is being run headlessly, just early-exit.
  if (IsHeadless()) {
    return;
  }

  // In a testing situation, make the callback and early-exit as well.
  ...
  // The call to do the popup menu takes the location of the upper-left. Wiggle
  // the menu a bit to the upper-left further to get a better position.
  NSPoint point = NSMakePoint(NSMinX(bounds), NSMaxY(bounds));
  point.x -= 8;
  point.y += 2;

  // Do the popup.
  [_menu popUpMenuPositioningItem:item atLocation:point inView:view];
}
