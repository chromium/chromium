...
static_assert(DigestSizeForHashKind(kSha1) == kSha1Size);
static_assert(DigestSizeForHashKind(kSha256) == kSha256Size);
static_assert(DigestSizeForHashKind(kSha384) == kSha384Size);
static_assert(DigestSizeForHashKind(kSha512) == kSha512Size);
...
TEST(HashTest, Sha256) {
  ...
  for (const auto& c : cases) {
    ...
    auto computed_digest = Sha256(base::as_byte_span(message));
...
TEST(HashTest, Sha384) {
  ...
  std::vector<uint8_t> computed_digest(DigestSizeForHashKind(kSha384));
  Hash(kSha384, base::as_byte_span(message), computed_digest);
...
TEST(HashTest, Sha512) {
  ...
  for (const auto& c : cases) {
    ...
    PrepareTestCase(c, &message, digest);
    auto computed_digest = Sha512(base::as_byte_span(message));
    EXPECT_EQ(digest, computed_digest);
  }
}
...
TEST(HashDeathTest, WrongDigestSizeDies) {
  ...
  EXPECT_DEATH_IF_SUPPORTED(Hash(HashKind::kSha256, input, small_digest), "");
  EXPECT_DEATH_IF_SUPPORTED(Hash(HashKind::kSha256, input, big_digest), "");
  EXPECT_DEATH_IF_SUPPORTED(Hash(HashKind::kSha256, "input", small_digest), "");
  EXPECT_DEATH_IF_SUPPORTED(Hash(HashKind::kSha256, "input", big_digest), "");
}

TEST(HashDeathTest, UseAfterFinishDies) {
  Hasher hasher(HashKind::kSha256);
...
TEST(HashTest, StringViewHash) {
  const std::array<uint8_t, kSha256Size> hash{
  ...
  EXPECT_EQ(hash, Sha256("Hello, World!"));
}

TEST(HashTest, HashKindEVPMDConversions) {
  constexpr auto kKinds = std::to_array<HashKind>({
      kSha1,
      kSha256,
      kSha384,
      kSha512,
  ...
  for (auto kind : kKinds) {
    ...
    auto result_kind = HashKindForEVPMD(md);
    ASSERT_TRUE(result_kind.has_value());
    EXPECT_EQ(kind, *result_kind);
  }
  EXPECT_FALSE(HashKindForEVPMD(EVP_md5()).has_value());
}
...
TEST(HashTest, HashFileSuccess) {
  ...
  for (const auto& c : kCases) {
    ...
    auto hash = base::span(hash_buffer).first(DigestSizeForHashKind(c.kind));

    file.Seek(base::File::FROM_BEGIN, 0);
    EXPECT_TRUE(HashFile(c.kind, &file, hash));
...
TEST(HashTest, HashFileFailure) {
  ...
  EXPECT_FALSE(HashFile(kSha256, &file, buffer));
