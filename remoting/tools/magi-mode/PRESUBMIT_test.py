...
class MockAffectedFile:
    def __init__(self, local_path, action='M'):
        ...
        self._action = action
    def LocalPath(self): return self._path
    def AbsoluteLocalPath(self): return os.path.normpath(os.path.join(os.path.abspath('fake_repo'), self._path))
...
class MockInputApi:
    ...
    def AffectedFiles(self, file_filter=None, include_deletes=False):
        return [f for f in self.affected_files if not file_filter or file_filter(f)]

    def ReadFile(self, affected_file):
        return self.files_content.get(affected_file.LocalPath(), "")

    def PresubmitLocalPath(self):
        return os.path.normpath(os.path.join(os.path.abspath('fake_repo'), 'remoting/tools/magi-mode'))

    def FilterSourceFile(self, affected_file, files_to_check=None):
        if not files_to_check: return True
        return any(affected_file.LocalPath().endswith(ext) for ext in ('.md',))
...
class MagiPresubmitTest(unittest.TestCase):
    ...
    def testReachability(self, mock_walk, mock_getsize, mock_exists):
        ...
        magi_dir = os.path.normpath(os.path.join(os.path.abspath('fake_repo'), 'remoting/tools/magi-mode'))
        ...
        self.mock_input.affected_files = [MockAffectedFile('remoting/tools/magi-mode/SKILL.md')]
        ...
        with patch('builtins.open', unittest.mock.mock_open(read_data='content\n')):
            results = PRESUBMIT.CheckMarkdownFiles(self.mock_input, self.mock_output)
    ...
    def testCodeBlockExclusion(self):
        ...
        long_line = '```\n' + 'A' * 100 + '\n```\n'
        self.mock_input.affected_files = [MockAffectedFile('remoting/tools/magi-mode/SKILL.md')]
        self.mock_input.files_content = {'remoting/tools/magi-mode/SKILL.md': long_line}

        magi_dir = os.path.normpath(os.path.join(os.path.abspath('fake_repo'), 'remoting/tools/magi-mode'))
        with patch('os.walk', return_value=[(magi_dir, [], ['SKILL.md'])]), patch('os.path.getsize', return_value=100):
            results = PRESUBMIT.CheckMarkdownFiles(self.mock_input, self.mock_output)
    ...
    def testIndentedBlockRobustness(self):
        ...
        self.mock_input.affected_files = [MockAffectedFile('remoting/tools/magi-mode/SKILL.md')]
        self.mock_input.files_content = {'remoting/tools/magi-mode/SKILL.md': content}
