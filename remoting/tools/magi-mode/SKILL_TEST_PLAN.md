...
1.  **Setup**: Create isolated directories for each test case under
    `remoting/tools/magi-mode/.temp/` and copy `testdata` into them.
...
3.  **Verification**: Read the output files generated by the subagents and
    verify they match the expected outputs defined in the test JSON files.
...
*   **Enforcement**: This rule should be presented to future invocations of
    Gemini to ensure they self-verify their changes.
