...
// Missing constants and functions from WebRTC
const int SOCKET_ERROR = -1;

inline int32_t TimeDiff32(uint32_t later, uint32_t earlier) {
...
inline void long_to_bytes(uint32_t val, void* buf) {
  *static_cast<uint32_t*>(buf) = base::HostToNet32(val);
}

inline void short_to_bytes(uint16_t val, void* buf) {
  *static_cast<uint16_t*>(buf) = base::HostToNet16(val);
}

inline uint32_t bytes_to_long(const void* buf) {
  return base::NetToHost32(*static_cast<const uint32_t*>(buf));
}

inline uint16_t bytes_to_short(const void* buf) {
  return base::NetToHost16(*static_cast<const uint16_t*>(buf));
}
...
PseudoTcp::PseudoTcp(IPseudoTcpNotify* notify, uint32_t conv)
  ...
  DCHECK(m_rbuf_len + MIN_PACKET < m_sbuf_len);
  ...
  m_msslevel = 0;
  m_largest = 0;
  DCHECK(MIN_PACKET > PACKET_OVERHEAD);
...
// retransmit segments
      ...
      VLOG(1) << "timeout retransmit (rto: " << m_rx_rto
...
IPseudoTcpNotify::WriteResult PseudoTcp::packet(uint32_t seq,
                                                ...
                                                uint32_t offset,
                                                uint32_t len) {
  DCHECK(HEADER_SIZE + len <= MAX_PACKET);
  ...
  VLOG(1) << "<-- <CONV=" << m_conv << "><FLG=" << static_cast<unsigned>(flags)
          << "><SEQ=" << seq << ":" << seq + len << "><ACK=" << m_rcv_nxt
  ...
  if (seg.flags & FLAG_CTL) {
    if (seg.len == 0) {
      ...
      if (m_state == TCP_LISTEN) {
        ...
        VLOG(1) << "State: TCP_SYN_RECEIVED";
        // m_notify->associate(addr);
        queueConnectMessage();
      } else if (m_state == TCP_SYN_SENT) {
        m_state = TCP_ESTABLISHED;
        VLOG(1) << "State: TCP_ESTABLISHED";
        adjustMTU();
        if (m_notify) {
      ...
      LOG(WARNING) << "Unknown control code: " << seg.data[0];
      return false;
    }
  }
  ...
  if ((seg.ack > m_snd_una) && (seg.ack <= m_snd_nxt)) {
    ...
    if (seg.tsecr) {
      ...
      if (rtt >= 0) {
        ...
        m_rx_rto = std::clamp(m_rx_srtt + std::max(1u, 4 * m_rx_rttvar),
                              MIN_RTO, MAX_RTO);
#if _DEBUGMSG >= _DBG_VERBOSE
        VLOG(1) << "rtt: " << rtt << "  srtt: " << m_rx_srtt
                << "  rto: " << m_rx_rto;
#endif  // _DEBUGMSG
      } else {
        LOG(WARNING) << "rtt < 0";
...
#if _DEBUGMSG >= _DBG_NORMAL
        VLOG(1) << "exit recovery";
#endif  // _DEBUGMSG
        m_dup_acks = 0;
      } else {
#if _DEBUGMSG >= _DBG_NORMAL
        VLOG(1) << "recovery retransmit";
#endif  // _DEBUGMSG
        if (!transmit(m_slist.begin(), now)) {
          closedown(ECONNABORTED);
          return false;
...
#if _DEBUGMSG >= _DBG_NORMAL
          VLOG(1) << "Recovered " << nAdjust << " bytes (" << m_rcv_nxt
                  << " -> " << m_rcv_nxt + nAdjust << ")";
#endif  // _DEBUGMSG
...
bool PseudoTcp::transmit(const SList::iterator& seg, uint32_t now) {
  ...
  while (true) {
    ...
    if (wres == IPseudoTcpNotify::WR_FAIL) {
      VLOG(1) << "packet failed";
      return false;
    }

    DCHECK(wres == IPseudoTcpNotify::WR_TOO_LARGE);

    while (true) {
      if (PACKET_MAXIMUMS[m_msslevel + 1] == 0) {
        VLOG(1) << "MTU too small";
        return false;
      }
...
#if _DEBUGMSG >= _DBG_NORMAL
    VLOG(1) << "Adjusting mss to " << m_mss << " bytes";
#endif  // _DEBUGMSG
  }

  if (nTransmit < seg->len) {
    VLOG(1) << "mss reduced to " << m_mss;
...
void PseudoTcp::attemptSend(SendFlags sflags) {
  ...
  while (true) {
    ...
    if (bFirst) {
      ...
      bFirst = false;
      VLOG(1) << "[cwnd: " << m_cwnd << "  nWindow: " << nWindow
              << "  nInFlight: " << nInFlight << "  nAvailable: " << nAvailable
              << "  nQueued: " << snd_buffered
              << "  nEmpty: " << available_space << "  ssthresh: " << m_ssthresh
              << "]";
    }
#endif  // _DEBUGMSG
...
void PseudoTcp::parseOptions(const char* data, uint32_t len) {
  ...
  if (options_specified.find(TCP_OPT_WND_SCALE) == options_specified.end()) {
    LOG(WARNING) << "Peer doesn't support window scaling";
...
void PseudoTcp::applyOption(char kind, const char* data, uint32_t len) {
  if (kind == TCP_OPT_MSS) {
    LOG(WARNING) << "Peer specified MSS option which is not supported.";
    // TODO(?): Implement.
  } else if (kind == TCP_OPT_WND_SCALE) {
    // Window scale factor.
    // http://www.ietf.org/rfc/rfc1323.txt
    if (len != 1) {
      LOG(WARNING) << "Invalid window scale option received.";
      return;
    }
