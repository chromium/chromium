...
TEST_F(AudioCapturerChromeOsTest, AudioHelperIsDestroyed) {
  audio_capturer_->Start(base::DoNothing());
  ...
  // Release the helper's raw_ptr before it's destroyed by the resetting of the
  // caputrer.
  audio_helper_chromeos_ = nullptr;
  ...
  // Allow the destruction task on the audio thread to complete.
  EXPECT_TRUE(base::test::RunUntil([&]() { return destroyed; }));
}
