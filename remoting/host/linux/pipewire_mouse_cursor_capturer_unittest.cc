...
static const webrtc::ScreenId kScreenId1 =
    GnomeDisplayConfig::GetScreenId(kMeta1);
static const webrtc::ScreenId kScreenId2 =
    GnomeDisplayConfig::GetScreenId(kMeta2);
...
class PipewireMouseCursorCapturerTest : public testing::Test {
 ...
 protected:
  ...
  PipewireMouseCursorCapturer capturer_{nullptr, stream_manager_.GetWeakPtr()};
...
PipewireMouseCursorCapturerTest::PipewireMouseCursorCapturerTest() {
  ...
  config.monitors[kMeta1] = CreateMonitorInfo(0, 0, 100, 100, /*scale=*/1.0);
  config.monitors[kMeta2] = CreateMonitorInfo(100, 0, 200, 200, /*scale=*/2.0);
  capturer_.OnDisplayConfig(config);
}
