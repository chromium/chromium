...
// TODO(yuweih): Factor out Linux-specific logic from this class.
#include <gtk/gtk.h>
  ...
  switch (result) {
    ...
    case mojom::OpenUrlResult::FAILURE: {
      std::string message = l10n_util::GetStringFUTF8(
          IDS_REMOTE_OPEN_URL_FAILED, base::UTF8ToUTF16(url_.spec()));
      LOG(ERROR) << "Failed to open URL remotely: " << url_.spec();
...
void RemoteOpenUrlClient::OpenOnFallbackBrowserInternal() {
  ...
  if (!url_.is_empty()) {
    gtk_launch_command.AppendArg(url_.spec());
  }
