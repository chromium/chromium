...
class OAuthHostStarterImpl : public HostStarter,
                             public gaia::GaiaOAuthClient::Delegate,
                             public remoting::ServiceClient::Delegate {
 public:
  OAuthHostStarterImpl(
  ...
  // OAuthHostStarterImpl implementation.
 ...
 private:
  ...
  base::WeakPtr<OAuthHostStarterImpl> weak_ptr_;
...
void OAuthHostStarterImpl::StartHost(Params params,
...
void OAuthHostStarterImpl::OnGetTokensResponse(const std::string& refresh_token,
                                               const std::string& access_token,
                                               int expires_in_seconds) {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&OAuthHostStarterImpl::OnGetTokensResponse, weak_ptr_,
...
void OAuthHostStarterImpl::OnRefreshTokenResponse(
    const std::string& access_token,
    int expires_in_seconds) {
...
void OAuthHostStarterImpl::OnGetUserEmailResponse(
    const std::string& user_email) {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OAuthHostStarterImpl::OnGetUserEmailResponse,
  ...
  if (!auth_code_exchanged_) {
    ...
    host_stopper_->StopLocalHost(
        ...
        base::BindOnce(&OAuthHostStarterImpl::OnLocalHostStopped,
...
void OAuthHostStarterImpl::OnHostRegistered(
  ...
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OAuthHostStarterImpl::OnHostRegistered,
                                  weak_ptr_, host_id, authorization_code));
    return;
  }
...
void OAuthHostStarterImpl::StartHostProcess() {
  // Start the host.
  ...
  daemon_controller_->SetConfigAndStart(
      ...
      base::BindOnce(&OAuthHostStarterImpl::OnHostStarted,
                     base::Unretained(this)));
}

void OAuthHostStarterImpl::OnLocalHostStopped() {
...
void OAuthHostStarterImpl::OnHostStarted(DaemonController::AsyncResult result) {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OAuthHostStarterImpl::OnHostStarted,
                                  weak_ptr_, result));
    return;
  }
...
void OAuthHostStarterImpl::OnOAuthError() {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&OAuthHostStarterImpl::OnOAuthError, weak_ptr_));
    return;
  }
...
void OAuthHostStarterImpl::OnNetworkError(int response_code) {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&OAuthHostStarterImpl::OnNetworkError,
                                  weak_ptr_, response_code));
    return;
  }
...
void OAuthHostStarterImpl::OnHostUnregistered() {
  if (!main_task_runner_->BelongsToCurrentThread()) {
    main_task_runner_->PostTask(
        FROM_HERE,
        base::BindOnce(&OAuthHostStarterImpl::OnHostUnregistered, weak_ptr_));
    return;
  }
...
std::unique_ptr<HostStarter> CreateOAuthHostStarter(
  ...
  return std::make_unique<OAuthHostStarterImpl>(
