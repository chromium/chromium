...
class BluetoothLocalGattServiceFlossTest : public testing::Test {
 public:
  ...
  std::string GenerateExpectedIdentifier(device::BluetoothUUID uuid) {
    return base::StringPrintf("-%s/%04x", uuid.value().c_str(),
...
TEST_F(BluetoothLocalGattServiceFlossTest, InstanceIdsResolved) {
  ...
  EXPECT_EQ(service->GetIdentifier(), GenerateExpectedIdentifier(service_uuid));
  EXPECT_EQ(characteristic->GetIdentifier(),
            GenerateExpectedIdentifier(characteristic_uuid));
  EXPECT_EQ(descriptor->GetIdentifier(),
            GenerateExpectedIdentifier(descriptor_uuid));
