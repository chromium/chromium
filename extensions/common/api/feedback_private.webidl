...
dictionary FeedbackInfo {
  ...
  // feedback report.
  sequence<LogsMapEntry> systemInformation;
...
dictionary SendFeedbackResult {
  ...
  required LandingPageType landingPageType;
};
...
dictionary ReadLogSourceResult {
  ...
  required long readerId;

  // Each DOMString in this array represents one line of logging that was
  // fetched from the log source.
  required sequence<DOMString> logLines;
};

// Listener callback for the onFeedbackRequested event.
callback OnFeedbackRequestedListener = undefined(FeedbackInfo feedback);

interface OnFeedbackRequestedEvent : ExtensionEvent {
  static undefined addListener(OnFeedbackRequestedListener listener);
  static undefined removeListener(OnFeedbackRequestedListener listener);
  static boolean hasListener(OnFeedbackRequestedListener listener);
};
...
interface FeedbackPrivate {
  ...
  static undefined openFeedback(FeedbackSource source);
  ...
  static attribute OnFeedbackRequestedEvent onFeedbackRequested;
};

partial interface Browser {
  static attribute FeedbackPrivate feedbackPrivate;
};
