...
class InvalidIfDefinedMacroNamesTest(unittest.TestCase):
    ...
    def testInvalidIfDefinedMacroNames(self):
        lines = [
            ...
            '#elifdef TARGET_IPHONE_SIMULATOR',
            ' # ifdef TARGET_IPHONE_SIMULATOR',
...
class CheckAndroidNullAwayAnnotatedClasses(unittest.TestCase):
    ...
    def testDetectsInClasses(self):
        ...
        results = PRESUBMIT._CheckAndroidNullAwayAnnotatedClasses(
    ...
    def testExcludesTestSupport(self):
        ...
        mock_input = MockInputApi()
        mock_input.files = [
            MockFile('path/test/Two.java', ['public class Two']),
        ]
        results = PRESUBMIT._CheckAndroidNullAwayAnnotatedClasses(
            mock_input, MockOutputApi())
        self.assertEqual(0, len(results))
...
class AssertNoJsInIosTest(unittest.TestCase):
    ...
    def testExistingFilesWarningOnly(self):
        ...
        input_api.files = [
            ...
            MockFile('ios/chrome/feature/resources/script2.js', [],
                     action='D'),
        ]
        results = PRESUBMIT.CheckNoJsInIos(input_api, MockOutputApi())
        self.assertEqual(1, len(results))
    ...
    def testMovedScriptWarningOnly(self):
        ...
        input_api.files = [
            ...
            MockFile('ios/chrome/renamed_feature/resources/script.js', [],
                     action='A'),
        ]
        results = PRESUBMIT.CheckNoJsInIos(input_api, MockOutputApi())
        self.assertEqual(1, len(results))
...
class CheckUnwantedDependenciesTest(unittest.TestCase):
    ...
    def testJavaImportEndToEnd(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            subprocess.run([git_cmd, 'init', '-q', temp_dir], check=True)
            ...
            subprocess.run([git_cmd, '-C', temp_dir, 'add', '.'], check=True)
