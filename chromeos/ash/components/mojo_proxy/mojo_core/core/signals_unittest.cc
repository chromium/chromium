...
TEST_F(SignalsTest, QueryInvalidArguments) {
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_INVALID_ARGUMENT,
            MojoQueryHandleSignalsState(MOJO_LEGACY_HANDLE_INVALID, &state));
...
TEST_F(SignalsTest, QueryMessagePipeSignals) {
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(a, &state));
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE, state.satisfied_signals);
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(b, &state));
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE, state.satisfied_signals);
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_READABLE |
                MOJO_LEGACY_HANDLE_SIGNAL_WRITABLE |
                MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED |
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            WaitForSignals(b, MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED));

  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(b, &state));
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED, state.satisfied_signals);
  EXPECT_EQ(MOJO_LEGACY_HANDLE_SIGNAL_PEER_CLOSED |
                MOJO_LEGACY_HANDLE_SIGNAL_QUOTA_EXCEEDED,
            state.satisfiable_signals);
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoClose(b));
}
...
TEST_F(SignalsTest, LocalPeers) {
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(a, &state));
  EXPECT_TRUE(state.satisfiable_signals &
              MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  EXPECT_FALSE(state.satisfied_signals & MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(c, &state));
  EXPECT_TRUE(state.satisfiable_signals &
              MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  EXPECT_FALSE(state.satisfied_signals & MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);

  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(d, &state));
  EXPECT_TRUE(state.satisfiable_signals &
              MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  EXPECT_FALSE(state.satisfied_signals & MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  ...
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(d, &state));
  EXPECT_FALSE(state.satisfiable_signals &
               MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
...
TEST_F(SignalsTest, RemotePeers) {
  ...
  EXPECT_FALSE(state.satisfied_signals & MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            WaitForSignals(a, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
  ...
  EXPECT_FALSE(state.satisfied_signals & MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
  EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
            WaitForSignals(b, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
                           MOJO_LEGACY_TRIGGER_CONDITION_SIGNALS_UNSATISFIED));

  RunTestClient("RemotePeersClient", [&](MojoHandle h) {
    // The bootstrap pipe should eventually signal remoteness.
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
              WaitForSignals(h, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
    ...
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
              WaitForSignals(a, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
                             MOJO_LEGACY_TRIGGER_CONDITION_SIGNALS_SATISFIED));
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(a, &state));
    EXPECT_TRUE(state.satisfied_signals &
                MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);

    // And so should |c| after we fuse |d| to |a|.
    MojoHandle c, d;
    CreateMessagePipe(&c, &d);
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoFuseMessagePipes(d, a, nullptr));
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK,
              WaitForSignals(c, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
                             MOJO_LEGACY_TRIGGER_CONDITION_SIGNALS_SATISFIED));
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(c, &state));
    EXPECT_TRUE(state.satisfied_signals &
                MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
    ...
    EXPECT_EQ(
        ...
        WaitForSignals(a, MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE,
                       MOJO_LEGACY_TRIGGER_CONDITION_SIGNALS_UNSATISFIED));
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoQueryHandleSignalsState(a, &state));
    EXPECT_FALSE(state.satisfied_signals &
                 MOJO_LEGACY_HANDLE_SIGNAL_PEER_REMOTE);
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoClose(a));
    EXPECT_EQ(MOJO_LEGACY_RESULT_OK, MojoClose(b));
  });
