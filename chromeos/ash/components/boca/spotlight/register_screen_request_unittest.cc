...
class RegisterScreenRequestTest : public testing::Test {
 public:
  RegisterScreenRequestTest() = default;
  void SetUp() override {
...
TEST_F(RegisterScreenRequestTest, InitRegisterScreenRequestSucceed) {
...
TEST_F(RegisterScreenRequestTest, InitRegisterScreenAndFail) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(request_handler(), HandleRequest(_))
      .WillOnce(DoAll(SaveArg<0>(&http_request),
