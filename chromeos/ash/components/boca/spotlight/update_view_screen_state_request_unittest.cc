...
class UpdateViewScreenStateRequestTest : public testing::Test {
 public:
  UpdateViewScreenStateRequestTest() = default;
  void SetUp() override {
...
TEST_F(UpdateViewScreenStateRequestTest,
       InitUpdateViewScreenStateRequestSucceed) {
  net::test_server::HttpRequest http_request;
  ...
  std::unique_ptr<UpdateViewScreenStateRequest> request =
  ...
  auto* contentData =
      ...
      "\"deviceId\":\"d1\"},\"user\":{\"gaiaId\":\"1\"}},\"viewScreenState\":"
      "4}";
...
TEST_F(UpdateViewScreenStateRequestTest, InitUpdateViewScreenStateAndFail) {
  net::test_server::HttpRequest http_request;
  EXPECT_CALL(request_handler(), HandleRequest(_))
      .WillOnce(DoAll(SaveArg<0>(&http_request),
