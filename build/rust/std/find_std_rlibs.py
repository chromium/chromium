...
def main():
    ...
    (
        parser.add_argument(
            "--rust-bin-dir", help="Path to Rust binaries", required=True
        ),
    )
    (
        parser.add_argument(
            "--target", help="Rust target triple", required=False
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--depfile-target", help="Target to key depfile around", required=True
    )
    parser.add_argument(
        "--extra-libs", help="List of extra non-libstd sysroot libraries"
    )
    parser.add_argument(
        "--rustc-revision",
    ...
    with open(args.depfile, 'w') as depfile:
        ...
        depfile.write(
            "%s:"
            % (os.path.join(args.output, "lib%s.rlib" % args.depfile_target))
        )
