...
# package.
PackageSizes = collections.namedtuple(
    'PackageSizes', ['compressed', 'uncompressed']
)
...
def CreateSizesHistogram(package_sizes):
    ...
    for name, size in package_sizes.items():
        histogram.append(
            CreateSizesHistogramItem(
                '%s_%s' % (name, 'compressed'), size.compressed, sizes_guid
            )
        )
        histogram.append(
            CreateSizesHistogramItem(
                '%s_%s' % (name, 'uncompressed'), size.uncompressed, sizes_guid
            )
        )
    return histogram
...
def CreateTestResults(test_status, timestamp):
    ...
    results = {
        'tests': {},
        'interrupted': False,
        'metadata': {'test_name_prefix': 'build/fuchsia/'},
        'version': 3,
...
def WriteGerritPluginSizeData(output_path, package_sizes):
    ...
    with open(output_path, 'w') as sizes_file:
        sizes_data = {
            name: size.compressed for name, size in package_sizes.items()
        }
...
def GetCompressedSize(file_path):
    ...
    try:
        ...
        proc = subprocess.Popen(
            compressor_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        proc.wait()
...
def GetBlobs(far_file, build_out_dir):
    ...
    blobs[meta_name] = Blob(
        meta_name, meta_hash, compressed, uncompressed, True
    )
    ...
    for blob_name, blob_hash in blob_name_hashes.items():
        ...
        blobs[blob_name] = Blob(
            blob_name, blob_hash, compressed, uncompressed, is_counted
        )
...
def GetPackageSizes(package_blobs):
    ...
    for package_name in package_blobs:
        ...
        package_sizes[package_name] = PackageSizes(
...
def GetBinarySizesAndBlobs(args, sizes_config):
    ...
    package_blobs = GetPackageBlobs(
        sizes_config['far_files'], args.build_out_dir
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=os.path.join(
            'tools', 'fuchsia', 'size_tests', 'fyi_sizes.json'
        ),
    ...
    if not os.path.isdir(args.build_out_dir):
        raise Exception(
            'Could not find build output directory "%s".' % args.build_out_dir
        )
    ...
    try:
        package_sizes, package_blobs = GetBinarySizesAndBlobs(
            args, sizes_config
        )
        ...
        all_tests_passed, test_status = GetTestStatus(
            package_sizes, sizes_config, test_completed
        )

        if results_directory:
            WriteTestResults(
                os.path.join(results_directory, 'test_results.json'),
                test_completed,
                test_status,
                timestamp,
            )
            with open(
                os.path.join(results_directory, 'perf_results.json'), 'w'
            ) as f:
                json.dump(sizes_histogram, f)
            WritePackageBlobsJson(
                os.path.join(results_directory, PACKAGES_BLOBS_FILE),
                package_blobs,
            )
            WritePackageSizesJson(
                os.path.join(results_directory, PACKAGES_SIZES_FILE),
                package_sizes,
            )
