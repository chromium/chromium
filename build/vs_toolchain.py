...
def _CopyDebugger(target_dir, target_cpu):
  ...
  dia_path = os.path.join(NormalizePath(os.environ['GYP_MSVS_OVERRIDE_PATH']),
                          'DIA SDK', 'bin')
  if target_cpu in ['x64', 'arm64']:
    # Use the amd64 version for x64 and arm64, emulating if needed, because
    # build tools are x64 for AMD64 builds.
    dia_path = os.path.join(dia_path, 'amd64')
  dia_path = os.path.join(dia_path, 'msdia140.dll')
