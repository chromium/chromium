            ...
            if not path.endswith(_IGNORED_SUFFIXES):
                if path not in self._unmatched_paths:
                    if self.unmatched_paths_count < _MAX_UNMATCHED_TO_LOG:
                        logging.warning(
                            'Could not find source path for %s', path
                        )
...
def _Parse(output_directory, executable_path):
    ...
    assert executable_path in dep_map, (
        'Failed to find rule that builds ' + executable_path
    )
...
def main():
    ...
    if num_unmatched > _MAX_UNMATCHED_TO_IGNORE:
        raise Exception(
            f'{num_unmatched} unmapped files (of {len(object_paths)}).'
            ' Likely a bug in ninja_parser.py'
        )

    if args.depfile:
        action_helpers.write_depfile(
            args.depfile, args.result_json, source_mapper.parsed_files
        )
