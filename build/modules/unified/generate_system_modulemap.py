    ...
    for c in range(start, len(modulemap)):
        if modulemap[c] == '{':
            ...
            if depth == 0:
                return c + 1
    raise ValueError(
        f'No closing brace for {modulemap[start : start + 100]}...'
    )


def modify_modulemap(
    content: str,
...
def parse_modulemap(
    ...
    content = modify_modulemap(
        modulemap_path.read_text(), modify_modules, modified_modules
    )
...
def calculate_transitive_headers(
    clang_args: list[str],
    include_dirs: list[Tuple[pathlib.Path, List[Header]]],
    ...
    sysroot_dirs = sorted(
        [_absolute(d) for d in sysroot_dirs],
            ...
            for include_dir, headers in include_dirs:
                for h in headers:
                    if not h.private and str(h.path) not in _DISABLED_HEADERS:
                        if all(
                            _requires_met(r, target_os, target_cpu)
                            for r in h.requires
                        ):
...
"""
        ...
        if ps.returncode != 0:
            print(
                f'Suggestion: Run `cd {os.getcwd()} && {sys.argv[0]} --debug',
    ...
    with io.StringIO() as s:
        ...
        for mm in modulemaps:
            ...
            mm_content = _SIMPLE_HEADER_RE.sub(
                ...
                modify_modulemap(
                    mm.read_text(), modify_modules, modified_modules
                ),
            )
        ...
        for header in headers:
            ...
            if header.module_name:
                ...
                module_name = (
                    f'"{header.path.name}_{modules[header.path.name]}"'
                )
            else:
                modules[header.path.name] = 1
...
def main(args, extra_args):
    ...
    for module in args.module:
        if module not in known_modules:
            ...
            raise ValueError(
                f'Module \'{module}\' not found in partial modulemaps. '
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--clang', type=pathlib.Path, help='Path to the Clang compiler binary.'
    )
    parser.add_argument(
        '--sysroot', type=pathlib.Path, help='Path to the sysroot directory.'
    )
    parser.add_argument(
        '--root_include_dir',
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            'Path to a modulemap to partially merge. '
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            'Name of a top-level module to selectively extract '
