...
class Compiler:
    def __init__(
        ...
        *,
        ...
        self.cpu = cpu
        self.sysroot_dir = (
            IncludeDir.SysrootModule if self.os.is_apple else IncludeDir.Sysroot
        )
        self.sysroot = None
    ...
    def include_dirs(self) -> list[tuple[pathlib.Path, IncludeDir]]:
        ...
        dirs = lines[
            lines.index('#include <...> search starts here:') + 1 : lines.index(
                'End of search list.'
            )
        ]
    ...
    def compile_all(self) -> dict[str, Header]:
        ...
        while uncompiled:
            ...
            if files is None:
                logging.warning(
                    "Failed to generate depfile while compiling %s", rel
                )
        ...
        for header in sorted(graph.values()):
            ...
            if header.abs is None:
                for d, kind in self.include_dirs:
                    if (
                        header.include_dir == kind
                        and (d / header.rel).is_file()
                    ):
            ...
            # likely are.
            if (
                header.compile_status == CompileStatus.NotCompiled
                and rdeps[header]
            ):
                ...
                header.deps = [
                    ...
                    if dep.rel != header.rel
                    or dep.include_dir > header.include_dir
                ]
