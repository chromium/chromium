...
def process_options():
    ...
    parser.add_option(
        ...
        help=(
            'Emit some extra debugging information (default off). This option '
    ...
    if options.src_dir:
        ...
        options.src_dir = os.path.dirname(
            os.path.dirname(os.path.realpath(__file__))
        )

    if not options.landmine_scripts:
        options.landmine_scripts = [
            os.path.join(options.src_dir, 'build', 'get_landmines.py')
        ]
...
def main():
    ...
    clobber_if_necessary(
        landmines, options.src_dir, os.path.normpath(landmines_path)
    )
