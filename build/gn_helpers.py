        ...
        for line in lines:
            ...
            if import_path.startswith("//"):
                ...
                if import_path.startswith("/"):
                    ...
                    raise GNError(
                        'Need /-prefix for an absolute path: %s' % import_path
                    )

            if not os.path.isabs(import_path):
                raise GNError(
                    'Unable to use relative path in import path: %s'
                    % import_path
                )
...
def CreateBuildCommand(output_directory):
    ...
    if output_directory and os.path.abspath(
        output_directory
    ...
    if siso_deps and ninja_deps:
        raise Exception(
            'Found both .siso_deps and .ninja_deps in '
