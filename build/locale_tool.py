...
def _ScanDirectoriesForFiles(scan_dirs, file_predicate):
    ...
    for src_dir in scan_dirs:
        for root, _, files in os.walk(src_dir):
            result.extend(
                os.path.join(root, f) for f in files if file_predicate(f)
            )
    return result
...
def _PrepareTinyGnWorkspace(work_dir, out_subdir_name='out'):
    ...
    _WriteFile(
        os.path.join(work_dir, '.gn'), 'buildconfig = "//BUILDCONFIG.gn"\n'
    )
...
''',
    ...
    _WriteFile(
        os.path.join(work_dir, 'toolchain', 'BUILD.gn'),
...
''',
    ...
    shutil.copyfile(
        os.path.join(_TOP_SRC_DIR, 'build', 'config', 'locales.gni'),
...
def _SortGrdElementsRanges(grd_lines, element_predicate):
    ...
    return _SortElementsRanges(
        grd_lines, element_predicate, _GetXmlLangAttribute
    )
...
def _CheckGrdElementRangeLang(grd_lines, start, end, wanted_locales):
    ...
    for pos in xrange(start, end):
        ...
        if not lang:
            errors.append(
                '%d: Missing "lang" attribute in <output> element' % pos + 1
            )
            continue
...
def _CheckGrdElementRangeAndroidOutputFilename(
    grd_lines, start, end, wanted_locales
):
    ...
    for pos in xrange(start, end):
        ...
        if not m:
            ...
            if not filename.endswith('.xml'):
                errors.append(
                    '%d: Filename should end with ".xml": %s'
                    % (pos + 1, filename)
                )

            dirname = os.path.basename(os.path.dirname(filename))
            prefix = (
                'values-%s' % resource_utils.ToAndroidLocaleName(cr_locale)
                if cr_locale != _DEFAULT_LOCALE
                else 'values'
            )
            if dirname != prefix:
                errors.append(
                    '%s: Directory name should be %s: %s'
...
def _CheckGrdAndroidOutputElements(grd_file, grd_lines, wanted_locales):
    ...
    errors = []
    for start, end in intervals:
        errors += _CheckGrdElementRangeLang(
            grd_lines, start, end, wanted_locales
        )
        errors += _CheckGrdElementRangeAndroidOutputFilename(
            grd_lines, start, end, wanted_locales
        )
    return errors
    ...
    for start, end in reversed(intervals):
        ...
        for locale in missing_locales:
            ...
            dst_line = src_line.replace(
                'lang="%s"' % src_locale, 'lang="%s"' % locale
            ).replace('values-%s/' % src_locale, 'values-%s/' % android_locale)
...
def _CheckGrdTranslationElementRange(grd_lines, start, end, wanted_locales):
    ...
    for pos in xrange(start, end):
        ...
        if not m:
            errors.append(
                '%d: Missing path attribute in <file> element' % pos + 1
            )
        else:
            filename = m.group(1)
...
def _CheckGrdTranslations(grd_file, grd_lines, wanted_locales):
    ...
    for start, end in intervals:
        ...
        errors += _CheckGrdTranslationElementRange(
            grd_lines, start, end, wanted_locales
        )
    return errors
...
def _AddMissingLocalesInGrdTranslations(grd_file, grd_lines, wanted_locales):
    ...
    for start, end in reversed(intervals):
        ...
        if not src_line:
            raise Exception(
                'Cannot find <file> element with "%s" lang attribute'
                % src_locale
            )
        ...
        for locale in missing_locales:
            dst_line = src_line.replace(
                'lang="%s"' % src_locale, 'lang="%s"' % locale
            ).replace('_%s.xtb' % src_locale, '_%s.xtb' % locale)
            grd_lines.insert(line_count, dst_line)
            line_count += 1
...
def _CheckGnOutputsRange(gn_lines, start, end, wanted_locales):
    ...
    for pos in xrange(start, end):
        ...
        if cr_locale in locales:
            errors.append(
                '%s: Redefinition of output for "%s" locale'
...
def _UpdateLocalesInExpectationLines(
    pyl_lines, wanted_locales, available_width=79
):
    ...
    while line_num < line_count:
        ...
        while line_num < line_count and not pyl_lines[
            line_num
        ].rstrip().endswith('],'):
            line_num += 1
            continue
...
'''
    ...
    def RegisterExtraArgs(self, group):
        group.add_argument(
            '--as-json', action='store_true', help='Output as JSON list.'
        )
        group.add_argument(
            '--type',
...
class _CheckInputFileBaseCommand(_Command):
    ...
    def RegisterExtraArgs(self, group):
        ...
        group.add_argument('input', nargs='*', help='Input file(s) to check.')
        group.add_argument(
            '--fix-inplace',
            action='store_true',
