...
def _GetSourceDirsFromSourceFiles(source_files):
    ...
    """
    return list(
        set(os.path.dirname(source_file) for source_file in source_files)
    )


def _CreateSourcesJsonFile(
    source_dirs, input_path, sources_json_file, src_root, root_build_dir
):
    ...
    for s in source_dirs:
        ...
        if abs_source[: len(src_root)] != src_root:
            print(
                'Error: found source directory not under repository root: %s %s'
                % (abs_source, src_root)
            )
            return 1
...
def _RunInstrumentCommand(parser):
    ...
    if args.target_sources_file:
        source_files.extend(
            build_utils.ReadSourcesList(args.target_sources_file)
        )
