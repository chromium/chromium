...
`deps_info['processed_jar_path']` unless this comes from a prebuilt `.aar` archive.
...
collection of all `deps_info['processed_jar_path']` entries for the target and all
its dependencies.
...
def main(argv):
  ...
  parser.add_option('--processed-jar-path',
                    help='Path to .jar to use at runtime.')
  ...
  device_lib_options = ['processed_jar_path', 'dex_path']
  required_options_map = {
      ...
      'java_library': ['build_config', 'processed_jar_path'] + lib_options,
  ...
  if is_java_target:
    ...
    if options.processed_jar_path:
      deps_info['processed_jar_path'] = options.processed_jar_path
    if options.dex_path:
  ...
  if is_java_target or options.type == 'android_app_bundle':
    ...
    if (is_java_target and options.processed_jar_path
        and options.type != 'dist_jar'):
      device_classpath.append(options.processed_jar_path)
    device_classpath.extend(
        c.get('processed_jar_path') for c in all_library_deps
        if c.get('processed_jar_path'))
  ...
  if options.type in ('dist_jar', 'java_binary', 'robolectric_binary'):
    ...
    host_classpath = []
    if options.processed_jar_path:
      host_classpath.append(options.processed_jar_path)
    host_classpath.extend(c['processed_jar_path'] for c in all_library_deps)
    # Collect all the dist_jar host jars.
    dist_jar_host_jars = [
        c['processed_jar_path'] for c in all_dist_jar_deps
        if 'processed_jar_path' in c
    ]
  ...
  if is_java_target:
    ...
    config['javac']['processor_classpath'] = [
        c['processed_jar_path'] for c in processor_deps.Direct()
        if c.get('processed_jar_path')
    ]
    config['javac']['processor_classpath'] += [
        c['processed_jar_path'] for c in processor_deps.All('java_library')
    ]
  ...
  if options.type in ('android_apk', 'android_app_bundle',
    ...
    if options.trace_events_jar_dir:
      ...
      for processed_jar_path in device_classpath:
        file_path = processed_jar_path.replace('../', '')
