...
class _TransitiveValues:
    ...
    def RemoveSubtree(
        ...
        for key, value in self.__dict__.items():
            if not (
                key in _TransitiveValues._NEVER_REMOVE
                or (retain_processed_jars and key == 'all_processed_jars')
                or (
                    retain_unprocessed_jars
                    and key in ('all_unprocessed_jars', 'all_interface_jars')
                )
                or (
                    retain_resource_zips
                    and key in ('dependency_zips', 'dependency_zip_overlays')
                )
...
class _TransitiveValuesBuilder:
    ...
    def Build(self):
        ...
        self._ret.proguard_configs.update(
            (
                direct_deps if params.get('direct_deps_only') else all_deps
            ).collect('proguard_configs', flatten=True)
        )
    ...
    def _CollectClasspath(self, direct_deps, all_deps):
        ...
        ret.direct_interface_jars.update(
            direct_deps.collect('interface_jar_path')
        )
        ret.all_unprocessed_jars.update(
            all_deps.collect('unprocessed_jar_path')
        )
        ...
        if params.collects_processed_classpath():
            ...
            ret.all_processed_jars.update(
                all_deps.collect('processed_jar_path')
            )
    ...
    def _AddBaseModuleToClasspath(self):
        ...
        self._ret.direct_unprocessed_jars.add(
            base_module['unprocessed_jar_path']
        )
    ...
    def _CollectManifests(self, all_deps_without_under_test):
        ...
        params = self._params
        ret = self._ret
        ret.android_manifests.update(
            params.get('mergeable_android_manifests', [])
        )
    ...
    def _CollectExtraPackageNames(self, direct_deps, all_deps):
        ...
        self._ret.extra_package_names.update(
            resource_deps.collect('package_name')
        )
    ...
    def _RemoveJarsOwnedByDistJars(self, dist_jar_params):
        ...
        if dist_jar_params.get('direct_deps_only'):
            ...
            dist_jar_tvs.all_unprocessed_jars = deps.collect(
    ...
    def _RemoveParentModuleOverlap(self):
        ...
        if parent_module := self._params.parent_module():
            x = _TransitiveValuesBuilder(
                parent_module, remove_parent_module_overlap=False
            ).Build()
...
def _PromoteToCommonAncestor(modules, child_to_ancestors, field_name):
    ...
    for d in dupes:
        ...
        for ancestor in child_to_ancestors[owning_modules[0]]:
            ...
            raise Exception(
                'Should have already removed ancestor dupes. '
                + ','.join(owning_modules)
                + ' field='
                + field_name
                + ' dupes: '
...
def _DoPlatformChecks(params):
    ...
    if params.get('bypass_platform_checks') or params.get(
        'requires_robolectric'
    ):
        return
    ...
    if deps_not_support_android and params.supports_android():
        raise Exception(
            ...
            + '\n'.join('* ' + d['gn_target'] for d in deps_not_support_android)
        )
...
def _SuffixAssets(config, target_config, suffix_string=None):
    ...
    config['uncompressed_assets'] = helper(
        suffix_names, suffix, config['uncompressed_assets']
    )
...
def _CreateLintConfig(params, javac_config, manifest_config):
    ...
    if params.is_bundle():
        ...
        manifests = OrderedSet(
            p['android_manifest'] for p in params.module_deps()
        )
...
def main():
    ...
    if is_bundle:
        ...
        if deps_proguard_enabled and deps_proguard_disabled:
            raise Exception(
                'Deps %s have proguard enabled while deps %s have '
                'proguard disabled'
    ...
    if has_classpath:
        ...
        javac_full_classpath = list(tv.all_unprocessed_jars) + list(
            tv.all_input_jars_paths
        )
        ...
        if proguard_enabled:
            main_config['proguard_classpath_jars'] = sorted(
                tv.all_input_jars_paths
            )
    ...
    if params.is_compile_type():
        ...
        turbine_config['interface_classpath'] = list(
            tv.direct_interface_jars
        ) + list(tv.direct_input_jars_paths)
        ...
        turbine_config['processor_classpath'] = _SortClasspath(
            processor_deps.recursive()
        ).collect('processed_jar_path')
        ...
        javac_config['javac_full_interface_classpath'] = list(
            tv.all_interface_jars
    ...
    if is_bundle:
        ...
        module_configs_by_name = {
            m['module_name']: m.build_config_json() for m in module_deps
        }
        ...
        for n, c in module_configs_by_name.items():
            ...
            if n == 'base':
                ...
                main_config['android_manifest'] = module_params[
                    'android_manifest'
                ]
    ...
    if is_apk_or_module:
        ...
        main_config['merged_android_manifest'] = params[
        ...
        main_config['native']['secondary_abi_libraries'] = (
        ...
        if is_bundle_module:
            ...
            main_config['native']['secondary_abi_loadable_modules'] = (
                secondary_abi_loadable_modules
            )
            main_config['native']['secondary_abi_placeholders'] = (
                secondary_abi_placeholder_paths
            )
