...
def _ParseArgs(args):
    ...
    input_opts.add_argument(
        ...
        'another application at runtime, and also ensures all resource IDs are '
    ...
    input_opts.add_argument(
        '--apk-under-test-rtxt',
        help='Path to the R.txt file of the APK under test.',
    )
    input_opts.add_argument(
    ...
    input_opts.add_argument(
        '--static-library-version', help='Version code for static library.'
    )
    ...
    input_opts.add_argument(
        '--webp-binary', default='', help='Path to the cwebp binary.'
    )
    input_opts.add_argument(
    ...
    output_opts.add_argument(
        '--proto-path', help='Apk output for proto format.'
    )
    output_opts.add_argument(
    ...
    if options.static_library_version and (
        options.static_library_version != options.version_code
    ):
...
def _FixManifest(options, temp_dir):
    ...
    fixed_package = (
        options.arsc_package_name or options.manifest_package or orig_package
    )
    ...
    if options.version_code:
        manifest_utils.NamespacedSet(
            manifest_node, 'versionCode', options.version_code
        )
    if options.version_name:
        manifest_utils.NamespacedSet(
            manifest_node, 'versionName', options.version_name
        )
    ...
    for min_sdk_node in manifest_node.iter(
        '{%s}min-sdk' % manifest_utils.DIST_NAMESPACE
    ):
...
def _CreateKeepPredicate(
    resource_exclusion_regex, resource_exclusion_exceptions
):
    ...
    return lambda path: (
        not re.search(resource_exclusion_regex, path)
        or build_utils.MatchesGlob(path, resource_exclusion_exceptions)
    )
...
def _FilterAapt2Warnings(output):
    # Filter warnings about attributes already defined in the base package.
    # These happen in instrumentation tests where the test APK has additional
    # dependencies (e.g. material design) that define attributes already present
    # in the base APK (e.g. via appcompat).
    pattern = r'.*warn: attribute \'.*\' already defined in base package .*'
    return build_utils.FilterLines(output, pattern)
...
def _PackageApk(options, build):
    ...
    stderr_filter = None
    if options.apk_under_test_rtxt:
        stderr_filter = _FilterAapt2Warnings
    build_utils.CheckOutput(
        link_command,
        before_join_callback=before_join,
    ...
    if actual_package_id is not None and actual_package_id != package_id:
        raise Exception(
            'Invalid package ID 0x%x (expected 0x%x)'
    ...
    with build_utils.TempDir() as tempdir:
        ...
        with open(fixed_manifest, encoding='utf-8') as f:
            return manifest_utils.NormalizeManifest(
                f.read(),
                options.verification_version_code_offset,
...
def main(args):
    ...
    with resource_utils.BuildContext(
        temp_dir=path, keep_files=bool(debug_temp_resources_dir)
    ) as build:
        manifest_package_name = _PackageApk(options, build)

        # If --app-as-shared-lib is used, all resources will be
    ...
    if options.depfile:
        assert options.srcjar_out, (
            'Update first output below and remove assert.'
        )
        depfile_deps = (
            options.dependencies_res_zips
            + options.dependencies_res_zip_overlays
        ...
        action_helpers.write_depfile(
            options.depfile, options.srcjar_out, depfile_deps
        )
