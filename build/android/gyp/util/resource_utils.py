...
def IsAndroidLocaleQualifier(string):
    ...
    return _RE_ANDROID_LOCALE_QUALIFIER_1.match(
        string
...
def IterResourceFilesInDirectories(
    directories, ignore_pattern=AAPT_IGNORE_PATTERN
):
...
class ResourceInfoFile:
    ...
    def AddMapping(self, archive_path, source_path):
        ...
        if new_value != source_path:
            raise Exception(
                'Duplicate AddMapping for "{}". old={} new={}'.format(
                ...
                for v in values:
                    try:
                        if not v.startswith('0x'):
                            ...
                            new_values.append(
                                lookup_table[(resource_type, name)]
                            )
                        else:
                            new_values.append(v)
                    except:
        ...
        with open(r_txt_path, 'w', encoding='utf-8') as f:
            for entry in entries:
                ...
                if self.IsResourceFinal(entry):
                    value = re.sub(
                        r'0x(?:00|7f)',
                        '0x{:02x}'.format(self.final_package_id),
                        value,
                    )
                f.write(
                    '{} {} {} {}\n'.format(
                        entry.java_type, entry.resource_type, entry.name, value
                    )
                )
...
def CreateRJavaFiles(
    ...
    if apk_under_test_rtxt:
        for entry in _ParseTextSymbolsFile(
            apk_under_test_rtxt, fix_package_ids=True
        ):
...
class _ResourceBuildContext:
    ...
    def __init__(self, temp_dir=None, keep_files=False):
        ...
        self.proguard_main_dex_path = os.path.join(
            self.temp_dir, 'maindex.flags'
        )
        ...
        self.optimized_proto_path = os.path.join(
            self.temp_dir, 'out.opt.proto.ap_'
        )
...
def FilterAndroidResourceStringsXml(xml_file_path, string_predicate):
    ...
    if string_deletion:
        new_xml_data = GenerateAndroidResourceStringsXml(
            strings_map, namespaces
        )
