...
def CallAndWriteDepfileIfStale(
    on_stale_md5,
    options,
    record_path=None,
    input_paths=None,
    ...
    if hasattr(options, 'depfile') and options.depfile:
        action_helpers.write_depfile(
            options.depfile, output_paths[0], depfile_deps
        )
...
class Changes:
    ...
    def DescribeDifference(self):
        ...
        if self.old_metadata.StringsMd5() != self.new_metadata.StringsMd5():
            ndiff = difflib.ndiff(
                self.old_metadata.GetStrings(), self.new_metadata.GetStrings()
            )
        ...
        for path in self.IterModifiedPaths():
            ...
            lines.extend(
                '  -> Subpath added: ' + p for p in self.IterAddedSubpaths(path)
            )
            lines.extend(
                '  -> Subpath removed: ' + p
