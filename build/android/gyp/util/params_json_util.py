...
class _HashableList(list):
    ...
    def _check_not_sealed(self):
        assert not self._sealed, (
            'Cannot mutate a DepsList after it has been used.'
        )
...
class ParamsJson(dict):
    ...
    def javac_build_config_json(self):
        ...
        return get_build_config(
            self.build_config_path('.javac.build_config.json')
        )
    ...
    def needs_full_javac_classpath(self):
        """Returns True if the target type runs manifest_merger.py."""
        return (
            self.type in ('android_apk', 'android_app_bundle_module')
            or (
                self.is_library()
                and self.get('needs_full_javac_classpath', False)
            )
            or (self.type == 'dist_jar' and self.requires_android())
        )
    ...
    def has_classpath(self):
        ...
        if self.is_library():
            return bool(
                ...
                or self.get('bytecode_rewriter_target')
                or not self.is_prebuilt()
            )
    ...
    def needs_transitive_rtxt(self):
        ...
        return self.type == 'dist_aar' or (
            self.is_library() and not self.is_prebuilt()
        )
    ...
    def module_deps(self):
        ...
        deps = sorted(
            self.deps().of_type('android_app_bundle_module'),
    ...
    def parent_module(self):
        ...
        if self['module_name'] == 'base':
            assert not module_deps, (
                'Base module should not depend on '
    ...
    def base_module(self):
        ...
        if self.is_bundle():
            return next(
                x for x in self.deps() if x.get('module_name') == 'base'
            )
    ...
    def resource_deps(self):
        ...
        return (
            self.deps().recursive_resource_deps().of_type('android_resources')
        )
