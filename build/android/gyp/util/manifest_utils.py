...
def SetUsesSdk(
    manifest_node, target_sdk_version, min_sdk_version, max_sdk_version=None
):
...
def NormalizeManifest(
    manifest_contents, version_code_offset, library_version_offset
):
    _RegisterElementTreeNamespaces()
    ...
    for l in dom.toprettyxml(indent='  ').splitlines():
        ...
        if len(l) > _WRAP_LINE_LENGTH and any(x in l for x in _WRAP_CANDIDATES):
            ...
            if end_tag == '/>':
                out_lines.append(
                    '{}{}>'.format(indent, start_tag.replace('<', '</'))
                )
        else:
            out_lines.append(l)
