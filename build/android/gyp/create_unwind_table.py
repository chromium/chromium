...
def ReadFunctionCfi(stream: TextIO) -> Iterable[FunctionCfi]:
    ...
    for line in FilterToNonTombstoneCfi(stream):
        ...
        if cfi_init_match:
            ...
            if (
                current_function_address is not None
                and current_function_size is not None
            ):
                yield FunctionCfi(
                    current_function_size, tuple(current_function_address_cfi)
                )
            current_function_address = int(cfi_init_match.group(1), 16)
            current_function_size = int(cfi_init_match.group(2), 16)
            current_function_address_cfi = [
                AddressCfi(
                    int(cfi_init_match.group(1), 16), cfi_init_match.group(3)
                )
            ]
        else:
...
def EncodeStackPointerUpdate(offset: int) -> bytes:
    ...
    if 0x04 <= abs_offset <= 0x200:
        ...
        if abs_offset >= 0x104:
            instructions.append(
                instruction_code | ((abs_offset - 0x100 - 4) >> 2)
            )
        try:
            return EncodeAsBytes(*instructions)
        except ValueError as e:
            raise RuntimeError(
                'offset = %d produced out of range value' % offset
            ) from e
    else:
        # This only encodes positive sp movement.
        assert offset > 0, offset
        return EncodeAsBytes(
            0b10110010  # vsp = vsp + 0x204 + (uleb128 << 2)
...
def EncodePop(registers: Sequence[int]) -> bytes:
    ...
    Returns:
      A list of arm unwind instructions as bytes.
    """
    assert all(r in range(4, 16) for r in registers), (
        f'Can only pop r4 ~ r15. Registers:\n{registers}.'
    )
    ...
    non_lr_registers_continuous_from_r4 = sorted(non_lr_registers) == list(
        range(4, 4 + len(non_lr_registers))
    )

    if (
        pop_lr
        and 0 < len(non_lr_registers) <= 8
        and non_lr_registers_continuous_from_r4
    ):
        instructions.append(
            0b10101000 | (len(non_lr_registers) - 1)  # Pop r4-r[4+nnn], r14.
        )
    else:
        register_bits = 0
...
def EncodeAddressUnwind(address_unwind: AddressUnwind) -> bytes:
    ...
    if address_unwind.unwind_type == UnwindType.UPDATE_SP_AND_OR_POP_REGISTERS:
        return (
            ...
            if address_unwind.sp_offset
            else b''
        ) + (
            EncodePop(address_unwind.registers)
            if address_unwind.registers
            else b''
        )
    ...
    if address_unwind.unwind_type == UnwindType.RESTORE_SP_FROM_REGISTER:
        ...
        return EncodeAsBytes(
            0b10010000 | address_unwind.registers[0]  # Set vsp = r[nnnn].
        ) + (
            EncodeStackPointerUpdate(address_unwind.sp_offset)
            if address_unwind.sp_offset
            else b''
...
class UnwindInstructionsParser(abc.ABC):
    ...
    @abc.abstractmethod
    def ParseFromMatch(
        self, address_offset: int, cfa_sp_offset: int, match: re.Match
    ) -> Tuple[AddressUnwind, int]:
...
class PushOrSubSpParser(UnwindInstructionsParser):
    ...
    def ParseFromMatch(
        ...
        new_cfa_sp_offset, ra_cfa_offset = (
            int(group) if group else None for group in match.groups()[:2]
        )
        ...
        return AddressUnwind(
            ...
            sp_offset,
            tuple(registers),
        ), new_cfa_sp_offset or cfa_sp_offset
...
class VPushParser(UnwindInstructionsParser):
    ...
    def ParseFromMatch(
        ...
        if new_cfa_sp_offset is None:
            return (
                AddressUnwind(address_offset, UnwindType.NO_ACTION, 0, ()),
                cfa_sp_offset,
            )
...
class StoreSpParser(UnwindInstructionsParser):
    ...
    def ParseFromMatch(
        ...
        return AddressUnwind(
            address_offset,
            UnwindType.RESTORE_SP_FROM_REGISTER,
            sp_offset,
            (register,),
        ), new_cfa_sp_offset


def EncodeUnwindInstructionTable(
    complete_instruction_sequences: Iterable[bytes],
    ...
    ordered_sequences = sorted(
        ref_counts.keys(), key=ComputeScore, reverse=True
    )
...
def EncodeAddressUnwinds(
    ...
    complete_instruction_sequences = [
        b''.join(unwind_instructions[i:])
        for i in range(len(unwind_instructions))
    ]

    encoded_unwinds: List[EncodedAddressUnwind] = []
    for address_unwind, sequence in zip(
        sorted_address_unwinds, complete_instruction_sequences
    ):
...
# in the last page.
REFUSE_TO_UNWIND: Tuple[EncodedAddressUnwind, ...] = (
...
def EncodeFunctionUnwinds(
    function_unwinds: Iterable[FunctionUnwind], text_section_start_address: int
    ...
    for unwind in sorted_function_unwinds:
        ...
        if prev_func_end_address < unwind.address:
            ...
            gaps += 1
            yield EncodedFunctionUnwind(
                GetPageNumber(prev_func_end_address),
                GetPageOffset(prev_func_end_address),
                TRIVIAL_UNWIND,
            )

        yield EncodedFunctionUnwind(
            GetPageNumber(unwind.address),
    ...
    logging.info(
        '%d/%d gaps between functions filled with trivial unwind.',
        gaps,
...
def EncodeFunctionOffsetTable(
    ...
    for sequence in encoded_address_unwind_sequences:
        ...
        for address_offset, complete_instruction_sequence in sequence:
            ...
            function_offset_table += Uleb128Encode(
                instruction_offset
            ) + Uleb128Encode(
...
def EncodePageTableAndFunctionTable(
    ...
    """
    page_function_unwinds: Dict[int, List[EncodedFunctionUnwind]] = (
        collections.defaultdict(list)
    )
    for function_unwind in function_unwinds:
        page_function_unwinds[function_unwind.page_number].append(
            function_unwind
        )
    ...
    for page_number, same_page_function_unwinds in sorted(
        ...
        raw_page_table.extend(
            [function_table_index] * (number_of_empty_pages + 1)
        )
        assert page_number == len(raw_page_table) - 1
...
def ParseAddressCfi(
    ...
    for parser in parsers:
        ...
        address_unwind, cfa_sp_offset = parser.ParseFromMatch(
            address_cfi.address - function_start_address,
            prev_cfa_sp_offset,
            match,
        )
        ...
        in_epilogue = (
            ...
            and address_unwind.unwind_type
            != UnwindType.RESTORE_SP_FROM_REGISTER
        )

        return (
            address_unwind if not in_epilogue else None,
            in_epilogue,
            cfa_sp_offset,
        )
...
def GenerateUnwinds(
    function_cfis: Iterable[FunctionCfi],
    parsers: Tuple[UnwindInstructionsParser, ...],
    ...
    for function_cfi in function_cfis:
        ...
        for address_cfi in function_cfi.address_cfi:
            ...
            address_unwind, in_epilogue, cfa_sp_offset = ParseAddressCfi(
                address_cfi,
                function_cfi.address_cfi[0].address,
                parsers,
                cfa_sp_offset,
            )
        ...
        if address_unwinds:
            ...
            yield FunctionUnwind(
                function_cfi.address_cfi[0].address,
                function_cfi.size,
                tuple(address_unwinds),
            )
...
def EncodeUnwindInfo(
    page_table: bytes,
    function_table: bytes,
    function_offset_table: bytes,
    unwind_instruction_table: bytes,
) -> bytes:
...
def GenerateUnwindTables(
    ...
    encoded_address_unwind_sequences: List[
        Tuple[EncodedAddressUnwind, ...]
    ] = []
    ...
    function_offset_table, function_offset_table_offsets = (
        EncodeFunctionOffsetTable(
            encoded_address_unwind_sequences, unwind_instruction_table_offsets
        )
    )
...
def main():
    ...
    proc = subprocess.Popen(
        ['./' + args.dump_syms_path, args.input_path, '-v'],
        stdout=subprocess.PIPE,
        encoding='utf-8',
    )
    ...
    (
        ...
        function_offset_table,
        unwind_instruction_table,
    ) = GenerateUnwindTables(encoded_function_unwinds)
    unwind_info: bytes = EncodeUnwindInfo(
        page_table,
        function_table,
        function_offset_table,
        unwind_instruction_table,
    ...
    if proc.wait():
        logging.critical(
            'dump_syms exited with return code %d', proc.returncode
        )
