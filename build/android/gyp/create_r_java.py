...
def main(args):
    ...
    parser.add_argument(
        '--srcjar-out', required=True, help='Path to output srcjar.'
    )
    parser.add_argument(
        '--deps-rtxts',
        required=True,
    ...
    parser.add_argument(
        '--r-package', required=True, help='R.java package to use.'
    )
    ...
    action_helpers.write_depfile(
        options.depfile, options.srcjar_out, inputs=options.deps_rtxts
    )
