...
COMMON_CIPD_CACHE_DIR = os.path.join(
    constants.DIR_SOURCE_ROOT, '.android_emulator_cipd_cache'
)
...
def _FindMinSdkFile(apk_dir, min_sdk):
    ...
    with open(json_file) as f:
        ...
        for entry in data:
            if (
                entry['min_sdk'] > curr_min_sdk_version
        ...
        if not min_sdk_found:
            logging.error(
                'No suitable apk file found that suits the minimum sdk %d.',
                min_sdk,
            )
            return None

        logging.info(
            'Found apk file for mininum sdk %d: %r with version %r',
            min_sdk,
            min_sdk_found['file_name'],
...
def _ProcessSourceProps(source_prop_path, image_version):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), 'package.xml.template'), 'r'
    ) as f:
    ...
    package_xml_path = os.path.join(
        os.path.dirname(source_prop_path), 'package.xml'
    )
    ...
    signature_path = os.path.join(
        os.path.dirname(source_prop_path), _RAW_IMAGE_SIGNATURE_FILE
    )
...
class _AvdManagerAgent:
    ...
    def __init__(self, avd_home, sdk_root):
        ...
        fake_tools_dir = os.path.join(
            self._sdk_root, 'non-existent-tools', 'non-existent-version'
        )
        self._env.update(
            {
                'ANDROID_AVD_HOME': self._avd_home,
                'AVDMANAGER_OPTS': '-Dcom.android.sdkmanager.toolsdir=%s'
                % fake_tools_dir,
...
class AvdConfig:
    ...
    def __init__(self, avd_proto_path):
        ...
        self.avd_proto_name = os.path.splitext(
            os.path.basename(avd_proto_path)
        )[0]
    ...
    def emulator_home(self):
        ...
        """
        return os.path.join(
            COMMON_CIPD_ROOT, self.GetDestPath(self._config.avd_package)
        )
    ...
    def system_image_name(self):
        ...
        base_path = os.path.join(
            COMMON_CIPD_ROOT, self.GetDestPath(self.system_image_package)
        )
    ...
    def system_image_dir(self):
        ...
        if not self._system_image_dir:
            ...
            self._system_image_dir = os.path.join(
                base_path, *self.system_image_name.split(';')
            )
            if not os.path.exists(self._system_image_dir):
                raise AvdException(
                    'System image dir %r does not exist'
    ...
    def _qt_config_path(self):
        ...
        qt_config_dir = os.path.join(
            self.xdg_config_dir, 'Android Open Source Project'
        )
    ...
    def HasSnapshot(self, snapshot_name):
        ...
        snapshot_path = os.path.join(
            self._avd_dir, 'snapshots', snapshot_name, 'ram.bin'
        )
    ...
    def Create(
        ...
        avd_manager = _AvdManagerAgent(
            avd_home=self.avd_home, sdk_root=self.emulator_sdk_root
        )
        ...
        try:
            ...
            for pkg in self._config.additional_apk:
                ...
                if os.path.exists(apk_file):
                    logging.info(
                        'Adding additional apk for install: %s', apk_file
                    )
            ...
            for pkg in self._config.privileged_apk:
                ...
                if os.path.exists(apk_file):
                    logging.info(
                        'Adding privilege apk for install: %s', apk_file
                    )
                    privileged_apk_tuples.append(
                        (
                            apk_file,
                            self._config.install_privileged_apk_partition,
                        )
                    )
            ...
            assert instance.device is not None, (
                '`instance.device` not initialized.'
            )
            ...
            instance.device.WaitUntilFullyBooted(
                decrypt=True, timeout=360, retries=0
            )
            logging.info(
                'The build fingerprint of the system is %r',
                instance.device.build_fingerprint,
            )

            if additional_apks:
                for apk in additional_apks:
                    instance.device.Install(
                        apk, allow_downgrade=True, reinstall=True
                    )
                    package_name = apk_helper.GetPackageName(apk)
                    package_version = instance.device.GetApplicationVersion(
                        package_name
                    )
                    logging.info(
                        'The version for package %r on the device is %r',
                        package_name,
                        package_version,
                    )

            if privileged_apk_tuples:
                system_app.InstallPrivilegedApps(
                    instance.device, privileged_apk_tuples
                )
            ...
            if snapshot:
                logging.info(
                    'Wait additional 60 secs before saving snapshot for AVD'
                )
                time.sleep(60)
            ...
            multiInstanceLockFile = os.path.join(
                self._avd_dir, 'multiinstance.lock'
            )
            ...
            package_def_content = {
                ...
                'root': self.emulator_home,
                'install_mode': 'copy',
                'data': [
                    {'dir': os.path.relpath(self._avd_dir, self.emulator_home)},
                    {
                        'file': os.path.relpath(
                            self._root_ini_path, self.emulator_home
                        )
                    },
                    {
                        'file': os.path.relpath(
                            self._features_ini_path, self.emulator_home
                        )
                    },
                ],
            }
            ...
            with tempfile_ext.TemporaryFileName(
                ...
                cipd_create_cmd = [
                    ...
                    % self.system_image_package.version,
                ]
                if cipd_json_output:
    ...
    def _ProcessRawSystemImage(self):
        ...
        for root, _, files in os.walk(os.path.join(dest_path, 'system-images')):
            if _RAW_IMAGE_SIGNATURE_FILE in files:
                with open(
                    os.path.join(root, _RAW_IMAGE_SIGNATURE_FILE), 'r'
                ) as f:
        ...
        zip_pattern = os.path.join(
            dest_path, 'sdk-repo-linux-system-images-*.zip'
        )
        zip_files = glob.glob(zip_pattern)
        if not zip_files:
            raise AvdException(
                f'Cannot find raw system image file {zip_pattern!r}'
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for root, _, files in os.walk(temp_dir):
                if 'source.properties' in files:
                    if source_properties_dir:
                        raise AvdException(
                            'Found multiple source.properties files.'
                        )
            ...
            system_image_path = os.path.join(
                self.emulator_sdk_root, *package_path.split(';')
            )
            logging.info(
                'Moving system_image files to the final destination %r.',
                system_image_path,
            )
    ...
    def IsAvailable(self):
        ...
        for cipd_root, pkgs in self._IterCipdPackages(
            _PACKAGES_RUNTIME, check_version=False
        ):
            stdout = subprocess.run(
                ['cipd', 'installed', '--root', cipd_root],
                capture_output=True,
                check=False,
                encoding='utf8',
            ).stdout
    ...
    def Uninstall(self):
        ...
        for cipd_root, _ in self._IterCipdPackages(
            _PACKAGES_ALL, check_version=False
        ):
    ...
    def UninstallRawSystemImage(self):
        ...
        if self._config.HasField('raw_system_image_package'):
            ...
            if os.path.exists(target_path):
                logging.info(
                    'Deleting processed raw system image in %r', target_path
                )
    ...
    def _RebaseQcow2Images(self):
        ...
        for f in _BACKING_FILES:
            ...
            logging.info(
                'Rebasing the qcow2 image %r with the backing file %r',
                qcow2_image_path,
                backing_file_path,
            )
            cmd_helper.RunCmd(
                [
                    ...
                    os.path.relpath(
                        backing_file_path, os.path.dirname(qcow2_image_path)
                    ),
                    qcow2_image_path,
                ]
            )
    ...
    def _IterCipdPackages(self, packages, check_version=True):
        ...
        for pkg in self._ListPackages(packages):
            if pkg.version:
                ...
                raise AvdException(
                    'Expecting a version for the package %s' % pkg.package_name
                )
    ...
    def _InstallCipdPackages(self, packages, check_version=True):
        ...
        for cipd_root, pkgs in self._IterCipdPackages(
            packages, check_version=check_version
        ):
    ...
    def _UpdateConfigs(self):
        ...
        hw_ini_glob_pattern = os.path.join(
            self._avd_dir, 'snapshots', '*', 'hardware.ini'
        )
...
class _AvdInstance:
    ...
    def Start(
        ...
        if self.HasSystemSnapshot():
            ...
            if read_only:
                logging.info(
                    'System snapshot found. Set "read_only=False" '
                    'to load it properly.'
                )
                read_only = False
        ...
        with (
            tempfile_ext.TemporaryFileName() as socket_path,
            ...
            if not gpu_mode:
                gpu_mode = (
                    self._avd_config.avd_launch_settings.gpu_mode
            ...
            try:
                with measures.time_consumption(
                    'emulator', 'start', 'listen_for_serial'
                ):
        ...
        try:
            ...
            if ensure_system_settings:
                ...
                self.device.WaitUntilFullyBooted(
                    timeout=360 if is_slow_start else 90, retries=retries
                )
    ...
    def Stop(self, force=False):
        ...
        if self._output_file:
            if self._debug_tags:
                if self._output_manager:
                    self._output_manager.ArchiveArchivedFile(
                        self._output_file, delete=True
                    )
...
def _EnsureSystemSettings(device):
    ...
    adb_output = device.RunShellCommand(
        get_long_press_timeout_cmd, check_return=True
    )
    ...
    if device.build_version_sdk >= version_codes.BAKLAVA:
        ...
        if not device.is_desktop and device.IsApplicationInstalled(
