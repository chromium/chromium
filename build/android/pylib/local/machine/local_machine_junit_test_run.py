...
class LocalMachineJunitTestRun(test_run.TestRun):
    ...
    def _CreateJvmArgsList(self, for_listing=False, allow_debugging=True):
        ...
        if self._test_instance.coverage_dir and not for_listing:
            ...
            if self._test_instance.coverage_on_the_fly:
                ...
                jvm_args.append(
                    '-Djacoco-agent.destfile=%s' % jacoco_coverage_file
                )
    ...
    def _QueryTestJsonConfig(
        self, temp_dir, allow_debugging=True, enable_shadow_allowlist=False
    ):
        ...
        jvm_args = self._CreateJvmArgsList(
            for_listing=True, allow_debugging=allow_debugging
        )
        if jvm_args:
        ...
        if enable_shadow_allowlist and self._test_instance.shadows_allowlist:
            cmd += [
                '--shadows-allowlist',
                self._test_instance.shadows_allowlist,
            ]
    ...
    def _MakeJob(
        self, shard_id, temp_dir, test_group, properties_jar_path, json_config
    ):
    ...
    def _GetJsonConfig(self):
        with tempfile_ext.NamedTemporaryDirectory() as temp_dir:
            return self._QueryTestJsonConfig(
                temp_dir, allow_debugging=False, enable_shadow_allowlist=True
            )
    ...
    def GetTestsForListing(self):
        ...
        for config in json_config.get('disabled', {}).values():
            for class_name, methods in config.items():
                ret.extend(
                    f'{class_name}.{method} (disabled)' for method in methods
                )
        ret.sort()
        return ret
    ...
    def _RunTestsInternal(self, temp_dir, results, raw_logs_fh):
        if self._test_instance.json_config:
            ...
            try:
                ...
                results.append(
                    _MakeUnknownFailureResult('Filter matched no tests')
                )
                return
        ...
        if shard_filter:
            logging.warning(
                'Running test shards: %s using %s concurrent process(es)',
        ...
        jobs = [
            self._MakeJob(
                i, temp_dir, test_groups[i], properties_jar_path, json_config
            )
            for i in shard_list
        ]
        ...
        log_lines = []
        current_test = None
        for line in RunCommandsAndSerializeOutput(
            jobs, num_workers, quiet=self._test_instance.quiet
        ):
            if raw_logs_fh:
        ...
        if failed_jobs and not self._test_instance.quiet:
            for job in failed_jobs:
                print(
                    f'To re-run failed shard {job.shard_id}, use --json-config '
        ...
        num_expected = sum(
            len(methods)
            for classes in json_config['configs'].values()
        ...
        if num_actual != num_expected:
            ...
            if self._test_instance.run_disabled:
                sb.append(
                    'The missing tests could be due to not using '
        ...
        if json_config.get('disabled') and not self._test_instance.run_disabled:
            num_disabled = sum(
                len(methods)
                for classes in json_config['disabled'].values()
                for methods in classes.values()
            )
...
def _MakeUnknownFailureResult(name, details=None):
    ...
    results_list = [
        base_test_result.BaseTestResult(
            name, base_test_result.ResultType.UNKNOWN, log=details
        )
    ]
    test_run_results = base_test_result.TestRunResults()
    test_run_results.AddResults(results_list)
...
def RunCommandsAndSerializeOutput(jobs, num_workers, quiet=False):
    ...
    def run_proc(idx):
        ...
        job = jobs[idx]
        proc = cmd_helper.Popen(
            job.cmd, stdout=s_out, stderr=s_err, env=getattr(job, 'env', None)
        )
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as pool:
        ...
        yield from _StreamFirstShardOutput(
            jobs[0], futures[0].result(), quiet=quiet
        )
