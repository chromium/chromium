    ...
    def decorator(f):
        ...
        def wrapper(dev, *args, **kwargs):
            try:
                ...
                logging.exception(
                    'Shard timed out: %s(%s)', f.__name__, str(dev)
                )
...
class LocalDeviceEnvironment(environment.Environment):
    def __init__(self, args, output_manager, _error_func):
        ...
        if hasattr(args, 'skia_gold_consider_unsupported'):
            self._skia_gold_consider_unsupported = (
                args.skia_gold_consider_unsupported
            )
        ...
        def prepare_device(d):
            ...
            if force_main_user:
                ...
                if d.GetCurrentUser() != main_user:
                    logging.info(
                        'Switching to the main user with id %s', main_user
                    )
            ...
            if d.build_version_sdk >= version_codes.BAKLAVA and d.HasRoot():
                ...
                if not d.is_desktop and d.IsApplicationInstalled(
                    device_utils.GBOARD_PKG
                ):
    ...
    def TearDown(self):
        ...
        for m in self._logcat_monitors:
            try:
                ...
                logging.exception(
                    'Failed to stop logcat monitor for %s',
                    m.adb.GetDeviceSerial(),
                )
            except IOError:
                logging.exception(
                    'Failed to locate logcat for device %s',
                    m.adb.GetDeviceSerial(),
                )
    ...
    def DenylistDevice(self, device, reason='local_device_failure'):
        ...
        with self._devices_lock:
            self._devices = [
                d for d in self._devices if str(d) != device_serial
            ]
