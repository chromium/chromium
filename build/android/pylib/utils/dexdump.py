...
def Dump(apk_path):
    ...
    try:
        ...
        parsed_dex_files = []
        for dex_file in build_utils.ExtractAll(
            apk_path, dexfile_dir, pattern='*classes*.dex'
        ):
            ...
            clean_xml = BAD_XML_CHARS.sub('\ufffd', output_xml)
...
def _ParseAnnotations(dexRaw: str) -> Dict[int, Annotations]:
    ...
    annotationsBlockMatcher = re.compile('^Class #.*annotations:$')
    # We want to retrieve the index of the class
    # Eg: Class #12 annotations -> 12
    classIndexMatcher = re.compile('(?<=#)[0-9]*')
    ...
    for line in dexRaw.splitlines():
        if currentAnnotationsForClass is None:
            ...
            if annotationsBlockMatcher.match(line):
                ...
                currentAnnotationsForClass = Annotations(
            ...
            if line.startswith('Annotations on class'):
                currentAnnotationsBlock = (
                    currentAnnotationsForClass.classAnnotations
                )
            elif line.startswith('Annotations on method'):
                method = methodMatcher.findall(line)[0]
                currentAnnotationsBlock = {}
                currentAnnotationsForClass.methodsAnnotations[method] = (
                    currentAnnotationsBlock
                )

            # If we match against any other type of annotations
            # we will ignore them
            elif line.startswith('Annotations on'):
            ...
            # being used)
                ...
                currentAnnotationsBlock is not None
                and line.strip().startswith('VISIBILITY_RUNTIME')
            ):
                annotationName, annotationValuesStr = annotationMatcher.findall(
                    line
                )[0]
                ...
                currentAnnotationsBlock.update(
...
def _ParseRootNode(root, annotations: Dict[int, Annotations]):
    ...
    for child in root:
        if child.tag == 'package':
            ...
            parsed_node, classCount = _ParsePackageNode(
                child, classCount, annotations
            )
...
def _ParsePackageNode(
    package_node, classCount: int, annotations: Dict[int, Annotations]
):
    ...
    for child in package_node:
        if child.tag == 'class':
            classes[child.attrib['name']] = _ParseClassNode(
                child, classCount, annotations
            )
            classCount += 1
...
def _ParseClassNode(
    class_node, classIndex: int, annotations: Dict[int, Annotations]
):
    ...
    return {
        ...
        'annotations': annotations.get(
            classIndex, Annotations(classAnnotations={}, methodsAnnotations={})
        ),
    }
