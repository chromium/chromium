...
DexHeader = collections.namedtuple(
    'DexHeader', ','.join(t[0] for t in _DEX_HEADER_FMT)
)
...
_MethodIdItem = collections.namedtuple(
    'MethodIdItem', 'type_idx,proto_idx,name_idx'
)
...
class _ClassDefItemList(_MemoryItemList):
    def __init__(self, reader, offset, size):
        ...
        def factory(x):
            return _ClassDefItem(
                *(x.ReadUInt() for _ in range(len(_ClassDefItem._fields)))
            )
...
class _DexReader:
    ...
    def _DecodeMUtf8(self, string_length, offset):
        ...
        for _ in range(string_length):
            a = self.ReadUByte()
            if a == 0:
                raise _MUTf8DecodeError(
                    'Early string termination encountered',
                    string_length,
                    offset,
                )
            if (a & 0x80) == 0x00:
                ...
                if (b & 0xC0) != 0x80:
                    raise _MUTf8DecodeError(
                        'Error in byte 2', string_length, offset
                    )
                ...
                if (b & 0xC0) != 0x80 or (c & 0xC0) != 0x80:
                    raise _MUTf8DecodeError(
                        'Error in byte 3 or 4', string_length, offset
                    )
...
class _MUTf8DecodeError(Exception):
    def __init__(self, message, length, offset):
        message += (
            ' (decoded string length: {}, string data offset: {:#x})'.format(
                length, offset
            )
        )
...
class DexFile:
    ...
    def __init__(self, data):
        ...
        self.type_item_list = _TypeIdItemList(
            self.reader, self.header.type_ids_off, self.header.type_ids_size
        )
...
class _DumpClasses(_DumpCommand):
    def Run(self):
        for class_item in self._dexfile.class_def_item_list:
            ...
            superclass_string = self._dexfile.GetTypeString(
                class_item.superclass_idx
            )
            interfaces = self._dexfile.GetTypeListStringsByOffset(
                class_item.interfaces_off
            )
            access_flags = DexFile.ResolveClassAccessFlags(
                class_item.access_flags
            )
            print(
                '{} (superclass={}, interfaces={}, access_flags={})'.format(
    ...
    if os.path.splitext(args.input)[1] in ('.apk', '.jar', '.zip', '.aab'):
        with zipfile.ZipFile(args.input) as z:
            ...
            if not dex_file_paths:
                print(
                    'Error: {} does not contain any classes.dex files'.format(
                        args.input
                    )
                )
                sys.exit(1)
