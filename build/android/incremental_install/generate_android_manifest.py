...
def _ParseArgs(args):
    ...
    parser.add_argument(
        '--dst-manifest', required=True, help='The output modified manifest.'
    )
    parser.add_argument(
...
def _ProcessManifest(path, disable_isolated_processes):
    ...
    if real_acf:
        app_node.set(
            _AddNamespace('appComponentFactory'),
            _INCREMENTAL_APP_COMPONENT_FACTORY,
        )
    ...
    for i, instrumentation_node in enumerate(instrumentation_nodes):
        ...
        instrumentation_node.set(
            _AddNamespace('name'), _INCREMENTAL_INSTRUMENTATION_CLASSES[i]
        )
        _CreateMetaData(
            app_node,
            _META_DATA_INSTRUMENTATION_NAMES[i],
...
def main(raw_args):
    ...
    new_manifest_data = _ProcessManifest(
        options.src_manifest, options.disable_isolated_processes
    )
