...
def _InstallBundle(
    devices, apk_helper_instance, modules, fake_modules, locales=None
):
...
def _SetWebViewProvider(devices, package_name):
    ...
    def switch_provider(device):
        if device.build_version_sdk < version_codes.NOUGAT:
            logging.error(
                'No need to switch provider on pre-Nougat devices (%s)',
                device.serial,
            )
        else:
...
def _ResolveActivity(
    device, package_name, category, action, preferred_activity=None
):
    # E.g.:
    ...
    if not matched_entries:
        raise Exception(
            f'Did not find {category_text}, {action_text} in\n{data}'
        )
    ...
    if len(activity_names) > 1:
        ...
        raise Exception(
            'Found multiple launcher activities:\n * '
...
def _ReadDeviceFlags(device, command_line_flags_file):
    ...
    old_flags = device.RunShellCommand(
        f'cat {device_path} 2>/dev/null',
        as_root=True,
        shell=True,
        check_return=False,
        raw_output=True,
...
def _UpdateDeviceFlags(device, command_line_flags_file, new_flags):
    if not command_line_flags_file:
        if new_flags:
            logging.warning(
                'Command-line flags are not configured for this target.'
            )
        return
    ...
    if new_flags is None:
        if old_flags:
            logging.warning(
                'Using pre-existing command-line flags: %s', old_flags
            )
        return

    if new_flags != old_flags:
        adb_command_line.CheckBuildTypeSupportsFlags(
            device, command_line_flags_file
        )
        ...
        if new_flags:
            logging.info(
                'Updated flags file: %s with value: %s', device_path, new_flags
            )
...
def _LaunchUrl(
    ...
    def launch(device):
        activity = _ResolveActivity(
            device, package_name, category, action, preferred_activity
        )
        ...
        launch_intent = intent.Intent(
            action=action, activity=activity, data=url, package=package_name
        )
...
def _RunGdb(
    ...
    if not pid:
        debug_process_name = _NormalizeProcessName(
            debug_process_name, package_name
        )
...
def _RunLldb(
    ...
    print(
        _Colorize(
            'All subsequent output is from connect_lldb.sh script.',
            colorama.Fore.YELLOW,
        )
    )
...
def _PrintPerDeviceOutput(devices, results, single_line=False):
    for d, result in zip(devices, results):
        ...
        sys.stdout.write(
            _Colorize(
                '{} ({}):'.format(d, d.build_description), colorama.Fore.YELLOW
            )
        )
...
def _ParseMeminfo(usage):
    ...
    for metric in metrics:
        match_metric = re.search(
            r'^\s+' + metric + r':\s+(\d+)', usage, re.MULTILINE
        )
        if match_metric:
...
def _PrintMemUsageSummary(num_processes, total_pss, total_uss, summary_metrics):
    ...
    print(
        'Total USS (private clean+dirty): %d KB (%.1f MB)'
...
def _RunMemUsage(devices, package_name, query_app=False):
    ...
    for result in _PrintPerDeviceOutput(devices, all_results):
        if not result:
            ...
            if num_processes > 0:
                _PrintMemUsageSummary(
                    num_processes, total_pss, total_uss, summary_metrics
                )
...
def _DuHelper(device, path_spec, run_as=None):
    ...
    lines = device.RunShellCommand(
        cmd_str, run_as=run_as, shell=True, check_return=False
    )
    output = '\n'.join(lines)
    ...
    if output.startswith('run-as:'):
        ...
        lines = device.RunShellCommand(
            cmd_str, as_root=True, shell=True, check_return=False
        )
    ret = {}
    try:
        for line in lines:
...
def _RunDiskUsage(devices, package_name):
    ...
    def disk_usage_helper(d):
        ...
        package_output = re.sub(
            r'Hidden system packages:.*?^\b',
            '',
            package_output,
            flags=re.S | re.M,
        )
        ...
        try:
            ...
            lib_path = re.search(
                r'(?:legacyN|n)ativeLibrary(?:Dir|Path)=(.*)', package_output
            ).group(1)
        except AttributeError as e:
            raise Exception(
                'Error parsing dumpsys output: ' + package_output
            ) from e

        if code_path.startswith('/system'):
            logging.warning(
                'Measurement of system image apks can be innacurate'
            )
        ...
        for m in re.finditer(
            awful_wrapping + r'([\s\S]+?)[\],]', package_output
        ):
        ...
        data_dir_sizes = _DuHelper(
            d, '%s/{*,.*}' % data_dir, run_as=package_name
        )
        ...
        if code_cache_dir:
            ...
            code_cache_sizes = _DuHelper(
                d, '%s/{*,.*}' % code_cache_dir, run_as=package_name
            )
    ...
    for result in _PrintPerDeviceOutput(devices, all_results):
        ...
        print_sizes(
            'odex (compilation_filter=%s)' % compilation_filter, odex_sizes
        )
        if show_warning:
            logging.warning('For a more realistic odex size, run:')
            logging.warning(
                '    %s compile-dex [speed|speed-profile]', sys.argv[0]
            )
...
class _LogcatProcessor:
    ...
    class NativeStackSymbolizer:
        ...
        def AddLine(self, parsed_line, dim):
            ...
            # pass them through.
            is_crash_line = parsed_line.tag == 'DEBUG' or (
                self._STACK_PATTERN.match(parsed_line.message)
            )
    ...
    _DALVIK_IGNORE_PATTERN = re.compile(
        '|'.join(
            [
                r'^Added shared lib',
                r'^Could not find ',
    ...
    def __init__(
        ...
        self._pid_pattern = re.compile(
            r'Start proc (\d+):{}/'.format(package_name)
        )
        ...
        self.nonce = 'Chromium apk_operations.py nonce={}'.format(
            random.random()
        )
        ...
        if user_regex:
            ...
            if not self._user_defined_highlight:
                print(
                    _Colorize(
                        'Rejecting invalid regular expression: {}'.format(
                            user_regex
                        ),
    ...
    def _PrintParsedLine(self, parsed_line, dim=False):
        if LOGCAT_LEVELS.find(parsed_line.priority) < self._log_level_idx:
            return
        if self._filter and not (
            self._filter.search(parsed_line.tag)
            or self._filter.search(parsed_line.message)
        ):
            return
        if self._exit_on_match and self._exit_on_match.search(
            parsed_line.message
        ):
        ...
        msg_style = (
            pid_style
            if not user_match
            else (colorama.Fore.GREEN + colorama.Style.BRIGHT)
        )
        ...
        tag = _Colorize(
            '{:8}'.format(parsed_line.tag),
            pid_style + ('' if dim else colorama.Style.BRIGHT),
        )
        priority = _Colorize(
            parsed_line.priority, self._GetPriorityStyle(parsed_line.priority)
        )
    ...
    def ProcessLine(self, line):
        ...
        if (
            ...
            or self._verbose
            or (
                log.priority == 'F'  # Java crash dump
                or log.tag in self._ALLOWLISTED_TAGS
            )
        ):
            if nonce_found:
    ...
    for line in device.adb.Logcat(logcat_format='threadtime', iter_timeout=1):
        try:
            ...
            if timeout is not None and now - start_time >= timeout:
                print(
                    f'Stopping logcat because timeout of {timeout}s was reached.'
                )
                return
            ...
            if logcat_processor.FoundNonce():
                ...
                if logcat_processor.HasActivePids():
                    ...
                    if dead_since is None:
                        ...
                        - max(
                            dead_since,
                            logcat_processor.last_stack_seen_time or 0,
                        )
                        >= 1.0
                    ):
                        print(
                            'Stopping logcat because the process stopped.'
...
def _GetPackageProcesses(device, package_name):
    ...
    return [
        p
        for p in device.ListProcesses(package_name)
...
def _RunPs(devices, package_name):
    ...
    all_processes = parallel_devices.pMap(
        lambda d: _GetPackageProcesses(d, package_name)
    ).pGet(None)
...
def _RunProfile(
    ...
    with tempfile.NamedTemporaryFile() as fh:
        ...
        simpleperf.ConvertSimpleperfToPprof(
            host_simpleperf_out_path, host_build_directory, pprof_out_path
        )
        print(
            textwrap.dedent(
                """
...
class _StackScriptContext:
    ...
    def __init__(
        self, output_directory, apk_path, bundle_generation_info, quiet=False
    ):
    ...
    def _CreateStaging(self):
        ...
        if bundle_generation_info:
            ...
            with zipfile.ZipFile(
                bundle_generation_info.bundle_apks_path, 'r'
            ) as z:
                files_to_extract = [
...
def _GenerateMissingAllFlagMessage(devices):
    return (
        'More than one device available. Use --all to select all devices, '
...
class _Command:
    ...
    def _CreateApkHelpers(self, args, incremental_apk_path, install_dict):
        ...
        if self.apk_helper is None:
            if args.apk_path:
                ...
                bundle_apks_path = (
                    os.path.splitext(args.bundle_path)[0] + '.apks'
                )
    ...
    def _FindSupportedDevices(self, devices):
        ...
        logging.debug(
            'App supports (requires 32bit: %r, is webview: %r): %r',
            requires_32_bit,
            is_webview,
            app_abis,
        )
        ...
        for device in devices:
            ...
            if device_primary_abi in ('x86', 'x86_64'):
                device_abis = [
                    abi for abi in device_abis if not abi.startswith('arm')
                ]
    ...
    def ProcessArgs(self, args):
        ...
        if self.needs_package_name and not args.package_name:
            if self._CreateApkHelpers(
                ...
                self._parser.error(
                    'One of --package-name or --apk-path is required.'
                )
        ...
        if self.need_device_args:
            ...
            if not self._CreateApkHelpers(
                ...
                fully_supported, not_supported_reasons = (
                    self._FindSupportedDevices(available_devices)
                )
                reason_string = '\n'.join(
                ...
                if args.devices:
                    if reason_string:
                        logging.warning(
                            'Devices not supported: %s', reason_string
                        )
                    self.devices = available_devices
                    ...
                    raise Exception(
                        'Cannot find any supported devices for this app.\n\n'
                        f'{reason_string}'
                    )
            ...
            try:
                if len(self.devices) > 1:
                    if not self.supports_multiple_devices:
                        self._parser.error(
                            device_errors.MultipleDevicesError(self.devices)
                        )
                    if not args.all and not args.devices:
                        self._parser.error(
                            _GenerateMissingAllFlagMessage(self.devices)
                        )
...
class _PackageInfoCommand(_Command):
    ...
    def Run(self):
        ...
        if len(str(self.apk_helper.GetVersionCode())) == 9:
            ...
            is_webview = (
                _IsWebViewProvider(self.apk_helper) and not _IsTrichrome()
            )
            x = android_chrome_version.TranslateVersionCode(
                str(self.apk_helper.GetVersionCode()), is_webview
            )
            print(
                f'Decoded versionCode: build_number={x.build_number} '
    ...
    def _RegisterExtraArgs(self, group):
        if self.is_bundle:
            group.add_argument(
                '--locales',
                action='append',
                help='Locale splits to install (english is in base, so always installed).',
            )
            group.add_argument(
                '-m',
                '--module',
...
class _SetWebViewProviderCommand(_Command):
    ...
    description = (
        "Sets the device's WebView provider to this APK's package name."
    )
    needs_package_name = True
    needs_apk_helper = True
    ...
    def Run(self):
        if not _IsWebViewProvider(self.apk_helper):
            raise Exception(
                'This package does not have a WebViewLibrary meta-data '
...
class _LaunchCommand(_Command):
    name = 'launch'
    description = (
        'Sends a launch intent for the APK or bundle after first '
    ...
    def Run(self):
        ...
        if self.args.wait_for_java_debugger:
            if self.apk_helper and not self.apk_helper.GetIsDebuggable():
                raise Exception(
                    'Passed --wait-for-java-debugger flag but did not set '
...
class _ArgvCommand(_Command):
    ...
    def Run(self):
        ...
        outputs = parallel_devices.pMap(
            lambda d: _ReadDeviceFlags(d, command_line_flags_file) or ''
        ).pGet(None)
...
"""
    ...
    def Run(self):
        _RunGdb(
            self.devices[0],
            self.args.package_name,
            self.args.debug_process_name,
            self.args.pid,
            self.args.output_directory,
            self.args.target_cpu,
            self.args.port,
            self.args.ide,
...
"""
    ...
    def _RegisterExtraArgs(self, group):
        ...
        group.add_argument(
            '--ndk-dir', help='Select alternative NDK root directory.'
        )
        group.add_argument(
            '--lldb-server', help='Select alternative on-device lldb-server.'
        )
...
"""
    ...
    def Run(self):
        ...
        if self.is_test_apk and self.additional_apk_helpers:
            for additional_apk_helper in self.additional_apk_helpers:
                extra_package_names.append(
                    additional_apk_helper.GetPackageName()
                )
        ...
        if self.args.exit_on_crash:
            ...
            pkg = (
                '(?:' + '|'.join(re.escape(n) for n in all_packages_names) + ')'
            )
            exit_on_match += (
                f'Fatal signal.*{pkg}|App crashed.*{pkg}|Force stopping.*{pkg}'
            )
        try:
            _RunLogcat(
                self.devices[0],
    ...
    def _RegisterExtraArgs(self, group):
        ...
        group.add_argument(
            '--timeout', type=float, help='Exit logcat after this many seconds.'
        )
        group.add_argument(
            '--exit-on-crash',
            action='store_true',
...
class _MemUsageCommand(_Command):
    ...
    def Run(self):
        _RunMemUsage(
            self.devices, self.args.package_name, query_app=self.args.query_app
        )
...
class _ShellCommand(_Command):
    name = 'shell'
    description = (
        'Same as "adb shell <command>", but runs as the apk\'s uid '
...
class _CompileDexCommand(_Command):
    ...
    def Run(self):
        outputs = _RunCompileDex(
            self.devices, self.args.package_name, self.args.compilation_filter
        )
...
class _PrintCertsCommand(_Command):
    ...
    def _RegisterExtraArgs(self, group):
        group.add_argument(
            ...
            action='store_true',
            help=(
                "Print the certificate's full signature, Base64-encoded. "
    ...
    def Run(self):
        ...
        if self.is_bundle:
            ...
            with tempfile.NamedTemporaryFile() as f:
                logging.warning(
                    'Bundles are not signed until turned into .apk files.'
                )
                logging.warning(
                    'Showing signing info based on associated keystore.'
                )
                cmd = [
                    keytool,
                    '-exportcert',
                    '-keystore',
                    self.bundle_generation_info.keystore_path,
                    '-storepass',
                    self.bundle_generation_info.keystore_password,
                    '-alias',
                    self.bundle_generation_info.keystore_alias,
                    '-file',
                    f.name,
                ]
            ...
            def run_apksigner(min_sdk_version):
                ...
                env['PATH'] = os.path.pathsep.join(
                    [os.path.join(_JAVA_HOME, 'bin'), env.get('PATH')]
                )
...
class _ProfileCommand(_Command):
    name = 'profile'
    description = (
        'Run the simpleperf sampling CPU profiler on the currently-'
    ...
    def _RegisterExtraArgs(self, group):
        group.add_argument(
            ...
            default='browser',
            help=(
                'Which process to profile. This may be a process name or pid '
        ...
        group.add_argument(
            ...
            default=None,
            help=(
                '(Optional) Profile only a single thread. This may be either a '
        ...
        group.add_argument(
            ...
            default='cpu-cycles',
            help=(
                'A comma separated list of perf events to capture '
    ...
    def Run(self):
        ...
        _RunProfile(
            ...
            self.args.package_name,
            self.args.output_directory,
            self.args.profile_output,
            self.args.profile_process,
            self.args.profile_thread,
            self.args.profile_events,
            extra_args,
        )
...
class _RunCommand(_InstallCommand, _LaunchCommand, _LogcatCommand):
    name = 'run'
    description = (
        'Install, launch, and show logcat (when targeting one device).'
    )
    all_devices_by_default = False
    supports_multiple_devices = True
...
class _BuildBundleApks(_Command):
    ...
    description = (
        'Build the .apks archive from an Android app bundle, and '
    ...
    def _RegisterExtraArgs(self, group):
        group.add_argument(
            '--output-apks',
            required=True,
            help='Destination path for .apks file.',
        )
        group.add_argument(
            '--minimal',
            action='store_true',
...
class _StackCommand(_Command):
    ...
    def Run(self):
        ...
        try:
            ...
            if proc.wait():
                raise Exception(
                    'stack script returned {}'.format(proc.returncode)
                )
        finally:
            context.Close()
