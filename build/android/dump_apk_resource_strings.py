...
class ResourceStringValues:
    ...
    def AddValue(self, res_name, res_config, res_value):
        ...
        if res_name is not self.res_name and res_name != self.res_name:
            if self.res_name is None:
                ...
                print(
                    'WARNING: Resource key ignored (%s, should be %s)'
                    % (res_name, self.res_name)
                )

        if self.res_values.setdefault(res_config, res_value) is not res_value:
            print(
                'WARNING: Duplicate value definition for [config %s]: %s '
                '(already has %s)'
    ...
    def ToStringList(self, res_id):
        ...
        values = sorted(
            [(str(config), value) for config, value in self.res_values.items()]
        )
        if res_id is None:
            ...
            result = [
                ...
                % (res_id, self.res_name, len(values))
            ]
...
class ResourceStringMap:
    ...
    def ToStringList(self, omit_ids=False):
        ...
        # ID.
        for res_id, _ in sorted(
            res_map.items(), key=lambda x: (x[1].res_name, x[0])
        ):
...
assert (
    _RE_BUNDLE_STRING_DEFAULT_VALUE.match(
        '        (default) - [STR] "More options"'
    ).group(1)
    == "More options"
)
    ...
    while True:
        ...
        if need_value:
            ...
            if not m:
                # Should not happen
                sys.stderr.write(
                    'WARNING: Missing value for string ID 0x%08x "%s"'
...
def main(args):
    ...
    if IsFilePathABundle(options.input_file):
        ...
        def create_string_map():
            return ParseBundleResources(
                options.bundletool_path, options.input_file
            )
