    ...
    try:
        ...
        if popen is not None:
            try:
                ...
                logging.warning(
                    'Failed to kill %s. Process may be leaked.', str(popen.pid)
                )
...
def main():
    ...
    if os.environ.get('SWARMING_TASK_ID'):
        ...
        if not logdog_butler_bin or not os.path.exists(logdog_butler_bin):
            parser.error(
                'Either --logdog-bin-cmd must be specified and valid or '
    ...
    with tempfile.TemporaryDirectory() as temp_directory:
        if logdog_butler_bin:
            if os.path.exists(logdog_butler_bin):
                ...
                logging.warning(
                    '--logdog-bin-cmd specified, but binary was not found.'
        ...
        with NoLeakingProcesses(logdog_proc):
            with NoLeakingProcesses(
                subprocess.Popen(test_cmd, env=test_env)
            ) as test_proc:
