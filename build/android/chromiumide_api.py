...
def _gn_gen(output_dir: str) -> None:
    ...
    cmd = [
        sys.executable,
        os.path.join(_DEPOT_TOOLS_PATH, 'gn.py'),
        'gen',
        output_dir,
    ]
...
def _process_sources(
    source_files: List[str], output_dir: str, source_path_set: Set[str]
) -> None:
...
def _process_params(
    ...
    class_path_set: Set[str],
    source_jar_set: Set[str],
) -> None:
    ...
    if target_sources_file := params.get('target_sources_file'):
        ...
        _process_sources(
            build_utils.ReadSourcesList(
                os.path.join(output_dir, target_sources_file)
            ),
            output_dir,
            source_path_set,
        )
...
def _scan_params(output_dir: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    return (
        sorted(source_path_set),
        sorted(class_path_set),
        sorted(source_jar_set),
    )
...
def _extract_source_jar(source_jar: str) -> str:
    ...
    subprocess.check_call(
        [
            os.path.join(
                _SRC_ROOT, 'third_party', 'jdk', 'current', 'bin', 'jar'
            ),
            '-x',
            '-f',
