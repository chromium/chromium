...
def ZapTimestamp(filename):
    ...
    if filename.endswith('.tlb'):
        ...
        assert (
            contents[custom_off : custom_off + 6] == b'\x08\x00\x3e\x00\x00\x00'
        )
        assert re.match(
            rb'Created by MIDL version 8\.\d\d\.\d{4} '
            rb'at ... Jan 1. ..:..:.. 2038\n',
            contents[custom_off + 6 : custom_off + 6 + 0x3E],
        )
        # Second: Type uint32 (0x13) storing 0x7fffffff (followed by WW / 0x57 pad)
        assert (
            contents[custom_off + 6 + 0x3E : custom_off + 6 + 0x3E + 8]
            == b'\x13\x00\xff\xff\xff\x7f\x57\x57'
        )
        # Third: Type uint32 (0x13) storing MIDL compiler version.
        assert (
            contents[custom_off + 6 + 0x3E + 8 : custom_off + 6 + 0x3E + 8 + 2]
            == b'\x13\x00'
        )
        ...
        contents = re.sub(
            rb'File created by MIDL compiler version 8\.\d\d\.\d{4} \*/\r\n'
            rb'/\* at ... Jan 1. ..:..:.. 2038',
            rb'File created by MIDL compiler version 8.xx.xxxx */\r\n'
            rb'/* at a redacted point in time',
            contents,
        )
        contents = re.sub(
            rb'    Oicf, W1, Zp8, env=(.....) \(32b run\), '
            rb'target_arch=(AMD64|X86) 8\.\d\d\.\d{4}',
            rb'    Oicf, W1, Zp8, env=\1 (32b run), target_arch=\2 8.xx.xxxx',
            contents,
        )
        ...
        contents = contents.replace(
            b'#endif !_MIDL_USE_GUIDDEF_', b'#endif // !_MIDL_USE_GUIDDEF_'
        )
        ...
        contents = contents.replace(
            b'0x801026c, /* MIDL Version 8.1.620 */',
...
def overwrite_guids_proxy(proxy_file, dynamic_guids):
    ...
    for key in dynamic_guids:
        contents = re.sub(
            get_uuid_format_proxy_file(key),
...
def setguid(contents, offset, guid):
    ...
    struct.pack_into(
        '<IHH8s', contents, offset, *(guid.fields[0:3] + (guid.bytes[8:],))
    )
    ...
    *flags,
    ...
    if not os.path.isdir(source):
        ...
        if sys.platform != 'win32':
            print(
                'Directory %s needs to be populated from Windows first' % source
            )
            return 1
    ...
    for source_file in common_files:
        ...
        if not os.path.isfile(file_path):
            ...
            if sys.platform != 'win32':
                print(
                    'File %s needs to be generated from Windows first'
                    % file_path
                )
                return 1
    ...
    if dynamic_guids != 'none':
        ...
        dynamic_guids = re.sub(
            'PLACEHOLDER-GUID-', '', dynamic_guids, flags=re.I
        )
    ...
    if dynamic_guids:
        ...
        idl = posixpath.join(
            outdir, os.path.splitext(os.path.basename(idl_template))[0] + '.idl'
        )
    ...
    args = (
        ...
        + (['/tlb', tlb] if tlb else [])
        + ['/h', h]
        + (['/dlldata', dlldata] if dlldata else [])
        + ['/iid', iid]
