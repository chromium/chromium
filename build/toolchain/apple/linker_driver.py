...
def main(args):
    ...
    try:
        ...
        # Run the linker driver actions, in the order specified by the actions
        # list.
...
def run_dsym_util(dsym_path_prefix, full_args):
    ...
    tools_paths = _find_tools_path(full_args)
...
def set_dsymutil_path(dsymutil_path, full_args):
    ...
    an alternate dsymutil. This action is always processed before the
    run_dsym_util action.
...
def set_strip_path(strip_path, full_args):
    ...
    an alternate strip. This action is always processed before the run_strip
    action.
...
def _find_tools_path(full_args):
...
_LINKER_DRIVER_ACTIONS = [
    ('dsymutilpath,', set_dsymutil_path),
    ('dsym,', run_dsym_util),
    ('unstripped,', run_save_unstripped),
    ('strippath,', set_strip_path),
    ('strip,', run_strip),
]
