...
bool MatchesExpectedType(crosapi::mojom::HandleType expected_handle_type,
...
void DidGetDriveFSMetadata(crosapi::mojom::HandleType expected_handle_type,
  ...
  if (error != drive::FILE_ERROR_OK || metadata.is_null() ||
      ...
      !MatchesExpectedType(expected_handle_type, metadata->type)) {
...
void GetCloudIdentifier(const base::FilePath& virtual_path,
  ...
  // TODO(b/279580265) handle `kFileSystemTypeProvided` for e.g. OneDrive
  // support.
