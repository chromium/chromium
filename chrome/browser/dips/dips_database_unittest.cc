...
TEST_P(DIPSDatabaseQueryTest, EnsureLastInteractionBeforeRangeStart) {
  // Verify that the |last_interaction| should be strictly less than
  // |range_start|.
  EXPECT_CHECK_DEATH(
      db_->GetSitesThatBounced(Time::FromDoubleT(0), Time::FromDoubleT(1)));
  EXPECT_CHECK_DEATH(
      db_->GetSitesThatBounced(Time::FromDoubleT(0), Time::FromDoubleT(0)));
}
