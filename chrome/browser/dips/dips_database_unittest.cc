...
TEST_P(DIPSDatabaseTest, AddUpdateQueryDeleteBounce) {
  ...
  EXPECT_EQ(state1->site_storage_time, storage_time1);
  EXPECT_FALSE(state1->user_interaction_time.has_value());
  ...
  EXPECT_EQ(state2->user_interaction_time, interaction_time2);

  // Update the second.
  absl::optional<base::Time> storage_time2 = Time::FromDoubleT(3);
  state2->site_storage_time = storage_time2;
  EXPECT_TRUE(db_->Write(site2, state2->site_storage_time,
                         state2->user_interaction_time));
  // Query the second again.
  absl::optional<StateValue> updated_state2 = db_->Read(site2);
  ASSERT_TRUE(updated_state2.has_value());
  EXPECT_EQ(updated_state2->site_storage_time, storage_time2);
  EXPECT_EQ(updated_state2->user_interaction_time, interaction_time2);
