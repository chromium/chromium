...
IN_PROC_BROWSER_TEST_P(DIPSTabHelperBrowserTest, SitesInOpenTabsAreExempt) {
  ...
  // Open b.test on a new tab in the same window/profile.
  chrome::NewTab(browser());
  ASSERT_TRUE(ui_test_utils::NavigateToURL(browser(), GURL("http://b.test")));
  ...
  // Verify that the DIPS record for b.test is still present, because the
  // tracker was exempt.
  ASSERT_TRUE(GetDIPSState(GURL("http://b.test")).has_value());
}
#endif  // !BUILDFLAG(IS_ANDROID)
...
// This feature is not implemented on Android.
// Multi-profile is implemented differently on ChromeOS, which relies on the
