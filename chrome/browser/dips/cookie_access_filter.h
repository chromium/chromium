...
class CookieAccessFilter {
 public:
  ...
  // Clear `result` and fill it with the indices of the URLs that accessed
  // cookies. Returns true iff every previously-recorded cookie access was
  // successfully matched to a URL in `urls`. (Note: this depends on the order
  // of previous calls to AddAccess()).
