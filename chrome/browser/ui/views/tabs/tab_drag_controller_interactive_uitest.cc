...
class TabDragControllerInactiveWaiter {
 public:
  TabDragControllerInactiveWaiter() = default;
  TabDragControllerInactiveWaiter(const TabDragControllerInactiveWaiter&) =
      delete;
  TabDragControllerInactiveWaiter& operator=(
      const TabDragControllerInactiveWaiter&) = delete;
  ~TabDragControllerInactiveWaiter() = default;

  void Wait() {
    CHECK(TabDragController::IsActive());
    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
        FROM_HERE,
        base::BindOnce(&TabDragControllerInactiveWaiter::CheckForInactive,
...
void DragToSeparateWindowStep3(DetachToBrowserTabDragControllerTest* test,
                               TabStrip* not_attached_tab_strip,
                               TabStrip* attached_tab_strip) {
  // Should now be attached to attached_tab_strip.
  ...
  ASSERT_TRUE(TabDragController::IsActive());
  EXPECT_FALSE(GetIsDragged(test->browser()));
  EXPECT_TRUE(test->IsTabDraggingInfoCleared(not_attached_tab_strip));
  EXPECT_TRUE(test->IsTabDraggingInfoSet(attached_tab_strip));

  // Drag to the trailing end of the tabstrip to ensure we're in a predictable
  // spot within the strip.
  test->StopAnimating(attached_tab_strip);
  ASSERT_TRUE(test->DragInputToCenterNotifyWhenDone(
      attached_tab_strip->tab_at(1), base::BindLambdaForTesting([=]() {
...
void DragToSeparateWindowStep2(DetachToBrowserTabDragControllerTest* test,
  ...
  if (test::PlatformSupportsScreenCoordinates()) {
    ...
    EXPECT_TRUE(test->DragInputToNotifyWhenDone(
        target_point, std::move(step3), test->GetWindowHint(target_tab_strip)));
  } else {
    EXPECT_TRUE(test->DragInputToCenterNotifyWhenDone(target_tab_strip,
                                                      std::move(step3)));
  }
}
...
IN_PROC_BROWSER_TEST_P(DetachToBrowserTabDragControllerTest,
  ...
  test::TabDragControllerInactiveWaiter waiter;
  Tab* tab = tab_strip->tab_at(0);
  ASSERT_TRUE(PressInputAtCenter(tab));
  auto step3 =
      base::BindOnce(&DragToSeparateWindowStep3, this, tab_strip, tab_strip2);
  ASSERT_TRUE(DragInputToCenterNotifyWhenDone(
      tab,
  ...
  // We can't use QuitDraggingObserver here because for system-DnD-based tab
  // dragging that waits until the DnD session ends, which won't happen until we
  // release the mouse.
  // We also can't use BrowserChangeObserver to wait for the browser holding the
  // dragged tabs to be removed when attaching to tab_strip2, because the DnD
  // session may also use a RunLoop. This loop will be the inner-most one, and
  // we can't quit BrowserChangeObserver's loop until the DnD loop quits, which
  // again won't happen until we release the mouse.
  // Thus we have to use multiple steps and *AndNotify() methods to drive the
  // test flow, and wait here until TabDragController becomes fully inactive.
  waiter.Wait();
...
void DragToSeparateWindowDuringDragEndStep3(
...
IN_PROC_BROWSER_TEST_P(DetachToBrowserTabDragControllerTest,
  ...
  // See DragToSeparateWindow for why we drive the test like this.
  test::TabDragControllerInactiveWaiter waiter;
  Tab* tab = tab_strip->tab_at(0);
  ASSERT_TRUE(PressInputAtCenter(tab));
  auto step3 = base::BindOnce(&DragToSeparateWindowDuringDragEndStep3, this,
...
IN_PROC_BROWSER_TEST_P(DetachToBrowserTabDragControllerTestWithTabbedSystemApp,
  ...
  // Move to the first tab and drag it enough so that it detaches, but not
  // enough that it attaches to browser2.
  DragTabAndNotify(tab_strip1,
                   base::BindOnce(&DragToSeparateWindowStep2, this, tab_strip1,
                                  tab_strip2, base::DoNothing()));
