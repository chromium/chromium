...
IN_PROC_BROWSER_TEST_F(SidePanelCoordinatorTest, ChangeSidePanelWidth) {
  ...
  // Set the browser width so that two thirds of the browser would be less than
  // the minimum side panel width.
  gfx::Rect current_browser_bounds(browser()->GetBrowserView().GetBounds());
  new_bounds = current_browser_bounds;
  new_bounds.set_width((min_side_panel_width - 1) * 3 / 2);
  browser()->GetBrowserView().SetBounds(new_bounds);
  browser_width = browser()->GetBrowserView().GetLocalBounds().width();
  two_thirds_browser_width = browser_width * 2 / 3;
