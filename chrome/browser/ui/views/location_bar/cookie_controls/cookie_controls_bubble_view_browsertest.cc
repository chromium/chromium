...
IN_PROC_BROWSER_TEST_F(CookieControlsBubbleViewBrowserTest,
  ...
  // There's a little bit of a race here, as the completed reload will also
  // cause the bubble to close. But that requires a trip through the renderer
  // and back, and even when the alternative pathway wins, the test passes. So
  // it avoids flakes, but doesn't gives 100% coverage.
  bubble_view()->GetWidget()->CloseWithReason(
