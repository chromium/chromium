...
constexpr int kIllustrationHeight = 56;
...
std::unique_ptr<NonAccessibleImageView> BuildIllustrationView(bool is_dark) {
  ...
  int preferred_width = kIllustrationHeight *
                        illustration->GetPreferredSize().width() /
                        illustration->GetPreferredSize().height();
  illustration->SetImageSize(gfx::Size(preferred_width, kIllustrationHeight));
  return illustration;
}
...
CombinedSelectorSheetView::BuildStepSpecificContent() {
  return std::make_pair(BuildMechanismsView(), AutoFocus::kNo);
}

std::unique_ptr<HoverListView>
CombinedSelectorSheetView::BuildMechanismsView() {
  return std::make_unique<HoverListView>(
      std::make_unique<TransportHoverListModel>(
          static_cast<CombinedSelectorSheetModel*>(model())->dialog_model()));
}
