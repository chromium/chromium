...
class ReloadButtonAccessibilityTest : public InteractiveBrowserTest,
 ...
 public:
  ...
  ui::AXNodeData GetReloadAXNodeData(ui::TrackedElement* el,
                                     const char* file,
                                     int line) {
    if (auto* const view_el = el->AsA<views::TrackedElementViews>()) {
      ...
      ui::AXNode* body_element = root->GetFirstUnignoredChild();
      if (!body_element) {
        ADD_FAILURE_AT(file, line) << "Could not get AXNode body_element";
        return {};
      }
      ui::AXNode* reload_button = body_element->GetFirstUnignoredChild();
      if (!reload_button) {
        ADD_FAILURE_AT(file, line) << "Could not get AXNode reload_button";
        return {};
      }
  ...
  static ui::AXNodeData GetAXNodeData(const ui::TrackedElement* el) {
    ui::AXNodeData data;
    if (auto* const view_el = el->AsA<views::TrackedElementViews>()) {
      view_el->view()->GetViewAccessibility().GetAccessibleNodeData(&data);
    } else if (auto* const webui_el = el->AsA<ui::TrackedElementWebUI>()) {
      ui::AXNode* root =
          webui_el->handler()->web_contents()->GetAccessibilityRootNode();
      if (root) {
  ...
  static std::u16string GetReloadButtonTooltip(const ui::TrackedElement* el) {
    return GetAXNodeData(el).GetString16Attribute(
        ax::mojom::StringAttribute::kDescription);
  }

  static bool GetReloadButtonHasPopup(const ui::TrackedElement* el) {
    return GetAXNodeData(el).GetIntAttribute(
...
IN_PROC_BROWSER_TEST_P(ReloadButtonAccessibilityTest, NormalReload) {
  ASSERT_TRUE(embedded_test_server()->Start());
...
#endif
IN_PROC_BROWSER_TEST_P(ReloadButtonAccessibilityTest, MAYBE_ContextMenu) {
  ASSERT_TRUE(embedded_test_server()->Start());
  GURL url = embedded_test_server()->GetURL("/title1.html");
  RunTestSequence(
      InstrumentTab(kWebContentsElementId),
      ...
      // Right-click without DevTools: should not show a menu.
      WaitForReloadButtonHasPopup(false),
      ...
      // Wait for DevTools to affect the reload button state.
      // The reload button should now indicate it has a popup.
      WaitForReloadButtonHasPopup(true));
}
