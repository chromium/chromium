...
class FakeThemeService : public ThemeService {
 public:
  ...
  void BuildAutogeneratedThemeFromColor(SkColor color) override {
    color_ = color;
    theme_id_ = ThemeService::kAutogeneratedThemeID;
    NotifyThemeChanged();
  }

  void UseTheme(ui::SystemTheme system_theme) override {
    theme_id_ = ThemeHelper::kDefaultThemeID;
  ...
  std::string GetThemeID() const override {
    return theme_id_;
  }

 private:
  raw_ptr<ThemeSyncableService> theme_syncable_service_ = nullptr;
  std::string theme_id_ = ThemeHelper::kDefaultThemeID;
  SkColor color_ = 0;
};
