...
class DiceWebSigninInterceptionBubbleBrowserTest
 ...
 public:
  ...
  AvatarToolbarButtonInterface* GetAvatarButton() {
  ...
  views::BubbleAnchor GetAvatarAnchor(Browser* browser_arg = nullptr) {
    ...
    BrowserView* browser_view =
        BrowserView::GetBrowserViewForBrowser(browser_arg);
    return browser_view->toolbar_button_provider()
        ->GetAvatarToolbarButtonInterface()
        ->GetBubbleAnchor(*browser_arg);
  }
  ...
  GetTestBubbleParametersWithInterceptType(
    ...
    // Produces a color that meets the necessary conditions (DCHECKs in profile
    // colors used). Mirrors they way the DiceWebSigninInterceptor sets the
    // color.
    auto profile_highlight_color =
        GetAutogeneratedThemeColors(GenerateNewProfileColor(entry).color)
            .frame_color;

    return WebSigninInterceptor::Delegate::BubbleParameters(
        intercept_type, account, primary_account, profile_highlight_color);
  }
