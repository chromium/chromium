...
class HighEfficiencyChipInteractiveTest : public InteractiveBrowserTest {
 public:
  ...
  // Creates a new tab with the given URL and waits for it to load
  MultiStep CreateNewTab(GURL url, const ui::ElementIdentifier& contents_id) {
    return Steps(Do(base::BindLambdaForTesting([=]() {
                   util_->LoadPageInNewTab(url, true);
                   InstrumentTab(browser(), contents_id);
                 })),
                 WaitForWebContentsNavigation(contents_id));
  }
  ...
  MultiStep InstrumentTabAt(size_t index, const ui::ElementIdentifier& tab_id) {
    return Steps(SelectTab(kTabStripElementId, index),
                 Do(base::BindLambdaForTesting(
                     [&]() { InstrumentTab(browser(), tab_id); })));
  }
...
IN_PROC_BROWSER_TEST_F(HighEfficiencyChipInteractiveTest, PageActionChipShows) {
  RunTestSequence(InstrumentTabAt(0, kFirstTabContents),
                  NavigateTab(test_url_, kFirstTabContents),
                  CreateNewTab(test_url_, kSecondTabContents),
...
IN_PROC_BROWSER_TEST_F(HighEfficiencyChipInteractiveTest,
  ...
  RunTestSequence(
      InstrumentTabAt(0, kFirstTabContents),
      NavigateTab(test_url_, kFirstTabContents),
      CreateNewTab(test_url_, kSecondTabContents),
      EnsureNotPresent(kHighEfficiencyChipElementId),
      SelectTab(kTabStripElementId, 1),
...
IN_PROC_BROWSER_TEST_F(HighEfficiencyChipInteractiveTest,
  ...
  RunTestSequence(
      ...
      InstrumentTabAt(2, kPerformanceSettingsTab),
