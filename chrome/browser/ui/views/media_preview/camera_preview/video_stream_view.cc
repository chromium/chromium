...
void VideoStreamView::OnPaint(gfx::Canvas* canvas) {
  ...
  // Only use round corners if there are no horizontal or vertical bars.
  const float tolerance = 0.1;
  bool should_round_corners =
      fabs(frame_aspect_ratio - targeted_aspect_ratio_) <= tolerance;

  if (should_round_corners) {
    canvas->sk_canvas()->clipRRect(
        SkRRect::MakeRectXY(
            SkRect::MakeXYWH(x, y, rendered_frame_width, rendered_frame_height),
