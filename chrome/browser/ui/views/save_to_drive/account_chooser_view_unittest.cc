...
TEST_F(AccountChooserViewTest, SingleAccount) {
  ...
  AccountChooserView* account_chooser_view = anchor_view_->AddChildView(
      std::make_unique<AccountChooserView>(accounts, std::nullopt));
  ...
  EXPECT_TRUE(VerifyAccountChooserRow(
      ...
      body_view_children.at(1)->children().front(), accounts.front()));
...
TEST_F(AccountChooserViewTest, MultiAccount) {
  ...
  AccountChooserRadioGroupView* body_view =
      static_cast<AccountChooserRadioGroupView*>(children.at(1));
