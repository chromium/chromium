...
TEST_F(SessionRestoreInfoBarPrefsTest, IncrementInfoBarShownCountByType) {
  EXPECT_EQ(0, profile_.GetPrefs()->GetInteger(
                   prefs::kSessionRestoreInfoBarTimesShown));
  EXPECT_EQ(0, profile_.GetPrefs()->GetInteger(
  ...
  IncrementInfoBarShownCount(
      profile_.GetPrefs(),
      SessionRestoreInfoBarDelegate::InfobarMessageType::kTurnOffFromSession);
