...
struct ToolResponse {
  ...
  // If true, indicates that the tool executed with an obvious visual response,
  // and the UI/agent should not explicitly announce or narrate its completion
  // to the user.
...
class AiOverlayDialogTools {
 public:
  ...
  virtual void OpenUrl(const std::string& url,
                       bool new_tab,
                       base::OnceCallback<void(ToolResult)> callback) = 0;
  ...
  virtual void SwitchTab(const std::string& query,
                         base::OnceCallback<void(ToolResult)> callback) = 0;
  // Close the current browser tab.
  virtual void CloseCurrentTab(
      base::OnceCallback<void(ToolResult)> callback) = 0;
  // Go back to the previous page in history.
  virtual void GoBack(base::OnceCallback<void(ToolResult)> callback) = 0;
  // Go forward to the next page in history.
  virtual void GoForward(base::OnceCallback<void(ToolResult)> callback) = 0;
  // Refresh the current page.
  virtual void ReloadPage(base::OnceCallback<void(ToolResult)> callback) = 0;
  // --- General DOM Tools ---
  ...
  virtual void PlayVideo(base::OnceCallback<void(ToolResult)> callback) = 0;

  // Pause video playback.
  virtual void PauseVideo(base::OnceCallback<void(ToolResult)> callback) = 0;
