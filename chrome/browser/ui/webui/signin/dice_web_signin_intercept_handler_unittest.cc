...
class DiceWebSigninInterceptHandlerTestBase : public testing::Test {
 public:
  ...
  base::DictValue GetInterceptionParameters(
      ...
      const std::string& profile_name = "Primary Profile") {
    ...
    DiceWebSigninInterceptHandler handler(
        {interception_type(), intercepted_account, primary_account,
         ...
         /*show_managed_disclaimer=*/false, account_preview_preference},
...
TEST_P(DiceWebSigninInterceptHandlerChromeSigninInterceptionTest,
       ChromeSigninSubtitleWithAccountPreviewData) {
  if (GetParam() == signin::Tribool::kTrue) {
    // Supervised users show parental controls subtitle regardless of preview
    // data.
    return;
  }
  ...
  std::u16string device_str =
      l10n_util::GetStringUTF16(IDS_ACCOUNT_PREVIEW_DEVICE_PHONE);
  std::string expected_subtitle = l10n_util::GetStringFUTF8(
      IDS_ACCOUNT_PREVIEW_PROMO_SUBTITLE_PASSWORDS_WITH_DEVICE, device_str);
  const std::string* subtitle_ptr = parameters.FindString("subtitle");
  ASSERT_TRUE(subtitle_ptr);
  EXPECT_EQ(*subtitle_ptr, expected_subtitle);
  ...
  base::DictValue parameters_no_device =
      GetInterceptionParameters(pref_no_device, "Primary Profile 2");
  std::string expected_subtitle_no_device =
      l10n_util::GetStringUTF8(IDS_ACCOUNT_PREVIEW_PROMO_SUBTITLE_BOOKMARKS);
  const std::string* subtitle_no_device_ptr =
      parameters_no_device.FindString("subtitle");
  ASSERT_TRUE(subtitle_no_device_ptr);
  EXPECT_EQ(*subtitle_no_device_ptr, expected_subtitle_no_device);
}
