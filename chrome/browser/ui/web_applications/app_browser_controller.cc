...
void AppBrowserController::UpdateThemeSupplier() {
  ...
  // the theme supplier here. Otherwise the colorIds for the window caption bar
  // will be remapped through `AutogeneratedThemeSupplier`, and colors will be
  ...
  if (ignore_custom_colors || no_custom_colors || non_tabbed_no_frame_color) {
    theme_supplier_ = nullptr;
    if (auto* theme_observer =
            BrowserWindowThemeObserver::From(browser_.get())) {
      theme_observer->NotifyThemeChanged(BrowserThemeChangeType::kWebAppTheme);
    }
    return;
  }
  ...
  theme_supplier_ = AutogeneratedThemeSupplier::CreateForWebAppColors(
