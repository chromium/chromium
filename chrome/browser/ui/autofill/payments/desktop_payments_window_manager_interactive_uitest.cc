...
class DesktopPaymentsWindowManagerInteractiveUiTest : public UiBrowserTest {
 public:
  ...
  void ShowUi(const std::string& name) override {
    ...
    static_cast<DesktopPaymentsWindowManager*>(
        client->GetPaymentsWindowManager())
        ->CreatePopupForTesting(popup_opening_url_);
  }
  ...
  bool VerifyUi() override {
    ...
    if (popup_web_contents->GetURL() != popup_opening_url_) {
      return false;
    }
 ...
 private:
  GURL popup_opening_url_ = GURL("https://site.example");
};

IN_PROC_BROWSER_TEST_F(DesktopPaymentsWindowManagerInteractiveUiTest,
                       InvokeUi_TabModalBrowserPopup) {
  ShowAndVerifyUi();
}
