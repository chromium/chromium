...
class SaveAutofillAiDataControllerImplTest : public DialogBrowserTest {
 public:
  SaveAutofillAiDataControllerImplTest() = default;
  SaveAutofillAiDataControllerImplTest(
      const SaveAutofillAiDataControllerImplTest&) = delete;
  SaveAutofillAiDataControllerImplTest& operator=(
      const SaveAutofillAiDataControllerImplTest&) = delete;
  ...
  void ShowUi(const std::string& name) override {
    ...
    controller_ =
        SaveAutofillAiDataControllerImpl::FromWebContents(web_contents);
    CHECK(controller_);
    if (name == "UpdateEntity") {
      std::pair<autofill::EntityInstance, autofill::EntityInstance> entities =
          GetUpdateEntities();
      controller_->OfferSave(std::move(entities.first),
                             std::move(entities.second), base::NullCallback());
      return;
    } else if (name == "SaveNewEntity") {
      controller_->OfferSave(autofill::test::GetPassportEntityInstance(),
  ...
  SaveAutofillAiDataControllerImpl* controller() { return controller_; }

 private:
  raw_ptr<SaveAutofillAiDataControllerImpl> controller_ = nullptr;
};

IN_PROC_BROWSER_TEST_F(SaveAutofillAiDataControllerImplTest,
