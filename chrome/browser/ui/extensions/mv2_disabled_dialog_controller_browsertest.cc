...
class Mv2DisabledDialogControllerBrowserTest : public InteractiveBrowserTest {
 public:
  Mv2DisabledDialogControllerBrowserTest() = default;
  ~Mv2DisabledDialogControllerBrowserTest() override = default;
...
// time it was shown.
IN_PROC_BROWSER_TEST_F(Mv2DisabledDialogControllerBrowserTest, NoUserAction) {
  RunTestSequence(
      // Extension is disabled due to the MV2 deprecation stage.
      CheckResult(
          [&]() {
...
IN_PROC_BROWSER_TEST_F(Mv2DisabledDialogControllerBrowserTest,
  ...
  RunTestSequence(
      ...
      CheckResult(
          ...
          true),
      // Trigger the dialog.
      // Note: We need to manually compute the affected extensions because there
      // is no way to preserve policy-installed extensions between tests, and
      // therefore we cannot trigger the dialog in between tests.
      Do([this]() {
...
// finished.
// Load an MV2 extension with an icon.
IN_PROC_BROWSER_TEST_F(Mv2DisabledDialogControllerBrowserTest,
                       PRE_IconsLoaded) {
...
IN_PROC_BROWSER_TEST_F(Mv2DisabledDialogControllerBrowserTest, IconsLoaded) {
  RunTestSequence(
