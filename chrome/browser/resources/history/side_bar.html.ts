...
<cr-menu-selector id="menu" selected="${this.selectedPage}"
    ...
    @iron-activate="${this.onMenuIronActivate_}" selected-attribute="selected">
  <a id="history" role="menuitem" class="page-item cr-nav-menu-item"
      href="${this.getHistoryItemHref_()}" path="${this.getHistoryItemPath_()}"
  ...
  <a id="syncedTabs" role="menuitem" href="/syncedTabs"
      class="page-item cr-nav-menu-item" path="syncedTabs"
      @click="${this.onItemClick_}">
    <cr-icon icon="cr:devices"></cr-icon>
    $i18n{openTabsMenuItem}
    <cr-ripple></cr-ripple>
  </a>
  <a role="menuitem" id="clear-browsing-data" class="cr-nav-menu-item"
...
<div id="footer" ?hidden="${!this.showFooter_}">
  ...
  <div id="google-account-footer" ?hidden="${!this.showGoogleAccountFooter_}"
