...
function createOSSettingsRoutes() {
  ...
  // Bluetooth section.
  r.BLUETOOTH =
      createSection(r.BASIC, mojom.BLUETOOTH_SECTION_PATH, Section.kBluetooth);
  ...
  if (!loadTimeData.getBoolean('isGuest')) {
    ...
    if (loadTimeData.getBoolean('isNearbyShareSupported')) {
      r.NEARBY_SHARE = createSubpage(
          r.MULTIDEVICE, mojom.NEARBY_SHARE_SUBPAGE_PATH, Subpage.kNearbyShare);
    }
  }
  ...
  if (loadTimeData.valueExists('isKerberosEnabled') &&
    ...
    r.KERBEROS =
        createSection(r.BASIC, mojom.KERBEROS_SECTION_PATH, Section.kKerberos);
  ...
  // Device section.
  r.DEVICE = createSection(r.BASIC, mojom.DEVICE_SECTION_PATH, Section.kDevice);
  r.POINTERS =
  ...
  r.EXTERNAL_STORAGE_PREFERENCES = createSubpage(
      r.STORAGE, mojom.EXTERNAL_STORAGE_SUBPAGE_PATH, Subpage.kExternalStorage);
  ...
  r.APP_NOTIFICATIONS = createSubpage(
      r.APPS, mojom.APP_NOTIFICATIONS_SUBPAGE_PATH, Subpage.kAppNotifications);
  ...
  if (loadTimeData.valueExists('androidAppsVisible') &&
    ...
    r.ANDROID_APPS_DETAILS = createSubpage(
        r.APPS, mojom.GOOGLE_PLAY_STORE_SUBPAGE_PATH, Subpage.kGooglePlayStore);
  ...
  r.FINGERPRINT = createSubpage(
      r.LOCK_SCREEN, mojom.FINGERPRINT_SUBPAGE_PATH_V2, Subpage.kFingerprintV2);
  ...
  // Printing section.
  r.OS_PRINTING =
      createSection(r.ADVANCED, mojom.PRINTING_SECTION_PATH, Section.kPrinting);
  ...
  r.ABOUT = new Route('/' + mojom.ABOUT_CHROME_OS_SECTION_PATH);
