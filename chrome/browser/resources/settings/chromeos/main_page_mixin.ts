...
export const MainPageMixin = dedupingMixin(
    ...
    Constructor<MainPageMixinInterface> => {
      ...
      class MainPageMixinInternal extends superclassBase implements
        ...
        // Do not initialize manually. This is initialized by Polymer.
        ...
        override connectedCallback() {
          ...
          // currentRouteChanged() expects |scroller| to be defined and is
          // called by this super method in RouteObserverMixin.
        ...
        private ensureSectionForRoute_(route: Route): Promise<HTMLElement> {
          ...
          return new Promise(resolve => {
            if (this.shouldExpandAdvanced_(route)) {
              this.dispatchCustomEvent('hide-container');
              waitFn(() => {
                this.loadAdvancedPage().then(() => {
                  resolve(this.querySection(route.section)!);
                });
              });
            } else {
              waitFn(() => {
        ...
        private async enterSubpage_(route: Route) {
          ...
          this.dispatchCustomEvent('subpage-expand');
          ...
          this.dispatchCustomEvent('settings-section-expanded');
          this.dispatchCustomEvent('show-container');
        }
        ...
        private enterMainPage_(oldRoute: Route): Promise<void> {
          ...
          return new Promise((resolve) => {
            requestAnimationFrame(() => {
              ...
              this.dispatchCustomEvent('showing-main-page');
              resolve();
            });
          });
        }
        ...
        private async scrollToSection_(route: Route) {
          const section = await this.ensureSectionForRoute_(route);
          this.dispatchCustomEvent('showing-section', {detail: section});
          this.dispatchCustomEvent('show-container');
        }
