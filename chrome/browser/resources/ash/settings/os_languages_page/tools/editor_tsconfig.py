...
def _relative_to_with_parents(path: pathlib.Path, other: pathlib.Path) -> str:
    ...
    while not path.is_relative_to(other):
        if other == other.parent:
            raise ValueError(
                f'path ({path}) and other ({other}) do not share any parents'
            )
        other = other.parent
        parts.append('..')
...
def _convert_tsconfig_for_editor(
    ...
    for file in files:
        ...
        if file_path.is_relative_to(original_root_dir):
            ...
            source_path = editor_dir.joinpath(
                editor_root_dir, relative_path
            ).resolve()
    ...
    editor_tsconfig = {
        **(
            {
                'extends': _rebase_relative_path(
                    tsconfig['extends'], original_dir, editor_dir
                )
            }
        ...
        'compilerOptions': {
            'rootDirs': [
                ...
                _rebase_relative_path(
                    original_root_dir, original_dir, editor_dir
                ),
            ],
            'noEmit': True,
            'paths': {
...
def main(argv: List[str]) -> None:
    ...
    parser.add_argument(
        'original_tsconfig',
        type=pathlib.Path,
        help='path to tsconfig.json generated by ts_library in the out directory',
    )
    parser.add_argument(
        '--root_dir',
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            'removes all files named lazy_load from the tsconfig to prevent '
    ...
    if not (
        original_tsconfig.stem.startswith('tsconfig_')
        and original_tsconfig.suffix == '.json'
    ):
        raise ValueError(
            f'original_tsconfig ({original_tsconfig}) is not'
    ...
    while gen_dir.name != "gen":
        if gen_dir == gen_dir.parent:
            raise ValueError(
                f'original_tsconfig ({original_tsconfig}) does'
