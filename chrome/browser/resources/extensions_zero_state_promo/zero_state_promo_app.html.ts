...
<div id="headerContainer">
  ${this.showChipsV2_ || this.showChipsV3_ ? html`
    <h3 id="title">$i18n{extensionsZeroStateIphHeaderV2}</h3>
  ` : html`
    <h3 id="title">$i18n{extensionsZeroStateIphHeader}</h3>
  `}
  <cr-icon-button id="dismissButton" class="icon-clear"
...
${this.showPlainLinksUi_ ? html`
  <div id="anchorContainer">
    ...
    <a id="couponsLink" @click="${this.onCouponsButtonClick_}" is="action-link">
      $i18n{extensionsZeroStateIphShoppingCategoryLink},
    </a>
    <a id="writingLink" @click="${this.onWritingButtonClick_}" is="action-link">
  ...
  <div id="buttonContainer">
    <cr-button id="closeButton" @click="${this.onDismissButtonClick_}">
        ...
        <cr-chip id="productivityButton"
          ...
          <cr-icon
              icon="${this.webuiRoundedIconsEnabled_ ?
                  ...
                  'zero-state-promo:productivity-old'}">
          </cr-icon>
          $i18n{extensionsZeroStateIphProductivityCategoryLabel}
        </cr-chip>
      </div>
      <div class="v1Chip">
        <cr-chip id="aiButton" @click="${this.onAiButtonClick_}">
          <cr-icon
              icon="${this.webuiRoundedIconsEnabled_ ?
                  'zero-state-promo:spark' :
                  'zero-state-promo:ai-old'}">
          </cr-icon>
