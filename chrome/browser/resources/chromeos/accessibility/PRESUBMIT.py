...
def _CheckNoJsChanges(input_api, output_api):
  ...
  for f in input_api.AffectedFiles(False):  # ignores deleted files.
    if f.LocalPath().endswith('js') and f.LocalPath().find('braille_ime') == -1:
      results.append(output_api.PresubmitError(
        "Illegal file modification: %s\n\n"
        "Modifying JavaScript files in the accessibility/ subdirectory is not "
        "allowed during the TypeScript migration. Please migrate this file to "
        "TypeScript before making any changes." % f.LocalPath()))
  return results
...
def _CommonChecks(input_api, output_api):
    ...
    results = []
    results.extend(_CheckNoJsChanges(input_api, output_api))
    return results
