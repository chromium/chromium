...
<div id="logOptionsContainer">
  ...
  <label>
    <input id="logFileinfo" type="checkbox"
        @change="${this.onLogFileinfoChange_}" .checked="${this.logFileInfo_}">
...
<div id="logCheckboxContainer">
  ...
  ${this.eventTypes_.map(item => html`
    <label>
      <input id="logType${item.type}" type="checkbox" .checked="${item.enabled}"
          .value="${item.type}" @change="${this.onLogTypeChange_}">
...
<div id="logContainer">
  ${this.filteredLogs_.length === 0 ? html`
    <span>$i18n{logNoEntriesText}</span>
  ` : ``}
  ${this.filteredLogs_.map(item => html`
    <p>
      <span class="type-tag log-type-${item.type.toLowerCase()}">
        ${item.type}
      </span>
      <span class="level-tag log-level-${item.level.toString().toLowerCase()}">
        ${item.level}
      </span>
