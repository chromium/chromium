...
function maybeRunIfResourceId(runFunction: (resourceId: string) => void) {
...
function initialize() {
  ...
  joinFlowButton?.addEventListener('click', () => {
    maybeRunIfResourceId(
        (resourceId: string) => window.data_sharing_sdk.runJoinFlow(
            resourceId,
  ...
  const inviteFlowButton = document.getElementById(
      'create-invite-flow-button',
  );
  inviteFlowButton?.addEventListener('click', () => {
    maybeRunIfResourceId(window.data_sharing_sdk.runInviteFlow);
  });

  const manageFlowButton = document.getElementById(
      'create-manage-flow-button',
  );
