...
TEST_F(InstallerDownloaderInfoBarDelegateTest, ClickAcceptButtonOnInfoBarView) {
  ...
  EXPECT_CALL(mock_accept_cb_, Run).Times(1);
  ...
  views::test::ButtonTestApi(accept_button).NotifyClick(ui::test::TestEvent());
}
...
TEST_F(InstallerDownloaderInfoBarDelegateTest,
  ...
  views::test::ButtonTestApi(dismiss_button).NotifyClick(ui::test::TestEvent());
}
