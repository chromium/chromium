...
class FakeHttpsLatencySampler : public Sampler {
 public:
  ...
  explicit FakeHttpsLatencySampler(TelemetryData telemetry_data)
      : telemetry_data_(telemetry_data) {}

  void CollectTelemetry(TelemetryCallback callback) override {
    std::move(callback).Run(telemetry_data_);
  }

 private:
  const TelemetryData telemetry_data_;
};
...
TelemetryData NetworkSamplerTestHelper(
    const std::vector<FakeNetworkData>& networks_data) {
  content::BrowserTaskEnvironment task_environment_;

  TelemetryData latency_data;
  latency_data.mutable_networks_telemetry()
      ->mutable_https_latency_data()
      ->set_latency_ms(145);
  auto https_latency_sampler =
      std::make_unique<FakeHttpsLatencySampler>(latency_data);
  ...
  NetworkSampler network_metric_sampler(https_latency_sampler.get());
  network_metric_sampler.CollectTelemetry(
      base::BindLambdaForTesting([&result](TelemetryData telemetry_data) {
        result = std::move(telemetry_data);
      }));

  EXPECT_EQ(
      result.networks_telemetry().https_latency_data().latency_ms(),
      latency_data.networks_telemetry().https_latency_data().latency_ms());
  return result;
}

TEST(NetworkSamplerTest, WifiConnecting) {
  const std::vector<FakeNetworkData> networks_data = {
      {"guid1", shill::kStateConfiguration, shill::kTypeWifi, 10,
       false /* is_portal */, true /* is_visible */, true /* is_configured */}};

  TelemetryData result = NetworkSamplerTestHelper(networks_data);
...
TEST(NetworkSamplerTest, WifiInvisibleNotConnected) {
  const std::vector<FakeNetworkData> networks_data = {
...
TEST(NetworkSamplerTest, WifiPortal) {
  const std::vector<FakeNetworkData> networks_data = {
...
TEST(NetworkSamplerTest, MixTypesAndConfigurations) {
  const std::vector<FakeNetworkData> networks_data = {
