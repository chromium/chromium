...
#ifndef CHROME_BROWSER_ASH_POLICY_STATUS_COLLECTOR_LEGACY_DEVICE_STATUS_COLLECTOR_H_
#define CHROME_BROWSER_ASH_POLICY_STATUS_COLLECTOR_LEGACY_DEVICE_STATUS_COLLECTOR_H_
...
// This class is essentially a copy of device_status_collector.cc before it
// was refactored to support granular reporting controls. This class is being
// used as an option to revert changes after launch if needed. it will be
// created and used by status_uploader.cc if the policy
// EnableDeviceGranularReporting is set to false.
// Collects and summarizes the status of an enterprise-managed ChromeOS device.
class LegacyDeviceStatusCollector
 ...
 public:
  ...
  // |pref_service|.
  LegacyDeviceStatusCollector(PrefService* pref_service,
                              chromeos::system::StatisticsProvider* provider);

  LegacyDeviceStatusCollector(const LegacyDeviceStatusCollector&) = delete;
  LegacyDeviceStatusCollector& operator=(const LegacyDeviceStatusCollector&) =
      delete;

  ~LegacyDeviceStatusCollector() override;
 ...
 private:
  ...
  base::WeakPtrFactory<LegacyDeviceStatusCollector> weak_factory_{this};
};
