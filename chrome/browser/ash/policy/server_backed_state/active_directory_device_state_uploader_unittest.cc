...
class ActiveDirectoryDeviceStateUploaderTest
 ...
 protected:
  ...
  void InitUploader() {
    ...
    certificate_uploader_impl_ =
        std::make_unique<ash::attestation::EnrollmentCertificateUploaderImpl>(
            cloud_policy_client);
    certificate_uploader_impl_->set_attestation_flow_for_testing(
        &attestation_flow_);
    certificate_uploader_impl_->set_retry_limit_for_testing(0);
    certificate_uploader_impl_->set_retry_delay_for_testing(base::TimeDelta());
    uploader_->SetEnrollmentCertificateUploaderForTesting(
        std::move(certificate_uploader_impl_));
