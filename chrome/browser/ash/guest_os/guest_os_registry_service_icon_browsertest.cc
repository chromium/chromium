...
class GuestOsRegistryServiceIconTest : public InProcessBrowserTest {
 public:
  ...
  void LoadIconAndValidate(const std::string& app_id,
                           bool expect_loaded,
...
IN_PROC_BROWSER_TEST_F(GuestOsRegistryServiceIconTest, LoadIconOnce) {
  ...
  LoadIconAndValidate(app_id, true, run_loop.QuitClosure());
  run_loop.Run();
...
IN_PROC_BROWSER_TEST_F(GuestOsRegistryServiceIconTest, LoadIconTwice) {
  ...
  LoadIconAndValidate(app_id, true, run_loop1.QuitClosure());
  LoadIconAndValidate(app_id, true, run_loop2.QuitClosure());
  run_loop1.Run();
  run_loop2.Run();
  ExpectIconFiles(app_id);
}
...
IN_PROC_BROWSER_TEST_F(GuestOsRegistryServiceIconTest, AddRemoveAddAppIcon) {
  ...
  LoadIconAndValidate(app_id, false, run_loop1.QuitClosure());
  run_loop1.Run();
  ...
  LoadIconAndValidate(app_id, true, run_loop2.QuitClosure());
  run_loop2.Run();
  EXPECT_TRUE(base::PathExists(icon_dir));
  ...
  LoadIconAndValidate(app_id, false, run_loop3.QuitClosure());
  run_loop3.Run();

  // Add the app. Ensure 2nd fetch of icon from VM succeeds.
  AddApp();
  base::RunLoop run_loop4;
  LoadIconAndValidate(app_id, true, run_loop4.QuitClosure());
  run_loop4.Run();
  EXPECT_TRUE(base::PathExists(icon_dir));
}
