...
void DozeModePowerStatusScheduler::Start() {
  ...
  user_active_timer_.Start(
      FROM_HERE, kUserIdleTimeout,
      base::BindRepeating(&DozeModePowerStatusScheduler::OnUserActiveTimerTick,
                          Unretained(this)));
...
void DozeModePowerStatusScheduler::OnArcSessionStopped(
  ...
  // When ARC stopped, reset all observations and stop all timers.
  Stop();
}
...
void DozeModePowerStatusScheduler::MaybeUpdatePowerStatus() {
  PowerStatus new_status = CalculatePowerStatus();
...
void DozeModePowerStatusScheduler::ReportDailyMetrics(
  ...
  if (!total_duration.is_zero()) {
    base::UmaHistogramPercentage(
        ...
        static_cast<int>(100 *
                         (real_power_status_duration / total_duration)));
  }
}
