...
bool ShouldDivert(const storage::FileSystemURL& url) {
  ...
  // created files) then we'd need to initialize the Diversion File's contents
  // (via a FileStreamWriter), just after the StartDiverting call, before
  // running the EnsureFileExistsCallback. There would be the usual trade-offs
  // here between lazy and eager initialization of those file contents.
  const std::string& extension = url.virtual_path().FinalExtension();
  return (extension == ".crdownload") || (extension == ".crswap");
}
...
// static
bool DiversionBackendDelegate::ShouldDivertForTesting(
    const storage::FileSystemURL& url) {
