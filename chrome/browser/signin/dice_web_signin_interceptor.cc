...
void DiceWebSigninInterceptor::MaybeInterceptWebSignin(
  ...
  if (IsInInconsistentStateWithPrimaryAccount(account_id, identity_manager_)) {
    profile_metrics_service_->UmaHistogramBoolean(
        "Signin.SigninPending.InconsistentStateInvoked", true);
  }
...
void DiceWebSigninInterceptor::OnInterceptionReadyToBeProcessed(
  ...
  if (switch_to_entry) {
    highlight_color =
        switch_to_entry->GetProfileThemeColors().profile_color_seed;
  } else {
    highlight_color = GetAutogeneratedThemeColors(profile_color).frame_color;
  }
  ...
  WebSigninInterceptor::Delegate::BubbleParameters bubble_parameters(
      *interception_type, info, GetPrimaryAccountInfo(identity_manager_),
      highlight_color, show_link_data_option, show_managed_disclaimer);
...
void DiceWebSigninInterceptor::ProceedWithProfileCreation(
  ...
  state_->dice_signed_in_profile_creator_ =
      std::make_unique<DiceSignedInProfileCreator>(
          profile_, state_->account_id_, state_->secondary_accounts_,
