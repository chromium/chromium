...
// browser shutdown.
IN_PROC_BROWSER_TEST_P(ChromeKeepAliveURLBrowserTest,
                       SendRequestAfterBrowserShutdown) {
  const std::string method = GetParam();
  auto request_handler =
      std::move(RegisterRequestHandlers({kKeepAliveEndpoint})[0]);
  ASSERT_TRUE(embedded_test_server()->Start());
  ...
  // The keepalive request may be sent before shutting down, but processed by
  // the server after shutting down.
  request_handler->WaitForRequest();
...
IN_PROC_BROWSER_TEST_P(ChromeKeepAliveURLBrowserTest,
  ...
  ASSERT_TRUE(content::NavigateToURL(web_contents(), keepalive_page_url));
  // Ensure the keepalive request is sent before shutting down, and the browser
  // is now pending to receive response from this handler.
  request_handler->WaitForRequest();
