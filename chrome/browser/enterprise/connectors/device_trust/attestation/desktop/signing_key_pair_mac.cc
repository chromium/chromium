...
namespace {
class SigningKeyPairMac : public SigningKeyPair {
 public:
  // SigningKeyPair:
...
SigningKeyPairMac::GetTpmBackedKeyProvider() {
  return crypto::GetUnexportableKeyProvider();
}

bool SigningKeyPairMac::StoreKeyPair(KeyTrustLevel trust_level,
                                     std::vector<uint8_t> wrapped) {
  NOTIMPLEMENTED();
  return false;
}

SigningKeyPair::KeyInfo SigningKeyPairMac::LoadKeyPair() {
  NOTIMPLEMENTED();
...
std::unique_ptr<SigningKeyPair> SigningKeyPair::CreatePlatformKeyPair() {
  ...
  return std::make_unique<SigningKeyPairMac>();
}
