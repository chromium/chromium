...
This causes the provided test hook to be set as the current hook for each
runtime. What specifically each runtime does after that differs (covered in each
runtime's specific documentation, e.g. [OpenXR Testing][openxr testing]), but the
important part is that the runtimes will start calling into the test via Mojo to
get controller and headset data instead of attempting to use the real
implementation.
...
[openxr testing]: https://chromium.googlesource.com/chromium/src/+/HEAD/device/vr/openxr/README.md#testing
