...
// origin is not isolated and the exception is not applied.
IN_PROC_BROWSER_TEST_F(
  ...
  // b.com will be marked for isolation when loaded in a subframe.
  // But the exception won't be followed until it is loaded in a future browsing
  // context.
  ...
  if (content::SiteIsolationPolicy::UseDedicatedProcessesForAllSites()) {
    ...
    // followed so this case doesn't apply.
    // So in these cases, we are verifying that new exceptions are respected in
    // sub-frames.
...
// first, then if the origin is loaded in a subframe later, the origin will
// be isolated and the exception will be applied in both cases.
IN_PROC_BROWSER_TEST_F(JavascriptOptimizerBrowserTest,
...
// Test that when a rule is removed during a session, then the origin will
// still be isolated but the updated js-opt setting will be applied.
IN_PROC_BROWSER_TEST_F(JavascriptOptimizerBrowserTest,
  ...
  // Before any navigation, a site with an exception is
  // not isolated.
...
IN_PROC_BROWSER_TEST_F(JavascriptOptimizerBrowserTest,
  ...
  // Since this exception matches the default, it will
  // not be isolated.
  ASSERT_TRUE(content::NavigateToURL(
      web_contents(),
      embedded_https_test_server().GetURL("sub.a.com", "/simple.html")));
...
// Test that exceptions which match the main frame are
// not propagated down to subframes from different sites. Additionally, if the
// subframe is later navigated to a site that matches the main frame, the main
// frame's exception will apply.
IN_PROC_BROWSER_TEST_F(JavascriptOptimizerBrowserTest,
