...
// Timeout of the PageLoadMetricsTestWaiter can happen on Linux and Window,
// though rarely, due to fast shutdown process. For example, the browser side
// observer could be destroyed before the UKM IPC is received.
#if BUILDFLAG(IS_WIN) || BUILDFLAG(IS_LINUX) || BUILDFLAG(IS_CHROMEOS_LACROS)
#define MAYBE_INP_ReportBeforeNavigatingAwayToCrossOrigin \
  DISABLED_INP_ReportBeforeNavigatingAwayToCrossOrigin
#else
#define MAYBE_INP_ReportBeforeNavigatingAwayToCrossOrigin \
  INP_ReportBeforeNavigatingAwayToCrossOrigin
#endif
IN_PROC_BROWSER_TEST_F(InteractionToNextPaintTest,
                       MAYBE_INP_ReportBeforeNavigatingAwayToCrossOrigin) {
  // Add waiter to wait for the interaction is arrived in browser.
  auto waiter = std::make_unique<page_load_metrics::PageLoadMetricsTestWaiter>(
      web_contents());
  waiter->AddNumInteractionsExpectation(1);
...
#define MAYBE_INP_ReportBeforeEnteringBFCache \
  DISABLED_INP_ReportBeforeEnteringBFCache
#else
#define MAYBE_INP_ReportBeforeEnteringBFCache INP_ReportBeforeEnteringBFCache
#endif
IN_PROC_BROWSER_TEST_F(InteractionToNextPaintTest,
                       MAYBE_INP_ReportBeforeEnteringBFCache) {
