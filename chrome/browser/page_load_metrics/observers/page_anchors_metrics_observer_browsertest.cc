...
class PageAnchorsMetricsObserverBrowserTest
 ...
 public:
  ...
  void WaitForUKMEntry(const std::string& entry_name,
    ...
    ASSERT_GE(ukm_recorder_->GetEntriesByName(entry_name).size(),
              expected_entries_size);
  }
...
IN_PROC_BROWSER_TEST_F(PageAnchorsMetricsObserverBrowserTest,
                       NavigateAwayShouldRecordUkmData) {
  ResetUKM();
...
IN_PROC_BROWSER_TEST_F(PageAnchorsMetricsObserverBrowserTest,
  ...
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorAnchorElementMetrics::kEntryName, 2u);
  ASSERT_TRUE(HistoryGoBack(web_contents()));
  // UserInteractions for page 2 should be recorded to UKM.
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorUserInteractions::kEntryName, 2u);
  ...
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorAnchorElementMetrics::kEntryName, 3u);
  ASSERT_TRUE(HistoryGoForward(web_contents()));
  // UserInteractions for page 1 should be recorded to UKM.
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorUserInteractions::kEntryName, 3u);
  ...
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorAnchorElementMetrics::kEntryName, 4u);
  NavigateAway();
  // UserInteractions for page 2 should be recorded to UKM.
  WaitForUKMEntry(
      ukm::builders::NavigationPredictorUserInteractions::kEntryName, 4u);
