...
TEST_P(IwaCacheClientDeathTest, CreateClient) {
  if (GetParam().should_crash && GetParam().session_type == kUser) {
    EXPECT_DEATH(
        { IwaCacheClient cache_client = IwaCacheClient(); },
        "Unsupported session type for IWA caching");
  } else if (GetParam().should_crash) {
    EXPECT_DEATH(
        { IwaCacheClient cache_client = IwaCacheClient(); },
        "IwaCacheClient should only be created inside mgs or kiosk sessions "
