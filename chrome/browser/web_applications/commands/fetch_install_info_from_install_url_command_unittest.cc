 ...
 protected:
  std::unique_ptr<WebAppInstallInfo> CreateAndRunCommand(
    ...
    if (disable_web_app_info) {
      install_page_state.page_install_info = nullptr;
    } else {
      install_page_state.page_install_info =
          std::make_unique<WebAppInstallInfo>();
      install_page_state.page_install_info->start_url = install_url;
      install_page_state.page_install_info->manifest_id =
          GenerateManifestIdFromStartUrlOnly(install_url);
      install_page_state.page_install_info->scope =
          install_url.GetWithoutFilename();
      install_page_state.page_install_info->user_display_mode =
