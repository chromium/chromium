...
OverrideShortcutsForTesting() {                                 // IN-TEST
  ...
  // Initialize all directories used.
#if defined(OS_WIN)
  DCHECK(scoped_override->desktop.CreateUniqueTempDir());
  DCHECK(scoped_override->application_menu.CreateUniqueTempDir());
  DCHECK(scoped_override->quick_launch.CreateUniqueTempDir());
  DCHECK(scoped_override->startup.CreateUniqueTempDir());
#elif defined(OS_MAC)
  DCHECK(scoped_override->chrome_apps_folder.CreateUniqueTempDir());
#elif defined(OS_LINUX)
  DCHECK(scoped_override->desktop.CreateUniqueTempDir());
#endif
