...
/** Tests SerialNotificationManager behaviour and its delegate. */
@RunWith(BaseRobolectricTestRunner.class)
@EnableFeatures({NotificationFeatureMap.CACHE_NOTIIFICATIONS_ENABLED})
public class SerialNotificationManagerTest {
    ...
    private Intent createIntent(boolean isConnected, boolean isIncognito) {
        ...
        intent.setAction(SerialNotificationManager.ACTION_SERIAL_UPDATE);
        intent.putExtra(SerialNotificationManager.NOTIFICATION_ID_EXTRA, NOTIFICATION_ID);
        intent.putExtra(SerialNotificationManager.NOTIFICATION_URL_EXTRA, TEST_URL.getSpec());
        intent.putExtra(SerialNotificationManager.NOTIFICATION_IS_CONNECTED, isConnected);
        intent.putExtra(SerialNotificationManager.NOTIFICATION_IS_INCOGNITO, isIncognito);
        return intent;
    }
    ...
    public void setUp() {
        ...
        mManager = new SerialNotificationManager(mMockNotificationManager, mDelegate);
    }
    ...
    public void test_connectedShowsNotification() {
        ...
        assertNotificationEquals(
                "Connected to a serial port", "Tap to return to " + TEST_URL_FORMATTED);
    }
    ...
    public void test_connectedInIncognitoShowsNotification() {
        ...
        assertFalse(mServiceStopped);
        assertEquals(1, mTabsBroughtToFront);
        assertNotificationEquals("Connected to a serial port", "Tap to return to the site");
    }
}
