...
class GeminiAppTabHelper : public content::WebContentsObserver {
 public:
  ...
  // Creates a new instance for `web_contents` if and only if:
  // (a) the Gemini app preinstallation feature is enabled, and
  // (b) the specified `web_contents` is not off the record.
  static std::unique_ptr<GeminiAppTabHelper> MaybeCreate(
