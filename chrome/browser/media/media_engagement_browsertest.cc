...
IN_PROC_BROWSER_TEST_F(MediaEngagementContentsObserverPrerenderBrowserTest,
  ...
  // AddAutoplayFlags should be called twice because activating the prerendered
  // page will call AddAutoplayFlags again.
  EXPECT_CALL(client, AddAutoplayFlags(testing::_, testing::_)).Times(2);
  ...
  // AddAutoplayFlags should not be called in prerendering.
  EXPECT_CALL(prerendered_client, AddAutoplayFlags(testing::_, testing::_))
      .Times(0);
  prerender_helper().NavigatePrerenderedPage(host_id, prerender_url);
