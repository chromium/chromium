...
IN_PROC_BROWSER_TEST_F(WebRtcSameOriginPolicyBrowserTest,
  ...
  // we open the target page second so that we can easily navigate it without
  // switching focus.
  content::WebContents* capturing_tab = OpenTestPageInNewTab(kMainHtmlPage);
  ...
  // Because the target page was opened second, we don't need to switch before
  // navigating it. We navigate to a FileURL so that the origin will change,
  // which should trigger the capture to end.
...
IN_PROC_BROWSER_TEST_F(WebRtcSameOriginPolicyBrowserTest,
  ...
  // Because the target page was opened second, we don't need to switch before
  // navigating it. We navigate using the embedded_test_server so that the
