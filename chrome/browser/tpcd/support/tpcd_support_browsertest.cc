...
IN_PROC_BROWSER_TEST_F(TpcdSupportBrowserTest,
  ...
  EXPECT_EQ(settings->GetCookieSetting(
                kEnrolledSiteWithToken, GURL(),
                {net::CookieSettingOverride::k3pcdSupport}, nullptr),
            CONTENT_SETTING_BLOCK);
  EXPECT_EQ(settings->GetCookieSetting(
                kEnrolledSiteWithToken, embedding_site,
                {net::CookieSettingOverride::k3pcdSupport}, nullptr),
            CONTENT_SETTING_BLOCK);
  ...
  // TODO (crbug.com/1466156): Once CookieSetting call-sites have been updated
  // to pass in `CookieSettingOverride::k3pcdSupport`, actually attempt to read
  // the enrolled site's cookie as a third-party.
  ...
  EXPECT_EQ(settings->GetCookieSetting(
                enrolled_site_diff_path, embedding_site,
                {net::CookieSettingOverride::k3pcdSupport}, nullptr),
            CONTENT_SETTING_ALLOW);
...
IN_PROC_BROWSER_TEST_F(TpcdSupportBrowserTest,
  ...
  EXPECT_EQ(settings->GetCookieSetting(
                enrolled_site_subdomain, embedding_site,
                {net::CookieSettingOverride::k3pcdSupport}, nullptr),
            CONTENT_SETTING_ALLOW);
}
