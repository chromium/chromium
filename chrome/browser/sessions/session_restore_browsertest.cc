...
// Tests that trying to restore an entry with a large URL (causing the
// serialized version to have an empty URL instead) won't crash, and instead
// will restore about:blank. Disabled for some platforms that somehow crashed
// on restore.
// Regression test for https://crbug.com/1299335.
#if !BUILDFLAG(IS_CHROMEOS) && !BUILDFLAG(IS_WIN) && !BUILDFLAG(IS_LINUX) && \
    !defined(ADDRESS_SANITIZER)
IN_PROC_BROWSER_TEST_F(SessionRestoreTest,
                       RestoringEntryWithVeryLargeUrlShouldNotCrash) {
  // Navigate to a very long data: URL.
  const std::u16string long_text(100000, 'x');
  GURL long_data_url(u"data:text/html,<html>" + long_text + u"</html>");
  ui_test_utils::NavigateToURLWithDisposition(
      browser(), long_data_url, WindowOpenDisposition::CURRENT_TAB,
      ui_test_utils::BROWSER_TEST_WAIT_FOR_LOAD_STOP);

  // Verify that the very long data: URL committed successfully.
  content::WebContents* contents =
      browser()->tab_strip_model()->GetActiveWebContents();
  EXPECT_EQ(2, contents->GetController().GetEntryCount());
  EXPECT_EQ(long_data_url,
            contents->GetController().GetLastCommittedEntry()->GetURL());

  QuitBrowserAndRestore(browser());

  // Restoring the entry with the very long data: URL will result in
  // about:blank, because the serialized version of the entry has an empty URL,
  // and empty URLs are rewritten to about:blank during session restore.
  Browser* new_browser = QuitBrowserAndRestore(browser());
  content::NavigationController& new_controller =
      new_browser->tab_strip_model()->GetActiveWebContents()->GetController();
  EXPECT_EQ(2, new_controller.GetEntryCount());
  EXPECT_EQ(GURL(url::kAboutBlankURL),
            new_controller.GetLastCommittedEntry()->GetURL());
}
#endif
