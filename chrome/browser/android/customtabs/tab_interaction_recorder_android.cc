...
void AutofillObserverImpl::OnAfterFormsSeen(
  ...
  // Set the had form data associated value in the main RenderFrameHost's
  // BackForwardCacheMetrics in the page. Note that thismeans that if |rfh| is a
  // subframe it's possible that the page will still be marked as having a form
  // data associated with it even though |rfh| had navigated to another document
  // and there is no longer any document with forms in the page.
