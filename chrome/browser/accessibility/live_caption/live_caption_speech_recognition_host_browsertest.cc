...
class MockLiveTranslateController : public LiveTranslateController {
 public:
  ...
  void GetTranslation(const std::string& result,
    ...
    std::move(callback).Run("translation string");
  }
...
IN_PROC_BROWSER_TEST_F(LiveCaptionSpeechRecognitionHostTest, TranslationCache) {
  ...
  // The translation should be cached, so there should not be a second request
  // to the LiveTranslateController.
  OnSpeechRecognitionRecognitionEvent(
      frame_host, "Pomeranians come in 23 different color combinations.",
      /* expected_success= */ true, /* is_final= */ false);
  base::RunLoop().RunUntilIdle();
  ...
  ASSERT_EQ("Some Pomeranians are even tricolored!",
  ...
  ASSERT_EQ("Are they the cutest dog breed in the world?",
  ...
  ASSERT_EQ("Absolutely.", GetTranslationRequests()[4]);
  ...
  OnSpeechRecognitionRecognitionEvent(frame_host, "First sentence.",
                                      /* expected_success= */ true,
                                      /* is_final= */ false);
  OnSpeechRecognitionRecognitionEvent(frame_host, "Third sentence.",
                                      /* expected_success= */ true,
                                      /* is_final= */ false);
  base::RunLoop().RunUntilIdle();
