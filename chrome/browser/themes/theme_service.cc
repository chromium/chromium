...
bool ThemeService::UsingPolicyTheme() const {
  return profile_->GetPrefs()->IsManagedPreference(themes::kPolicyThemeColor);
}
...
void ThemeService::BuildAutogeneratedThemeFromColor(SkColor color,
  ...
  SwapThemeSupplier(AutogeneratedThemeSupplier::CreateForColor(color));
  if (theme_supplier_) {
    if (store_in_prefs) {
...
SkColor ThemeService::GetPolicyThemeColor() const {
  ...
  return profile_->GetPrefs()->GetInteger(themes::kPolicyThemeColor);
}
