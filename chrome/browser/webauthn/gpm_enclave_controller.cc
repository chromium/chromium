...
void GPMEnclaveController::OnUVCapabilityKnown(bool can_make_uv_keys) {
  ...
  if (!can_make_uv_keys) {
    platform_uv_support_ = EnclaveManager::PlatformUvSupport::kNoUvKey;
  } else {
    platform_uv_support_ =
...
GPMEnclaveController::GetEnclaveUserVerificationMethod() {
  if (!enclave_manager_->IsReady() || !platform_uv_support_.has_value()) {
  ...
  EnclaveManager::UvKeyState uv_key_state =
      enclave_manager_->uv_key_state(*platform_uv_support_);
