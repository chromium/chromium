...
IN_PROC_BROWSER_TEST_F(SCTHashdanceBrowserTest,
  ...
  // Set up Safe Browsing preferences so hashdance is used.
  SetExtendedReportingEnabled(false);
  SetSafeBrowsingEnabled(true);
  ...
  // Reset the SCT auditing cache by disabling and re-enabling Safe Browsing.
  // Otherwise, the second report will be deduplicated and not sent.
  SetSafeBrowsingEnabled(false);
  SetSafeBrowsingEnabled(true);

  // Repeat and check that the report count is incremented again to 2.
  ASSERT_TRUE(ui_test_utils::NavigateToURL(
      browser(), https_server()->GetURL("hashdance.test", "/")));
  WaitForRequests(4);
  EXPECT_EQ(4u, requests_seen());
  EXPECT_EQ(
      "hashdance.test",
      GetLastSeenReport().certificate_report(0).context().origin().hostname());
  report_count = g_browser_process->local_state()->GetInteger(
      prefs::kSCTAuditingHashdanceReportCount);
