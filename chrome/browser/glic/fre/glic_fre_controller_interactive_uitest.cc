...
// TODO(https://crbug.com/422832680): Timing tests are flaky on Linux try bots.
// Fix when linux is a prioritized launch platform.
#if BUILDFLAG(IS_LINUX)
#define MAYBE_RecordsHoldTimeWhenContentLoadsFast \
  DISABLED_RecordsHoldTimeWhenContentLoadsFast
#else
#define MAYBE_RecordsHoldTimeWhenContentLoadsFast \
  RecordsHoldTimeWhenContentLoadsFast
#endif
IN_PROC_BROWSER_TEST_F(GlicFreControllerUiHoldTimeTest,
                       MAYBE_RecordsHoldTimeWhenContentLoadsFast) {
  auto server_running = fre_server().StartAcceptingConnectionsAndReturnHandle();
