...
void GlicExperimentalOptInDialogView::OnWidgetVisibilityChanged(
  ...
  // Announce once, and asynchronously: the widget is shown and then activated
  // synchronously by TabDialogManager, so posting a task ensures the
  // announcement is made after the dialog's window is on screen and has become
  // the key/active window. Announcements posted before that may be discarded
...
void GlicExperimentalOptInDialogView::AnnounceAccessibleTitle() {
  if (!web_view_ || !GetWidget() || !GetWidget()->IsVisible()) {
    return;
  }
