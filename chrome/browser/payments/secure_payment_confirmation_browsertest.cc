...
// is set. It is currently desktop-only.
#if !BUILDFLAG(IS_ANDROID)
IN_PROC_BROWSER_TEST_F(SecurePaymentConfirmationTest, OptOutRequiresFlagSet) {
  test_controller()->SetHasAuthenticator(true);
  NavigateTo("a.com", "/secure_payment_confirmation.html");
  std::vector<uint8_t> credential_id = {'c', 'r', 'e', 'd'};
  std::vector<uint8_t> user_id = {'u', 's', 'e', 'r'};
...
// The opt-out support is currently desktop-only.
#if !BUILDFLAG(IS_ANDROID)
