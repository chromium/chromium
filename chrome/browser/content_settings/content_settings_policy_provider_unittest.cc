...
TEST_F(PolicyProviderTest, ManagedDefaultMediaCaptureAllowed) {
  const struct {
  ...
  for (const auto& test_case : kTestCases) {
    ...
    // If policy is not set, we should get no rules.
    std::unique_ptr<RuleIterator> rule_iterator(
        provider.GetRuleIterator(test_case.content_type, false));
    EXPECT_FALSE(rule_iterator);

    // Set policy to blocked (false).
    prefs->SetManagedPref(test_case.pref_name,
                          std::make_unique<base::Value>(false));

    rule_iterator = provider.GetRuleIterator(test_case.content_type, false);
    ASSERT_TRUE(rule_iterator);
    EXPECT_TRUE(rule_iterator->HasNext());
    std::unique_ptr<Rule> rule = rule_iterator->Next();
    EXPECT_FALSE(rule_iterator->HasNext());
...
TEST_F(PolicyProviderTest, ManagedMediaCaptureAllowedForUrls) {
  const struct {
    const char* pref_name;
    ContentSettingsType content_type;
  } kTestCases[] = {
  ...
  for (const auto& test_case : kTestCases) {
    ...
    // Set policy to allow specific URL and an invalid wildcard pattern.
    // The wildcard should be ignored, but the valid URL should still be
    // applied.
    base::ListValue list;
    list.Append("*");
    list.Append(kTestSubdomainPattern);
    prefs->SetManagedPref(test_case.pref_name, std::move(list));
...
TEST_F(PolicyProviderTest, ObserveManagedAudioCaptureAllowedSettingsChange) {
  TestingProfile profile;
  sync_preferences::TestingPrefServiceSyncable* prefs =
      profile.GetTestingPrefService();
  PolicyProvider provider(prefs);
  ...
  // Set the managed pref.
  prefs->SetManagedPref(prefs::kManagedAudioCaptureAllowed,
                        std::make_unique<base::Value>(false));
  ::testing::Mock::VerifyAndClearExpectations(&mock_observer);
  ...
  prefs->RemoveManagedPref(prefs::kManagedAudioCaptureAllowed);
  provider.ShutdownOnUIThread();
}

// ManagedDefaultMediaStreamSetting is deprecated but takes effect if and only
// if no value is set for AudioCaptureAllowed.
TEST_F(PolicyProviderTest, ManagedDefaultMediaCaptureConflicts) {
  TestingProfile profile;
  sync_preferences::TestingPrefServiceSyncable* prefs =
      profile.GetTestingPrefService();
  PolicyProvider provider(prefs);

  // If ManagedDefaultMediaStreamSetting is set to ASK and AudioCaptureAllowed
  // is not set, the effective value is ASK.
  prefs->SetManagedPref(prefs::kManagedDefaultMediaStreamSetting,
                        std::make_unique<base::Value>(CONTENT_SETTING_ASK));
  std::unique_ptr<RuleIterator> rule_iterator =
      provider.GetRuleIterator(ContentSettingsType::MEDIASTREAM_MIC, false);
  ASSERT_TRUE(rule_iterator);
  EXPECT_TRUE(rule_iterator->HasNext());
  std::unique_ptr<Rule> rule = rule_iterator->Next();
  EXPECT_FALSE(rule_iterator->HasNext());
  EXPECT_EQ(CONTENT_SETTING_ASK, ValueToContentSetting(rule->value));
  rule_iterator.reset();

  // If ManagedDefaultMediaStreamSetting is set to either BLOCK/ASK and
  // AudioCaptureAllowed is set to False, the effective value is BLOCK.
  prefs->SetManagedPref(prefs::kManagedAudioCaptureAllowed,
                        std::make_unique<base::Value>(false));
  rule_iterator =
      provider.GetRuleIterator(ContentSettingsType::MEDIASTREAM_MIC, false);
  ...
  // If ManagedDefaultMediaStreamSetting is set to either BLOCK/ASK and
  // AudioCaptureAllowed is set to True, the effective value is DEFAULT (no
  // forced policy).
  prefs->SetManagedPref(prefs::kManagedAudioCaptureAllowed,
                        std::make_unique<base::Value>(true));
  rule_iterator =
  ...
  // No change if in this state the ManagedDefaultMediaStreamSetting policy
  // has its value updated (after the PrefProvider is already initialized).
  prefs->SetManagedPref(prefs::kManagedDefaultMediaStreamSetting,
                        std::make_unique<base::Value>(CONTENT_SETTING_BLOCK));
  rule_iterator =
      provider.GetRuleIterator(ContentSettingsType::MEDIASTREAM_MIC, false);
  ...
  // However, if the AudioCaptureAllowed policy is then unset, the previously
  // set value for ManagedDefaultMediaStreamSetting should now take effect.
  prefs->SetManagedPref(prefs::kManagedAudioCaptureAllowed,
                        std::make_unique<base::Value>(false));
  rule_iterator =
      provider.GetRuleIterator(ContentSettingsType::MEDIASTREAM_MIC, false);
...
TEST_F(PolicyProviderTest, UnmanagedDefaultContentSettingsIgnored) {
  TestingProfile profile;
  sync_preferences::TestingPrefServiceSyncable* prefs =
      profile.GetTestingPrefService();

  // Set unmanaged user preferences for default content settings.
  prefs->SetUserPref(prefs::kManagedDefaultMediaStreamSetting,
                     base::Value(CONTENT_SETTING_BLOCK));
  prefs->SetUserPref(prefs::kManagedDefaultCookiesSetting,
                     base::Value(CONTENT_SETTING_BLOCK));

  // Set unmanaged user preferences for audio/video capture.
  prefs->SetUserPref(prefs::kManagedAudioCaptureAllowed, base::Value(false));
  prefs->SetUserPref(prefs::kManagedVideoCaptureAllowed, base::Value(false));
  ...
  rule_iterator =
      provider.GetRuleIterator(ContentSettingsType::MEDIASTREAM_CAMERA, false);
  EXPECT_FALSE(rule_iterator);

  rule_iterator = provider.GetRuleIterator(ContentSettingsType::COOKIES, false);
  EXPECT_FALSE(rule_iterator);

  provider.ShutdownOnUIThread();
}

TEST_F(PolicyProviderTest, ManagedPolicyOverridesUnmanagedPreference) {
  TestingProfile profile;
  sync_preferences::TestingPrefServiceSyncable* prefs =
      profile.GetTestingPrefService();

  // Set unmanaged user pref.
  prefs->SetUserPref(prefs::kManagedDefaultCookiesSetting,
  ...
  // Remove the managed pref and verify the default reverts to unconstrained,
  // ignoring the remaining user pref.
  prefs->RemoveManagedPref(prefs::kManagedDefaultCookiesSetting);
