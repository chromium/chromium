...
TEST_F(AIManagerKeyedServiceTest, AITextSessionSet) {
  ...
  AIManagerKeyedService::AITextSessionSet* sessions =
      ai_manager->GetSessionsFromContext(mock_host());
  ASSERT_EQ(0u, sessions->sessions_.size());
  ...
  run_loop.Run();
  ASSERT_EQ(1u, sessions->sessions_.size());

  // After resetting the session, the `AITextSessionSet` becomes empty again.
  mock_session.reset();
  task_environment()->RunUntilIdle();
  ASSERT_EQ(0u, sessions->sessions_.size());
}
