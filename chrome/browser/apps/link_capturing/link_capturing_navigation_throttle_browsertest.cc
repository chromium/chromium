...
class LinkCapturingNavigationThrottleBrowserTest : public InProcessBrowserTest {
 public:
  ...
  void WaitForAppLaunch() {
    web_app::WebAppTestLaunchedObserver(app_id_).WaitUntilLaunched();
  }

  void CheckForSimpleTabAndAppWindow() {
    EXPECT_EQ(GetBrowserListState(),
              base::StringPrintf("{{%s}, {%s}}", simple_url_.spec().c_str(),
                                 start_url_.spec().c_str()));
  }

  std::string GetBrowserListState() {
    std::vector<std::string> browser_states;
    base::ranges::transform(
        *BrowserList::GetInstance(), std::back_inserter(browser_states),
        [](Browser* browser) {
          std::vector<std::string> tab_states;
          for (int i = 0; i < browser->tab_strip_model()->count(); ++i) {
            tab_states.push_back(browser->tab_strip_model()
                                     ->GetWebContentsAt(i)
                                     ->GetVisibleURL()
                                     .spec());
          }
          return base::StrCat({"{", base::JoinString(tab_states, ", "), "}"});
        });
    return base::StrCat({"{", base::JoinString(browser_states, ", "), "}"});
  }
