...
IN_PROC_BROWSER_TEST_F(ContextMenuIncognitoFilterBrowserTest,
  ...
  // Ensure we don't keep the web contents observer attached beyond the
  // cross-OTR navigation.
  ASSERT_EQ(tab->GetUserData("mattwashere2"), nullptr);
  ...
  // The response was a 302 redirect followed by a 200, and the navigation went
  // from normal-->OTR browsing.
