...
// TODO(crbug.com/436662421): This test validates the nullptr checks that were
// added to ActorOverlayViewController to handle moving tabs to unsupported
// window types. Once the nullptr checks are cleaned up, we can delete this
// test.
IN_PROC_BROWSER_TEST_F(ActorOverlayTest,
                       ViewControllerHandlesNullWindowController) {
  Profile* const profile = browser()->profile();
  // The main browser() is our normal window with a non-null window controller.
  Browser* const normal_browser = browser();
  ASSERT_EQ(normal_browser->type(), Browser::TYPE_NORMAL);
  ASSERT_NE(normal_browser->browser_window_features()
                ->actor_overlay_window_controller(),
            nullptr);

  // Create a popup window, which will have a null window controller.
  Browser* const popup_browser = CreateBrowserForPopup(profile);
  ASSERT_EQ(popup_browser->type(), Browser::TYPE_POPUP);
  ASSERT_EQ(popup_browser->browser_window_features()
                ->actor_overlay_window_controller(),
            nullptr);

  // Add a new tab to the normal browser.
  tabs::TabInterface* const tab_to_move =
      tabs::TabInterface::GetFromContents(&chrome::NewTab(normal_browser));
  ASSERT_EQ(normal_browser->tab_strip_model()->count(), 2);

  // Start an Actor task on the tab. The overlay should be visible in the normal
  // browser.
  ActorUiStateManagerInterface* state_manager =
      ActorKeyedService::Get(profile)->GetActorUiStateManager();
  TestFuture<ActionResultPtr> result;
  state_manager->OnUiEvent(
      StartingToActOnTab(tab_to_move->GetHandle(), TaskId(1)),
      result.GetCallback());
  ExpectOkResult(result);
  ASSERT_TRUE(base::test::RunUntil(
      [&]() { return IsActorOverlayVisible(normal_browser); }));
  ASSERT_TRUE(base::test::RunUntil(
      [&]() { return IsActorOverlayChildVisible(normal_browser); }));

  // Move the tab, at index 1, to the popup window. In the new window,
  // SetWindowController will be called with nullptr.
  chrome::MoveTabsToExistingWindow(normal_browser, popup_browser, {1});
  ASSERT_EQ(popup_browser->tab_strip_model()->count(), 2);
  ASSERT_EQ(normal_browser->tab_strip_model()->count(), 1);

  // The overlay should now be hidden, as it has no window to attach to.
  ASSERT_TRUE(base::test::RunUntil(
      [&]() { return !IsActorOverlayVisible(normal_browser); }));
  ASSERT_EQ(NumActorOverlayChildren(popup_browser), 0u);

  // Move the tab back to the original normal browser.
  chrome::MoveTabsToExistingWindow(popup_browser, normal_browser, {1});
  ASSERT_EQ(normal_browser->tab_strip_model()->count(), 2);

  // The overlay should be visible. This checks that the controller correctly
  // handled the transition and re-attached its WebView to the valid window
  // controller.
  ASSERT_TRUE(base::test::RunUntil(
      [&]() { return IsActorOverlayVisible(normal_browser); }));
  ASSERT_TRUE(base::test::RunUntil(
