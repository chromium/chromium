...
class WebAccessibleResourcesUseDynamicUrlApiTest : public ExtensionApiTest {
 public:
  WebAccessibleResourcesUseDynamicUrlApiTest() {
    feature_list_.InitAndEnableFeature(
        extensions_features::kExtensionDynamicURLRedirection);
  }
...
IN_PROC_BROWSER_TEST_F(WebAccessibleResourcesUseDynamicUrlApiTest,
                       UseDynamicUrl) {
  // Load extension.
  TestExtensionDir extension_dir;
  static constexpr char kManifestStub[] = R"({
  ...
  static constexpr char kScriptTemplate[] = R"(
    ...
    // Run tests.
    const testCases = [
      // [title, isAllowed, filename, useDynamicUrl] are the arguments.
...
// TODO(crbug.com/40245983): Exercise these resources being used in iframes in a
// web page. The navigation flow goes through a different path than resource
// fetching.
