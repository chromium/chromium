...
class SocketsTcpApiTest : public ExtensionApiTest {
 public:
  SocketsTcpApiTest() {
    // Enable the features required for the test to verify that the correct
    // NetworkAnonymizationKey was used for the DNS lookup.
...
IN_PROC_BROWSER_TEST_F(SocketsTcpApiTest, SocketTcpExtension) {
  ...
  scoped_refptr<const Extension> test_extension =
      LoadExtension(test_data_dir_.AppendASCII("sockets_tcp/api"));
  ...
  EXPECT_EQ(0, result2.resolved_addresses.size());
