...
class WebContentsHiddenObserver : public content::WebContentsObserver {
 public:
  ...
  ~WebContentsHiddenObserver() override = default;
...
void WebViewAPITest::LaunchApp(const std::string& app_location) {
  ...
  const Extension* extension =
      LoadExtension(test_data_dir_.AppendASCII(app_location));
...
void WebViewAPITest::CloseAppWindows() {
  AppWindowRegistry::AppWindowList app_window_list =
      AppWindowRegistry::Get(profile())->app_windows();
  for (AppWindow* app_window : app_window_list) {
...
void WebViewAPITest::StartTestServer(const std::string& app_location) {
  ...
  base::ScopedAllowBlockingForTesting allow_blocking;
  // Ensure we serve files from chrome/test/data/extensions/.
  base::FilePath test_data_dir = test_data_dir_.AppendASCII(app_location);
...
class WebViewAPITestUserAgentOverride : public WebViewAPITest {
 public:
  void SetUp() override {
