...
IN_PROC_BROWSER_TEST_F(CommandsApiTest, InactivePageActionDoesntTrigger) {
  ...
  SendKeyPressToAction(browser_window_interface(), *extension, ui::VKEY_F,
...
// Simulate the user setting the keybinding to override the print shortcut.
  ...
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_L, control_is_modifier, false, false,
      command_is_modifier));

  // Activate the shortcut.
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_P, control_is_modifier, false, false,
      command_is_modifier));
...
IN_PROC_BROWSER_TEST_F(CommandsApiTest, DontOverwriteSystemShortcuts) {
  ...
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_F, false, true, true, false));
  ...
  // Send Ctrl+F.
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_F, true, false, false, false));
  // Send Alt+Shift+F.
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_F, false, true, true, false));
  EXPECT_TRUE(alt_shift_f_listener.WaitUntilSatisfied());
...
IN_PROC_BROWSER_TEST_F(CommandsApiTest,
  ...
  ASSERT_TRUE(SendBookmarkKeyPressSync(browser_window_interface()));
  EXPECT_TRUE(test_listener.WaitUntilSatisfied());
  EXPECT_EQ(std::string("basics browser action"), test_listener.message());
}
...
IN_PROC_BROWSER_TEST_F(CommandsApiTest, MAYBE_ContinuePropagation) {
  ...
  // initially.
  ASSERT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_F, true, true, false, false));
  ASSERT_TRUE(catcher.GetNextResult());
...
IN_PROC_BROWSER_TEST_P(ActionCommandsApiTest,
  ...
  EXPECT_TRUE(ui_test_utils::SendKeyPressSync(
      browser_window_interface(), ui::VKEY_U, false, true, true, false));
