...
public interface CustomLinkOperations {
    /**
     * Adds a custom link with {@code name} and {@code url}, which must not collide with an existing
     * custom link.
     *
    ...
    /**
     * Assigns a link identified by {@code keyUrl} to a custom link specified with {@code name} and
     * {@code url}. This may modify an existing custom link, or convert a Most Visited link to a
     * custom link.
     *
    ...
    /**
     * Deletes a custom link identified by {@code keyUrl}, no-op if non-existent.
     *
    ...
    /**
     * Tests the existence of a custom link identified by {@code keyUrl}.
     *
     * @param keyUrl The URL to search.
     * @return Whether a custom link identified by {@code keyUrl} exists.
     */
    boolean hasCustomLink(GURL keyUrl);

    /**
     * Moves a custom link identified by {@code keyUrl} to a new position, and shift all other
