...
public class ChromeBrowserProviderImpl extends ChromeBrowserProvider.Impl {
    ...
    public static final Uri BROWSER_CONTRACTS_BOOKMAKRS_API_URI =
            buildContentUri(BROWSER_CONTRACT_API_AUTHORITY, BOOKMARKS_PATH);

    public static final Uri BROWSER_CONTRACTS_SEARCHES_API_URI =
            buildContentUri(BROWSER_CONTRACT_API_AUTHORITY, SEARCHES_PATH);

    public static final Uri BROWSER_CONTRACTS_HISTORY_API_URI =
            buildContentUri(BROWSER_CONTRACT_API_AUTHORITY, HISTORY_PATH);

    public static final Uri BROWSER_CONTRACTS_COMBINED_API_URI =
            buildContentUri(BROWSER_CONTRACT_API_AUTHORITY, COMBINED_PATH);
    ...
    private static final String[] SUGGEST_PROJECTION =
            new String[] {BookmarkColumns.ID, BookmarkColumns.TITLE, BookmarkColumns.URL,
    ...
    private Cursor getBookmarkHistorySuggestions(
            String selection, String[] selectionArgs, String sortOrder, boolean excludeHistory) {
        ...
        Cursor cursor = queryBookmarkFromAPI(
                SUGGEST_PROJECTION, urlWhere.toString(), selectionArgs, sortOrder);
        ...
        switch (match) {
            ...
            case URL_MATCH_API_HISTORY_CONTENT:
                cursor = queryBookmarkFromAPI(
                        projection, buildHistoryWhereClause(selection), selectionArgs, sortOrder);
                break;
            ...
            case URL_MATCH_API_BOOKMARK_CONTENT:
                cursor = queryBookmarkFromAPI(
                        projection, buildBookmarkWhereClause(selection), selectionArgs, sortOrder);
                break;
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case URI_MATCH_BOOKMARKS_ID:
                result = ChromeBrowserProviderImplJni.get().removeBookmark(
                        mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, bookmarkId);
                break;
            case URL_MATCH_API_BOOKMARK_ID:
            ...
            case URL_MATCH_API_SEARCHES_ID:
                result =
                        removeSearchFromAPI(buildWhereClause(bookmarkId, selection), selectionArgs);
                break;
            case URL_MATCH_API_SEARCHES:
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case URI_MATCH_BOOKMARKS_ID:
                ...
                result = ChromeBrowserProviderImplJni.get().updateBookmark(
                        mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, bookmarkId,
                        url, title, parentId);
                updateLastModifiedBookmarkFolder(parentId);
                break;
            case URL_MATCH_API_BOOKMARK_ID:
                result = updateBookmarkFromAPI(
                        values, buildWhereClause(bookmarkId, selection), selectionArgs);
                break;
            case URL_MATCH_API_BOOKMARK:
                result = updateBookmarkFromAPI(values, selection, selectionArgs);
                break;
            case URL_MATCH_API_SEARCHES_ID:
                result = updateSearchTermFromAPI(
                        values, buildWhereClause(bookmarkId, selection), selectionArgs);
                break;
            case URL_MATCH_API_SEARCHES:
            ...
            case URL_MATCH_API_HISTORY_CONTENT:
                result = updateBookmarkFromAPI(
                        values, buildHistoryWhereClause(selection), selectionArgs);
                break;
            case URL_MATCH_API_HISTORY_CONTENT_ID:
                result = updateBookmarkFromAPI(
                        values, buildHistoryWhereClause(bookmarkId, selection), selectionArgs);
                break;
            case URL_MATCH_API_BOOKMARK_CONTENT:
                result = updateBookmarkFromAPI(
                        values, buildBookmarkWhereClause(selection), selectionArgs);
                break;
            case URL_MATCH_API_BOOKMARK_CONTENT_ID:
                result = updateBookmarkFromAPI(
                        values, buildBookmarkWhereClause(bookmarkId, selection), selectionArgs);
                break;
            default:
    ...
    private long addBookmark(ContentValues values) {
        ...
        long id = ChromeBrowserProviderImplJni.get().addBookmark(mNativeChromeBrowserProvider,
                ChromeBrowserProviderImpl.this, url, title, isFolder, parentId);
    ...
    private long addBookmarkFromAPI(ContentValues values) {
        ...
        return ChromeBrowserProviderImplJni.get().addBookmarkFromAPI(mNativeChromeBrowserProvider,
                ChromeBrowserProviderImpl.this, row.mUrl, row.mCreated, row.mIsBookmark, row.mDate,
                row.mFavicon, row.mTitle, row.mVisits, row.mParentId);
    }

    private Cursor queryBookmarkFromAPI(
        ...
        return ChromeBrowserProviderImplJni.get().queryBookmarkFromAPI(mNativeChromeBrowserProvider,
                ChromeBrowserProviderImpl.this, projection, selection, selectionArgs, sortOrder);
    }

    private int updateBookmarkFromAPI(
            ContentValues values, String selection, String[] selectionArgs) {
        BookmarkRow row = BookmarkRow.fromContentValues(values);
        return ChromeBrowserProviderImplJni.get().updateBookmarkFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, row.mUrl,
                row.mCreated, row.mIsBookmark, row.mDate, row.mFavicon, row.mTitle, row.mVisits,
                row.mParentId, selection, selectionArgs);
    }

    private int removeBookmarkFromAPI(String selection, String[] selectionArgs) {
        return ChromeBrowserProviderImplJni.get().removeBookmarkFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, selection,
                selectionArgs);
    }

    private int removeHistoryFromAPI(String selection, String[] selectionArgs) {
        return ChromeBrowserProviderImplJni.get().removeHistoryFromAPI(mNativeChromeBrowserProvider,
                ChromeBrowserProviderImpl.this, selection, selectionArgs);
    }
    ...
    private long addSearchTermFromAPI(ContentValues values) {
        ...
        return ChromeBrowserProviderImplJni.get().addSearchTermFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, row.mTerm, row.mDate);
    }

    private int updateSearchTermFromAPI(
            ContentValues values, String selection, String[] selectionArgs) {
        SearchRow row = SearchRow.fromContentValues(values);
        return ChromeBrowserProviderImplJni.get().updateSearchTermFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, row.mTerm, row.mDate,
                selection, selectionArgs);
    }

    private Cursor querySearchTermFromAPI(
            String[] projectionIn, String selection, String[] selectionArgs, String sortOrder) {
        String[] projection = null;
        if (projectionIn == null || projectionIn.length == 0) {
        ...
        return ChromeBrowserProviderImplJni.get().querySearchTermFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, projection, selection,
                selectionArgs, sortOrder);
    }

    private int removeSearchFromAPI(String selection, String[] selectionArgs) {
        return ChromeBrowserProviderImplJni.get().removeSearchTermFromAPI(
                mNativeChromeBrowserProvider, ChromeBrowserProviderImpl.this, selection,
                selectionArgs);
    }
    ...
    private void ensureNativeSideInitialized() {
        ...
        if (mNativeChromeBrowserProvider == 0) {
            mNativeChromeBrowserProvider =
                    ChromeBrowserProviderImplJni.get().init(ChromeBrowserProviderImpl.this);
        }
    }
    ...
    @NativeMethods
    interface Natives {
        long init(ChromeBrowserProviderImpl caller);

        long addBookmark(long nativeChromeBrowserProvider, ChromeBrowserProviderImpl caller,
                String url, String title, boolean isFolder, long parentId);

        int removeBookmark(
                long nativeChromeBrowserProvider, ChromeBrowserProviderImpl caller, long id);
        int updateBookmark(long nativeChromeBrowserProvider, ChromeBrowserProviderImpl caller,
                long id, String url, String title, long parentId);
        long addBookmarkFromAPI(long nativeChromeBrowserProvider, ChromeBrowserProviderImpl caller,
        ...
        int updateBookmarkFromAPI(long nativeChromeBrowserProvider,
                ChromeBrowserProviderImpl caller, String url, Long created, Boolean isBookmark,
                Long date, byte[] favicon, String title, Integer visits, long parentId,
                String selection, String[] selectionArgs);
        int removeBookmarkFromAPI(long nativeChromeBrowserProvider,
                ChromeBrowserProviderImpl caller, String selection, String[] selectionArgs);
        int removeHistoryFromAPI(long nativeChromeBrowserProvider, ChromeBrowserProviderImpl caller,
                String selection, String[] selectionArgs);
        long addSearchTermFromAPI(long nativeChromeBrowserProvider,
        ...
        SQLiteCursor querySearchTermFromAPI(long nativeChromeBrowserProvider,
                ChromeBrowserProviderImpl caller, String[] projection, String selection,
                String[] selectionArgs, String sortOrder);
        int updateSearchTermFromAPI(long nativeChromeBrowserProvider,
                ChromeBrowserProviderImpl caller, String search, Long date, String selection,
                String[] selectionArgs);
        int removeSearchTermFromAPI(long nativeChromeBrowserProvider,
                ChromeBrowserProviderImpl caller, String selection, String[] selectionArgs);
    }
}
