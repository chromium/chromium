...
import org.chromium.chrome.browser.compositor.overlay_panel.contextualsearch.ContextualSearchCalloutControl.CalloutListener;
import org.chromium.chrome.browser.compositor.overlay_panel.contextualsearch.ContextualSearchImageControl.ImageListener;
...
public class ContextualSearchBarControl {
    ...
    /**
     * The opacity of the Bar's Search Term. This text control may not be initialized until the
     * opacity is set beyond 0.
     */
    ...
    /**
     * Resets the SearchBar text opacity when a new search context is set. The search context is
     * made visible and the search term invisible.
     */
    ...
    /**
     * Resets the SearchBar text opacity when a new search term is set. The search term is made
     * visible and the search context invisible.
     */
    ...
    /**
     * Should be called when an onShowPress() event occurs on the Bar. See {@code
    ...
    /**
     * Updates the UI state for the SearchBar text. The search context view will fade out while the
     * search term fades in.
     *
     ...
     * animate the carousel away. The caller needs to call this when an expanding animation has
     * reached its maximum height, but may call it repeatedly as long as the Bar keeps growing.
     */
    ...
    /**
     * Returns the current height of the portion of the Related Searches UI that is visible due to
     * animation.
     */
