...
def CompressUsingLZMA(
    build_dir, compressed_file, input_file, verbose, fast, strip_time=False
):
...
def CopyAllFilesToStagingDir(
    ...
    CopySectionFilesToStagingDir(
        config, 'GENERAL', staging_dir, build_dir, verbose
    )
    if distribution:
        ...
        if config.has_section(distribution):
            CopySectionFilesToStagingDir(
                config, distribution, staging_dir, build_dir, verbose
            )
    if enable_hidpi == '1':
        CopySectionFilesToStagingDir(
            config, 'HIDPI', staging_dir, build_dir, verbose
        )

    if include_snapshotblob == '1':
        CopySectionFilesToStagingDir(
            config, 'SNAPSHOTBLOB', staging_dir, build_dir, verbose
        )

    if include_dxc == '1':
        CopySectionFilesToStagingDir(
            config, 'DXC', staging_dir, build_dir, verbose
        )

    if component_build != '1' and component_ffmpeg_build == '1':
        CopySectionFilesToStagingDir(
            config, 'FFMPEG', staging_dir, build_dir, verbose
        )
...
def CopySectionFilesToStagingDir(
    config, section, staging_dir, src_dir, verbose
):
    ...
    for option in config.options(section):
        ...
        if options.use_static_angle == 'True' and option.lower() in [
            ...
            if verbose:
                print(
                    'Skipping ANGLE library in static build: {}'.format(option)
                )
            continue
        ...
        if verbose and not src_paths:
            print(
                'No matches found for {} in {}'.format(
...
def GetPrevVersion(build_dir, temp_dir, last_chrome_installer, output_name):
    ...
    RunSystemCommand(cmd, options.verbose)
    dll_path = glob.glob(
        os.path.join(temp_dir, 'Chrome-bin', '*', 'chrome.dll')
    )
...
def Readconfig(input_file, current_version):
    ...
    variables['VersionDir'] = os.path.join(
        variables['ChromeDir'], current_version
    )
...
def CreateArchiveFiles(options, staging_dir, current_version, prev_version):
    ...
    if options.depfile:
        ...
        for root, dirs, files in os.walk(os.path.join(staging_dir, CHROME_DIR)):
            for filename in files:
                staging_contents.append(
                    path_fixup(os.path.join(root, filename))
                )
    ...
    if options.last_chrome_installer:
        prev_archive_file = os.path.join(
            options.last_chrome_installer, options.output_name + ARCHIVE_SUFFIX
        )
        patch_file = os.path.join(
            options.build_dir, patch_name_prefix + PATCH_FILE_EXT
        )
...
def PrepareSetupExec(options, current_version, prev_version):
    ...
    if options.setup_exe_format == 'FULL':
        ...
        prev_setup_file = os.path.join(
            options.last_chrome_installer, SETUP_EXEC
        )
        new_setup_file = os.path.join(options.build_dir, SETUP_EXEC)
        patch_file = os.path.join(
            options.build_dir, SETUP_PATCH_FILE_PREFIX + PATCH_FILE_EXT
        )
...
def CreateResourceInputFile(
    ...
    if component_build:
        installer_dir = os.path.join(
            staging_dir, CHROME_DIR, current_version, 'Installer'
        )
        resources += [
            (file, 'BD', os.path.join(installer_dir, file))
...
# |insert_before|.
def CopyAndAugmentManifest(
    build_dir, output_dir, manifest_name, inserted_string, insert_before
):
...
def DoComponentBuildTasks(
    ...
    staged_dlls = [
        os.path.normcase(os.path.relpath(os.path.join(dir, file), version_dir))
        for dir, _, files in os.walk(version_dir)
        for file in files
        if fnmatch.fnmatch(file, '*.dll')
    ]
    component_dll_filenames = []
    for component_dll in [
        dll
        for dll in build_dlls
        if os.path.normcase(os.path.relpath(dll, build_dir)) not in staged_dlls
    ]:
    ...
    for dll_filename in component_dll_filenames:
        version_assembly_dll_additions.append(
            "  <file name='%s'/>" % dll_filename
        )
    CopyAndAugmentManifest(
        build_dir,
        version_dir,
...
def main(options):
    ...
    setup_file = PrepareSetupExec(
        options, current_build_number, prev_build_number
    )
...
def _ParseOptions():
    ...
    parser.add_option(
        '-d', '--distribution', help='Name of Chromium Distribution. Optional.'
    )
    parser.add_option(
        '-s',
    ...
    if not options.resource_file_path:
        options.resource_file_path = os.path.join(
            options.build_dir, MINI_INSTALLER_INPUT_FILE
        )
