...
using complex_tasks::CommerceHintAgent;
...
TEST(CommerceHintAgentTest, MAYBE_RegexBenchmark) {
  ...
  for (int length = 16; length <= (1L << 20); length *= 4) {
    ...
    // With string copy, all of them would take at least 400us, assuming string
    // length is 1M chars, and running x86-64 Release build on Linux
    // workstation. Same for the rest. x86-64 Debug build is ~3X slower, and
    // Debug build on Pixel 2 is 3~10X slower.

    float elapsed_us = BenchmarkIsAddToCart(str);
    // Typical value is ~3us.
