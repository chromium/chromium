var verbose = 0;
var overlayClassName = 'clank_shopping_overlay';
var showClassName = 'show_' + overlayClassName;
...
function getLargeImages(root, atLeast, relaxed = false) {
  ...
  function shouldStillKeep(image) {
    ...
    // picture tag.
    if (image.parentElement.tagName == 'PICTURE') return true;
    return false;
  }
  for (const image of candidates) {
    if (verbose > 0) console.log('offsetHeight', image, image.offsetHeight);
...
function extractImage(item) {
  ...
  if (!multipleImagesSupported()) {
    if (verbose > 0) console.assert(images.length == 1, 'image extraction error', item, images);
    if (images.length != 1) {
      return null;
    }
  }
...
function chooseTitle(elementArray) {
  return elementArray.reduce(function(a, b) {
    ...
    if (verbose > 1) console.log('className score', a_score, b_score, a_str, b_str, a, b);
    ...
    if (verbose > 1) console.log('getBoundingClientRect', a.getBoundingClientRect(), b.getBoundingClientRect(), a, b);
    ...
    if (verbose > 1) console.log('font size', a_size, b_size, a, b);
...
function extractTitle(item) {
  ...
  const possible_titles = item.querySelectorAll('a, span, p, div, h1, h2, h3, h4, h5, strong');
  let titles = [];
  for (const title of possible_titles) {
    ...
    if (title.offsetWidth <= 1 || title.offsetHeight <= 1) continue;
  ...
  if (titles.length > 1) {
    if (verbose > 1) console.log('all generic titles', item, titles);
    titles = [chooseTitle(titles)];
  }

  if (verbose > 0) console.log('titles', item, titles);
...
function adjustBeautifiedCents(priceElement) {
  ...
  if (verbose > 0) console.log('cents', cents, priceElement);
  if (cents.length == 2
...
function extractPrice(item) {
  ...
  for (const price of item.querySelectorAll('span, b, p, div, h3')) {
    ...
    if (verbose > 1) console.log('price candidate', candidate, price);
    if (price.childElementCount > 0) {
      ...
      if (price.innerText.trim() == price.lastElementChild.innerText.trim()
        ...
        if (verbose > 1) console.log('skip redundant parent', price);
        continue;
      }
    }
    // TODO: check child elements recursively.
    if (anyLineThroughInAncentry(price)) {
      if (verbose > 1) console.log('line-through', price);
      continue;
    }
  ...
  if (verbose > 0) console.log('captured_prices', captured_prices);
...
function extractItem(item) {
  ...
  if (imageUrl == null) {
    if (verbose > 0) console.warn('no images found', item);
    return null;
  }
  url = extractUrl(item);
  ...
  if (url == null) {
    if (verbose > 0) console.warn('no url found', item);
    return null;
  }
  title = extractTitle(item);
  if (title == null) {
    if (verbose > 0) console.warn('no title found', item);
    return null;
  }
  ...
  if (price == null) {
    if (verbose > 0) console.warn('no price found', item);
    return null;
  }
...
function styleForOverlay() {
  ...
  style.sheet.insertRule('body.show_' + overlayClassName +
    ' .' + overlayClassName + `
    {
      ...
      height: 100%;
      width: 100%;
      background: rgba(128, 134, 138, 0.126);
      border: dashed rgb(128, 134, 138) 3px;
      border-radius: 10px;
      display: block;
      text-align: right;
      z-index: 9999; /* craigslist uses 999 */
      /* pointer-events: none; */ /* touch pass through */
    }
  `, 0);
   style.sheet.insertRule('body.show_' + overlayClassName +
     ' .' + overlayClassName + `[checked]
     {
       background: rgba(26, 115, 232, 0.126); /* rgb(227, 238, 252); */
       border: solid rgb(26, 115, 232) 2px; /* modern_blue_600 */
     }
   `, 0);
  style.sheet.insertRule('body.show_' + overlayClassName +
    ' .' + overlayClassName + ` div
    {
      ...
      text-align: right;
      padding: 0 0 16px 16px; /* slightly larger touch area */
    }
  `, 0);
...
function extractOneItem(item, extracted_items, processed, output, overlay) {
  ...
  if (item.getBoundingClientRect().height < 50) {
    if (verbose > 0) console.log('too short', item);
    return;
  }
  if (item.scrollHeight > 1000) {
    if (verbose > 0) console.log('too tall', item);
    return;
  }
  ...
  if (item.querySelectorAll('img, amp-img').length == 0)  {
    if (verbose > 0) console.log('no image', item);
    return;
  }
  if (!item.textContent.match(priceRegex)) {
    if (verbose > 0) console.log('no price', item);
    return;
  }
  //console.log('extracted_items', extracted_items);
  if (hasOverlap(item, extracted_items)) {
    if (verbose > 0) console.log('overlap', item);
    return;
  }
  if (processed.has(item)) return;
  processed.add(item);
  if (verbose > 0) console.log('trying', item);
...
function extractAllItems(root, overlay=false) {
  ...
  if (verbose > 0) console.log(items);
...
extractAllItems(getCartRootElement(), false);
