...
TEST(UserDataDir, EmptyResultsInDefault) {
  ...
  GetUserDataDirectoryImpl({}, kFakeInstallConstants, &result, &invalid);
...
TEST(UserDataDir, InvalidResultsInDefault) {
  ...
  GetUserDataDirectoryImpl(L"--user-data-dir=<>|:", kFakeInstallConstants,
                           &result, &invalid);
  EXPECT_TRUE(result.ends_with(kUserDataDirNameSuffix));
  EXPECT_EQ(L"<>|:", invalid);
}
...
TEST(UserDataDir, RegistrySettingsInHKLMOverrides) {
  ...
  GetUserDataDirectoryImpl(L"--user-data-dir=xxx", kFakeInstallConstants,
                           &result, &invalid);
...
TEST(UserDataDir, HasTempUserDataDirInHeadlessMode) {
  std::wstring result;
  std::wstring invalid;
  GetUserDataDirectoryImpl(L"--headless", kFakeInstallConstants, &result,
                           &invalid);
...
TEST(UserDataDir, HasNoTempUserDataDirInOldHeadlessMode) {
  std::wstring result;
  std::wstring invalid;
  GetUserDataDirectoryImpl(L"--headless=old", kFakeInstallConstants, &result,
                           &invalid);
...
TEST(UserDataDir, HasNoHeadlessTempUserDataDirIfProvidedInCommandLine) {
  ...
  GetUserDataDirectoryImpl(
      L"--headless --user-data-dir=" + cmd_line_user_data_dir,
