...
class SingleFileTarReader {
 public:
  ...
  absl::optional<uint64_t> tar_content_size() const {
    return tar_content_size_;
  }
 ...
 private:
  ...
  static absl::optional<uint64_t> ReadOctalNumber(
      base::span<const uint8_t> buffer);

  // Populated once the size has been parsed. The value 0 means the file in
  // the tar is empty.
  absl::optional<uint64_t> tar_content_size_;
