...
export function firmwareUpdateAppTest() {
  ...
  function getFirmwareUpdateDialog() {
    return page.shadowRoot.querySelector('firmware-update-dialog')
        .shadowRoot.querySelector('#firmwareUpdateDialog');
  }
  ...
  test('OpenFirmwareDialog', async () => {
    await flushTasks();
    assertFalse(!!getFirmwareUpdateDialog());
    // Open dialog for first firmware update card.
    getUpdateCards()[0].shadowRoot.querySelector(`#updateButton`).click();
    await flushTasks();
    assertTrue(getFirmwareUpdateDialog().open);
  });
}
