...
// Fork SUBMIT SUITE
// =============================================================================
suite('ContextualTasksComposeboxForkSubmitTest', () => {
  let testProxy: TestContextualTasksBrowserProxy;
  let mockComposeboxPageHandler: TestMock<ComposeboxPageHandlerRemote>&
      ComposeboxPageHandlerRemote;
  let mockSearchboxPageHandler: TestMock<SearchboxPageHandlerRemote>&
  ...
  test('ComposeboxSubmitSendsQueryBeforeAutocomplete', async () => {
    ...
    assertEquals(
        ...
        'Input should be cleared, but input = ' + getInputValue(inputElement));
  });
  ...
  test('empty query submits for Deep Search follow-up', async () => {
    ...
    assertTrue(disabledButton.disabled, 'Submit button should be disabled');
...
// =============================================================================
// Fork INJECT-INPUT SUITE
// =============================================================================
suite('ContextualTasksComposeboxForkInjectInputTest', () => {
  ...
  async function createPendingUpload() {
    ...
    assertTrue(
        innerComposebox.attachedContext.get(FAKE_TOKEN_STRING) !== undefined,
  ...
  test('InjectInputSubmitAfterInjectionTrue', async () => {
    ...
    const submitBaseline = mockSearchboxPageHandler.getCallCount('submitQuery');
  ...
  test('InjectInputSubmitAfterInjectionFalse', async () => {
    ...
    assertEquals(
        submitBaseline, mockSearchboxPageHandler.getCallCount('submitQuery'));
  });
  ...
  test('InjectInputPopulatesInjectedFileFields', async () => {
    ...
    const expectedUrl = 'chrome://image?url=' + encodeURIComponent(THUMBNAIL);
  ...
  test('LateAutocompleteResultsGatedByQueryId', async () => {
    ...
    const queryArgs = mockSearchboxPageHandler.getArgs(QUERY_AUTOCOMPLETE_FN);
    ...
    innerComposebox.addEventListener('result-changed', e => {
      resultChangedResults.push((e as CustomEvent<AutocompleteResult>).detail);
    });
    ...
    assertEquals(
        suggestionsBefore.queryId, wrapperState.zeroStateSuggestions_.queryId);
    assertEquals(
        suggestionsBefore.input, wrapperState.zeroStateSuggestions_.input);
    assertEquals(
    ...
    assertEquals(activeQueryId, wrapperState.zeroStateSuggestions_.queryId);
  ...
  test('LateResultRejectedWhileSubmitDeferredOnPendingUpload', async () => {
    const {innerComposebox, wrapper} = parts;
    ...
    searchboxCallbackRouterRemote.autocompleteResultChanged(
        createAutocompleteResultForTesting({
          ...
          input: '',
          matches: [createAutocompleteMatch({allowedToBeDefaultMatch: false})],
        }));
    await flushAndSettle();
    ...
    assertEquals(null, innerComposebox.getDropdownElement().result);
    assertEquals(
        0, resultChangedCount, 'The stale result must not fire result-changed');
    assertEquals(
        suggestionsBefore.queryId, wrapperState.zeroStateSuggestions_.queryId);
    assertEquals(
        suggestionsBefore.input, wrapperState.zeroStateSuggestions_.input);
  ...
  test('Submit button disabled if no input supports unimodal', async () => {
    ...
    assertEquals(
        0, innerComposebox.pendingUploads.size, 'pendingUploads should be 0');
    assertTrue(innerComposebox.submitEnabled, 'submitEnabled should be true');
    assertFalse(
        innerComposebox.canSubmitFilesAndInput,
  ...
  test('Injected input can be added, then deleted from AIM', async () => {
    ...
    const carousel =
        innerComposebox.shadowRoot.querySelector<ComposeboxFileCarouselElement>(
            '#carousel');
