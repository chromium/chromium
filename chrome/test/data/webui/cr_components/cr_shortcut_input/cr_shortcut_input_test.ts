...
suite('CrShortcutInputTest', function() {
  ...
  test('allowCtrlAltShortcuts_CtrlAlt', async function() {
    ...
    await microtasksFinished();
    assertEquals(isMac ? 'Ctrl + Option + A' : 'Ctrl + Alt + A', field.value);
  ...
  test('allowCtrlAltShortcuts_CommandAlt', async function() {
    ...
    await microtasksFinished();
    assertEquals('Command + Option + A', field.value);
    assertEquals('Command+Alt+A', input.shortcut);
  });

  test('OptionDisplayOnMac', async function() {
    input.shortcut = 'Alt+Space';
    await microtasksFinished();
    const field = input.$.input;
    assertEquals('Option + Space', field.value);
  });
