...
import {CrLinkRowElement, CrPolicyIndicatorElement, CrToggleElement, DevicePageBrowserProxyImpl, NoteAppInfo, NoteAppLockScreenSupport, Route, Router, routes, setDisplayApiForTesting, settingMojom, SettingsDevicePageElement, SettingsToggleButtonElement} from 'chrome://os-settings/os_settings.js';
...
suite('<settings-stylus> for device page', () => {
  ...
  suite('stylus', () => {
    ...
    setup(async () => {
      await init();
      const page = showAndGetDeviceSubpage('stylus', routes.STYLUS) as
          SettingsStylusElement;
      stylusPage = page;
      const selectElement =
          page.shadowRoot!.querySelector<HTMLSelectElement>('#selectApp');
      assertTrue(!!selectElement);
      appSelector = selectElement;
      const div1 = page.shadowRoot!.querySelector<HTMLElement>('#no-apps');
      assertTrue(!!div1);
      noAppsDiv = div1;
      const div2 = page.shadowRoot!.querySelector<HTMLElement>('#waiting');
      assertTrue(!!div2);
      waitingDiv = div2;
    ...
    function keepLastNoteOnLockScreenToggle(): SettingsToggleButtonElement|
        null {
      return stylusPage.shadowRoot!.querySelector(
    ...
    test('stylus tools prefs', () => {
      ...
      // toggle is disabled.
      const toolsToggle =
          stylusPage.shadowRoot!.querySelector<HTMLButtonElement>(
