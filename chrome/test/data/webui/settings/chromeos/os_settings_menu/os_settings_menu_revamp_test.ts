...
suite('<os-settings-menu>', () => {
  ...
  suite('Internet menu item', () => {
    ...
    async function setHotspotAvailable(): Promise<void> {
      const hotspotInfo = {
    ...
    test(
        'Hotspot shows when no network connected but hotspot is available',
        async () => {
          loadTimeData.overrideValues({isHotspotEnabled: true});

          await createMenu();

          await setHotspotAvailable();

          const internetMenuItem = getInternetMenuItem();
          assertEquals('Hotspot available', internetMenuItem.sublabel);
        });

    test(
        'Default internet description shows when no network connected and hotspot is unavailable',
        async () => {
          loadTimeData.overrideValues({isHotspotEnabled: false});
    ...
    test(
        ...
        async () => {
          ...
          // Remove the VPN network, since the hotspot is not available for now,
          // the default description "Wi-Fi, mobile data" should be shown.
          networkConfigRemote.removeNetworkForTest(vpnNetwork);
          await flushTasks();
          assertEquals('Wi-Fi, mobile data', internetMenuItem.sublabel);

          // Set hotspot available, 'Hotspot available' should be shown now.
          await setHotspotAvailable();
          assertEquals('Hotspot available', internetMenuItem.sublabel);
        });
  });
