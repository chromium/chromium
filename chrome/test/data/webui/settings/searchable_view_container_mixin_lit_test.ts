...
import {loadTimeData, Router, routes, SearchableViewContainerMixinLit, SettingsViewMixinLit} from 'chrome://settings/settings.js';
...
suite('SearchableViewContainerMixinLit', function() {
    ...
    override render() {
      return html`
        <div id="container" scrollable>
          <cr-view-manager ?show-all="${this.shouldShowAll}">
            <test-parent-view-lit slot="view" id="parentView0">
              <span>ParentView0</span>
              <button data-child-trigger="childView0"></button>
            </test-parent-view-lit>

            <test-parent-view-lit slot="view" id="parentView1">
  ...
  test('shouldShowAll', async function() {
    function assertShowAll(showAll: boolean) {
      assertEquals(
          showAll,
          testElement.shadowRoot.querySelector('cr-view-manager')!.hasAttribute(
              'show-all'));
    }
  ...
  test('searchContents', async function() {
    const parentViews = testElement.shadowRoot.querySelectorAll<HTMLElement>(
        'test-parent-view-lit[slot=view]');
