...
suite('General', () => {
  ...
  suite('KeyboardNavigation', function() {
    setup(() => {
      loadTimeData.overrideValues({
    ...
    test('expands and collapses folders with arrow keys', async () => {
      // Re-initialize with tree view enabled.
      powerBookmarksList = await initializeUi(bookmarksApi);

      const folderRow = getPowerBookmarksRowElement(powerBookmarksList, '5');
      assertTrue(!!folderRow);
      assertFalse(
          folderRow.toggleExpand, 'Folder should be initially collapsed');
    ...
    test(
        'moves focus to first child on right arrow if already open',
        async () => {
          // Re-initialize with tree view enabled.
          powerBookmarksList = await initializeUi(bookmarksApi);
          ...
          const childRow = getPowerBookmarksRowElement(powerBookmarksList, '6');
