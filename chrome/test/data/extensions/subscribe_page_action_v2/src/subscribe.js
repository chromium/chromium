...
const unknownName =
    chrome.i18n.getMessage('rss_subscription_unknown_feed_name');
...
function navigate() {
  const select = document.getElementById('readerDropdown');
  const url = feedReaderList[select.selectedIndex].url.replace(
  ...
  if (storageEnabled) {
    ...
    const alwaysUse = document.getElementById('alwaysUse');
...
function main() {
  ...
  // Set the token.
  const tokenArray = new Uint32Array(4);
  crypto.getRandomValues(tokenArray);
  token = [].join.call(tokenArray);

  styleSheet = '<link rel=\'stylesheet\' type=\'text/css\' href=\'' +
      chrome.runtime.getURL('style.css') + '\'>';
  frameScript = window.domAutomationController !== undefined ?
      '<script src=\'' + chrome.runtime.getURL('test_support.js') + '\'></' +
          'script>' :
      '';
...
function handleFeedParsingFailed(error) {
  ...
  let html =
      '<body><span id="error" class="item_desc">' + error + '</span></body>';
  ...
  const error_frame = createFrame('error', html);
...
function createFrame(frame_id, html) {
  ...
  frame.src = 'data:text/html;charset=utf-8,<html>' + csp +
      '<!--Token:' + extension_id + token + '-->' + html + '</html>';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const button = document.getElementById('rss_subscription_subscribe_button');
