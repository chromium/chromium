...
loadScript.then(async function() {
  chrome.test.runTests([
    ...
    function tabsOnCreated() {
      ...
      chrome.tabs.create(
          {windowId: firstWindowId, url: pageUrl('f'), selected: true},
          pass(function(tab) {}));
    },
    ...
    function tabsOnAttached() {
      ...
      moveAndListen(
          testTabId, {windowId: secondWindowId, index: 0}, pass(function() {
            moveAndListen(testTabId, {windowId: firstWindowId, index: 1});
          }));
    },

    function tabsOnDetached() {
      function moveAndListen(
          tabId, oldWindowId, oldIndex, properties, callback) {
        chrome.test.listenOnce(
            chrome.tabs.onDetached, function(detachedTabId, info) {
              // Ensure notification is correct.
      ...
      // Move tab to second window, then back to first.
      moveAndListen(
          testTabId, firstWindowId, 1, {windowId: secondWindowId, index: 0},
          pass(function() {
            moveAndListen(
                testTabId, secondWindowId, 0,
    ...
    function tabsOnZoomChange() {
      chrome.tabs.setZoom(testTabId, 1, function() {
        chrome.test.listenOnce(
            chrome.tabs.onZoomChange, function(zoomChangeInfo) {
    ...
    function windowsOnCreated() {
      chrome.test.listenOnce(chrome.windows.onCreated, function(window) {
        ...
        chrome.tabs.query(
            {windowId: window.id}, pass(function(tabs) {
