...
const tests = [
  ...
  function getAllInWindow() {
    chrome.tabs.getAllInWindow(
        secondWindowId, pass(function(tabs) {
          ...
          assertTrue(
              newTabUrls.includes(tabs[1].url),
              `Unexpected URL: ${tabs[1].url}`);
          assertEq(pageUrl('a'), tabs[2].url);
        }));

    chrome.tabs.getAllInWindow(thirdWindowId, pass(function(tabs) {
  ...
  function updateSelect() {
    chrome.tabs.getAllInWindow(
        secondWindowId, pass(function(tabs) {
          ...
          // Select tab[1].
          chrome.tabs.update(
              tabs[1].id, {active: true}, pass(function(tab1) {
                ...
                chrome.tabs.getAllInWindow(
                    secondWindowId, pass(function(tabs) {
                      ...
                      // Select tab[2].
                      chrome.tabs.update(
                          tabs[2].id, {active: true}, pass(function(tab2) {
                            ...
                            chrome.tabs.getAllInWindow(
                                secondWindowId, pass(function(tabs) {
                                  assertEq(
                                      false,
                                      tabs[1].active || tabs[1].selected);
                                  assertEq(
                                      true, tabs[2].active && tabs[2].selected);
                                }));
                          }));
                    }));
              }));
  ...
  function update() {
    chrome.tabs.get(testTabId, pass(function(tab) {
                      ...
                      // Update url.
                      chrome.tabs.update(
                          testTabId, {url: pageUrl('c')}, pass(function(tab) {
  ...
  function testOpenEmptyPopup() {
    chrome.tabs.query({}, pass(function(tabs) {
                        ...
                        chrome.windows.create(
                            {type: 'popup'}, pass(function(window) {
  ...
  function testCreatePopupAndMoveTab() {
    ...
    chrome.tabs.create(
        {url: 'about:blank'}, pass(function(tab) {
          chrome.windows.create(
              {type: 'popup', tabId: tab.id}, pass(function(window) {
    ...
    chrome.tabs.create(
        {url: 'about:blank'}, pass(function(tab) {
          chrome.windows.create(
              {type: 'popup', url: 'about:blank', tabId: tab.id},
...
(async function() {
  ...
  if (isAndroid) {
    testsToRun = tests.filter((t) => {
      return !skipForAndroid.includes(t.name);
    });
  }
