  ...
  expect(
      [
        ...
        },
        {
          ...
          retval: {
            requestHeaders:
                [{name: 'User-Agent', binaryValue: toCharCodes('FoobarUA')}],
          },
        },
        {
          label: 'onSendHeaders',
...
function modifyResponseHeaders() {
  ...
  navigateAndWait(getURL('simpleLoad/a.html'), function() {
    fetch(getURLSetHeader())
        .then((response) => {
          chrome.test.assertTrue(
              response.headers.get('Foo') == null, 'Header was not removed.');
        })
        .catch((e) => {
          chrome.test.fail();
        });
...
function testLoadCORSImage(crossOriginAttributeValue) {
  ...
  const requestedUrl = getServerURL(
      'cors/intercepted_by_extension.gif?' + Math.random(), 'original.tld');
  const frameUrl = getServerURL(
  ...
  const redirectTarget = getServerURL(
      ...
      [
        // events
        {
          ...
          details: {
            type: 'image',
            url: requestedUrl,
            // Frame URL unavailable because requests are filtered by
            // type=image.
            frameUrl: 'unknown frame URL',
            initiator: initiator,
            documentId: 1,
