...
const allTests = [
  function testEventListenerTarget() {
    ...
    cancelButton.addEventListener(
        EventType.FOCUS, function onFocusTarget(event) {
          setTimeout(function() {
  ...
  function testEventListenerBubble() {
    const okButton = rootNode.firstChild.children[0];
    assertEq('Ok', okButton.name);
    let okButtonGotEvent = false;
    okButton.addEventListener(EventType.FOCUS, function onFocusBubble(event) {
      okButtonGotEvent = true;
      okButton.removeEventListener(EventType.FOCUS, onFocusBubble);
    });
    rootNode.addEventListener(
        EventType.FOCUS, function onFocusBubbleRoot(event) {
  ...
  function testStopPropagation() {
    ...
    cancelButton.addEventListener(
        EventType.FOCUS, function onFocusStopProp(event) {
          cancelButton.removeEventListener(EventType.FOCUS, onFocusStopProp);
          event.stopPropagation();
          setTimeout(
              (function() {
                rootNode.removeEventListener(
                    EventType.FOCUS, onFocusStopPropRoot);
                chrome.test.succeed();
              }).bind(this),
              0);
        });
  ...
  function testEventListenerCapture() {
    ...
    rootNode.addEventListener(
        EventType.FOCUS, function onFocusCaptureRoot(event) {
          assertEq('focus', event.type);
          assertEq(okButton, event.target);
