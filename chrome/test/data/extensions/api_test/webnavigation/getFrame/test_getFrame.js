...
ready.then(async function() {
  ...
  chrome.test.runTests([
    ...
    function testGetInvalidFrame() {
      chrome.webNavigation.getFrame(
          {tabId: tab.id, frameId: 1, processId: processId}, function(details) {
    ...
    function testGetFrameNoValues() {
      chrome.webNavigation.getFrame({}, function(details) {
        chrome.test.assertEq(null, details);
    ...
    function testGetFrameNoFrameId() {
      chrome.webNavigation.getFrame(
          {tabId: tab.id, processId: processId}, function(details) {
            chrome.test.assertEq(null, details);
            chrome.test.assertLastError(kNotSpecifiedErrorMessage);
            chrome.test.succeed();
          });
    ...
    async function testGetFrameDocumentId() {
      chrome.webNavigation.getFrame(
          {tabId: tab.id, documentId: documentId}, function(details) {
            chrome.test.assertEq(
                {
                  errorOccurred: false,
                  url: url,
    ...
    function testGetFrameInvalidDocumentId() {
      chrome.webNavigation.getFrame(
          {tabId: tab.id, frameId: 0, processId: processId, documentId: '42AB'},
          function(details) {
      ...
      const done = chrome.test.listenForever(
          chrome.webNavigation.onCommitted, function(details) {
            ...
            chrome.webNavigation.getAllFrames(
                {tabId: tab.id}, function(frameDetails) {
                  chrome.test.assertEq(
                      [{
                        errorOccurred: false,
      ...
      const done = chrome.test.listenForever(
          chrome.webNavigation.onCommitted, function(details) {
            ...
            if (details.documentLifecycle === 'prerender') {
              // Inject a script that activates the pre-rendered page.
              chrome.tabs.executeScript(
                  tab.id, {code: `window.location.href = './a.html';`});
              return;
            }
