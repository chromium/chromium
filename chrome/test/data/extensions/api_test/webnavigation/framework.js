  ...
  expectedEventData.forEach(function(exp) {
    if (exp.event == name) {
      ...
      if ('transitionQualifiers' in exp.details) {
        const idx = exp.details['transitionQualifiers'].indexOf(
            'maybe_client_redirect');
        if (idx >= 0) {
          expDetails = deepCopy(exp.details);
          expDetails['transitionQualifiers'].splice(idx, 1);
          altDetails = deepCopy(expDetails);
          altDetails['transitionQualifiers'].push('client_redirect');
        } else {
          expDetails = exp.details;
          altDetails = exp.details;
        }
      } else {
        expDetails = exp.details;
        altDetails = exp.details;
      }
      if (deepEq(expDetails, details) || deepEq(altDetails, details)) {
        if (!found) {
          found = true;
          label = exp.label;
          exp.event = undefined;
