...
const selectedTest = location.hash.slice(1);
  ...
  chrome.vpnProvider.createConfig('some config name', function() {
    ...
    chrome.vpnProvider.createConfig('some config name', function() {
      chrome.test.assertEq(
          'Name not unique.', chrome.runtime.lastError.message);
      chrome.vpnProvider.createConfig('', function() {
        chrome.test.assertEq(
            'Empty name not supported.', chrome.runtime.lastError.message);
        chrome.test.succeed();
      });
    });
  });
...
function destroyConfigFailures() {
  chrome.vpnProvider.destroyConfig('nonexistent', function() {
    chrome.test.assertEq(
        'Unauthorized access.', chrome.runtime.lastError.message);
    chrome.test.succeed();
  });
}
...
function setParameterFailures() {
  ...
  function recurse(index) {
    ...
    chrome.vpnProvider.setParameters(params, function() {
      chrome.test.assertEq(
          error, chrome.runtime.lastError.message, 'Test ' + index + ' failed');
      recurse(index + 1);
    });
  }
...
const testRoutines = {
  ...
  createConfigConnectForBind: function() {
    chrome.vpnProvider.onPlatformMessage.addListener(function(
        config_name, message) {
  ...
  createConfigConnectAndDisconnect: function() {
    ...
    chrome.vpnProvider.onPacketReceived.addListener(function(data) {
      ...
      const packet = new Uint8Array([100, 101, 97, 100, 98, 101, 101, 102]);
      chrome.test.assertEq(packet, new Uint8Array(data));
      chrome.test.succeed();
    });
    const onNotifyComplete = function() {
      chrome.test.assertEq(chrome.runtime.lastError, undefined);
      chrome.vpnProvider.sendPacket(new ArrayBuffer(0), function() {
        chrome.test.assertEq(
            chrome.runtime.lastError.message, 'Can\'t send an empty packet.');
        // The variable packet contains the string 'feebdaed'.
        const packet = new Uint8Array([102, 101, 101, 98, 100, 97, 101, 100]);
    ...
    const onSetParameterComplete = function() {
      chrome.test.assertEq(chrome.runtime.lastError, undefined);
      chrome.vpnProvider.notifyConnectionStateChanged(
          'connected', onNotifyComplete);
    };
    chrome.vpnProvider.onPlatformMessage.addListener(function(
        config_name, message) {
      chrome.test.assertEq(config_name, 'testconfig');
  ...
  platformMessage: function() {
    let i = 0;
    chrome.vpnProvider.onPlatformMessage.addListener((config_name, message) => {
      chrome.test.assertEq(config_name, 'testconfig');
      if (message === 'connected') {
        chrome.test.assertEq(i, 0);
        chrome.test.succeed();
