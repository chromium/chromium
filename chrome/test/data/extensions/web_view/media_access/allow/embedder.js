...
embedder.assertCorrectMediaEvent_ = function(e) {
  ...
  if (e.url.indexOf(embedder.baseGuestURL)) {
    embedder.failTest(
        'Wrong url: ' + e.url + ', expected url to start with ' +
        embedder.baseGuestURL);
    return false;
  }
  return true;
...
embedder.tests.testAllowAndThenDeny = function() {
  ...
  let calledAllow = false;
  let callCount = 0;
  let exceptionCount = 0;
  const checkAndCall = function(e) {
    if (!embedder.assertCorrectMediaEvent_(e)) {
      return;
    }
  ...
  const onPermissionRequest1 = function(e) {
    checkAndCall(e);
  };
  const onPermissionRequest2 = function(e) {
    checkAndCall(e);
  };
  ...
  const checkAndCall = function(e) {
    ...
    if (callbackCount === 2) {
      if (exceptionCount === 1) {
        ...
        embedder.failTest(
            'Expected exceptionCount 1, but got ' + exceptionCount);
      }
    }
  };
