...
embedder.registerAndWaitForPostMessage_ = function(webview, testName) {
  const onPostMessageReceived = function(e) {
    const data = JSON.parse(e.data);
    if (data[1] === '' + testName) {
      embedder.assertTrue(
          'access-denied' === data[0],
...
embedder.assertCorrectMediaEvent_ = function(e) {
  ...
  embedder.assertTrue(
      e.url.startsWith(embedder.baseGuestURL), 'wrong e.url, ' + e.url);
};
...
embedder.tests.testDenyThenAllowThrows = function testDenyThenAllowThrows() {
  ...
  const callDenyAndCheck = function(e) {
    try {
      if (!denyCalled) {
        e.request.deny();
        denyCalled = true;
...
function runTest(testName) {
  const testFunction = embedder.testNamesToTestMap[testName];
  if (!testFunction) {
