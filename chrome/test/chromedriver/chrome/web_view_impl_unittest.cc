 ...
 public:
  ...
  Status SendCommandAndGetResult(const std::string& method,
    ...
    if (method == "Page.getFrameTree") {
      ...
      for (base::Value& frame : one_shot_child_frames_) {
        children.Append(frame.Clone());
      }
      one_shot_child_frames_.clear();
  ...
  void AddSingleShotChildFrame(base::Value::Dict frame) {
    one_shot_child_frames_.Append(std::move(frame));
  }

 private:
  Status status_;
  base::Value::Dict result_;
  base::Value::List one_shot_child_frames_;
};
...
TEST(CallUserSyncScript, ElementIdAsResultRootFrame) {
  BrowserInfo browser_info;
  base::Value::Dict dict = GenerateResponse(4321);
  std::unique_ptr<base::Value> result;
  std::unique_ptr<FakeDevToolsClient> client_uptr =
      std::make_unique<FakeDevToolsClient>("root");
  client_uptr->set_result(dict);
...
TEST(CallUserSyncScript, ElementIdAsResultChildFrameErrors) {
  ...
  view.GetFrameTracker()->SetContextIdForFrame("bad", "irrelevant");
  {
    ...
    client_ptr->AddSingleShotChildFrame(std::move(bad_child));
  ...
  result.reset();
  {
    ...
    client_ptr->AddSingleShotChildFrame(std::move(another_good_child));
    EXPECT_TRUE(StatusOk(
...
TEST_P(CallUserSyncScriptArgs, RepeatedSeparator) {
  ...
  client_ptr->AddSingleShotChildFrame(std::move(extra_child));
