...
// PackageInstallWaiter waits for the package matching a given name to be
// installed.
class PackageInstallWaiter : public ArcAppListPrefs::Observer {
 public:
  PackageInstallWaiter(ArcAppListPrefs* arc_app_list_prefs,
                       const std::string_view package)
      : prefs_(arc_app_list_prefs), package_(package) {
    scoped_observation_.Observe(arc_app_list_prefs);
  }

  void Wait() {
    if (prefs_->IsPackageInstalled(package_)) {
      return;
    }
  ...
  void OnPackageInstalled(
    ...
    // Stop wait on install/update notification.
    if (package_info.package_name == package_) {
      wait_loop_.Quit();
    }
  }
  void OnPackageListInitialRefreshed() override {
    // Stop wait if the package is included in the initial package list.
    if (prefs_->IsPackageInstalled(package_)) {
      wait_loop_.Quit();
    }
  }
...
// ActiveWindowWaiter waits for a window to be activated in a given root window.
...
void ChromeOSIntegrationArcMixin::WaitForPackageInstall(
    const std::string& package) {
  PackageInstallWaiter(ArcAppListPrefs::Get(GetActiveUserBrowserContext()),
                       package)
      .Wait();
}
...
aura::Window* ChromeOSIntegrationArcMixin::LaunchAndWaitForWindow(
  ...
  aura::Window* const window =
      ActiveWindowWaiter(ash::Shell::GetPrimaryRootWindow()).Wait();
...
void ChromeOSIntegrationArcMixin::SetUpCommandLine(
  ...
  env->SetVar("XDG_RUNTIME_DIR", "/run/chrome");
