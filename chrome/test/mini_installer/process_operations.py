...
def VerifyProcessExpectation(expectation_name, expectation, variable_expander):
  ...
  Args:
    expectation_name: Path to the process being verified (may contain variables
        to be expanded).
...
def CleanProcess(expectation_name, expectation, variable_expander):
  ...
  for proc in psutil.process_iter():
    try:
      ...
      # Ignore processes for which the path cannot be determined. AccessDenied
      # is expected for pid 0, pid 4, and any others that the current process
      # does not have access to query.
      continue
    pid = proc.pid
    try:
      proc.kill()
