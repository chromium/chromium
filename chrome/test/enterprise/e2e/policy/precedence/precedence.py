...
class PrecedenceTest(ChromeEnterpriseTestCase):
  ...
  def test_CloudPolicyOverridesPlatformPolicyEnabled(self):
    ...
    self.SetPolicy(
      self.win_config['dc'], 'CloudPolicyOverridesPlatformPolicy', 1, 'DWORD'
    )
    self.RunCommand(self.win_config['client'], 'gpupdate /force')
    d = os.path.dirname(os.path.abspath(__file__))
    logging.info('CloudPolicyOverridesPlatformPolicy DISABLED')
    output = self.RunWebDriverTest(
      self.win_config['client'], os.path.join(d, 'precedence_webdriver_test.py')
    )
  ...
  def test_CloudPolicyOverridesPlatformPolicyDisabled(self):
    ...
    self.SetPolicy(
      self.win_config['dc'], 'CloudPolicyOverridesPlatformPolicy', 0, 'DWORD'
    )
    self.RunCommand(self.win_config['client'], 'gpupdate /force')
    d = os.path.dirname(os.path.abspath(__file__))
    logging.info('CloudPolicyOverridesPlatformPolicy DISABLED')
