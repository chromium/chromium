...
#include "chrome/test/fuzzing/mojo_js_fuzzing/digital_identity_request_fuzzer_grammar.h"
#include "chrome/test/fuzzing/mojo_js_fuzzing/digital_identity_request_fuzzer_grammar.pb.h"
...
// rule.
class DigitalIdentityRequestInProcessFuzzer
    : public InProcessBinaryProtoFuzzer<
          domatolpm::generated::digital_identity_request_fuzzer_grammar::
              fuzzcase> {
 public:
  using FuzzCase =
      domatolpm::generated::digital_identity_request_fuzzer_grammar::fuzzcase;
  DigitalIdentityRequestInProcessFuzzer();
...
REGISTER_BINARY_PROTO_IN_PROCESS_FUZZER(DigitalIdentityRequestInProcessFuzzer)

void DigitalIdentityRequestInProcessFuzzer::SetUpOnMainThread() {
  ...
  CHECK(ui_test_utils::NavigateToURL(
      browser(), embedded_https_test_server().GetURL(
                     "/DigitalIdentityRequest_in_process_fuzzer.html")));
...
DigitalIdentityRequestInProcessFuzzer::DigitalIdentityRequestInProcessFuzzer()
...
int DigitalIdentityRequestInProcessFuzzer::Fuzz(const FuzzCase& fuzz_case) {
  static Environment env;
  domatolpm::Context ctx;
  CHECK(domatolpm::digital_identity_request_fuzzer_grammar::handle_fuzzer(
      &ctx, fuzz_case));
  ...
  std::ignore = content::ExecJs(rfh, js_str);
  return 0;
}
