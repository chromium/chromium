...
from test_analysis import (
    ...
    filter_coverage_tests_for_platform,
...
class GraphAnalysisUnittest(unittest.TestCase):
    def test_test_generation(self):
        ...
        coverage_filename = os.path.join(
            TEST_DATA_DIR, "test_unprocessed_coverage.md"
        )
        ...
        with (
            ...
            for platform in TestPlatform:
                ...
                build_action_node_graph(
                    generated_tests_root_node, platform_tests
                )
                trim_graph_to_platform_actions(
                    generated_tests_root_node, platform
                )
                generated_tests_by_platform[platform] = (
                    generate_framework_tests(
                        generated_tests_root_node, platform
                    )
                )
            ...
            for (
                platform_set,
                tests,
            ) in required_coverage_by_platform_set.items():
                ...
                with open(
                    expected_filename, "r", encoding="utf-8"
                ) as expected_tests_file:
