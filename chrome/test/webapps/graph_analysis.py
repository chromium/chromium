...
def build_action_node_graph(
    root_node: ActionNode, tests: List[CoverageTest]
) -> None:
    """
...
def generage_graphviz_dot_file(
    root_node: ActionNode, platform: Optional[TestPlatform]
) -> str:
    def get_all_nodes_and_assign_graph_ids(
        root: ActionNode,
    ) -> List[ActionNode]:
        current_graph_id = 0
    ...
    for node in nodes:
        color_str = (
            "seagreen"
            if platform is None
            or platform in node.action.full_coverage_platforms
            else "sandybrown"
        )
...
# given platform.
def trim_graph_to_platform_actions(
    root_node: ActionNode, platform: TestPlatform
) -> None:
    """
    Removes any nodes and actions from the graph that are not supported by the
    given platform.
...
def generate_coverage_file_and_percents(
    ...
    partial_coverage = (
        actions_fully_covered + actions_partially_covered
    ) / total_actions
    logging.info(f"Coverage for {platform}:")
    logging.info(
        f"Full coverage: {full_coverage:.0%}, "
