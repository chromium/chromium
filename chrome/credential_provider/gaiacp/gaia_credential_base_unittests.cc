...
TEST_F(GcpGaiaCredentialBaseAdScenariosTest,
  ...
  EXPECT_TRUE(SUCCEEDED(SetUserProperty(
      existing_user_sid.Copy(), base::UTF8ToWide(kKeyDomain), domain_name)));

  EXPECT_TRUE(SUCCEEDED(SetUserProperty(
      existing_user_sid.Copy(), base::UTF8ToWide(kKeyUsername), user_name)));
