...
void AddSoftwareSecureWidevine(std::vector<content::CdmInfo>* cdms) {
  ...
  if (bundled_widevine && !hinted_widevine) {
    ...
    bool choose_bundled;
    if (base::FeatureList::IsEnabled(media::kSupportCdmComponentDowngrade)) {
      // Generally we want to pick the Component Update selected version, except
      // in the case the bundled CDM is newer than the selected CDM and is
      // different than the previously bundled CDM.
      choose_bundled =
          bundled_version > hinted_version &&
          bundled_version != GetBundledVersionDuringLastComponentUpdate();
    } else {
      // As this does not support version downgrades, choose the highest version
      // CDM, preferring the bundled CDM over the selected one if the versions
      // are the same.
