...
class ASH_EXPORT VcEffectsDelegate {
 public:
  ...
  void AddEffect(const VcHostedEffect* effect);
  ...
  // of the hosted effect, delegate that hosting only a single effect can safely
  // ignore it. `value` is the integer value the user just set, which can be
  // ignored if the effect is of type `kToggle`.
  virtual void OnEffectControlActivated(int effect_id, int value) = 0;

  const std::vector<const VcHostedEffect*>& effects() { return effects_; }

 private:
  std::vector<const VcHostedEffect*> effects_;
};
