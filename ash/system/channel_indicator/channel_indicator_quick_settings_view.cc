...
constexpr int kFeedbackButtonMarginVertical = 6;
constexpr int kFeedbackButtonMarginHorizontal = 16;

constexpr int kFeedbackButtonLargeCornerRadius = 16;
constexpr int kFeedbackButtonSmallCornerRadius = 4;
constexpr SkScalar kFeedbackButtonCorners[] = {
    kFeedbackButtonSmallCornerRadius, kFeedbackButtonSmallCornerRadius,
    kFeedbackButtonLargeCornerRadius, kFeedbackButtonLargeCornerRadius,
    kFeedbackButtonLargeCornerRadius, kFeedbackButtonLargeCornerRadius,
    kFeedbackButtonSmallCornerRadius, kFeedbackButtonSmallCornerRadius};
...
// FeedbackButton provides a styled button, for devices on a
// non-stable release track, that allows the user to submit feedback.
class ASH_EXPORT FeedbackButton : public IconButton {
 public:
  explicit FeedbackButton(version_info::Channel channel)
      : IconButton(base::BindRepeating([] {
        ...
        channel_(channel) {
    SetBorder(views::CreateEmptyBorder(gfx::Insets::VH(
        kFeedbackButtonMarginVertical, kFeedbackButtonMarginHorizontal)));
  ...
  void PaintButtonContents(gfx::Canvas* canvas) override {
    ...
    canvas->DrawPath(
        SkPath().addRoundRect(gfx::RectToSkRect(GetLocalBounds()),
                              kFeedbackButtonCorners, SkPathDirection::kCW),
        flags);
...
ChannelIndicatorQuickSettingsView::ChannelIndicatorQuickSettingsView(
  ...
  feedback_button_ = AddChildView(std::make_unique<FeedbackButton>(channel));
}
...
bool ChannelIndicatorQuickSettingsView::IsFeedbackButtonVisibleForTesting() {
