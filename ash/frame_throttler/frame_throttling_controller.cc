...
void FrameThrottlingController::OnCompositingShuttingDown(
    aura::WindowTreeHost* window_tree_host) {
  host_to_ids_map_.erase(window_tree_host);
  UpdateThrottling();
}
...
void FrameThrottlingController::AddObservation(aura::WindowTreeHost* host) {
  DCHECK(!window_tree_host_observation_.IsObservingSource(host));
  window_tree_host_observation_.AddObservation(host);
}

void FrameThrottlingController::RemoveObservation(aura::WindowTreeHost* host) {
  DCHECK(window_tree_host_observation_.IsObservingSource(host));
  window_tree_host_observation_.RemoveObservation(host);
}
