...
class PieMenuButton : public views::Button,
 ...
 public:
  ...
  PieMenuButton(int button_id,
      ...
      : button_id_(button_id), icon_(icon) {
    SetTooltipText(button_label_text);
  ...
  void StateChanged(ButtonState old_state) override { SchedulePaint(); }
  ...
  void PaintButtonContents(gfx::Canvas* canvas) override {
    ...
    const std::u16string text = GetTooltipText();
    const auto& font_list = views::Label::GetDefaultFontList();
    const gfx::Size text_size = GetTextSize(text, font_list);
    ...
    const auto text_color = color_provider->GetColor(kColorAshTextColorPrimary);
    canvas->DrawStringRectWithFlags(text, font_list, text_color, text_bounds,
 ...
 private:
  void RefreshIconImage() {
    ...
    icon_image_ = gfx::CreateVectorIcon(
        *icon_, color_provider->GetColor(kColorAshIconColorPrimary));
  }
...
void PieSubMenuContainerView::AddMenuButton(
    int button_id,
    const std::u16string& button_label_text,
    const gfx::VectorIcon* icon) {
  buttons_.push_back(AddChildView(
  ...
  owner_menu_view_->OnPieMenuButtonAdded(buttons_.back());
}
