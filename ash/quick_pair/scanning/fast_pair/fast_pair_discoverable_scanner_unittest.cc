...
class FastPairDiscoverableScannerTest : public testing::Test {
 public:
  void SetUp() override {
    ...
    data_parser_remote_.Bind(std::move(fast_pair_data_parser_), nullptr);

    EXPECT_CALL(*mock_process_manager(), GetProcessReference)
        .WillRepeatedly([&](QuickPairProcessManager::ProcessStoppedCallback) {
          auto process_reference = std::make_unique<
              MockQuickPairProcessManager::MockProcessReference>();

          EXPECT_CALL(*(process_reference.get()), GetFastPairDataParser)
              .WillRepeatedly([&]() { return data_parser_remote_; });
