...
class FastPairDataEncryptorImpl : public FastPairDataEncryptor {
 public:
  class Factory {
   public:
    static void CreateAsync(
        ...
        base::OnceCallback<void(std::unique_ptr<FastPairDataEncryptor>)> on_get_instance_callback);
   ...
   protected:
    ...
    virtual void CreateInstance(
        scoped_refptr<Device> device,
        base::OnceCallback<void(std::unique_ptr<FastPairDataEncryptor>)> on_get_instance_callback) = 0;
  ...
  const absl::optional<std::array<uint8_t, kPublicKeyByteSize>>& GetPublicKey() override;

  void ParseDecryptedResponse(
      const std::vector<uint8_t>& encrypted_response_bytes,
      base::OnceCallback<void(const absl::optional<DecryptedResponse>&)> callback) override;

  void ParseDecryptedPasskey(
      const std::vector<uint8_t>& encrypted_passkey_bytes,
      base::OnceCallback<void(const absl::optional<DecryptedPasskey>&)> callback) override;

  ~FastPairDataEncryptorImpl() override;

 protected:
  explicit FastPairDataEncryptorImpl(const fast_pair_encryption::KeyPair& key_pair);
  explicit FastPairDataEncryptorImpl(const std::array<uint8_t, kPrivateKeyByteSize>& secret_key);
  FastPairDataEncryptorImpl(const FastPairDataEncryptorImpl&) = delete;
  FastPairDataEncryptorImpl& operator=(const FastPairDataEncryptorImpl&) = delete;

 private:
  void QuickPairProcessStoppedOnResponse(QuickPairProcessManager::ShutdownReason shutdown_reason);
  void QuickPairProcessStoppedOnPasskey(QuickPairProcessManager::ShutdownReason shutdown_reason);
  ...
  absl::optional<std::array<uint8_t, kPublicKeyByteSize>> public_key_ = absl::nullopt;
