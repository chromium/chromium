...
bool ValidateInputSize(const std::vector<uint8_t>& encrypted_bytes) {
  if (encrypted_bytes.size() != kBlockSizeBytes) {
    QP_LOG(WARNING) << __func__ << ": Encrypted bytes should have size = " << kBlockSizeBytes
...
// static
void FastPairDataEncryptorImpl::Factory::SetFactoryForTesting(Factory* g_test_factory) {
...
void FastPairDataEncryptorImpl::Factory::CreateAsync(
  ...
  if (g_test_factory_) {
    g_test_factory_->CreateInstance(std::move(device), std::move(on_get_instance_callback));
    return;
  }

  if (device->protocol == Protocol::kFastPairInitial
      || device->protocol == Protocol::kFastPairRetroactive) {
    CreateAsyncWithKeyExchange(std::move(device), std::move(on_get_instance_callback));
  } else if (device->protocol == Protocol::kFastPairSubsequent) {
    CreateAsyncWithAccountKey(std::move(device), std::move(on_get_instance_callback));
  } else {
...
void FastPairDataEncryptorImpl::Factory::CreateAsyncWithKeyExchange(
  ...
  FastPairRepository::Get()->GetDeviceMetadata(
      metadata_id,
      base::BindOnce(&FastPairDataEncryptorImpl::Factory::DeviceMetadataRetrieved,
                     std::move(device),
...
void FastPairDataEncryptorImpl::Factory::DeviceMetadataRetrieved(
  ...
  absl::optional<fast_pair_encryption::KeyPair> key_pair =
      fast_pair_encryption::GenerateKeysWithEcdhKeyAgreement(public_anti_spoofing_key);
...
const std::array<uint8_t, kBlockSizeBytes> FastPairDataEncryptorImpl::EncryptBytes(
...
const absl::optional<std::array<uint8_t, kPublicKeyByteSize>>& FastPairDataEncryptorImpl::GetPublicKey() {
  return public_key_;
}

void FastPairDataEncryptorImpl::ParseDecryptedResponse(
    const std::vector<uint8_t>& encrypted_response_bytes,
    base::OnceCallback<void(const absl::optional<DecryptedResponse>&)> callback) {
  ...
  quick_pair_process::ParseDecryptedResponse(
      ...
      std::move(callback),
      base::BindOnce(&FastPairDataEncryptorImpl::QuickPairProcessStoppedOnResponse,
                     weak_ptr_factory_.GetWeakPtr()));
}

void FastPairDataEncryptorImpl::ParseDecryptedPasskey(
    const std::vector<uint8_t>& encrypted_passkey_bytes,
    base::OnceCallback<void(const absl::optional<DecryptedPasskey>&)> callback) {
  ...
  quick_pair_process::ParseDecryptedPasskey(
      ...
      std::move(callback),
      base::BindOnce(&FastPairDataEncryptorImpl::QuickPairProcessStoppedOnPasskey,
                     weak_ptr_factory_.GetWeakPtr()));
}

void FastPairDataEncryptorImpl::QuickPairProcessStoppedOnResponse(
    QuickPairProcessManager::ShutdownReason shutdown_reason) {
  QP_LOG(WARNING) << "Quick Pair process stopped while decrypting response due to error: "
                  << shutdown_reason;
}

void FastPairDataEncryptorImpl::QuickPairProcessStoppedOnPasskey(
    QuickPairProcessManager::ShutdownReason shutdown_reason) {
  QP_LOG(WARNING) << "Quick Pair process stopped while decrypting passkey due to error: "
                  << shutdown_reason;
}
