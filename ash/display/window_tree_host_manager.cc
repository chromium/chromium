...
void WindowTreeHostManager::DeleteHost(AshWindowTreeHost* host_to_delete) {
  Shell::Get()->frame_throttling_controller()->RemoveObservation(
      host_to_delete->AsWindowTreeHost());
...
AshWindowTreeHost* WindowTreeHostManager::AddWindowTreeHostForDisplay(
  ...
  aura::WindowTreeHost* host = ash_host->AsWindowTreeHost();
  DCHECK(Shell::Get()->frame_throttling_controller());
  Shell::Get()->frame_throttling_controller()->AddObservation(host);
