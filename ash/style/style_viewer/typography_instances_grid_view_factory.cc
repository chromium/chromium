...
class TypographyGridView : public SystemUIComponentsGridView {
 public:
  ...
  void AddTypographySample(const TypographyEntry& entry) {
    views::Label* label = AddInstance(
        std::u16string(entry.name),
        std::make_unique<views::Label>(std::u16string(entry.sentence)));
...
CreateTypographyInstancesGridView() {
  static constexpr auto kEntries = std::to_array<TypographyEntry>(
