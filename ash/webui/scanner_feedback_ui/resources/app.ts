...
(PolymerElement) {
  ...
  // Used in template:
  declare private stringSource: StringSource;
  declare openUrl: (url: string) => void;
  declare extraInfoCallback: () => Promise<any>;
  ...
  declare private screenshotUrl?: string;
  ...
  constructor() {
    ...
    this.stringSource = {
      ...
      // We cannot use i18n template replacement here as it only works in JS
      // files in HTML string templates. Additionally, double quotes are HTML
      // escaped with template replacement, which is not suitable for JS use as
    ...
    this.extraInfoCallback = () =>
        FEEDBACK_INFO_PROMISE.then(feedbackInfo => feedbackInfo.actionDetails);
    this.revertToPreviousScreen = () => PAGE_HANDLER_REMOTE.closeDialog();
    this.submitFeedback = (userDescription: string) =>
