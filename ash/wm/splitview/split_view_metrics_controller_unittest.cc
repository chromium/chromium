...
TEST_F(SplitViewMetricsControllerTest, RecordSnappedWindowsDuration) {
  ...
  histogram_tester.ExpectBucketCount(kSnapTwoWindowsDurationHistogramName, 0,
                                     1);
  ...
  window_state1->OnWMEvent(&snap_right);
  histogram_tester.ExpectBucketCount(kSnapTwoWindowsDurationHistogramName, 3,
                                     1);
  ...
  histogram_tester.ExpectBucketCount(kSnapTwoWindowsDurationHistogramName,
                                     kSnapTwoWindowsDurationHistogramMaxCount,
                                     1);
  ...
  // `window3` to the same desk. Test we don't record anything.
  window_state1->OnWMEvent(&snap_left);
  AdvanceClock(base::Minutes(3));
  desks_controller->NewDesk(DesksCreationRemovalSource::kKeyboard);
