...
std::string::size_type AddNodesFromString(LegacyBookmarkModel* model,
  ...
  while (end_pos != std::string::npos) {
    ...
    if (node_name != "]") {
      ...
      if (tell == folder_tell) {
        ...
        end_pos =
            AddNodesFromString(model, new_node, model_string, end_pos + 1);
      } else {
        std::string url_string("http://");
        url_string += std::string(node_name.begin(), node_name.end());
        url_string += ".com";
        model->AddURL(node, index, base::UTF8ToUTF16(node_name),
                      GURL(url_string));
        ++end_pos;
      }
      ++index;
...
std::string LegacyBookmarkModelStringFromNode(
  ...
  for (const auto& child : node->children()) {
    ...
    if (child->is_folder()) {
      child_string +=
          ":[ " + LegacyBookmarkModelStringFromNode(child.get()) + "]";
    }
    child_string += ' ';
  }
  return child_string;
