...
// Tests that suggestions are not reloaded on foreground if the omnibox does not
// have focus.
TEST_F(OmniboxMediatorTest, DoesNotReloadSuggestionsWhenNotEditing) {
  OCMStub([mock_text_controller_ hasFocus]).andReturn(NO);
...
TEST_F(OmniboxMediatorTest, DoesNotReloadSuggestionsWhenHasText) {
  OCMStub([mock_text_controller_ hasFocus]).andReturn(YES);
...
TEST_F(OmniboxMediatorTest, ReloadsSuggestionsWhenEditingAndEmpty) {
  ...
  fake_clipboard->SetClipboardURL(GURL("http://example.com"), base::Minutes(1));
