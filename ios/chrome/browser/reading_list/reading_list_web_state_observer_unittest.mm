...
class ReadingListWebStateObserverTest : public web::WebTest {
 public:
  ReadingListWebStateObserverTest() {
    ...
    fake_navigation_manager->SetLastCommittedItem(last_committed_item_.get());
 ...
 protected:
  ...
  FakeNavigationManager* fake_navigation_manager_;
...
TEST_F(ReadingListWebStateObserverTest, TestLoadReadingListFailure) {
  ...
  fake_navigation_manager_->GetPendingItem()->SetURL(url);
  test_web_state_.SetLoading(true);
  test_web_state_.OnPageLoaded(web::PageLoadCompletionStatus::FAILURE);
  test_web_state_.SetLoading(false);

  EXPECT_FALSE(fake_navigation_manager_->ReloadCalled());
  EXPECT_EQ(fake_navigation_manager_->GoToIndexCalled(), -1);
  // Check that |GetLastCommittedItem()| has not been altered.
  EXPECT_EQ(fake_navigation_manager_->GetLastCommittedItem()->GetVirtualURL(),
            GURL());
  EXPECT_EQ(fake_navigation_manager_->GetLastCommittedItem()->GetURL(), GURL());
...
TEST_F(ReadingListWebStateObserverTest, TestLoadReadingListDistilledCommitted) {
  ...
  fake_navigation_manager_->SetPendingItem(nullptr);
  fake_navigation_manager_->GetLastCommittedItem()->SetURL(url);
  test_web_state_.SetLoading(true);
  test_web_state_.OnPageLoaded(web::PageLoadCompletionStatus::FAILURE);
  test_web_state_.SetLoading(false);

  EXPECT_FALSE(fake_navigation_manager_->ReloadCalled());
  EXPECT_EQ(fake_navigation_manager_->GoToIndexCalled(),
            fake_navigation_manager_->GetLastCommittedItemIndex());
  EXPECT_EQ(fake_navigation_manager_->GetLastCommittedItem()->GetVirtualURL(),
            url);
  EXPECT_EQ(fake_navigation_manager_->GetLastCommittedItem()->GetURL(),
            distilled_url);
  EXPECT_TRUE(entry->IsRead());
}
...
TEST_F(ReadingListWebStateObserverTest, TestLoadReadingListDistilledPending) {
  ...
  fake_navigation_manager_->SetPendingItem(nil);
  fake_navigation_manager_->GetLastCommittedItem()->SetURL(url);
  test_web_state_.SetLoading(true);
  test_web_state_.OnPageLoaded(web::PageLoadCompletionStatus::FAILURE);
  test_web_state_.SetLoading(false);
