...
class ActuationTargetJavaScriptFeatureTest : public IOSChromeTestWithWebState {
 protected:
  ActuationTargetJavaScriptFeatureTest()
  ...
  ActuationTargetJavaScriptFeature* feature() {
    return ActuationTargetJavaScriptFeature::GetInstance();
  }
...
TEST_F(ActuationTargetJavaScriptFeatureTest, JsReturnsUnexpectedType) {
  ...
  web::test::ExecuteJavaScriptForFeature(web_state(),
        ...
        __gCrWeb.getRegisteredApi('actuation_target').addFunction(
  ...
  base::test::TestFuture<base::expected<
      ActuationTargetJavaScriptFeature::TargetFrameResult, ActuationError>>
      future;
...
TEST_F(ActuationTargetJavaScriptFeatureTest, JsReturnsError) {
  web::WebFrame* main_frame = WaitForMainFrame(feature());
  ASSERT_TRUE(main_frame);
  web::test::ExecuteJavaScriptForFeature(web_state(),
                                         base::SysUTF8ToNSString(R"(
...
TEST_F(ActuationTargetJavaScriptFeatureTest, TargetsMainFrame_Success) {
...
TEST_F(ActuationTargetJavaScriptFeatureTest,
...
TEST_F(ActuationTargetJavaScriptFeatureTest,
  ...
  EXPECT_FALSE(result.has_value());
  EXPECT_EQ(result.error().code,
            ActuationErrorCode::kActuationTargetFrameNotRegistered);
}

TEST_F(ActuationTargetJavaScriptFeatureTest,
       TargetsIframe_FrameIdNotRegistered) {
  web::WebFrame* main_frame = WaitForMainFrame(feature());
  ASSERT_TRUE(main_frame);
  ...
  EXPECT_FALSE(result.has_value());
  EXPECT_EQ(result.error().code,
            ActuationErrorCode::kActuationTargetFrameNotFoundById);
}

TEST_F(ActuationTargetJavaScriptFeatureTest,
       TargetIframe_ByCoordinate_Success) {
  int kIframeSize = 100;
...
TEST_F(ActuationTargetJavaScriptFeatureTest, MaxDepthExceeded) {
  web::WebFrame* main_frame = WaitForMainFrame(feature());
  ASSERT_TRUE(main_frame);
  ...
  feature()->GetTargetFrame(
      ...
      ActuationTargetJavaScriptFeature::kMaxTargetIframeDepth);

  auto result = future.Get();
  EXPECT_FALSE(result.has_value());
  EXPECT_EQ(result.error().code,
            ActuationErrorCode::kActuationTargetMaxDepthExceeded);
}
