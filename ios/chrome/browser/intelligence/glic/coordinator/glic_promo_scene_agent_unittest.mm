...
class GLICPromoSceneAgentTest : public PlatformTest {
 public:
  GLICPromoSceneAgentTest() : PlatformTest() {
    ...
    agent_ = [[GLICPromoSceneAgent alloc]
...
// Tests that the GLIC promo single registers in the promo manager.
TEST_F(GLICPromoSceneAgentTest, TestGLICPromoRegistration) {
  feature_list_.InitAndEnableFeature(kPageActionMenu);
  EXPECT_CALL(*promos_manager_.get(),
              RegisterPromoForSingleDisplay(promos_manager::Promo::GLICPromo))
      .Times(1);
  scene_state_.activationLevel = SceneActivationLevelForegroundActive;
}

// Tests that the GLIC promo is not registered in the promo
// manager when the eligibility criteria is not met.
TEST_F(GLICPromoSceneAgentTest, TestGLICPromoNoRegistration) {
  EXPECT_CALL(*promos_manager_.get(),
              RegisterPromoForSingleDisplay(promos_manager::Promo::GLICPromo))
      .Times(0);
  scene_state_.activationLevel = SceneActivationLevelForegroundActive;
