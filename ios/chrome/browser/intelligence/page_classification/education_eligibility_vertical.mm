...
// Returns whether `category_id` matches `prefix` as a taxonomy path prefix.
// Matches if `category_id` equals `prefix`, or if `category_id` starts with
// `prefix` followed immediately by a path delimiter ('/').
bool IsCategorySubpath(std::string_view category_id, std::string_view prefix) {
  if (category_id == prefix) {
    return true;
  }
  if (!base::StartsWith(category_id, prefix)) {
    return false;
  }
  return prefix.ends_with('/') || category_id[prefix.size()] == '/';
}
...
bool IsEligibleCategoryInternal(
  ...
  for (std::string_view prefix : education_category_prefixes) {
    if (IsCategorySubpath(category_id, prefix)) {
      return true;
    }
  }
  return false;
