...
// Tests that DeleteProfile(...) reports failure to delete the profile.
TEST_F(ProfileDeleterIOSTest, DeleteProfile_FailureStorageLoaded) {
  base::test::TaskEnvironment env;
  base::ScopedTempDir scoped_temp_dir;
  ASSERT_TRUE(scoped_temp_dir.CreateUniqueTempDir());
  const base::FilePath storage_dir = scoped_temp_dir.GetPath();
  ...
  // Load the storage and keep it alive, this should cause the deletion of
  // the profile to fail (when trying to delete the storage).
  WKWebsiteDataStore* storage = web::GetDataStoreForBrowserState(profile.get());
  ASSERT_NE(storage, nil);
  ...
  // The profile should not have been deleted and the failure reported.
  EXPECT_TRUE(base::DirectoryExists(profile_dir));
  EXPECT_EQ(deleted_name, profile_name);
