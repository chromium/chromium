...
TEST_F(FollowJavaScriptFeatureTest, NoRSSLink) {
  ...
  FollowJavaScriptFeature::GetInstance()->GetFollowWebPageURLs(
      web_state(), base::BindOnce(^(FollowWebPageURLs* urls) {
        ASSERT_TRUE(urls);
        EXPECT_NSEQ(net::NSURLWithGURL(example_url), urls.webPageURL);
        EXPECT_EQ(0ul, urls.RSSLinks.count);
...
TEST_F(FollowJavaScriptFeatureTest, SingleAlternateRssXMLLink) {
  ...
  FollowJavaScriptFeature::GetInstance()->GetFollowWebPageURLs(
      web_state(), base::BindOnce(^(FollowWebPageURLs* urls) {
        ASSERT_TRUE(urls);
        EXPECT_NSEQ(net::NSURLWithGURL(example_url), urls.webPageURL);
        ASSERT_EQ(1ul, urls.RSSLinks.count);
        EXPECT_NSEQ(@"http://example.com/rss",
                    [urls.RSSLinks[0] absoluteString]);
        completion_called = true;
      }));
...
TEST_F(FollowJavaScriptFeatureTest, MultipleRSSLinks) {
  ...
  FollowJavaScriptFeature::GetInstance()->GetFollowWebPageURLs(
      web_state(), base::BindOnce(^(FollowWebPageURLs* urls) {
        ASSERT_TRUE(urls);
        EXPECT_NSEQ(net::NSURLWithGURL(example_url), urls.webPageURL);
        ASSERT_EQ(3ul, urls.RSSLinks.count);
        EXPECT_NSEQ(@"http://example.com/rss",
                    [urls.RSSLinks[0] absoluteString]);
        EXPECT_NSEQ(@"http://example.com/rss2",
                    [urls.RSSLinks[1] absoluteString]);
        EXPECT_NSEQ(@"http://example.com/rss3",
                    [urls.RSSLinks[2] absoluteString]);
        completion_called = true;
      }));
