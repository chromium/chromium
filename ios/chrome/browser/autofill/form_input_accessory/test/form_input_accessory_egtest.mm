...
constexpr char kFormPassportNumber[] = "passportNumber";
constexpr char kFormPassportOwnerName[] = "passportOwnerName";
constexpr char kFormPassportCountryCode[] = "passportCountryCode";
constexpr char kFormPassportIssueDate[] = "passportIssueDate";
constexpr char kFormPassportExpirationDate[] = "passportExpirationDate";

constexpr NSString* kPassportEntityType = @"Passport";
constexpr NSString* kPassportNumber = @"LR1234567";
constexpr NSString* kPassportOwnerName = @"Pippi Långstrump";
constexpr NSString* kPassportCountry = @"Sweden";
constexpr NSString* kPassportIssueDate = @"2010-09-01";
constexpr NSString* kPassportExpirationDate = @"2019-08-30";

constexpr char kFormVehicleOwner[] = "vehicleOwner";
constexpr char kFormVehicleMake[] = "vehicleMake";
constexpr char kFormVehicleModel[] = "vehicleModel";
constexpr char kFormVehicleYear[] = "vehicleYear";
constexpr char kFormVehicleLicensePlate[] = "vehicleLicensePlate";
constexpr char kFormVehiclePlateState[] = "vehiclePlateState";
constexpr char kFormVehicleVIN[] = "vehicleVIN";

constexpr NSString* kVehicleOwner = @"Knecht Ruprecht";
constexpr NSString* kVehicleMake = @"BMW";
constexpr NSString* kVehicleModel = @"Series 2";
constexpr NSString* kVehicleYear = @"2025";
constexpr NSString* kVehicleLicensePlate = @"123456";
constexpr NSString* kVehiclePlateState = @"California";
constexpr NSString* kVehicleVIN = @"12312345";
constexpr NSString* kVehicleChipAccessibilityLabel = @"BMW, Vehicle · Series 2";
...
// autofill::CalculateFieldSignatureForField and
// autofill::CalculateFormSignature.
constexpr std::string_view kVehicleOverrideParam =
    "3343056218385819478_1092605073_178-"
    "3343056218385819478_2435123338_179-"
    "3343056218385819478_3040116536_185-"
    "3343056218385819478_1294171443_7-"
    "3343056218385819478_3811017579_176-"
    "3343056218385819478_2296829005_186-"
...
// Helper function for setting up the configs for Autofill AI.
- (void)configureAutofillAiWithConfig:(AppLaunchConfiguration&)config
...
- (AppLaunchConfiguration)appConfigurationForTestCase {
  ...
  if ([self isRunningTest:@selector(testFillPassportForm)] ||
    ...
    [self configureAutofillAiWithConfig:config
                          overrideParam:kPassportOverrideParam];
...
// Save a passport entity into the local database so the suggestion is
// available.
- (void)savePassportEntity {
  GREYAssertTrue([AutofillAppInterface savePassportEntity],
                 @"Failed to save passport entity.");
}

// Save a vehicle entity into the local database so the suggestion is available.
- (void)saveVehicleEntity {
  GREYAssertTrue([AutofillAppInterface saveVehicleEntity],
                 @"Failed to save vehicle entity.");
}
...
// Loads simple vehicle page on localhost.
...
// Tests that an Autofill AI vehicle suggestion successfully fills a vehicle
// form and can be undone.
- (void)testFillAndUndoVehicleForm {
  [self saveVehicleEntity];
  [self loadVehiclePage];

  // Enhanced Autofill only works for signed in users.
  [SigninEarlGrey signinWithFakeIdentity:[FakeSystemIdentity fakeIdentity1]];

  // Tap the vehicle make field to trigger suggestions.
  [[EarlGrey selectElementWithMatcher:chrome_test_util::WebViewMatcher()]
      performAction:chrome_test_util::TapWebElementWithId(kFormVehicleMake)];

  id<GREYMatcher> vehicle_chip =
      grey_accessibilityLabel(kVehicleChipAccessibilityLabel);

  // Wait for the suggestion form to appear.
  [ChromeEarlGrey waitForUIElementToAppearWithMatcher:vehicle_chip];

  [[EarlGrey selectElementWithMatcher:vehicle_chip] performAction:grey_tap()];

  // Verify that the page is filled properly.
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleOwner value:kVehicleOwner];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleMake value:kVehicleMake];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleModel value:kVehicleModel];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleYear value:kVehicleYear];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleLicensePlate
                                 value:kVehicleLicensePlate];
  [self verifyFieldWithIdHasBeenFilled:kFormVehiclePlateState
                                 value:kVehiclePlateState];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleVIN value:kVehicleVIN];

  // Tap the vehicle make field to see the undo option.
  [[EarlGrey selectElementWithMatcher:chrome_test_util::WebViewMatcher()]
      performAction:chrome_test_util::TapWebElementWithId(kFormVehicleMake)];
  ...
  // Tap on the undo autofill chip.
  [[EarlGrey selectElementWithMatcher:undoChipMatcher]
      performAction:grey_tap()];

  // Verify that all the fields are empty.
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleOwner value:@""];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleMake value:@""];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleModel value:@""];
  [self verifyFieldWithIdHasBeenFilled:kFormVehicleYear value:@""];
