...
// user action metrics.
bool ValidateShortcutItemAndRecordMetrics(
...
- (instancetype)initWithShortcutItem:(UIApplicationShortcutItem*)shortcutItem
  ...
  if ((self = [super initWithSceneState:sceneState isColdStart:isColdStart])) {
    ...
    _handleShortcutItem = ValidateShortcutItemAndRecordMetrics(shortcutItem);
  }
  return self;
}
...
- (void)execute {
  ...
  if (_shortcutHandler) {
    _shortcutHandler(_handleShortcutItem);
  }
}
