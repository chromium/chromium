...
XcodeTVOSSimulatorRuntimeDMGCipdPath = (
  'infra_internal/ios/xcode/tvos_runtime_dmg'
)
...
def _is_legacy_xcode_package(xcode_app_path):
  ...
  runtimes_in_xcode = glob.glob(
    os.path.join(
      xcode_app_path, XcodeIOSSimulatorRuntimeRelPath, '*.simruntime'
    )
  )
...
def _install_runtime(
  mac_toolchain, install_path, xcode_build_version, ios_version
):
...
def move_runtime(runtime_cache_folder, xcode_app_path):
  ...
  """
  xcode_runtime_folder = os.path.join(
    xcode_app_path, XcodeIOSSimulatorRuntimeRelPath
  )
...
def select(xcode_app_path):
  ...
  output += subprocess.check_output(
    run_first_launch_cmd, stderr=subprocess.STDOUT
  ).decode('utf-8')
  ...
  output += subprocess.check_output(
    xcrun_simctl_cmd, stderr=subprocess.STDOUT
  ).decode('utf-8')
...
def install(mac_toolchain, xcode_build_version, xcode_app_path, **runtime_args):
  ...
  * If installed Xcode isn't legacy (without runtime bundled), install and copy
  ...
  is_legacy_xcode_package = (
    mac_util.is_macos_13_or_higher() or _is_legacy_xcode_package(xcode_app_path)
  )
  ...
  if not is_legacy_xcode_package and runtime_args.get('ios_version'):
    ...
    _install_runtime(
      mac_toolchain, runtime_cache_folder, xcode_build_version, ios_version
    )
...
def get_latest_runtime_build_cipd(
  xcode_version, platform_type: constants.IOSPlatformType, platform_version: str
):
  ...
  runtime_build_match = re.search(
    XcodeSimulatorRuntimeBuildTagRegx, output, re.MULTILINE
  )
  runtime_version_match = re.search(
    XcodeSimulatorRuntimeVersionTagRegx, output, re.MULTILINE
  )
  ...
  if runtime_info is None:
    ...
    if existing_runtime_info is not None:
      iossim_util.delete_simulator_runtime(
        existing_runtime_info['identifier'], True
      )
      iossim_util.remove_stale_simulators_from_cache()

    _install_runtime_dmg(
      mac_toolchain,
      runtime_cache_folder,
      platform_type,
      platform_version,
...
def using_xcode_11_or_higher():
  ...
  return distutils.version.LooseVersion(
    '11.0'
  ) <= distutils.version.LooseVersion(version()[0])
...
def validate_local_xcode_install(xcode_build_version):
  ...
  if xcode_build_version.lower() != local_version.lower():
    raise test_runner_errors.LocalRunXcodeError(
      xcode_build_version, local_version
    )
...
def validate_local_runtime(
  ...
  Raises:
    test_runner_errors.LocalRunRuntimeError when the requested iOS version is
     not installed locally
  """
  runtime_build = get_latest_runtime_build_cipd(
    xcode_build_version, platform_type, platform_version
  )
  ...
  if not local_runtime:
    raise test_runner_errors.LocalRunRuntimeError(
      platform_version, runtime_build
    )
...
def ensure_xcode_ready_in_apps(xcode_build_version):
  ...
  if check_xcode_exists_in_apps(xcode_build_version):
    ...
    LOGGER.warning(
      f"Specified Xcode version {xcode_build_version} not found "
      "in /Applications."
    )
  ...
  if is_local_run():
    ...
    if platform_version:
      assert device_type is not None, (
        "platform_version requires a device_type string"
      )
      try:
