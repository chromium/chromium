...
_SRC_DIR = os.path.join(
  _THIS_DIR, os.path.pardir, os.path.pardir, os.path.pardir, os.path.pardir
)
...
def _load_crashing_seed():
  ...
  return (
    seed_json['variations_compressed_seed'],
...
class VariationsSimulatorParallelTestRunner(SimulatorParallelTestRunner):
  ...
  def _user_data_dir(self):
    ...
    app_data_path = iossim_util.get_app_data_directory(
      self.host_app_bundle_id, self.udid
    )
    return os.path.join(
      app_data_path, 'Library', 'Application Support', 'Google', 'Chrome'
    )
  ...
  def _reset_last_fetch_time(self):
    ...
    seed_helper.update_local_state(
      self._user_data_dir(),
      {
        _LOCAL_STATE_VARIATIONS_LAST_FETCH_TIME_KEY: str(
