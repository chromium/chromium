...
//
// NOTE: These assert helpers will only work if contained within a registered
// CrWebApi function. If used in other arbitrary locations, for example directly
// in an event handler, the error will not be reported because it will be
// redacted to the general text "Script Error" in the error handler.

import {CrWebError} from '//ios/web/public/js_messaging/resources/gcrweb_error.js';

const assertionErrorMessagePrefix = 'CrWeb Assertion:';
...
export function assert<T>(value: T, message: string): asserts value {
  ...
  throw new CrWebError(`${assertionErrorMessagePrefix} ${message}`);
}
...
export function assertInstanceof<T>(
  ...
  throw new CrWebError(`${assertionErrorMessagePrefix} Value ${
      value} is not of type ${type.name || typeof type}`);
}
 ...
 * statements referring to enums, because it results in a build time error if
 * the 'case' statements are not exhaustive. At runtime it behaves identically
 * to assertNotReached.
 */
