...
class VectorProvider(SingleChildProvider):
  ...
  def populate(self):
    ...
    addr = (
      self.valobj.GetValueAsAddress()
      if lldbtype.IsPointerType()
...
class SmartPointerProvider(SingleChildProvider):
  ...
  def __init__(
    self, raw_name, valobj, internal_dict, child_name=DEREFERENCE_NAME
  ):
    super().__init__(valobj)
...
class MemberProvider(SingleChildProvider):
  ...
  def populate(self):
    if self.raw_storage is None:
      ...
      self.pointer_type = (
        self.raw_storage.GetType().GetCanonicalType().GetName()
      )
    ...
    if self.pointer_type == 'cppgc::internal::RawPointer':
      ...
      sign_bit = 0x80000000
      decompressed = (
        ((compressed ^ sign_bit) - sign_bit) << pointer_shift
      ) & cage_base
      data_pointer = self.valobj.CreateValueFromAddress(
        '$$dereference$$', decompressed, pointee_type
      )
...
class WTFStringProvider(SingleChildProvider):
  ...
  def populate(self):
    ...
    hash_and_flags = (
      impl.GetChildMemberWithName('hash_and_flags_')
