...
class StructGeneratorTest(unittest.TestCase):
  def testGenerateIntField(self):
    self.assertEqual(
      'const int foo_bar', GenerateField({'type': 'int', 'field': 'foo_bar'})
    )
  ...
  def testGenerateEnumField(self):
    self.assertEqual(
      ...
      GenerateField(
        {'type': 'enum', 'field': 'foo_foo', 'ctype': 'MyEnumType'}
      ),
    )
  ...
  def testGenerateArrayField(self):
    self.assertEqual(
      ...
      GenerateField(
        {'type': 'array', 'field': 'bar_bar', 'contents': {'type': 'int'}}
      ),
    )
  ...
  def testGenerateClassField(self):
    self.assertEqual(
      ...
      GenerateField(
        {'type': 'class', 'field': 'bar', 'ctype': 'std::optional<bool>'}
      ),
    )
  ...
  def testGenerateStruct(self):
    schema = [
      ...
      {'type': 'string', 'field': 'bar_foo', 'default': 'dummy'},
      {
        'type': 'array',
        'field': 'bar_bar',
        'contents': {'type': 'enum', 'ctype': 'MyEnumType'},
      },
    ]
    struct = (
      'struct MyTypeName {\n'
