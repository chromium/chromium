...
def error(elem: ET.Element, msg: str) -> NoReturn:
...
def get_attr(elem: ET.Element, tag: str, regex: Optional[str] = None) -> str:
  """Get an attribute.
  ...
  if regex and not re.fullmatch(regex, attr):
    error(
      elem,
      (
        f"has '{tag}' attribute '{attr}' which does not match regex '{{regex}}'"
      ),
    )
  return attr
...
def get_optional_attr(
  elem: ET.Element, tag: str, regex: Optional[str] = None
) -> Optional[str]:
  """Get an attribute.
...
def get_text_children(
  ...
  for child in children:
    ...
    if regex and not re.fullmatch(regex, text):
      error(
        elem,
        (f"has '{tag}' node '{text}' which does not match regex '{{regex}}'"),
      )
    result.append(text)
  return result
...
def check_children(
  ...
  optional_children: Optional[Set[str]] = None,
) -> None:
...
def check_child_names_unique(elem: ET.Element, tag: str) -> None:
  ...
  """
  names = [child.attrib.get('name', '') for child in elem if child.tag == tag]
