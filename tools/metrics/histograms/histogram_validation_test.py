...
class HistogramValidationTest(unittest.TestCase):
  ...
  def testGetFilesToCheck_HistogramModified(self):
    target_path = str(
        (path_util.CHROMIUM_SRC_PATH / 'tools' / 'metrics' / 'histograms' /
         'metadata' / 'uma' / 'histograms.xml').resolve())
    affected_files = [
    ...
    variants_path = str((path_util.CHROMIUM_SRC_PATH / 'tools' / 'metrics' /
                         'histograms' / 'variants.xml').resolve())
    result = histogram_validation.get_files_to_check(
        affected_files,
        read_file_fn,
        variants_paths=[variants_path],
    ...
    self.assertEqual(pathlib.Path(result.files_to_check[0].path),
  ...
  @mock.patch('chromium_src.tools.metrics.histograms.'
              'histogram_utils._path_contents')
  def testGetFilesToCheck_VariantsModified(self, mock_path_contents):
    ...
    self.assertEqual(result.files_to_check[0].new_contents,
                     histograms_content.splitlines())
  ...
  def testGetOldAndNewVariants_Modified(self):
    ...
    res = histogram_validation.get_old_and_new_variants(affected, read_file_fn,
                                                        [target_path])
  ...
  def testGetHistogramNames_WithVariants(self):
    ...
    variants_xml = ('<histogram-configuration><variants name="MockVariants">'
