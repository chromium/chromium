...
class TestDependencyResolver(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.root_path = pathlib.Path(self.test_dir)
  ...
  def _create_file(self, relative_path, content=''):
    ...
    full_path.write_text(content.replace('TEST_CASE ', ''), encoding='utf-8')
    return full_path
  ...
  def test_direct_import_module(self):
    ...
    self.assertIn(pathlib.Path('main.py'), deps)
    self.assertIn(pathlib.Path('utils.py'), deps)
    self.assertEqual(deps[pathlib.Path('main.py')], [pathlib.Path('utils.py')])
  ...
  def test_import_with_alias(self):
    ...
    self.assertEqual(deps[pathlib.Path('main.py')], [pathlib.Path('data.py')])
  ...
  def test_from_import_module(self):
    ...
    expected = pathlib.Path('core') / 'logic.py'
    self.assertEqual(deps[pathlib.Path('main.py')], [expected])
  ...
  def test_symbol_fallback_to_file(self):
    ...
    self.assertEqual(deps[pathlib.Path('main.py')], [pathlib.Path('lib.py')])
...
import TEST_CASE chromium_src.tools.metrics.target
    ...
    self.assertEqual(deps[pathlib.Path('main.py')], [pathlib.Path('target.py')])
  ...
  def test_circular_dependency_safety(self):
    ...
    deps_a = dependency_solver.get_all_dependencies(graph, pathlib.Path('a.py'))
    self.assertEqual(deps_a, {pathlib.Path('b.py'), pathlib.Path('a.py')})
