...
def GetArchitecture(binary, xcode_path):
    ...
    architecture that matches the host. If such an architecture is not present
    in
    ...
    architectures = (
        subprocess.check_output([lipo_path, '-archs', binary], encoding='utf-8')
        .strip()
        .split(' ')
    )
    ...
    raise Exception(
        'Host architecture ' + host_arch + ' not present in fat binary'
    )
...
def ShowModuleInitializers(binary, xcode_path):
    """Gathers the module initializers for |binary| and symbolizes the
    addresses.
    """
...
def GetStaticInitializerSection(load_commands):
    ...
    Static initializers are stored in sections with flag
...
def GetModuleInitializers(binary, architecture, xcode_path):
    ...
    for sectname, segname, flags in static_initializer_sections:
        ...
        # sections below building `sect_address` can be removed. The logic to
        # add
        # the __TEXT base address will remain.
        if architecture not in ('arm64', 'x86_64'):
            raise Exception(
                "Parsing otool's S_INIT_FUNC_OFFSETS output on"
                " architectures other than arm64 on x86_64 is unsupported."
            )
        ...
        if architecture == 'arm64':
            ...
            # in S_INIT_FUNC_OFFSETS arm64 are 32 bits simply trim the first
            # column
            sect_address = [line.split('\t')[1].strip() for line in lines[2:]]
            sect_address = (' '.join(sect_address)).split(' ')

        if architecture == 'x86_64':
            # For x86_64 otool dumps as byte-oriented output. Here, trim the
            # first
...
def SymbolizeAddresses(binary, architecture, addresses, xcode_path):
    """Given a |binary| and a list of |addresses|, symbolizes them using
    atos.
    """
    if xcode_path:
        atos_path = os.path.join(
            xcode_path, 'Contents', 'Developer', 'usr', 'bin', 'atos'
        )
    else:
        atos_path = 'atos'
...
def main(args):
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            'Optional custom path to xcode binaries. By default, commands'
            ' such as `otool` will be run as `/usr/bin/otool` which only works'
