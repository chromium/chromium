...
def CheckChangeOnUpload(input_api, output_api):
  ...
  # than Linux.
  if (
    'chromium.ycm_extra_conf.py' in affected_files
    or 'ninja_output.py' in affected_files
    or any([input_api.re.match(r'tests(/|\\)', f) for f in affected_files])
  ):
    results += input_api.RunTests(
      input_api.canned_checks.GetUnitTests(
        input_api, output_api, ['tests/chromium.ycm_extra_conf_unittest.py']
      )
    )
