...
class Chromium_ycmExtraConfTest(unittest.TestCase):
  ...
  def NormalizeString(self, string):
    return (
      string.replace(self.out_dir, '[OUT]')
  ...
  def setUp(self):
    ...
    self.test_data_path = os.path.join(
      self.actual_chrome_root, 'tools', 'vim', 'tests', 'data'
    )
    self.ycm_extra_conf = imp.load_source(
  ...
  def testGetClangOptionsForKnownCppFile(self):
    clang_options = self.ycm_extra_conf.GetClangOptionsFromNinjaForFilename(
  ...
  def testOutDirNames(self):
    ...
    self.assertIn(
      '-I%s/a' % self.NormalizeString(out_dir),
      self.NormalizeStringsInList(clang_options),
    )
    self.assertIn(
      '-I%s/tag-one' % self.NormalizeString(out_dir),
      self.NormalizeStringsInList(clang_options),
    )
  ...
  @unittest.skipIf(
    os.name == "nt", "Test fails with path differences on Windows."
  )
