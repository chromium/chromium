...
fs_config_desc = usb_descriptors.ConfigurationDescriptor(
  bmAttributes=0xC0, MaxPower=50
)

fs_interface_desc = usb_descriptors.InterfaceDescriptor(bInterfaceNumber=0)
...
hs_interface_desc = usb_descriptors.InterfaceDescriptor(bInterfaceNumber=0)
...
class GadgetTest(unittest.TestCase):
  ...
  def test_get_microsoft_os_string_descriptor(self):
    ...
    os_string_descriptor = g.ControlRead(
      0x80, usb_constants.Request.GET_DESCRIPTOR, 0x03EE, 0x0000, 0x12
    )
    self.assertEqual(
      os_string_descriptor,
  ...
  def test_get_bos_descriptor(self):
    ...
    g.AddDeviceCapabilityDescriptor(
      usb_descriptors.ContainerIdDescriptor(ContainerID=container_id.bytes_le)
    )
