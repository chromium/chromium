...
GCS_SYMBOLS = {
  'symbols.zip',
  'Monochrome_symbols.zip',
  'Monochrome_symbols-secondary.zip',
}


def GetTraceBreakpadSymbols(
  cloud_storage_bucket, metadata, breakpad_output_dir, dump_syms_path=None
):
  ...
  if metadata.version_number is None:
    raise Exception(
      'Failed to extract trace version number: ' + metadata.trace_file
    )
  ...
  if metadata.os_name == OSName.ANDROID:
    ...
    rename_breakpad.RenameBreakpadFiles(
      breakpad_output_dir, breakpad_output_dir
    )
    ...
    raise Exception(
      'Trace OS "%s" is not supported: %s'
      % (metadata.os_name, metadata.trace_file)
    )

  flag_utils.GetTracingLogger().info(
    'Breakpad symbols located at: %s', os.path.abspath(breakpad_output_dir)
  )
...
def GetAndroidSymbols(cloud_storage_bucket, metadata, breakpad_output_dir):
  ...
  for arch_folder in possible_arch_folders:
    possible_gcs_folder = (
      'android-B0urB0N/' + metadata.version_number + '/' + arch_folder
    )
    ...
    # code.
    if _IsAndroidVersionCodeInFile(
  ...
  if gcs_folder is None:
    raise RuntimeError(
      'Failed to determine architecture folder: ' + str(metadata._trace_file)
    )
  ...
  for symbol in GCS_SYMBOLS:
    ...
    if not _FetchAndUnzipGCSFile(
      cloud_storage_bucket, gcs_symbol_file, symbol_zip_file, unzip_output_dir
    ):
      flag_utils.GetTracingLogger().warning(
        'Failed to find symbols on GCS: %s', gcs_symbol_file
      )
    else:
...
def _IsAndroidVersionCodeInFile(
  cloud_storage_bucket, version_code, possible_gcs_folder, local_folder
):
  ...
  if not _FetchGCSFile(
    cloud_storage_bucket, gcs_version_code_file, local_version_code_file
  ):
    flag_utils.GetTracingLogger().debug(
...
def _FetchBreakpadSymbols(cloud_storage_bucket, metadata, breakpad_output_dir):
  ...
  if metadata.os_name == OSName.LINUX:
    ...
    ) and 'arm' in metadata.architecture.lower():
      folder = 'mac-arm64'
    else:
      if metadata.architecture is None:
        flag_utils.GetTracingLogger().warning(
    ...
    raise Exception(
      'Expected OS "%s" to be Linux or Mac: %s'
      % (metadata.os_name, metadata.trace_file)
    )
  ...
  if not _FetchAndUnzipGCSFile(
    cloud_storage_bucket, gcs_zip_file, breakpad_zip, breakpad_output_dir
  ):
    if not _FetchAndUnzipGCSFile(
      cloud_storage_bucket, gcs_file, breakpad_zip, breakpad_output_dir
    ):
      raise Exception('Failed to find symbols on GCS: %s[.zip].' % (gcs_file))


def _FetchAndUnzipGCSFile(
  cloud_storage_bucket, gcs_file, gcs_output, output_dir
):
...
def _FetchGCSFile(cloud_storage_bucket, gcs_file, output_file):
  ...
  if cloud_storage.Exists(cloud_storage_bucket, gcs_file):
    flag_utils.GetTracingLogger().info(
      'Downloading files from GCS: %s', gcs_file
    )
