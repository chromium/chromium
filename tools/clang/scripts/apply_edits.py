  ...
  for line in sys.stdin:
    ...
    try:
      ...
      if not path:
        continue
      edits[path].append(
        Edit(edit_type, int(offset), int(length), replacement.encode("utf-8"))
      )
    except ValueError:
...
def _FindStartOfPreviousLine(contents, index):
  ...
  if (
    index > 0
    and contents[index - 1] in _NEWLINE_CHARACTERS
    and contents[index - 1] != contents[index]
  ):
    index = index - 1
...
def _SkipOverPreviousComment(contents, index):
  ...
  if not re.search(rb"^  \s*  (  //  |  \*  )", prev_text, re.VERBOSE):
    return index
...
def _InsertIncludeHeader(filepath, header_line_to_add, contents, is_system):
  ...
  if is_system:
    ...
    regex_text += (
      _INCLUDE_INSERTION_POINT_REGEX_TEMPLATE % primary_header_basename
    )

  match = re.search(
    regex_text.encode("utf-8"), contents, re.MULTILINE | re.VERBOSE
  )
  ...
  # Make the edit.
  return (
    contents[:insertion_point] + replacement_text + contents[insertion_point:]
  )
...
def _ApplyReplacement(filepath, contents, edit, last_edit):
  ...
  if not edit.replacement:
    contents = _ExtendDeletionIfElementIsInList(
      original_contents, contents, edit.offset, edit.length
    )
  return contents
...
def _ApplyIncludeHeader(filepath, contents, edit, last_edit, is_system):
  ...
  if is_system:
    ...
    header_line_to_add += '"%s"' % name
  return _InsertIncludeHeader(
    filepath, header_line_to_add.encode("utf-8"), contents, is_system
  )
...
def _ApplySingleEdit(filepath, contents, edit, last_edit):
  if edit.edit_type == 'r':
    ...
    raise ValueError(
      'Unrecognized edit directive "%s": %s\n' % (edit.edit_type, filepath)
    )
    return contents
...
def _ApplyEditsToSingleFile(filepath, edits):
  with open(filepath, 'rb+') as f:
    contents = f.read()
    (contents, edit_count, error_count) = _ApplyEditsToSingleFileContents(
...
def _ApplyEdits(edits):
  ...
  for k, v in edits.items():
    ...
    sys.stdout.write(
      'Applied %d edits (%d errors) to %d files [%.2f%%]\r'
...
def _ExtendDeletionIfElementIsInList(
  original_contents, contents, offset, length
):
  ...
  def notify(left_offset, right_offset):
    ...
    sys.stdout.write(
      'Extended deletion of "%s" to "%s" in "...%s..."\n'
...
""",
  ...
  return _ApplyEdits(
    {k: v for k, v in edits.items() if os.path.realpath(k) in filenames}
  )
