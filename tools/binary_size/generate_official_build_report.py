...
_GSUTIL = os.path.join(
  _DIR_SOURCE_ROOT, 'third_party', 'depot_tools', 'gsutil.py'
)
...
def _UploadSizeFile(size_path, version, arch):
  ...
  dst_url = os.path.join(
    _REPORTS_GS_URL, version, arch, report_basename + '.size'
  )
...
def main():
  ...
  if version_parts[-1] < 3:
    ...
    logging.info(
      'Not regenerating canary_reports.json. Version=%s', args.version
    )
