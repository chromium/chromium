  ...
  def do_GET(self):
    if self.path == "/discover.json":
      remote_discovery_url = (
        "http://localhost:{remote_port}/json/version".format(
...
from local to remote:
        ssh root@$DUT_IP -L {remote_port}:localhost:{remote_port}
            '''.format(
            remote_port=remote_port, remote_discovery_url=remote_discovery_url
          )
        }
...
if __name__ == '__main__':
  try:
    ...
    parser.add_argument(
      "debugger_port", default=debugger_port, type=int, nargs='?'
    )
    parser.add_argument(
      "--nolaunch",
    ...
    Handler = partial(
      CORSRequestHandler, directory=os.path.dirname(os.path.abspath(__file__))
    )
    ...
    if not args.nolaunch:
      webbrowser.open(
        "http://localhost:" + str(debugger_port) + "/app.html",
        new=1,
        autoraise=True,
      )
