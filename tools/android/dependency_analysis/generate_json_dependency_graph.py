...
class JavaClassJdepsParser:
    ...
    def parse_raw_jdeps_output(
        self, build_target: str, jdeps_output: str, prefixes: Tuple[str, ...]
    ):
    ...
    def parse_line(
        ...
        key_to, nested_to = class_dependency.split_nested_class_from_key(dep_to)
...
def _run_gn_desc_list_dependencies(
    ...
    return subprocess_utils.run_command(
        [gn_path, 'desc', '--all', str(build_output_dir), target, 'deps'],
        cwd=src_path,
    )
...
def run_and_parse_list_java_targets(
    build_output_dir: pathlib.Path, show_ninja: bool, src_path: pathlib.Path
) -> JarTargetDict:
...
def parse_original_targets_and_jars(
    gn_desc_output: str, build_output_dir: pathlib.Path, cr_position: int
) -> JarTargetDict:
    ...
    for build_target_line in gn_desc_output.split('\n'):
        ...
        jar_path = _get_jar_path_for_target(
            build_output_dir, build_target, cr_position
        )
        jar_dict[original_build_target] = jar_path
    return jar_dict


def _get_jar_path_for_target(
    build_output_dir: pathlib.Path, build_target: str, cr_position: int
) -> pathlib.Path:
...
def parse_args():
    ...
    arg_parser.add_argument(
        '--jdeps-path', help='Path to the jdeps executable.'
    )
    arg_parser.add_argument(
        '-g', '--gn-path', default='gn', help='Path to the gn executable.'
    )
    arg_parser.add_argument(
        '--skip-rebuild',
        action='store_true',
...
def main():
    ...
    if args.skip_rebuild:
        ...
        subprocess.run(
            cmd + to_recompile, capture_output=not args.show_ninja, check=True
        )
    ...
    with multiprocessing.Pool(jdeps_process_number) as pool:
        jdeps_outputs = pool.map(
            functools.partial(
                jar_utils.run_jdeps, jdeps_path=jdeps_path, verbose=args.verbose
            ),
            target_jars.values(),
        )
    ...
    for raw_jdeps_output, build_target in zip(
        jdeps_outputs, target_jars.keys()
    ):
        if raw_jdeps_output is None:
            continue
        logging.debug(f'Parsing jdeps for {build_target}')
        jdeps_parser.parse_raw_jdeps_output(
            build_target, raw_jdeps_output, prefixes=tuple(args.prefixes)
        )

    class_graph = jdeps_parser.graph
    logging.info(
        f'Parsed class-level dependency graph, '
    ...
    logging.info(
        f'Created package-level dependency graph, '
    ...
    logging.info(
        f'Created target-level dependency graph, '
