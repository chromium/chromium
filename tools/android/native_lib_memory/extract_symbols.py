...
def _GetSymbolNameToFilename(build_directory):
  ...
  symbol_names_filename = zip(
    pool.map(symbol_extractor.SymbolNamesFromLlvmBitcodeFile, object_filenames),
    object_filenames,
  )
  pool.close()
  result = {}
...
def WriteReachedData(filename, page_to_reached_data):
  ...
  for offset, data in page_to_reached_data.items():
    json_object.append(
      {'offset': offset, 'total': data['total'], 'reached': data['reached']}
    )
...
def CodePagesToObjectFiles(symbols_to_object_files, code_pages_to_symbols):
  ...
  logging.warning(
    '%d unmatched symbols (total size %d).',
...
def WriteCodePageAttribution(
  page_to_object_files, text_filename, json_filename
):
  ...
  with open(text_filename, 'w') as f:
    for page_offset in sorted(page_to_object_files.keys()):
      size_and_filenames = [
        (kv[1], kv[0]) for kv in page_to_object_files[page_offset].items()
      ]
      ...
      json_data.append(
        {
          'offset': page_offset,
          'accounted_for': total_size,
          'size_and_filenames': size_and_filenames,
        }
      )
      f.write(
        'Page Offset: %d * 4k (accounted for: %d)\n'
...
def CreateArgumentParser():
  ...
  parser.add_argument(
    ...
    help=('Native Library, e.g. libchrome.so or libmonochrome.so'),
  )
  parser.add_argument(
    '--reached-symbols-file',
    type=str,
  ...
  parser.add_argument(
    ...
    ' by extract_resident_pages.py',
    required=False,
  )
  parser.add_argument(
    '--build-directory', type=str, help='Build directory', required=True
  )
  parser.add_argument(
  ...
  parser.add_argument(
    '--port', type=int, default=8000, help='Port to use for the HTTP server.'
  )
  return parser
...
def main():
  ...
  page_to_object_files = CodePagesToObjectFiles(
    object_files_symbols, page_to_symbols
  )
  ...
  if args.reached_symbols_file:
    ...
    WriteReachedData(
      os.path.join(args.output_directory, 'reached.json'), reached_data
    )
  ...
  if args.start_server:
    ...
    httpd = socketserver.TCPServer(
      ('', args.port), server.SimpleHTTPRequestHandler
    )
