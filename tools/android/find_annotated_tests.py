...
def _GetTests(test_apks, apk_output_dir):
  ...
  for test_apk in test_apks:
    ...
    test_jar = os.path.join(
      apk_output_dir, constants.SDK_BUILD_TEST_JAVALIB_DIR, '%s.jar' % test_apk
    )
...
def _GetReportMeta(utc_script_runtime_string, total_test_count):
  ...
  if time_string_search is None:
    raise Exception(
      'Timestamp format incorrect, expected all digits, got %s' % raw_string
    )
...
def _GetReport(test_apks, script_runtime_string, apk_output_dir):
  ...
  report_data = {'metadata': report_meta, 'tests': test_data}
  return report_data
...
def main():
  ...
  parser.add_argument(
    '--json-output-dir', required=True, help='JSON file output dir'
  )
  parser.add_argument(
    '--apk-output-dir', required=True, help='The output directory of test apks'
  )
  parser.add_argument(
    '--timestamp-string',
  ...
  parser.add_argument(
    '-v', '--verbose', action='store_true', default=False, help='INFO verbosity'
  )
  ...
  apk_output_dir = os.path.abspath(
    os.path.join(constants.DIR_SOURCE_ROOT, arguments.apk_output_dir)
  )
  ...
  json_output_path = os.path.join(
    arguments.json_output_dir, '%s-android-chrome.json' % script_runtime_string
  )
