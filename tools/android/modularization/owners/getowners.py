...
def main():
  ...
  required_arg_group.add_argument(
    '--git-dir', required=True, help='Root directory to search for owners.'
  )
  required_arg_group.add_argument(
    '-o', '--output', required=True, help='File to write the result json to.'
  )
  ...
  if arguments.no_cache:
    ...
    if arguments.cache_dir is None:
      arguments.cache_dir = os.path.join(
        chromium_root, 'out', 'getowners_cache'
      )
  ...
  paths_to_search = owners_input.get_android_folders(
    chromium_root, arguments.limit_to_dir
  )
...
def _process_requested_path(
  ...
  dir_metadata = owners_dir_metadata.build_dir_metadata(
    all_dir_metadata, requested_path
  )
...
def _fetch_git_data(
  ...
  start_time = time.time()
  git_log = owners_git.get_log(
    chromium_root, requested_path.path, 182, follow, cache_dir
  )
  elapsed_time = time.time() - start_time
  ...
  for commit_msg in git_log.split(line_delimiter):
    author_re = re.search(
      author_search, commit_msg, re.IGNORECASE | re.MULTILINE
    )
    if author_re:
    ...
    reviewer_re = re.findall(
      reviewer_search, commit_msg, re.IGNORECASE | re.MULTILINE
    )
  ...
  git_data.number_of_files = owners_git.get_total_files(
    chromium_root, requested_path.path
  )
...
def _build_owners_info(
  chromium_root: str, owners_filepath: str
) -> owners_data.Owners:
  '''Creates a synthetic representation of an OWNERS file.'''

  if not owners_filepath:
    return None
...
def _propagate_down_owner_variables(
  chromium_root: str, owners: owners_data.Owners
) -> None:
