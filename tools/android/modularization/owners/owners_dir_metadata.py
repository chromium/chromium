...
def build_dir_metadata(
  all_dir_metadata: Dict, requested_path: owners_data.RequestedPath
) -> owners_data.DirMetadata:
  '''Creates a synthetic representation of an DIR_METADATA file.'''
  return _build_dir_metadata_recursive(
    all_dir_metadata, pathlib.Path(requested_path.path)
  )
...
def _build_dir_metadata_recursive(
  all_dir_metadata: Dict, path: pathlib.Path
) -> owners_data.DirMetadata:
  # Use memoized value
  ...
  if len(path.parts) > 1:
    parent_dir_metadata = _build_dir_metadata_recursive(
      all_dir_metadata, path.parent
    )
