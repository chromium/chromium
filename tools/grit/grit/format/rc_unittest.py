...
END'''.strip()
  ...
  def testRcIncludeFile(self):
    ...
    output = util.StripBlankLinesAndComments(buf.getvalue())
    expected = (
      _PREAMBLE + 'TEXT_ONE           TXT                "%s"\n'
      'TEXT_TWO           TXT                "%s"'
      % (util.normpath('/temp/bingo.txt').replace('\\', '\\\\'), 'bingo2.txt')
    )
    # hackety hack to work on win32&lin
    output = re.sub(r'"[c-zC-Z]:', '"', output)
    self.assertEqual(expected, output)
  ...
  def testRcIncludeFlattenedHtmlFile(self):
    ...
    build.RcBuilder.ProcessNode(
      root, DummyOutput('rc_all', 'en', output_file), buf
    )
    output = util.StripBlankLinesAndComments(buf.getvalue())
  ...
  def testSubstitutionHtml(self):
    ...
    self.assertTrue(
      ar_file == os.path.join(output_dir, 'ar_toolbar_about.html')
    )
