...
class DagCoarseIdAssigner(BaseCoarseIdAssigner):
  ...
  Start ID assignment in resource_ids is structured a Series-Parallel Graph,
  which
  ...
  * Operation 2: A <-- B becomes A <== B (add parallel edge).
  ...
  Each vertex (A, B, ...) is a start ID. S = globally minimal ID. T = \infty
  is an
  ...
  (3) When vertex A has its ID assigned, the weight of each edge "A <--"
  (i.e., an
      item) can have its weight (ID allotment) computed via quota assignment
      of A.
  ...
  For (1a), we start with DFS traversal of the (transposed, i.e., reversed)
  graph
  ...
  * Traverse a B <-- Y only when all X <-- B have been traversed.
  ...
  The resulting sequence has the length as the number of edges, and has the
  useful
  ...
  Here, "X|Y" denotes backtracking between visiting X and visiting Y. This
  appears
  ...
  * Operation 2: "...AB..." => "...A|AB..." (or "...A" => "...A|A").
  ...
  For (1b), we wish to build the graph from the sequence. This requires (1a)
  to be
