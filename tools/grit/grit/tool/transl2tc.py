...
class TranslationToTc(interface.Tool):
  ...
  translation of precisely the source RC file (not of an older or newer
  version).
  ...
  tool) to specify a file containing a list of message IDs to which output
  should
  ...
  def Run(self, globopt, args):
    ...
    if len(args) != 3:
      self.Out(
        'This tool takes exactly three arguments:\n'
    ...
    translations = self.ExtractTranslations(
      grd, source_rc, args[0], transl_rc, args[1]
    )
  ...
  def ExtractTranslations(
    ...
    for source_node in source_grd:
      ...
      assert 'name' in source_node.attrs, (
        'All nodes with cliques should have an ID'
      )
          ...
          if source_msg.GetRealContent() != current_msg.GetRealContent():
              ...
              for ix in range(len(transl_content)):
                if isinstance(transl_content[ix], tclib.Placeholder):
                  if not isinstance(current_content[ix], tclib.Placeholder):
                    ...
                    break
                  if (
                    transl_content[ix].GetOriginal()
                    != current_content[ix].GetOriginal()
                  ):
                    ok_to_fixup = (
                      False  # placeholders have likely been reordered
                    )
                    break
            ...
            if not ok_to_fixup:
              ...
              def Fixup(content, ix):
                if isinstance(content[ix], tclib.Placeholder) and content[
                  ix
                ].GetPresentation().startswith('TODO_'):
      ...
      for ix in range(len(transl_cliques)):
        ...
        # needed to refresh ID based on new placeholders
        source_msg.GenerateId()
        message_id = source_msg.GetId()
        translated_content = (
          transl_cliques[ix].GetMessage().GetPresentableContent()
        )
        ...
        if message_id in id2transl:
          ...
          if existing_translation != translated_content:
            original_text = (
              source_cliques[ix].GetMessage().GetPresentableContent()
            )
            self.Out(
              'Warning: Two different translations for "%s":\n'
              '  Translation 1: "%s"\n'
              '  Translation 2: "%s"\n'
    ...
    for message_id in list(id2transl.keys()):
      if (
        message_id not in current_message_ids
        or not current_grd.UberClique().BestClique(message_id).IsTranslateable()
        or (self.limits and message_id not in self.limits)
      ):
