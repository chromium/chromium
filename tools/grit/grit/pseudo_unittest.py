...
class PseudoUnittest(unittest.TestCase):
  ...
  def testConsecutiveVowels(self):
    ...
    self.assertTrue(
      out
      == pseudo.MapVowels("beauPeautiPifuPul weaPeathePer, aiPain't iPit?", 1)
    )

  def testCapitals(self):
    out = pseudo.PseudoString("HOWDIE DOODIE, DR. JONES")
    self.assertTrue(
      out == pseudo.MapVowels("HOPOWDIEPIE DOOPOODIEPIE, DR. JOPONEPES", 1)
    )
  ...
  def testPseudoMessage(self):
    ...
    self.assertTrue(
      ...
      == pseudo.MapVowels('HePelloPo USERNAME, hoPow aParePe youPyou?', 1)
    )
