...
class MessageCliqueUnittest(unittest.TestCase):
  def testClique(self):
    ...
    factory.FindCliqueAndAddTranslation(
      msg_de_f, 'de', constants.GENDER_FEMININE
    )
    ...
    self.assertIsNotNone(
      c.AllMessagesThatMatch(rex, True)[
        (pseudo.PSEUDO_LANG, constants.DEFAULT_GENDER)
      ]
    )
...
<grit latest_public_release="2" source_lang_id="en-US" current_release="3" base_dir=".">
  ...
  def testSemiIdenticalCliques(self):
    ...
    self.assertTrue(
      cliques[0]
      .MessageForLanguageAndGender('fr', constants.DEFAULT_GENDER)
      .GetRealContent()
      == 'Bonjour $1'
    )
    self.assertTrue(
  ...
  def testMissingTranslations(self):
    ...
    cliques[3].AddTranslation(
      tclib.Translation(
        text='Buongiorno', id=messages[3].GetId(), placeholders=[]
      ),
      'it',
      constants.DEFAULT_GENDER,
    )

    cliques[1].MessageForLanguageAndGender(
      'fr', constants.DEFAULT_GENDER, False, True
    )
    cliques[2].MessageForLanguageAndGender(
      'es', constants.DEFAULT_GENDER, False, True
    )

    # Non-default genders can still fall back to English
    cliques[2].MessageForLanguageAndGender(
      'es', constants.GENDER_MASCULINE, False, True
    )

    cliques[3].MessageForLanguageAndGender(
      'it', constants.DEFAULT_GENDER, False, False
    )

    # Non-default genders can fall back to the default constants without an error
    # or warning.
    cliques[3].MessageForLanguageAndGender(
      'it', constants.GENDER_MASCULINE, False, False
    )

    self.assertFalse(factory.HasMissingTranslations())

    cliques[0].MessageForLanguageAndGender(
      'de', constants.DEFAULT_GENDER, False, False
    )
    cliques[0].MessageForLanguageAndGender(
      'de', constants.GENDER_FEMININE, False, False
    )
    ...
    self.assertEqual(
      report.count('gender fallback to default gender example'), 0
    )
  ...
  def testBestCliqueSortIsStable(self):
    ...
    clique_no_description = factory.MakeClique(
      msg_no_description, translateable=True
    )
    self.assertTrue(factory.BestClique(clique_id) == clique_no_description)
    clique_id_description_b = factory.MakeClique(
      msg_id_description_b, translateable=True
    )
    self.assertTrue(factory.BestClique(clique_id) == clique_id_description_b)
    clique_id_description_a = factory.MakeClique(
      msg_id_description_a, translateable=True
    )
    self.assertTrue(factory.BestClique(clique_id) == clique_id_description_a)
    clique_description_y = factory.MakeClique(
      msg_description_y, translateable=True
    )
    self.assertTrue(factory.BestClique(clique_id) == clique_description_y)
    clique_description_x = factory.MakeClique(
      msg_description_x, translateable=True
    )
