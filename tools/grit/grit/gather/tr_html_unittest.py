...
class ParserUnittest(unittest.TestCase):
  ...
  def VerifyChunkingBasic(self, fold_whitespace):
    ...
    chunks = p.Parse(
      '<p>Hello <b>dear</b> how <i>are</i>you?<p>Fine!', fold_whitespace
    )
    self.assertEqual(
      chunks,
      [
        (False, '<p>', ''),
        (True, 'Hello <b>dear</b> how <i>are</i>you?', ''),
        (False, '<p>', ''),
        (True, 'Fine!', ''),
      ],
    )
    ...
    chunks = p.Parse(
      '<p> Hello <b>dear how <i>are you? <p> Fine!', fold_whitespace
    )
    self.assertEqual(
      chunks,
      [
        (False, '<p> ', ''),
        (True, 'Hello <b>dear how <i>are you?', ''),
        (False, ' <p> ', ''),
        (True, 'Fine!', ''),
      ],
    )
    ...
    chunks = p.Parse(
      '<b>Hello!</b> how are you?<p><i>I am fine.</i>', fold_whitespace
    )
    self.assertEqual(
      chunks,
      [
    ...
    chunks = p.Parse(
      "Hello! How are <b>you?</b><p><i>I'm fine!</i>", fold_whitespace
    )
    self.assertEqual(
      chunks,
      [
        (True, 'Hello! How are <b>you?</b>', ''),
        (False, '<p>', ''),
        (True, "<i>I'm fine!</i>", ''),
      ],
    )
  ...
  def VerifyChunkingDescriptions(self, fold_whitespace):
    ...
    chunks = p.Parse(
      '<!-- desc=bingo! --><B>Hello!</B> how are you?<P><I>I am fine.</I>',
      fold_whitespace,
    )
    self.assertEqual(
      chunks,
      [
        (True, '<B>Hello!</B> how are you?', 'bingo!'),
        (False, '<P>', ''),
        (True, '<I>I am fine.</I>', ''),
      ],
    )
    chunks = p.Parse(
      '<B><!-- desc=bingo! -->Hello!</B> how are you?<P><I>I am fine.</I>',
      fold_whitespace,
    )
    self.assertEqual(
      chunks,
    ...
    chunks = p.Parse(
      '<B>Hello!</B> how are you?<!-- desc=bingo! --><P><I>I am fine.</I>',
      fold_whitespace,
    )
    self.assertEqual(
      chunks,
  ...
  def VerifyChunkingReplaceables(self, fold_whitespace):
    ...
    p = tr_html.HtmlChunks()
    chunks = p.Parse(
      '<b>Hello!</b> how are you?<p [BINGO] [$~BONGO~$]><i>I am fine.</i>',
      fold_whitespace,
    )
    self.assertEqual(
      chunks,
  ...
  def VerifyChunkingMessageBreak(self, fold_whitespace):
    ...
    chunks = p.Parse(
      'Break<!-- message-break --> apart <!--message-break-->messages',
      fold_whitespace,
    )
    self.assertEqual(
      chunks,
    ...
    chunks = p.Parse(
      '<a href=\'google.com\'><!-- message-break -->Google'
      '<!--message-break--></a>',
      fold_whitespace,
    )
    self.assertEqual(
  ...
  def VerifyChunkingMessageNoBreak(self, fold_whitespace):
    ...
    chunks = p.Parse(
      'Please <!-- message-no-break --> <br />don\'t break', fold_whitespace
    )
    self.assertEqual(
      chunks,
      [(True, 'Please <!-- message-no-break --> <br />don\'t break', '')],
    )

    chunks = p.Parse(
      'Please <br /> break. <!-- message-no-break --> <br /> '
      'But not this time.',
      fold_whitespace,
    )
    self.assertEqual(
      ...
      [
        ...
        (
          True,
          'break. <!-- message-no-break --> <br /> But not this time.',
          '',
        ),
      ],
    )
  ...
  def testTranslateableAttributes(self):
    ...
    chunks = p.Parse(
      '<img src=bingo.jpg alt="hello there">'
    ...
    self.assertEqual(
      chunks,
      [
        (False, '<img src=bingo.jpg alt="', ''),
        (True, 'hello there', ''),
        (False, '"><input type=submit value="', ''),
        (True, 'hello', ''),
  ...
  def testTranslateableHtmlToMessage(self):
    ...
    self.assertTrue(
      pres == 'Hello BEGIN_BOLDX_USERNAME_XEND_BOLD, '
    ...
    pres = msg.GetPresentableContent()
    self.assertTrue(
      pres == 'BEGIN_FONT_1BEGIN_FONT_2Update!END_FONT_2 BEGIN_LINK'
    ...
    pres = msg.GetPresentableContent()
    self.assertTrue(
      pres == '''BEGIN_FONTBEGIN_LINKDesktopEND_LINKEND_FONTSPACE'''
    )
    ...
    pres = msg.GetPresentableContent()
    self.assertTrue(
      pres == 'BEGIN_BREAK_1BEGIN_BREAK_2BEGIN_CENTERBEGIN_FONT\xa92005'
    ...
    pres = msg.GetPresentableContent()
    self.assertTrue(pres == '&nbsp;-&nbsp;BEGIN_LINKCachedEND_LINK')
    ...
    pres = msg.GetPresentableContent()
    self.assertTrue(
      pres == 'You can read the BEGIN_LINK_1Privacy PolicyEND_LINK_1 and '
...
and BEGIN_LINK_2Privacy FAQEND_LINK_2 online.'''
    ...
    msg = tr_html.HtmlToMessage(
      '''Please <!-- message-no-break --><br /> don't break'''
    )
    pres = msg.GetPresentableContent()
...
class TrHtmlUnittest(unittest.TestCase):
  ...
  def testFoldWhitespace(self):
    ...
    html.Parse()
    self.assertEqual(
      html.skeleton_[1].GetMessage().GetPresentableContent(), 'Test  Message'
    )
...
<a href="http://desktop.google.com/preferences.html">Preferences&nbsp;Help</a>
    ...
    html.Parse()
    self.assertTrue(
      html.skeleton_[3].GetMessage().GetPresentableContent()
...
name=submit2></td>
    ...
    html.Parse()
    self.assertTrue(
      html.skeleton_[1].GetMessage().GetPresentableContent()
      == 'Save Preferences'
    )
  ...
  def testWhitespaceAfterInlineTag(self):
    ...
    html.Parse()
    self.assertTrue(
      html.skeleton_[1].GetMessage().GetRealContent()
...
<p>Other stuff</p>''')
  ...
  def testPreferencesHtml(self):
    ...
    for item in [
      x for x in html.skeleton_ if isinstance(x, clique.MessageClique)
    ]:
      if (
        item.GetMessage().GetRealContent() == '[STATUS-MESSAGE]'
        or item.GetMessage().GetRealContent() == '[ADDIN-DO] [ADDIN-OPTIONS]'
      ):
        self.fail()
  ...
  def testFewTranslateables(self):
    self.AssertNumberOfTranslateables(
      [
        'browser.html',
        'email_thread.html',
        'header.html',
  ...
  def testRegressionCjkHtmlFile(self):
    ...
    if False:
      html = self.HtmlFromFileWithManualCheck(
        util.PathFromRoot(r'grit/testdata/ko_oem_enable_bug.html')
      )
