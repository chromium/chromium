...
class IncludeNodeUnittest(unittest.TestCase):
  def testGetPath(self):
    ...
    self.assertEqual(
      root.ToRealPath(include_node.GetInputPath()),
  ...
  def testGetPathNoBasedir(self):
    ...
    expected_path = util.normpath(
      os.path.join('..', last_dir, 'flugel/kugel.pdf')
    )
    self.assertEqual(
      root.ToRealPath(include_node.GetInputPath()), expected_path
    )
