...
def CreateCommand(cmdline):
  ...
  cmd.AddArgument(
    ["-b", "--browsers"],
    "List of browsers to use. Comma-separated",
    type="string",
    required=True,
  )
  cmd.AddArgument(
    ["-bp", "--browserpaths"],
    "List of paths to browsers. Comma-separated",
    type="string",
    required=False,
  )
  cmd.AddArgument(
  ...
  cmd.AddArgument(["-u", "--url"], "URL to time")
  cmd.AddArgument(["-l", "--list"], "List of URLs to time", type="readfile")
...
def ExecuteTimeLoad(command):
  ...
  if len(browser_versions) != num_browsers:
    raise ValueError("--browserversions must be same length as --browser_paths")
  ...
  for b in xrange(num_browsers):
    ...
    if not scraper:
      raise ValueError(
        "could not find scraper for (%r, %r)" % (browsers[b], version)
      )
  ...
  for b in xrange(num_browsers):
    ...
    if browser_versions[b]:
      log_file.write(" %s" % browser_versions[b])
  log_file.write("\n")
  ...
  for b in xrange(num_browsers):
    result = scraper_list[b].Time(
      url_list, command["--size"], command["--timeout"], path=browser_paths[b]
    )
