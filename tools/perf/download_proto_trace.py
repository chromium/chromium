...
def Main():
  ...
  parser = argparse.ArgumentParser(
    description='Downloads the proto trace for an cloud storage html trace url.'
  )
  parser.add_argument(
    'html_trace_url', type=str, help='Looks like %s' % EXAMPLE_TRACE
  )
  parser.add_argument(
    '--download-dir',
    type=str,
  ...
  path = trace_downloader.DownloadProtoTrace(
    args.html_trace_url, download_dir=args.download_dir
  )
