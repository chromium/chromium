...
class Screenshot(legacy_page_test.LegacyPageTest):
  ...
  def ValidateAndMeasurePage(self, page, tab, results):
    try:
      ...
      logging.warning(
        "WaitForDocumentReadyStateToBeComplete() timeout, page: %s", page.name
      )
      return
    ...
    outpath = (
      os.path.abspath(os.path.join(self._png_outdir, page.file_safe_name))
      + '.png'
    )
    ...
    if self._dc_detect:
      for i in range(self._dc_extra_screenshots):
        ...
        logging.info(
          "Taking extra screenshot %d of %d.", i + 1, self._dc_extra_screenshots
        )
    ...
    image = image_util.FromRGBPixels(image_width, image_height, content_pixels)
    ...
    logging.info(
      "PNG file written successfully. (Took %f seconds)", time.time() - start
    )


def IsScreenshotWithinDynamicContentThreshold(
  base_screenshot,
  next_screenshot,
  content_pixels,
  num_total_pixels,
  ...
  while i < len(base_pixels):
    ...
    if (
      base_pixels[i] != next_pixels[i]
      or base_pixels[i + 1] != next_pixels[i + 1]
      or base_pixels[i + 2] != next_pixels[i + 2]
    ):
      content_pixels[i] = 0
  ...
  if (num_dynamic_pixels / num_total_pixels) >= dynamic_content_threshold:
    logging.error(
      "Percentage of pixels with dynamic content: %f",
