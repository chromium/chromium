...
CONFIG_FORMAT = (
  'Config must contain "v2_metric", "v3_metric", and "histogram_mappings."'
)
...
def CheckConfig(simple_config):
  ...
  if 'histogram_mappings' not in config:
    raise Exception(
      'config %s missing "histogram_mappings"\n%s' % (name, CONFIG_FORMAT)
    )
...
def OptionalGetHistogram(histogram_set, name, metric, version):
  ...
  if len(hists) > 1:
    raise Exception(
      'Multiple histograms named %s found for TBM%s metric %s'
      % (name, version, metric)
    )
  return hists[0]
...
def CompareSimpleHistograms(test_ctx, config, v2_histograms, v3_histograms):
  ...
  for v2_hist_name, v3_hist_info in config['histogram_mappings'].items():
    ...
    if v3_hist is None:
      ...
      raise Exception(
        'Histogram %s not produced by v3 metric\n%s' % (v3_hist_name, msg)
      )

    try:
      utils.AssertHistogramStatsAlmostEqual(
        test_ctx, v2_hist, v3_hist, precision
      )
      utils.AssertHistogramSamplesAlmostEqual(
        test_ctx, v2_hist, v3_hist, precision
      )
