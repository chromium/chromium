...
class DesktopPowerStorySet(story.StorySet):
  def __init__(self, shared_state=shared_page_state.SharedPageState):
    ...
    self.AddStory(
      trivial_sites.TrivialScrollingPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialBlinkingCursorPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialCanvasPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialWebGLPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialBlurAnimationPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialFullscreenVideoPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialGifPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
    self.AddStory(
      trivial_sites.TrivialAnimationPage(
        self, shared_state, wait_in_seconds=30, measure_memory=False
      )
    )
