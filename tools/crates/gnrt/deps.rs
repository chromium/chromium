...
mod tests {
    ...
    fn test_sample_output() {
        ...
        assert_eq!(dependencies[0].package_name, "cc");
        assert_eq!(dependencies[0].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[0].dependency_kinds.get(&DependencyKind::Build).unwrap().features,
            empty_str_slice
        );

        assert_eq!(dependencies[1].package_name, "more-asserts");
        assert_eq!(dependencies[1].epoch, Epoch::Minor(3));
        assert_eq!(
            dependencies[1].dependency_kinds.get(&DependencyKind::Development).unwrap().features,
            empty_str_slice
        );

        assert_eq!(dependencies[2].package_name, "once_cell");
        assert_eq!(dependencies[2].epoch, Epoch::Major(1));
        assert_eq!(
        ...
        assert_eq!(dependencies[3].package_name, "proc-macro2");
        assert_eq!(dependencies[3].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[3].dependency_kinds.get(&DependencyKind::Normal).unwrap().features,
            &["default", "proc-macro"]
        );

        assert_eq!(dependencies[4].package_name, "quote");
        assert_eq!(dependencies[4].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[4].dependency_kinds.get(&DependencyKind::Normal).unwrap().features,
            &["default", "proc-macro"]
        );

        assert_eq!(dependencies[5].package_name, "serde");
        assert_eq!(dependencies[5].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[5].dependency_kinds.get(&DependencyKind::Normal).unwrap().features,
            &["default", "derive", "serde_derive", "std"]
        );

        assert_eq!(dependencies[6].package_name, "serde_derive");
        assert_eq!(dependencies[6].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[6].dependency_kinds.get(&DependencyKind::Normal).unwrap().features,
            &["default"]
        );

        assert_eq!(dependencies[7].package_name, "syn");
        assert_eq!(dependencies[7].epoch, Epoch::Major(1));
        assert_eq!(
        ...
        assert_eq!(dependencies[8].package_name, "termcolor");
        assert_eq!(dependencies[8].epoch, Epoch::Major(1));
        assert_eq!(
            dependencies[8].dependency_kinds.get(&DependencyKind::Normal).unwrap().features,
            empty_str_slice
        );

        assert_eq!(dependencies[9].package_name, "unicode-ident");
        assert_eq!(dependencies[9].epoch, Epoch::Major(1));
        assert_eq!(
