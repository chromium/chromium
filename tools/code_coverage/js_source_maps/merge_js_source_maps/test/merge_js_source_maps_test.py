...
_SOURCE_MAP_TRANSLATOR = (
  _HERE_DIR.parent.parent
  / 'create_js_source_maps'
  / 'test'
  / 'translate_source_map.js'
).resolve()
...
class MergeSourceMapsTest(unittest.TestCase):
  ...
  def _writeResponseFileContents(self, sources, outputs, manifest_file):
    with tempfile.NamedTemporaryFile(
      mode='w+', dir=self._out_folder, delete=False
    ) as response_file:
      if manifest_file:
  ...
  def testMergingTwoSourcemaps(self):
    ...
    if num_sourcemaps != 1:
      self.fail(
        'Number of sourcemaps invalid, got: %d, want: 1' % num_sourcemaps
      )
    ...
    with open(
      os.path.join(_HERE_DIR, original_file_name), encoding='utf-8'
    ) as f:
      ...
      for line_num in range(len(original_file_lines)):
        ...
        if stripped_original_line == "// Copyright 2022 The Chromium Authors":
          stripped_original_line = (
            stripped_original_line + ". All rights reserved."
          )
  ...
  def testManifestFilesAreRewritten(self):
    ...
    input_fd, manifest_file = tempfile.mkstemp(
      dir=self._out_folder, text=True, suffix=".json"
    )
