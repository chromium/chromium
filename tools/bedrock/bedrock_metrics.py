...
def count_matching_files(
  ...
  for filepath in items_to_scan:
    ...
    if include_filename_strings:
      if not all(
        inc_str in filename_only for inc_str in include_filename_strings
      ):
...
def main():
  ...
  parser.add_argument(
    "build_directory", type=str, help="Build directory relative to src."
  )
  parser.add_argument(
  ...
  commit_hash = subprocess.run(
    "git rev-parse HEAD", shell=True, capture_output=True, text=True, check=True
  ).stdout.strip()
  ...
  unittest_total, _ = count_matching_files(
    abs_cb_directory, None, None, ["unittest"], None
  )
  ...
  production_total, _ = count_matching_files(
    abs_cb_directory, None, None, None, ["test"]
  )
  ...
  browser_owned_data = count_browser_owned_objects(
    abs_cb_directory
  ) + count_browser_user_data(abs_cb_directory)
