...
FILES_CFG = os.path.join(
  REPO_ROOT, 'chrome', 'tools', 'build', 'win', 'FILES.cfg'
)
...
def IsPEFile(path):
  return (
    os.path.isfile(path)
    and os.path.splitext(path)[1].lower() in PE_FILE_EXTENSIONS
    and os.path.basename(path) not in EXCLUDED_FILES
  )
...
def main(options, args):
  ...
  for file in os.listdir(directory):
    ...
    if (
      pe.OPTIONAL_HEADER.DllCharacteristics & NO_SEH_FLAG
      or (
      ...
      or pe.FILE_HEADER.Machine in (MACHINE_TYPE_AMD64, MACHINE_TYPE_ARM64)
    ):
      if options.verbose:
    ...
    if pe.FILE_HEADER.Machine in (MACHINE_TYPE_AMD64, MACHINE_TYPE_ARM64):
      if pe.OPTIONAL_HEADER.ImageBase <= 0xFFFFFFFF:
        print(
          "Checking %s ImageBase (0x%X < 4GB)... FAIL"
          % (path, pe.OPTIONAL_HEADER.ImageBase)
        )
        success = False
      elif options.verbose:
        print(
          "Checking %s ImageBase (0x%X > 4GB)... PASS"
          % (path, pe.OPTIONAL_HEADER.ImageBase)
        )
    ...
    if pe.FILE_HEADER.Machine == MACHINE_TYPE_AMD64:
      if IsInDefaultFileGroup(path) and IsCetExpected(path):
        ...
        for dbg_ent in pe.DIRECTORY_ENTRY_DEBUG:
          if (
            dbg_ent.struct.Type
            == pefile.DEBUG_TYPE['IMAGE_DEBUG_TYPE_EX_DLLCHARACTERISTICS']
          ):
            ...
            ex_dll_char_data = pe.__data__[
              ex_dll_offset : ex_dll_offset + ex_dll_length
            ]
