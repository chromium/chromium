...
class IDLParser(IDLLexer):
  ...
  def p_top_short(self, p):
    ...
    Filedoc = IDLNode(
      'Comment',
      self.lexobj.filename,
      p.lineno(2) - 1,
      p.lexpos(2) - 1,
      [self.BuildAttribute('NAME', ''), self.BuildAttribute('FORM', 'cc')],
    )
  ...
  def __init__(self):
    ...
    self.yaccobj = yacc.yacc(
      module=self, tabmodule=None, debug=False, optimize=0, write_tables=0
    )
...
def TestErrors(filename, filenode):
  ...
  if obj_cnt != pass_cnt:
    InfoOut.Log(
      "Mismatched pass (%d) vs. nodes built (%d)." % (pass_cnt, obj_cnt)
    )
  ...
  for i in range(pass_cnt):
    ...
    if obj_list[i] != comment:
      ErrOut.LogLine(
        filename, line, None, "OBJ %s : EXPECTED %s\n" % (obj_list[i], comment)
      )
      errors += 1
  ...
  if err_cnt != fail_cnt:
    InfoOut.Log(
      "Mismatched fail (%d) vs. errors seen (%d)." % (fail_cnt, err_cnt)
    )
...
def FindVersionError(releases, node):
  ...
  if node.IsA('Interface', 'Struct'):
    ...
    if first_list != comment_list:
      node.Error("Mismatch in releases: %s vs %s." % (comment_list, first_list))
      err_cnt += 1
