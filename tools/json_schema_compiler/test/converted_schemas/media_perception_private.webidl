...
dictionary State {
  ...
  sequence<VideoStreamParam> videoStreamParam;
  ...
  sequence<NamedTemplateArgument> namedTemplateArguments;
};
...
enum DistanceUnits { "UNSPECIFIED", "METERS", "PIXELS" };
...
dictionary FramePerception {
  ...
  sequence<PacketLatency> packetLatency;
  ...
  sequence<FramePerceptionType> framePerceptionTypes;
};
...
enum HotwordType { "UNKNOWN_TYPE", "OK_GOOGLE" };
...
dictionary MediaPerception {
  ...
  sequence<FramePerception> framePerceptions;

  // An array of audio perceptions.
  sequence<AudioPerception> audioPerceptions;

  // An array of audio-visual perceptions.
  sequence<AudioVisualPerception> audioVisualPerceptions;
...
dictionary Diagnostics {
  ...
  sequence<PerceptionSample> perceptionSamples;
};

// Listener callback for the onMediaPerception event.
...
// streams.
callback OnMediaPerceptionListener = undefined(MediaPerception mediaPerception);

interface OnMediaPerceptionEvent : ExtensionEvent {
  static undefined addListener(OnMediaPerceptionListener listener);
  static undefined removeListener(OnMediaPerceptionListener listener);
  static boolean hasListener(OnMediaPerceptionListener listener);
};

// Private API for receiving real-time media perception information.
[platforms = ("chromeos")] interface MediaPerceptionPrivate {
  ...
  // analytics process.
  static attribute OnMediaPerceptionEvent onMediaPerception;
};

partial interface Browser {
  static attribute MediaPerceptionPrivate mediaPerceptionPrivate;
};
