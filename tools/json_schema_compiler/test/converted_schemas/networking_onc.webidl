...
enum ClientCertificateType { "Ref", "Pattern" };
...
dictionary EAPProperties {
  ...
  required ClientCertificateType ClientCertType;
  DOMString Identity;
  DOMString Inner;
...
dictionary ManagedCellularProperties {
  ...
  sequence<CellularProviderProperties> HomeProvider;
...
dictionary WiFiProperties {
  [deprecated = "Removed in M131. There is no active ARP polling now."] boolean
...
// The <code>chrome.networking.onc</code> API is used for configuring network
// connections (Cellular, Ethernet, VPN or WiFi). This API is available in
// auto-launched Chrome OS kiosk sessions.
//
// Network connection configurations are specified following <a
...
//
// <b>NOTE</b>: Most dictionary properties and enum values use UpperCamelCase to
// match the ONC specification instead of the JavaScript lowerCamelCase
// convention.
interface ONC {
...
partial interface Networking {
  static attribute ONC onc;
};

partial interface Browser {
  static attribute Networking networking;
};
