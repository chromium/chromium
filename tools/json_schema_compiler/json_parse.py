...
try:
  ...
  def Parse(json_str):
    return json.loads(
      json_comment_eater.Nom(json_str), object_pairs_hook=OrderedDict
    )
  ...
  _SYS_PATH = sys.path[:]
  try:
    _SIMPLE_JSON_PATH = os.path.join(
      _FILE_PATH, os.pardir, os.pardir, 'third_party'
    )
    ...
    sys.path.insert(
      0,
      os.path.join(
        _FILE_PATH, os.pardir, os.pardir, 'third_party', 'json_schema_compiler'
      ),
    )
    import simplejson
