...
class _Generator(object):
  ...
  def _GenerateType(self, cpp_namespace, type_):
    ...
    if type_.functions:
      ...
      (
        c.Append(
          '%s::%s(%s&& rhs) noexcept = default;'
          % (classname_in_namespace, classname, classname)
        ).Append(
          '%s& %s::operator=(%s&& rhs) noexcept = default;'
          % (classname_in_namespace, classname_in_namespace, classname)
        )
      )
  ...
  def _GenerateInitializersAndBody(self, type_):
    ...
    for prop in type_.properties.values():
      ...
      if real_t.property_type == PropertyType.ENUM:
        ...
        t.property_type == PropertyType.ANY
  ...
  def _GenerateCloneItem(self, type_, field_name, is_optional):
    ...
    if (
      type_.property_type == PropertyType.ARRAY
      ...
      if is_optional:
        (
          ...
          .Append(
            self._util_cc_helper.AppendToContainer(
  ...
  def _GenerateTypePopulate(self, cpp_namespace, type_):
    ...
    (
      ...
      .Sblock(
        '    %s) {'
        % self._GenerateParams(('const base::DictValue& dict', '%(name)s& out'))
      )
    )
        ...
        (
          ...
          .Concat(
            self._GeneratePopulateVariableFromValue(
              type_.additional_properties, 'item.second', 'tmp', 'false'
            )
          )
    ...
    (c.Eblock('}').Substitute({'namespace': cpp_namespace, 'name': classname}))
    return c
  ...
  def _GenerateTypePopulateFromValue(self, cpp_namespace, type_):
    ...
    (
      ...
      .Sblock(
        '    %s) {'
        % self._GenerateParams(('const base::Value& value', '%(name)s& out'))
      )
    )
  ...
  def _GenerateTypePopulateProperty(self, prop, src, dst):
    ...
    if prop.optional:
      (
        c.Sblock('if (%(value_var)s) {').Concat(
  ...
  def _GenerateTypeFromValue(self, cpp_namespace, type_, is_dict):
    ...
    c.Append(
      'bool result = Populate(%s);' % self._GenerateArgs(('value', 'out'))
    )
  ...
  def _GenerateParseFromDictionaryForRootManifestType(
    ...
    c.Sblock(
      '%s) {' % self._GenerateParams(params, generate_error_messages=True)
    )
    ...
    c.Eblock('}')
    c.Substitute(
      {'classname_in_namespace': classname_in_namespace, 'classname': classname}
    )
    return c
  ...
  def _GenerateParseFromDictionaryForChildManifestType(
    ...
    # within |params|.
    c.Sblock(
      '%s) {' % self._GenerateParams(params, generate_error_messages=False)
    )
  ...
  def _ParsePropertyFromDictionary(self, property, is_root_manifest_type):
    ...
    if underlying_property_type == PropertyType.ENUM:
      ...
      underlying_item_type
      and underlying_item_type.property_type == PropertyType.ENUM
    ):
      # Array of enums.
      params = get_enum_params(
        underlying_item_type, include_optional_param=False
      )
    ...
    if is_root_manifest_type:
      c.Append(
        '::json_schema_compiler::manifest_parse_util::'
  ...
  def _GenerateObjectTypeToValue(self, cpp_namespace, type_):
    ...
    return c.Append().Append('return to_value_result;').Eblock('}')
  ...
  def _GenerateChoiceTypeToValue(self, cpp_namespace, type_):
    ...
    for choice in type_.choices:
      ...
      (
        ...
        .Append(
          'DCHECK(result.is_none()) << "Cannot set multiple choices for '
          '%s";' % type_.unix_name
        )
        .Cblock(
          self._CreateValueFromType(
            'result = base::Value(%s);', choice.name, choice, choice_var, True
          )
        )
        .Eblock('}')
      )
    (
      c.Append(
        'DCHECK(!result.is_none()) << "Must set at least one choice for '
        '%s";' % type_.unix_name
      )
  ...
  def _GenerateFunction(self, function):
    ...
    if function.returns_async:
      c.Concat(
        self._GenerateAsyncResponseArguments(
          'Results', function.returns_async.params
        )
      )
    ...
    if underlying_type.property_type == PropertyType.ARRAY:
      ...
      if item_type.property_type == PropertyType.ENUM:
        ...
        (
          ...
          .Append(
            '%s.emplace_back(%sToString(it));'
            % (enum_list_var, maybe_namespace)
          )
          .Eblock('}')
        )
  ...
  def _GenerateCreateValueFromType(self, type_, var, is_ptr):
    ...
    underlying_type = self._type_helper.FollowRef(type_)
    if (
      underlying_type.property_type == PropertyType.CHOICES
      ...
      raise NotImplementedError(
        'Conversion of %s to base::Value not implemented' % repr(type_.type_)
      )
  ...
  def _GenerateParamsCheck(self, function, var, failure_value):
    ...
    if num_required == len(function.params):
      ...
      c.Sblock(
        'if (%(var)s.size() < %(required)d || %(var)s.size() > %(total)d) {'
      )
    (
      c.Concat(
        self._AppendError16(
  ...
  def _GenerateFunctionParamsCreate(self, function):
    ...
    (
      c.Append('// static').Sblock(
        'std::optional<Params> Params::Create(%s) {'
    ...
    (
      c.Concat(
        self._GenerateParamsCheck(function, 'args', failure_value)
    ...
    (c.Append().Append('return params;').Eblock('}').Append())
  ...
  def _GenerateFunctionParamsCreateWithExpected(self, function):
    ...
    (
      c.Append('// static').Sblock(
        'base::expected<Params, std::u16string> '
  ...
  def _GeneratePopulatePropertyFromValue(
    self, prop, src_var, dst_class_var, failure_value
  ):
  ...
  def _GeneratePopulateVariableFromValue(
    ...
    if (
      ...
      if is_string_or_function:
        ...
        (
          c.Sblock('if (!temp.has_value()) {').Concat(
            self._AppendError16(
              'u"\'%%(key)s\': expected '
              + '%s, got " + %s'
              % (
      ...
      if is_ptr:
        if cpp_util.ShouldUseStdOptional(underlying_type):
          ...
          c.Append(
            '%(dst_var)s = ' + 'std::make_unique<%(cpp_type)s>(temp.value());'
          )
      else:
        c.Append('%(dst_var)s = *temp;')
      ...
      if is_ptr:
        ...
        (
          ...
          .Append(
            ...
            % self._GenerateArgs(('%(src_var)s.GetDict()', 'temp'))
          )
          .Append('  return %(failure_value)s;')
        ...
        (
          ...
          .Append(
            'if (!%%(cpp_type)s::Populate(%s)) {'
            % self._GenerateArgs(('%(src_var)s.GetDict()', '%(dst_var)s'))
          )
          .Append('  return %(failure_value)s;')
          .Append('}')
        )
      ...
      if item_type.property_type == PropertyType.ENUM:
        c.Concat(
          self._GenerateListValueToEnumArrayConversion(
            item_type, src_var, dst_var, failure_value, is_ptr=is_ptr
          )
        )
      else:
      ...
      if is_ptr:
        (
          ...
          .Append(
            'if (!%%(cpp_type)s::Populate(%s))'
            % self._GenerateArgs(('%(src_var)s', 'temp'))
          )
          .Append('  return %(failure_value)s;')
          .Append('%(dst_var)s = std::move(temp);')
        )
        ...
        (
          c.Append(
            'if (!%%(cpp_type)s::Populate(%s))'
            % self._GenerateArgs(('%(src_var)s', '%(dst_var)s'))
          ).Append('  return %(failure_value)s;')
        )
    elif underlying_type.property_type == PropertyType.ENUM:
      c.Concat(
        self._GenerateStringToEnumConversion(
          underlying_type, src_var, dst_var, failure_value
        )
      )
    elif underlying_type.property_type == PropertyType.BINARY:
      (
  ...
  def _GenerateListValueToEnumArrayConversion(
    self, item_type, src_var, dst_var, failure_value, is_ptr=False
  ):
    ...
    (
      ...
      .Concat(
        self._GenerateStringToEnumConversion(
          item_type, '(it)', 'tmp', failure_value
        )
      )
  ...
  def _GenerateStringToEnumConversion(
    self, type_, src_var, dst_var, failure_value
  ):
    ...
    if type_.namespace != self._namespace:
      namespace = cpp_util.GetCppNamespace(
        type_.namespace.environment.namespace_pattern, type_.namespace.unix_name
      )
      cpp_type_namespace = '%s::' % namespace
    (
      c.Append(
        'const std::string* %s = %s.GetIfString();' % (enum_as_string, src_var)
      )
      ...
      .Sblock(
        ...
        % (
          dst_var,
          self._type_helper.GetEnumDefaultValue(type_, self._namespace),
        )
      )
      .Concat(
        self._AppendError16(
  ...
  def _GenerateEnumToString(self, cpp_namespace, type_):
    ...
    c.Sblock(
      'const char* %sToString(%s enum_param) {' % (maybe_namespace, classname)
    )
    ...
    for enum_value in self._type_helper.FollowRef(type_).enum_values:
      ...
      (
        c.Append(
          'case %s: ' % self._type_helper.GetEnumValue(type_, enum_value)
  ...
  def _GenerateEnumFromString(self, cpp_namespace, type_):
    ...
    c.Sblock(
      '%s%s %sParse%s(std::string_view enum_string) {'
    ...
    for _, enum_value in enumerate(
      ...
      name = enum_value.name
      (
        c.Append('if (enum_string == "%s")' % name).Append(
          '  return %s;' % self._type_helper.GetEnumValue(type_, enum_value)
        )
      )
    (
      c.Append('return %s;' % self._type_helper.GetEnumNoneValue(type_)).Eblock(
        '}'
      )
    )
    return c
  ...
  def _GenerateAsyncResponseArguments(self, function_scope, params):
    ...
    (
      c.Sblock(
        'base::ListValue %(function_scope)sCreate(%(declaration_list)s) {'
      )
      .Append('base::ListValue create_results;')
      .Append(
        'create_results.reserve(%d);' % len(params) if len(params) else ''
      )
    )
    declaration_list = []
    for param in params:
      ...
      c.Cblock(
        self._CreateValueFromType(
          'create_results.Append(%s);', param.name, param.type_, param.unix_name
        )
      )
  ...
  def _GenerateEventNameConstant(self, event):
    ...
    c = Code()
    c.Append(
      'const char kEventName[] = "%s.%s";' % (self._namespace.name, event.name)
    )
    return c
