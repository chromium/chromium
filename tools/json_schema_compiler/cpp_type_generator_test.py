...
class CppTypeGeneratorTest(unittest.TestCase):
  def setUp(self):
    ...
    self.tabs = self.models['tabs'].AddNamespace(
      self.tabs_json[0], 'path/to/tabs.json'
    )
  ...
  def testGenerateIncludesAndForwardDeclarations(self):
    ...
    self.assertEqual(
      '#include <string_view>', manager.GenerateIncludes().Render()
    )
    self.assertEqual(
  ...
  def testGenerateIncludesAndForwardDeclarationsDependencies(self):
    ...
    dependency_tester = m.AddNamespace(
      self.dependency_tester_json[0], 'path/to/dependency_tester.json'
    )
    manager = CppTypeGenerator(
  ...
  def testGetCppTypeSimple(self):
    ...
    self.assertEqual(
      'int', manager.GetCppType(self.tabs.types['Tab'].properties['id'].type_)
    )
    self.assertEqual(
      'TabStatus',
  ...
  def testStringAsType(self):
    manager = CppTypeGenerator(
      self.models.get('font_settings'), _FakeSchemaLoader(None)
    )
    self.assertEqual(
      'std::string',
  ...
  def testArrayAsType(self):
    manager = CppTypeGenerator(
      self.models.get('browser_action'), _FakeSchemaLoader(None)
    )
    self.assertEqual(
      'std::vector<int>',
  ...
  def testGetCppTypeArray(self):
    manager = CppTypeGenerator(
      self.models.get('windows'), _FakeSchemaLoader(None)
    )
    self.assertEqual(
      'std::vector<Window>',
      manager.GetCppType(
    ...
    manager = CppTypeGenerator(
      self.models.get('permissions'), _FakeSchemaLoader(None)
    )
    self.assertEqual(
    ...
    manager = CppTypeGenerator(
      self.models.get('objects_movable'), _FakeSchemaLoader(None)
    )
    self.assertEqual(
  ...
  def testGetCppTypeIncludedRef(self):
    ...
    self.assertEqual(
      'std::vector<tabs::Tab>',
      manager.GetCppType(self.windows.types['Window'].properties['tabs'].type_),
    )
  ...
  def testGetCppTypeWithPadForGeneric(self):
    ...
    self.assertEqual(
      'bool',
      manager.GetCppType(
        self.permissions.functions['contains'].returns_async.params[0].type_
      ),
    )
  ...
  def testHardIncludesForEnums(self):
    ...
    manager = CppTypeGenerator(
      self.models.get('crossref_enums'), _FakeSchemaLoader(m)
    )

    self.assertEqual(
      '#include "path/to/simple_api.h"', manager.GenerateIncludes().Render()
    )
  ...
  def testHardIncludesForEnumArrays(self):
    ...
    manager = CppTypeGenerator(
      self.models.get('crossref_enums_array'), _FakeSchemaLoader(m)
    )
