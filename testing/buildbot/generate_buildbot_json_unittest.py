...
class FakeBBGen(generate_buildbot_json.BBJSONGenerator):
  def __init__(
    ...
    files = {
      ...
      os.path.join(
        pyl_files_dir, 'gn_isolate_map2.pyl'
      ): GPU_TELEMETRY_GN_ISOLATE_MAP,
      os.path.join(
        pyl_files_dir, 'gn_isolate_map_prefix.pyl'
      ): GN_ISOLATE_MAP_PREFIX,
      os.path.join(infra_config_dir, 'generated/project.pyl'): project_pyl,
      os.path.join(
        infra_config_dir, 'generated/luci/luci-milo.cfg'
      ): luci_milo_cfg,
...
class UnitTest(TestCase):
  ...
  def test_name_causes_error(self):
    ...
    fbb.check_input_file_consistency(verbose=True)
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, r'.*\bname field is set\b.*'
    ):
      fbb.check_output_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_isolate_name_causes_error(self):
    ...
    fbb.check_input_file_consistency(verbose=True)
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, r'.*\bisolate_name field is set\b.*'
    ):
      fbb.check_output_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_good_test_suites_are_ok(self):
    fbb = FakeBBGen(
      self.args, FOO_GTESTS_WATERFALL, FOO_TEST_SUITE, LUCI_MILO_CFG
    )
    fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_bad_composition_test_suites_are_caught(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'compound_suites may not refer to.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_composition_test_suites_no_conflicts(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Conflicting test definitions.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_composition_test_suites_no_duplicate_names(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, '.*may not duplicate basic test suite.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_unknown_test_suites_are_caught(self):
    fbb = FakeBBGen(
      self.args, UNKNOWN_TEST_SUITE_WATERFALL, FOO_TEST_SUITE, LUCI_MILO_CFG
    )
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Test suite baz_tests from machine.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_unknown_test_suite_types_are_caught(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Unknown test suite type foo_test_type.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_unrefed_test_suite_caught(self):
    fbb = FakeBBGen(
      self.args, FOO_GTESTS_WATERFALL, UNREFED_TEST_SUITE, LUCI_MILO_CFG
    )
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, '.*unreferenced.*bar_tests.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_gn_isolate_map_with_label_mismatch(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr,
      'key name.*foo_test.*label.*foo_test_tmp.*',
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_gn_isolate_map_using_implicit_gn_name(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr,
      'Malformed.*//chrome/foo_test.*for key.*foo_test.*',
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_ungenerated_output_files_are_caught(self):
    ...
    self.assertRegex(
      joined_lines,
      'File chromium.test.json did not have the following expected contents:.*',
    )
  ...
  def test_nonexistent_removal_raises(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'The following nonexistent machines.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_nonexistent_bot_raises(self):
    fbb = FakeBBGen(
      self.args, UNKNOWN_BOT_GTESTS_WATERFALL, FOO_TEST_SUITE, LUCI_MILO_CFG
    )
    with self.assertRaises(generate_buildbot_json.BBGenErr):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_test_suites_must_be_sorted(self):
    fbb = FakeBBGen(
      self.args, TEST_SUITE_SORTING_WATERFALL, TEST_SUITE_SORTED, LUCI_MILO_CFG
    )
    fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
    ...
    for unsorted in (
      ...
      fbb = FakeBBGen(
        self.args, TEST_SUITE_SORTING_WATERFALL, unsorted, LUCI_MILO_CFG
      )
      with self.assertRaises(generate_buildbot_json.BBGenErr):
        fbb.check_input_file_consistency(verbose=True)
      joined_lines = ' '.join(fbb.printed_lines)
...
class MixinTests(TestCase):
  ...
  def test_unreferenced(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, '.*mixins are unreferenced.*'
    ):
      fbb.check_input_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
  ...
  def test_no_duplicate_keys_basic_test_suite(self):
    fbb = FakeBBGen(
      self.args, FOO_GTESTS_WATERFALL, FOO_TEST_SUITE_NOT_SORTED, LUCI_MILO_CFG
    )
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr,
      (
  ...
  def test_type_assert_printing_help(self):
    fbb = FakeBBGen(
      self.args, FOO_GTESTS_WATERFALL, TEST_SUITES_SYNTAX_ERROR, LUCI_MILO_CFG
    )
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr,
      f'Invalid \\.pyl file '
  ...
  def test_args_field_not_list(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, '"args" must be a list'
    ):
      fbb.check_output_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
...
TEST_QUERY_BOTS_OUTPUT = {
    ...
    'gtest_tests': [
      {
        ...
        'merge': {'script': '//testing/merge_scripts/standard_gtest_merge.py'},
        'name': 'foo_test',
        'swarming': {
          'dimensions': {
            'device_os': 'LMY41U',
  ...
  'Android Builder': {'additional_compile_targets': ['bar_test']},
}
...
class ReplacementTests(TestCase):
  ...
  def test_replacement_invalid_key_not_valid(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Given replacement key *'
    ):
      fbb.check_output_file_consistency(verbose=True)
  ...
  def test_replacement_invalid_key_not_found(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Could not find *'
    ):
      fbb.check_output_file_consistency(verbose=True)
...
class MagicSubstitutionTests(TestCase):
  ...
  def test_invalid_function(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Magic substitution function *'
    ):
      fbb.check_output_file_consistency(verbose=True)
...
class MatrixCompositionTests(TestCase):
  ...
  def test_noexistent_ref(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Unable to find reference to *'
    ):
      fbb.check_output_file_consistency(verbose=True)
  ...
  def test_variants_pyl_no_ref(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'Missing variant definition for *'
    ):
      fbb.check_output_file_consistency(verbose=True)
  ...
  def test_invalid_skylab_matrix_with_variants(self):
    ...
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'skylab tests must specify cros_board.'
    ):
      fbb.check_output_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
...
class SwarmingTests(TestCase):
  ...
  def test_builder_with_no_os_dimension_fails(self):
    fbb = FakeBBGen(
      self.args, NO_OS_GTESTS_WATERFALL, MAC_TEST_SUITE, MAC_LUCI_MILO_CFG
    )
    fbb.check_input_file_consistency(verbose=True)
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr,
  ...
  def test_mac_builder_with_no_cpu_dimension_in_isolated_script_fails(self):
    ...
    fbb.check_input_file_consistency(verbose=True)
    with self.assertRaisesRegex(
      generate_buildbot_json.BBGenErr, 'cpu must be specified for mac'
    ):
      fbb.check_output_file_consistency(verbose=True)
    self.assertFalse(fbb.printed_lines)
