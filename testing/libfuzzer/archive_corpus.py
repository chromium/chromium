...
def main():
  ...
  parser.add_argument(
    'corpus_directories', metavar='corpus_dir', type=str, nargs='+'
  )
  parser.add_argument(
    '--output', metavar='output_archive_name.zip', required=True
  )
  ...
  for directory in args.corpus_directories:
    if not os.path.exists(directory):
      raise Exception(
        'The given seed_corpus directory (%s) does not exist.' % directory
      )
  ...
  if os.path.getsize(seed_corpus_path) > SEED_CORPUS_LIMIT_MB * 1024 * 1024:
    print(
      'Seed corpus %s exceeds maximum allowed size (%d MB).'
