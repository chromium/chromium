...
class IterateThroughResultsForUserUnittest(fake_filesystem_unittest.TestCase):
  def setUp(self) -> None:
    ...
    self._input_patcher = mock.patch.object(
      expectations.ExpectationProcessor, 'PromptUserForExpectationAction'
    )
    ...
    self.expectation_file = os.path.join(
      uu.ABSOLUTE_EXPECTATION_FILE_DIRECTORY, 'pixel_expectations.txt'
    )
...
"""
  ...
  def testIterateThroughResultsForUserIgnoreNoGroupByTags(self) -> None:
    ...
    self._expectations.IterateThroughResultsForUser(
...
@unittest.skipIf(sys.version_info[0] != 3, 'Python 3-only')
class CreateExpectationsForAllResultsUnittest(
  fake_filesystem_unittest.TestCase
):
  def setUp(self) -> None:
    self.setUpPyfakefs()
...
"""
  ...
  def testGroupByTags(self) -> None:
    ...
    self._expectations.CreateExpectationsForAllResults(
      ...
      True,
      True,
      self.build_fail_total_number_threshold,
...
class ModifyFileForResultUnittest(fake_filesystem_unittest.TestCase):
  def setUp(self) -> None:
    ...
    self._expectations = uu.UnitTestExpectationProcessor()
    self.expectation_file = os.path.join(
      uu.ABSOLUTE_EXPECTATION_FILE_DIRECTORY, 'expectation.txt'
    )
    uu.CreateFile(self, self.expectation_file)
    self._expectation_file_patcher = mock.patch.object(
      uu.UnitTestExpectationProcessor, 'GetExpectationFileForSuite'
...
"""
    ...
    with open(self.expectation_file, 'w') as outfile:
      outfile.write(expectation_file_contents)
    self._expectations.ModifyFileForResult(
      'some_file', 'some_test', ('win', 'win10'), '', 'Failure', False, True
    )
    expected_contents = (
      uu.TAG_HEADER
      + """\
...
"""
    ...
    with open(self.expectation_file, 'w') as outfile:
      outfile.write(expectation_file_contents)
    self._expectations.ModifyFileForResult(
      'some_file', 'some_test', ('win', 'win10'), '', 'Failure', True, True
    )
    expected_contents = (
      uu.TAG_HEADER
      + """\
...
"""
    ...
    with open(self.expectation_file, 'w') as outfile:
      outfile.write(expectation_file_contents)
    self._expectations.ModifyFileForResult(
      'some_file', 'foo_test', ('win', 'win10'), '', 'Failure', True, True
    )
    expected_contents = (
      uu.TAG_HEADER
      + """\
