...
public class TrackExitReasonsOfInterestTest {
    ...
    public void testWriteAppStateOnlyModifiesAppStateInFile() throws TimeoutException {
        ...
        int calls;
        MockAwContentsLifecycleNotifier testSupplier = new MockAwContentsLifecycleNotifier();
        TrackExitReasonsOfInterest.setStateSupplier(testSupplier::getAppState);
        for (@AppState int appState = AppState.UNKNOWN; appState < AppState.DESTROYED; appState++) {
            testSupplier.mState = appState;
    ...
    public void testWriteAppStateTaskRunnerOnlyUpdatesWithNewState() throws TimeoutException {
        ...
        MockAwContentsLifecycleNotifier testSupplier = new MockAwContentsLifecycleNotifier();
        TrackExitReasonsOfInterest.setStateSupplier(testSupplier::getAppState);
        testSupplier.mState = AppState.FOREGROUND;
        TrackExitReasonsOfInterest.writeLastWebViewState(null);
        // Simulate app going to background and calling updateAppState
        testSupplier.mState = AppState.BACKGROUND;
        TrackExitReasonsOfInterest.writeLastWebViewState(null);
        testSupplier.mState = AppState.FOREGROUND;
        TrackExitReasonsOfInterest.writeLastWebViewState(null);

        testSupplier.mState = AppState.DESTROYED;
        TrackExitReasonsOfInterest.writeLastWebViewState(null);
        data = TrackExitReasonsOfInterest.readLastExitInfo();
        // pids are updated
        assertTrue(data.mExitInfoPid == previousPid);
        // timestamps are updated
        assertTrue(data.mTimestampAtLastRecordingInMillis == timeAtLastRecording);
        assertEquals(data.mState, testSupplier.getAppState());
    }
