...
public class AwPrerenderTest extends AwParameterizedTest {
    ...
    // Tests prerendering falls back to network (not canceled) when ShouldInterceptRequest serves a
    // null response.
    @Test
    @LargeTest
    @Feature({"AndroidWebView"})
    @Features.DisableFeatures({BlinkFeatures.PRERENDER2_MEMORY_CONTROLS})
    public void testPrerenderingWithNullCustomResponse() throws Throwable {
        setSpeculativeLoadingAllowed(SpeculativeLoadingAllowedFlags.PRERENDER_ENABLED);
        loadInitialPage();
        ...
        // Let shouldInterceptRequest return a null response for the main resource of the
        // prerendered page. This should fall back prerendering navigation to network.
        shouldInterceptRequestHelper.setReturnValueForUrl(mPrerenderingUrl, null);

        int currentShouldInterceptRequestCallCount = shouldInterceptRequestHelper.getCallCount();

        injectSpeculationRulesAndWait(mPrerenderingUrl);

        // Ensure that ShouldInterceptRequest is called for the main resource...
        shouldInterceptRequestHelper.waitForCallback(currentShouldInterceptRequestCallCount);
        AwContentsClient.AwWebResourceRequest request =
                shouldInterceptRequestHelper.getRequestsForUrl(mPrerenderingUrl);
        Assert.assertNotNull(request);

        // ... and the setup script that is loaded from the main resource. This indicates that the
        // prerendering navigation fell back to network accordingly.
        shouldInterceptRequestHelper.waitForNext();
        final String scriptUrl = mTestServer.getURL(PRERENDER_SETUP_SCRIPT_URL);
        AwContentsClient.AwWebResourceRequest scriptRequest =
                shouldInterceptRequestHelper.getRequestsForUrl(scriptUrl);
    ...
    // Tests prerendering is canceled when ShouldInterceptRequest serves a closed file stream.
    @Test
    @LargeTest
    @Feature({"AndroidWebView"})
    @Features.DisableFeatures({BlinkFeatures.PRERENDER2_MEMORY_CONTROLS})
    public void testPrerenderingWithClosedFileStream() throws Throwable {
        setSpeculativeLoadingAllowed(SpeculativeLoadingAllowedFlags.PRERENDER_ENABLED);
        loadInitialPage();
        ...
        // Let shouldInterceptRequest return a closed file stream for the main resource of the
        // prerendered page. This should fail prerendering navigation.
        FileInputStream body = new FileInputStream(UrlUtils.getIsolatedTestFilePath(PRERENDER_URL));
        body.close();
        WebResourceResponseInfo response = new WebResourceResponseInfo("text/html", "utf-8", body);
        shouldInterceptRequestHelper.setReturnValueForUrl(mPrerenderingUrl, response);
        ...
        // Wait until prerendering is canceled due to the closed file stream.
        histogramWatcher.pollInstrumentationThreadUntilSatisfied();
    }
