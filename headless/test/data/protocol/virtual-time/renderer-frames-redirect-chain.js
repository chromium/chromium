...
(async function(testRunner) {
  ...
  // Frame A
  httpInterceptor.addResponse('http://www.example.com/frameA/', `<html>
        <head>
  ...
  httpInterceptor.addResponse(
      'http://www.example.com/frameA/1', null,
      ['HTTP/1.1 301 Moved', 'Location: /frameA/2']);

  httpInterceptor.addResponse(
      'http://www.example.com/frameA/2', `<p>FRAME A</p>`);

  // Frame B
  httpInterceptor.addResponse('http://www.example.com/frameB/', `<html>
  ...
  httpInterceptor.addResponse('http://www.example.com/iframe/', `<html>
        <head>
  ...
  httpInterceptor.addResponse(
      'http://www.example.com/iframe/1', null,
      ['HTTP/1.1 302 Found', 'Location: /iframe/2']);

  httpInterceptor.addResponse(
      'http://www.example.com/iframe/2', null,
      ['HTTP/1.1 301 Moved', 'Location: 3']);

  httpInterceptor.addResponse(
      'http://www.example.com/iframe/3', `<p>IFRAME B</p>`);
  ...
  httpInterceptor.hasRequestedUrls([
    ...
    'http://www.example.com/iframe/2', 'http://www.example.com/iframe/3'
  ]);
