...
(async function(testRunner) {
  ...
  httpInterceptor.addResponse(
      'http://www.example.com/#foo', null,
      ['HTTP/1.1 302 Found', 'Location: /1']);

  httpInterceptor.addResponse(
      'http://www.example.com/1#foo', null,
      ['HTTP/1.1 302 Found', 'Location: /2']);

  httpInterceptor.addResponse('http://www.example.com/2#foo', `<html>
        <body>
          <p id="content"></p>
          <script>
  ...
  testRunner.log(
      await session.evaluate(`document.getElementById('content').innerText`));
