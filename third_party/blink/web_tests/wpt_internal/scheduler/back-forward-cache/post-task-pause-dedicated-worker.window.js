// META: title=Scheduler tasks are paused in a dedicated Worker in BFCache.
...
// Verifies that a dedicated Worker is frozen when its client enters BFCache by
...
promise_test(async t => {
  ...
  const workerUrl =
      '/wpt_internal/scheduler/back-forward-cache/resources/timestamp-worker.js';
  ...
  const maxGap = await rc1.executeScript(async () => {
    const timestamps = await new Promise(resolve => {
      window.worker.onmessage = (e) => resolve(e.data);
      window.worker.postMessage('get_timestamps');
    });
