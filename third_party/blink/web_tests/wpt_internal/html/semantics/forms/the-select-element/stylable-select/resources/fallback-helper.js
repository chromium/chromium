...
async function generateReferenceFrame(numOptions,initialx,initialy,extraStyleRules) {
  const singleOption = `<div tabindex=0 class="stylable-select-option">${testSelectOptionText}</div>`
  const options = Array(numOptions).fill(singleOption).join('\n');
  const content = `
    <link rel=stylesheet href="resources/stylable-select-styles.css">

    <div id=container class="select stylable-select-container">
      <button class="stylable-select-button open" style="anchor-name:--button">
        <span class=stylable-select-selectedoption>Select</span>
        <div class=stylable-select-button-icon></div>
      </button>
      <div id=popover popover=auto style="position-anchor:--button" class=stylable-select-datalist>
        <div tabindex=0 autofocus class="stylable-select-option selected">Select</div>
        ${options}
      </div>
    </div>
    <style>
