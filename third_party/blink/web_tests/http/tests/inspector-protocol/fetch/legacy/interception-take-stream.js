(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  function checkData(str, pattern) {
    for (let i = 0; i < Math.ceil(str.length / pattern.length); ++i) {
      ...
      if (expected != fragment) {
        testRunner.log(`Data mistmatch near offset ${start}, got ${
  ...
  testRunner.runTestSuite([
    async function testBasicUsage() {
      ...
      testRunner.log(
          `data: ${result.data} (${result.data.length}) eof: ${result.eof}`);
    ...
    async function testSmallOverlappingReads() {
      ...
      const readSize = 333;
      const stream =
          await startRequestAndTakeStream(streamSize, {chunked: true});
      if (!stream)
        return;
      let data = await new Promise(fulfill => {
        ...
        function onResponse(response) {
          ...
          for (; pendingReads < 4; ++pendingReads)
            dp.IO.read({handle: stream, size: readSize})
                .then(r => onResponse(r.result));
        }
        for (; pendingReads < 4; ++pendingReads)
          dp.IO.read({handle: stream, size: readSize})
              .then(r => onResponse(r.result));
    ...
    async function testContinueAfterBodyTaken() {
      ...
      testRunner.log(`Attempting to continue as is after taking request: ${
