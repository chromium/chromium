(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  session.protocol.Network.onRequestWillBeSent(event => {
    ...
    var interceptionWaitingResolver =
        requestInterceptionWaitingMap.get(fileName);
  ...
  session.protocol.Fetch.onRequestPaused(async event => {
    var fileName = nameForUrl(event.params.request.url);
    // Because requestWillBeSent and interception requests come from different
    // processes, they may come in random order, This will syncronize them to
    // ensure requestWillBeSent is processed first and will stall here till it
    // does.
    if (!requestInterceptionWaitingMap.has(fileName))
      await new Promise(
          resolve => requestInterceptionWaitingMap.set(fileName, resolve));
  ...
  session.protocol.Network.onLoadingFailed(
      event => {throw 'This test should never fail to load a resource.'});

  var responseWasReceivedCallback = () => {
    throw 'Must be overriden first.'
  };
  session.protocol.Network.onLoadingFinished(async event => {
    ...
    var message = await session.protocol.Network.getResponseBody(
        {requestId: event.params.requestId});
    if (message.error)
      testRunner.log(message.error);
    var body = message.result.base64Encoded ? atob(message.result.body) :
                                              message.result.body;
  ...
  async function testUrls() {
    ...
    session.evaluate(
        `fetch('../network/resources/small-test-1.txt').then(r => r.text())`);
    await new Promise(resolve => responseWasReceivedCallback = resolve);
    session.evaluate(
        `fetch('../network/resources/small-test-2.txt').then(r => r.text())`);
    await new Promise(resolve => responseWasReceivedCallback = resolve);
    session.evaluate(
        `fetch('../resources/test-page.html').then(r => r.text())`);
    await new Promise(resolve => responseWasReceivedCallback = resolve);
    testRunner.log('');
  }
  ...
  await testUrls();
  await setPatterns(
      [{urlPattern: '*small-test-1.txt'}, {urlPattern: '*small-test-2.txt'}]);
  await testUrls();
  await setPatterns(
      [{urlPattern: '*small-test-1.txt'}, {urlPattern: '*small-*'}]);
  await testUrls();
