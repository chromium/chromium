(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  async function onRequestIntercepted(dp, e) {
    ...
    if (e.params.request.url ===
        'http://devtools.oopif-b.test:8000/inspector-protocol/resources/test-page.html')
      response.errorReason = 'Aborted';
    interceptionLog.push(
        e.params.request.url +
        (response.errorReason ? `: ${response.errorReason}` : ''));
  ...
  await loadPromise;
  let trees =
      await Promise.all(allTargets.map(target => target.Page.getFrameTree()));
