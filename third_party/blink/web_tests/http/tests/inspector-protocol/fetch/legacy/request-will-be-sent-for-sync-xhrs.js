(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  dp.Network.onRequestWillBeSent(
      event => testRunner.log('Network.requestWillBeSent: ' +
                              event.params.request.url));

  const url = testRunner.url('../../network/initial');
  ...
  await dp.Fetch.fulfillRequest({
    ...
    responseCode: 302,
    responseHeaders: [
      {name: 'Location', value: testRunner.url('../../network/redirect1')},
    ],
  });

  // First redirect should emit both Fetch.requestPaused and
  ...
  await dp.Fetch.fulfillRequest({
    ...
    responseCode: 302,
    responseHeaders: [
      {name: 'Location', value: testRunner.url('../../network/redirect2')},
    ],
  });

  // Second redirect should emit both Fetch.requestPaused and
  // Network.requestWillBeSent.
