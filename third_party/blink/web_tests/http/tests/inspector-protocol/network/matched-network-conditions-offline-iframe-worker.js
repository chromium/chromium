(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  const {dp, session} = await testRunner.startBlank(
      `Verify pattern matched network conditions offline emulation are applied to worker main script in a local iframe.`);
  ...
  testRunner.log('iframe worker script load result: ' + result);
