...
(async function(/** @type {import('test_runner').TestRunner} */ testRunner) {
  ...
  for (const elementId of ['grid1', 'grid2']) {
    const node = await helper.nodeWithId(elementId);
    nodeIds.push(node.id);
  }
  const {highlights: {gridHighlights}} =
      (await dp.Overlay.getGridHighlightObjectsForTest({nodeIds})).result;
  if (2 !== gridHighlights.length)
    testRunner.log('FAIL: expected ' + 2 + ' but got ' + gridHighlights.length);
  ...
  if (!((grid1HighlightExpected === firstHighlightResult &&
