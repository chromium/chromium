...
`, 'Nested @starting-style with tigther selector and nested divs');
  ...
  await dp.CSS.forceStartingStyle({nodeId: firstDivId, forced: true });
  await dp.CSS.forceStartingStyle({nodeId: secondDivId, forced: true });
  await cssHelper.loadAndDumpMatchingRulesForNode(firstDivId);
  await cssHelper.loadAndDumpMatchingRulesForNode(secondDivId);

  await dp.CSS.forceStartingStyle({nodeId: firstDivId, forced: false });
  await dp.CSS.forceStartingStyle({nodeId: secondDivId, forced: false });
  await cssHelper.loadAndDumpMatchingRulesForNode(firstDivId);
  await cssHelper.loadAndDumpMatchingRulesForNode(secondDivId);
