...
WebGPUExecutionContextToken GetExecutionContextToken(
  ...
  // WebGPU only supports 3 types of context tokens, DocumentTokens,
  // DedicatedWorkerTokens and ServiceWorkerToken. The token is sent to the GPU
  // process so that it can be cross-referenced against the browser process to
  // get an isolation key for caching purposes.
