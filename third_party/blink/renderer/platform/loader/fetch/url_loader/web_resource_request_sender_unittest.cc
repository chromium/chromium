 ...
 public:
  ...
  void StartAsync(std::unique_ptr<network::ResourceRequest> request,
    ...
    sender()->SendAsync(
        ...
        TRAFFIC_ANNOTATION_FOR_TESTS, false, std::vector<std::string>(),
        std::move(peer),
...
TEST_F(WebResourceRequestSenderTest, DelegateTest) {
  ...
  EXPECT_EQ(MOJO_RESULT_OK, mojo::CreateDataPipe(&options, &data_pipe_producer,
                                                 &data_pipe_consumer));
  client->OnStartLoadingResponseBody(std::move(data_pipe_consumer));

  uint32_t size = strlen(kTestPageContents);
  auto result = data_pipe_producer->WriteData(kTestPageContents, &size,
 ...
 public:
  void PerformTest(base::TimeTicks remote_request_start,
    ...
    EXPECT_EQ(MOJO_RESULT_OK,
              mojo::CreateDataPipe(&options, &data_pipe_producer,
                                   &data_pipe_consumer));
    client->OnStartLoadingResponseBody(std::move(data_pipe_consumer));
    data_pipe_producer.reset();  // The response is empty.
