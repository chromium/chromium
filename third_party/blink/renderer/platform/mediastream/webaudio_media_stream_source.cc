...
bool WebAudioMediaStreamSource::EnsureSourceIsStarted() {
  ...
  if (!media_stream_source_ ||
      !media_stream_source_->RequiresWebAudioConsumer()) {
    return false;
  }
  VLOG(1) << "Starting WebAudio media stream source.";
  media_stream_source_->SetWebAudioConsumer(this);
...
void WebAudioMediaStreamSource::EnsureSourceIsStopped() {
  ...
  if (media_stream_source_->RequiresWebAudioConsumer()) {
    media_stream_source_->RemoveWebAudioConsumer();
  }
