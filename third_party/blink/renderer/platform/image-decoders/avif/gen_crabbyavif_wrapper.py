...
_MSAN_DISABLE_START = """
// TODO(crbug.com/40240570, b/340592560): Memory sanitizer does not work well
// when the code crosses the Rust/C/C++ boundaries. Disable the tests under
// memory sanitizer until that is resolved.
...
def _generate_crabbyavif_file(source_file, replacements):
    ...
    # Copyright notice is 3 lines, insert everything after that.
    if source_file == "avif_image_decoder_test.cc":
        # Disable tests under memory sanitizer.
        contents.insert(3, _MSAN_DISABLE_START)
    contents.insert(3, _NOTICE)
    if source_file == "avif_image_decoder_test.cc":
        # Disable tests under memory sanitizer.
        contents.append(_MSAN_DISABLE_END)
    contents = "\n".join(contents)
