...
class TaskQueueThrottlerTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    wake_up_budget_pool_->AddThrottler(task_environment_.NowTicks(),
                                       task_queue_throttler_.get());
    timer_task_runner_ = timer_queue_->task_runner();
  }

  void TearDown() override {
    wake_up_budget_pool_->RemoveThrottler(task_environment_.NowTicks(),
                                          task_queue_throttler_.get());
  ...
  base::sequence_manager::TaskQueue::Handle CreateTaskQueue(
    ...
    return task_environment_.CreateTaskQueue(
  ...
  std::unique_ptr<TaskQueueThrottler> CreateThrottlerForTaskQueue(
    ...
    auto throttler = std::make_unique<TaskQueueThrottler>(
        queue, task_environment_.GetMockTickClock());
  ...
  void ForwardTimeToNextMinute() {
    task_environment_.FastForwardBy(
        task_environment_.NowTicks().SnappedToNextTick(base::TimeTicks(),
                                                       base::Minutes(1)) -
...
class AutoAdvancingProxyClock : public base::TickClock {
 public:
  explicit AutoAdvancingProxyClock(
  ...
  base::TimeTicks NowTicks() const override {
    if (!advance_interval_.is_zero()) {
      task_environment_->AdvanceClock(advance_interval_);
    }
...
class TaskQueueThrottlerWithAutoAdvancingTimeTest
 ...
 public:
  TaskQueueThrottlerWithAutoAdvancingTimeTest()
      : proxy_clock_(task_environment_) {}
  TaskQueueThrottlerWithAutoAdvancingTimeTest(
...
void TestTask(Vector<base::TimeTicks>* run_times,
              base::test::TaskEnvironment* task_environment) {
  run_times->push_back(RoundTimeToMilliseconds(task_environment->NowTicks()));
}

void ExpensiveTestTask(Vector<base::TimeTicks>* run_times,
                       base::test::TaskEnvironment* task_environment) {
  run_times->push_back(RoundTimeToMilliseconds(task_environment->NowTicks()));
  task_environment->AdvanceClock(base::Milliseconds(250));
}
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, TimerAlignment) {
  ...
  timer_task_runner_->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&TestTask, &run_times,
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Seconds(1),
                                     start_time_ + base::Seconds(1),
                                     start_time_ + base::Seconds(2),
                                     start_time_ + base::Seconds(9)));
}
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, WakeUpForNonDelayedTask) {
  ...
  timer_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&TestTask, &run_times,
                                base::Unretained(&task_environment_)));
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, WakeUpForDelayedTask) {
  ...
  task_environment_.FastForwardUntilNoTasksRemain();
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(2000.0)));
}
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Seconds(6),
                                     start_time_ + base::Seconds(16)));
}
...
TEST_F(TaskQueueThrottlerTest, TimeBasedThrottling) {
  ...
  std::unique_ptr<CPUTimeBudgetPool> pool = std::make_unique<CPUTimeBudgetPool>(
      "test", &tracing_controller_, task_environment_.NowTicks());

  pool->SetTimeBudgetRecoveryRate(start_time_, 0.1);
  pool->AddThrottler(start_time_, task_queue_throttler_.get());
  ...
  timer_task_runner_->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&ExpensiveTestTask, &run_times,
                     base::Unretained(&task_environment_)),
  ...
  pool->RemoveThrottler(task_environment_.NowTicks(),
                        task_queue_throttler_.get());
  run_times.clear();
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(3000),
                                     start_time_ + base::Milliseconds(3250)));
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000)));
  run_times.clear();

  LazyNow lazy_now_1(task_environment_.GetMockTickClock());
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(2000)));
  run_times.clear();

  LazyNow lazy_now_2(task_environment_.GetMockTickClock());
...
TEST_F(TaskQueueThrottlerTest, ImmediateTasksTimeBudgetThrottling) {
  ...
  // throttled.
  timer_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&ExpensiveTestTask, &run_times,
                                base::Unretained(&task_environment_)));
  timer_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&ExpensiveTestTask, &run_times,
                                base::Unretained(&task_environment_)));
  ...
  EXPECT_THAT(run_times,
              ElementsAre(start_time_, start_time_ + base::Milliseconds(2500)));
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  base::sequence_manager::TaskQueue::Handle second_queue =
      task_environment_.CreateTaskQueue(
  ...
  pool->SetTimeBudgetRecoveryRate(start_time_, 0.1);
  pool->AddThrottler(start_time_, task_queue_throttler_.get());
  pool->AddThrottler(start_time_, second_queue_throttler.get());
  wake_up_budget_pool_->AddThrottler(start_time_, second_queue_throttler.get());
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  Vector<base::TimeTicks> run_times;
  LazyNow lazy_now(task_environment_.GetMockTickClock());
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000),
                                     start_time_ + base::Milliseconds(1250)));
}
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  pool->AddThrottler(task_environment_.NowTicks(), task_queue_throttler_.get());
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest, MaxThrottlingDelay) {
  ...
  pool->SetTimeBudgetRecoveryRate(start_time_, 0.001);
  pool->AddThrottler(start_time_, task_queue_throttler_.get());
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000),
                                     start_time_ + base::Milliseconds(61250),
                                     start_time_ + base::Milliseconds(121500),
                                     start_time_ + base::Milliseconds(181750),
                                     start_time_ + base::Milliseconds(242000)));
}
...
TEST_F(TaskQueueThrottlerTest, ReportThrottling) {
  ...
  EXPECT_THAT(reported_throttling_times,
              ElementsAre(base::Milliseconds(1250), base::Milliseconds(2250)));
...
TEST_F(TaskQueueThrottlerTest, GrantAdditionalBudget) {
  ...
  pool->SetTimeBudgetRecoveryRate(start_time_, 0.1);
  pool->AddThrottler(start_time_, task_queue_throttler_.get());
  pool->GrantAdditionalBudget(start_time_, base::Milliseconds(500));
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000),
                                     start_time_ + base::Milliseconds(1250),
                                     start_time_ + base::Milliseconds(1500),
                                     start_time_ + base::Milliseconds(2500),
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000),
                                     start_time_ + base::Milliseconds(2000)));
...
TEST_F(TaskQueueThrottlerTest, TwoBudgetPools) {
  ...
  pool1->SetTimeBudgetRecoveryRate(start_time_, 0.1);
  pool1->AddThrottler(start_time_, task_queue_throttler_.get());
  pool1->AddThrottler(start_time_, second_queue_throttler.get());

  pool2->SetTimeBudgetRecoveryRate(start_time_, 0.01);
  pool2->AddThrottler(start_time_, task_queue_throttler_.get());
...
void RunChainedTask(Deque<base::TimeDelta> task_durations,
  ...
  run_times->push_back(RoundTimeToMilliseconds(task_environment->NowTicks()));
  task_environment->AdvanceClock(task_durations.front());
  task_durations.pop_front();

  queue->task_runner()->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(&RunChainedTask, std::move(task_durations),
                     base::Unretained(queue),
                     base::Unretained(task_environment), run_times, delay),
      delay);
}
}  // namespace
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  timer_task_runner_->PostDelayedTask(
      ...
      base::BindOnce(&RunChainedTask, MakeTaskDurations(10, base::TimeDelta()),
                     base::Unretained(timer_queue_.get()),
                     base::Unretained(&task_environment_), &run_times,
                     base::TimeDelta()),
      base::Milliseconds(100));
  ...
  EXPECT_THAT(
      run_times,
      ElementsAre(
          start_time_ + base::Seconds(1), start_time_ + base::Seconds(1),
          start_time_ + base::Seconds(1), start_time_ + base::Seconds(1),
          start_time_ + base::Seconds(1), start_time_ + base::Seconds(1),
          start_time_ + base::Seconds(1), start_time_ + base::Seconds(1),
          start_time_ + base::Seconds(1), start_time_ + base::Seconds(1)));
...
TEST_P(TaskQueueThrottlerWithAutoAdvancingTimeTest,
  ...
  timer_task_runner_->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(
          &RunChainedTask, MakeTaskDurations(10, base::Milliseconds(3)),
          base::Unretained(timer_queue_.get()),
          base::Unretained(&task_environment_), &run_times, base::TimeDelta()),
      base::Milliseconds(100));
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Milliseconds(1000),
                                     start_time_ + base::Milliseconds(1003),
                                     start_time_ + base::Milliseconds(1006),
                                     start_time_ + base::Milliseconds(1009),
                                     start_time_ + base::Milliseconds(2000),
                                     start_time_ + base::Milliseconds(2003),
                                     start_time_ + base::Milliseconds(2006),
                                     start_time_ + base::Milliseconds(2009),
                                     start_time_ + base::Milliseconds(3000),
                                     start_time_ + base::Milliseconds(3003)));
}
...
TEST_F(TaskQueueThrottlerTest,
  ...
  // of one-minute.
  wake_up_budget_pool_->SetWakeUpInterval(task_environment_.NowTicks(),
                                          base::Minutes(1));
  task_queue_throttler_->IncreaseThrottleRefCount();
  ...
  two_minutes_pool->SetWakeUpInterval(task_environment_.NowTicks(),
                                      base::Minutes(2));
  ...
  two_minutes_pool->AddThrottler(start_time_,
  ...
  EXPECT_EQ(one_minute_pool->last_wake_up_for_testing(),
            start_time_ + base::Minutes(1));
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Minutes(1)));
  ...
  // Clean up.
  two_minutes_pool->RemoveThrottler(task_environment_.NowTicks(),
                                    two_minutes_queue_throttler.get());
}
...
TEST_F(TaskQueueThrottlerTest,
  ...
  const base::TimeTicks start_time = task_environment_.NowTicks();
  ...
  task_environment_.FastForwardBy(base::Seconds(601));
  EXPECT_EQ(one_minute_pool->last_wake_up_for_testing(),
            start_time + base::Seconds(2));
  EXPECT_EQ(two_minutes_pool->last_wake_up_for_testing(),
            start_time + base::Seconds(3));
...
TEST_F(TaskQueueThrottlerTest,
  ...
  unaligned_pool->SetWakeUpInterval(task_environment_.NowTicks(),
                                    base::Minutes(1));
  ...
  unaligned_pool->AddThrottler(start_time_, unaligned_queue_throttler.get());
  ...
  task_environment_.FastForwardBy(base::Seconds(57));
  ...
  task_environment_.FastForwardBy(base::Seconds(117));
  EXPECT_EQ(aligned_pool->last_wake_up_for_testing(),
  ...
  // Clean up.
  unaligned_pool->RemoveThrottler(task_environment_.NowTicks(),
...
TEST_F(TaskQueueThrottlerTest, WakeUpBasedThrottling_EnableDisableThrottling) {
  ...
  timer_task_runner_->PostDelayedTask(
      ...
      base::BindOnce(&RunChainedTask, MakeTaskDurations(10, base::TimeDelta()),
                     base::Unretained(timer_queue_.get()),
                     base::Unretained(&task_environment_), &run_times, kDelay),
      kDelay);

  // Throttling is enabled. Only 1 task runs per |kTimeBetweenWakeUps|.
  task_environment_.FastForwardBy(kTimeBetweenWakeUps);

  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Seconds(60)));
  run_times.clear();

  // Disable throttling. All tasks can run.
  LazyNow lazy_now_1(task_environment_.GetMockTickClock());
  wake_up_budget_pool_->DisableThrottling(&lazy_now_1);
  task_environment_.FastForwardBy(5 * kDelay);

  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Seconds(70),
                                     start_time_ + base::Seconds(80),
                                     start_time_ + base::Seconds(90),
                                     start_time_ + base::Seconds(100),
                                     start_time_ + base::Seconds(110)));
  run_times.clear();
  ...
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Seconds(120),
                                     start_time_ + base::Seconds(180),
                                     start_time_ + base::Seconds(240),
                                     start_time_ + base::Seconds(300)));
}
...
TEST_F(TaskQueueThrottlerTest,
  ...
  ForwardTimeToNextMinute();
  const base::TimeTicks initial_time = task_environment_.NowTicks();
  wake_up_budget_pool_->SetWakeUpInterval(task_environment_.NowTicks(),
                                          base::Minutes(1));
  wake_up_budget_pool_->AllowLowerAlignmentIfNoRecentWakeUp(base::Seconds(1));
  Vector<base::TimeTicks> run_times;
...
TEST_F(TaskQueueThrottlerTest,
  ...
  task_environment_.FastForwardUntilNoTasksRemain();
  EXPECT_THAT(run_times, ElementsAre(start_time_ + base::Hours(1),
                                     start_time_ + base::Hours(1)));
}
...
TEST_F(TaskQueueThrottlerTest,
  ...
  timer_task_runner_->PostDelayedTask(
      FROM_HERE, base::BindLambdaForTesting([&]() {
        TestTask(&run_times, &task_environment_);
...
TEST_F(TaskQueueThrottlerTest,
  ...
  task_environment_.FastForwardUntilNoTasksRemain();
  EXPECT_THAT(run_times,
              ElementsAre(start_time_ + base::Hours(1),
                          start_time_ + base::Hours(1) + base::Minutes(1),
                          start_time_ + base::Hours(1) + base::Minutes(2)));
}
...
TEST_F(TaskQueueThrottlerTest,
  ...
  pool->SetTimeBudgetRecoveryRate(start_time_, 0.01);
  pool->AddThrottler(start_time_, task_queue_throttler_.get());
  ...
  timer_task_runner_->PostDelayedTask(
      FROM_HERE,
      base::BindOnce(
          &RunChainedTask, MakeTaskDurations(10, base::Milliseconds(7)),
          base::Unretained(timer_queue_.get()),
          base::Unretained(&task_environment_), &run_times, base::TimeDelta()),
      base::Milliseconds(100));
  ...
  EXPECT_THAT(run_times,
              ElementsAre(
                  ...
                  start_time_ + base::Milliseconds(1000),
                  start_time_ + base::Milliseconds(1007),
