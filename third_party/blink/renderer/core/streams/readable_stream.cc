...
class ReadableStream::PipeToEngine final
 ...
 private:
  ...
  class PipeToReadRequest final : public ReadRequest {
   public:
...
class ReadableStream::ByteStreamTeeEngine final
    : public GarbageCollected<ByteStreamTeeEngine> {
 public:
 ...
 private:
  class PullAlgorithm final : public StreamAlgorithm {
   public:
  ...
  void ForwardReaderError(ScriptState* script_state,
    ...
    StreamThenPromise(
        script_state->GetContext(),
        this_reader->closed_promise_->V8Promise(script_state->GetIsolate()),
        nullptr,
        MakeGarbageCollected<ScriptFunction>(
            script_state,
  ...
  void PullWithDefaultReader(ScriptState* script_state,
                             ExceptionState& exception_state) {
  ...
  void PullWithBYOBReader(ScriptState* script_state,
  ...
  class ByteTeeReadRequest final : public ReadRequest {
   public:
    explicit ByteTeeReadRequest(ByteStreamTeeEngine* engine)
        : engine_(engine) {}
    ...
    void ChunkSteps(ScriptState* script_state,
      ...
      event_loop->EnqueueMicrotask(WTF::BindOnce(
          &ByteTeeReadRequest::ChunkStepsBody, WrapPersistent(this),
          WrapPersistent(script_state), std::move(value)));
    }
  ...
  class ByteTeeReadIntoRequest final : public ReadIntoRequest {
   public:
...
v8::Local<v8::Promise> ReadableStream::ByteStreamTeeEngine::PullAlgorithm::Run(
...
void ReadableStream::ByteStreamTeeEngine::PullAlgorithm::Trace(
    Visitor* visitor) const {
  visitor->Trace(engine_);
  StreamAlgorithm::Trace(visitor);
}

class ReadableStream::ByteStreamTeeEngine::CancelAlgorithm final
    : public StreamAlgorithm {
 public:
...
void ReadableStream::ByteStreamTeeEngine::Start(
    ScriptState* script_state,
    ReadableStream* stream,
    ExceptionState& exception_state) {
  // https://streams.spec.whatwg.org/#abstract-opdef-readablebytestreamtee
