      ...
      if (element && element->IsScrollMarkerPseudoElement()) {
        current_node_ = static_cast<ScrollMarkerPseudoElement*>(element)
                            ->ScrollMarkerGroup();
                             ...
                             element->IsInterestButtonPseudoElement())) {
        // ::scroll-marker-group, ::scroll-button, and ::interest-button are
        // laid out as siblings of their originating element rather than as
        // descendants, so the originating element's display lock does not
        // affect them. Skip past the originating element when traversing
        // ancestors.
