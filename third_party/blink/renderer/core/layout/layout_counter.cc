...
CounterNode* GetParentCounter(const CounterNode& counter,
                              const WTF::AtomicString& identifier) {
  // Since the counters tree is organized in a way where
  // parent <-> child scope relationship means every counter from
  // the child scope has parent's first counter in its counters set
  // we just take the parent's scope first counter when counters()
  // are used.
  if (CountersScope* parent_scope = counter.Scope()->Parent()) {
    return &parent_scope->FirstCounter();
  }
  // counters() are allowed to leave the style scope.
  StyleContainmentScope* parent_style_scope =
      counter.Scope()->StyleScope()->Parent();
  if (parent_style_scope) {
    // Find the counters scope in which `counter` would be
    // in the parent style scope.
...
String LayoutCounter::OriginalText() const {
  ...
  if (!counter_->Separator().IsNull()) {
    ...
    // If the `counter` is the first counter of the scope, don't
    // append it to the result, as it's already in the `text`.
    if (counter == counter_node_) {
      counter = GetParentCounter(*counter, counter_->Identifier());
    }
    // While we have parent counter, append it to the final text.
    while (counter) {
      const CounterNode* next_counter =
          GetParentCounter(*counter, counter_->Identifier());
      if (next_counter) {
