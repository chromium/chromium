...

  {
    ...
    if (UNLIKELY(new_space.HasBlockFragmentation())) {
      ...
      if (new_space.IsInitialColumnBalancingPass()) {
        ...
        // If the fragmentainer size has changed, and there previously was space
        // shortage reported, we should re-run layout to avoid reporting the
        // same space shortage again.
