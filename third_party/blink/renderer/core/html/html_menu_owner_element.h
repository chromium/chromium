...
class CORE_EXPORT HTMLMenuOwnerElement : public HTMLElement,
 ...
 public:
  ...
  void UpdateViolationModeForMenuHierarchy();
  void ScheduleViolationModeUpdate();
 ...
 private:
  ...
  // root node of the menu hierarchy. UpdateViolationModeForMenuHierarchy checks
  // the root's value to determine if the entire hierarchy should switch its
  // ARIA role.
  int total_violations_in_tree_ = 0;
  // Maintained asynchronously by UpdateViolationModeForMenuHierarchy on every
  // HTMLMenuOwnerElement in the menu hierarchy.
  // UpdateViolationModeForMenuHierarchy compares this against the root menu's
  // (total_violations_in_tree_ > 0) to avoid calling
  // AXObjectCache::HandleAttributeChanged redundantly.
  bool is_in_violation_mode_ = false;
  bool is_violation_mode_update_scheduled_ = false;
};
