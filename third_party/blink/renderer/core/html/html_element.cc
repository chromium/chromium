...
void HTMLElement::NotifyAnchorElementObserver() {
  if (!GetAnchorElementObserver()) {
    bool needs_observer =
        FastHasAttribute(html_names::kAnchorAttr) || anchorElement();
    if (!needs_observer) {
      return;
    }
  }
