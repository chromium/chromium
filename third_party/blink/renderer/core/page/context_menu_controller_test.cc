...
// TODO: This test fails on ASAN bots in TransformWebPointerEvent.
#if defined(ADDRESS_SANITIZER)
#define MAYBE_ButtonWithInterestFor DISABLED_ButtonWithInterestFor
#else
#define MAYBE_ButtonWithInterestFor ButtonWithInterestFor
#endif
TEST_F(InterestForTouchscreenTest, MAYBE_ButtonWithInterestFor) {
  GetDocument()->documentElement()->SetInnerHTMLWithoutTrustedTypes(R"(
  ...
  WebGestureEvent pointerup_event(
      WebInputEvent::Type::kPointerUp, WebInputEvent::kNoModifiers,
      base::TimeTicks::Now(), WebGestureDevice::kTouchscreen);
  pointerup_event.SetPositionInWidget(gesture_location);
  GetWebView()->MainFrameWidget()->HandleInputEvent(
