...
When setting the display-locked property of an object, we remove all the
pending opacity updates of that document. We actually only need to remove only
the updates for objects that are in that display, but the check is too
expensive, so we remove all of the pending updates.
