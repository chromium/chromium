...
class CORE_EXPORT PointerEventFactory
 ...
 private:
  ...
  struct PointerAttributes : public GarbageCollected<PointerAttributes> {
  ...
  PointerAttributes* GetPointerAttributesForId(PointerId id) const;

  // We use int64_t to cover the whole range for PointerId with no
  // deleted hash value.
  HeapHashMap<int64_t,
              Member<PointerAttributes>,
              IntWithZeroKeyHashTraits<int64_t>>
      pointer_id_to_attributes_;

  struct PointerIdAndAttributes
      : public GarbageCollected<PointerIdAndAttributes> {
    PointerId id;
    Member<PointerAttributes> attributes;

    PointerIdAndAttributes(PointerId id, PointerAttributes* attributes)
        : id(id), attributes(attributes) {}

    void Trace(Visitor* visitor) const { visitor->Trace(attributes); }
  };
  ...
  HeapVector<Member<PointerIdAndAttributes>, kRemovedPointersCapacity>
