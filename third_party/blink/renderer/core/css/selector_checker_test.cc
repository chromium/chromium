 ...
 public:
  void SetUp() override {
    ...
    GetDocument().body()->SetInnerHTMLWithoutTrustedTypes(R"HTML(
        ...
        <div id="no-lang">No language</div>
      )HTML");
    UpdateAllLifecyclePhasesForTest();
  }
...
TEST_F(LangTest, SelectorParsing) {
  ...
  // Malformed but accepted by the parser:
  // EXPECT_FALSE(ValidateLang(":lang(x)"));
  // EXPECT_FALSE(ValidateLang(":lang(en-)"));
  // EXPECT_FALSE(ValidateLang(":lang(en--US)"));
}

TEST_F(LangTest, ExactLanguageMatch) {
...
TEST_F(LangTest, SpecificVariantMatch) {
...
TEST_F(LangTest, CaseInsensitiveMatch) {
  EXPECT_TRUE(MatchesLang(":lang(JA-HIRA-JP)", "ja-Hira-JP"));
  EXPECT_TRUE(MatchesLang(":lang(ja-hira-jp)", "ja-Hira-JP"));
  EXPECT_TRUE(MatchesLang(":lang(jA-hIrA-jP)", "ja-Hira-JP"));
}
...
TEST_F(LangTest, SingletonBlocking) {
  EXPECT_TRUE(MatchesLang(":lang(ja-JP)", "ja-Jpan-JP"));
...
TEST_F(LangTest, EmptyAndNoLangMatching) {
