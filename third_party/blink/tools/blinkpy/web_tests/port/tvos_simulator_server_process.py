...
# Custom version of ServerProcess that runs processes on the tvOS simulator.
class TVOSSimulatorServerProcess(ServerProcess):
    ...
    def __init__(self,
                 ...
                 more_logging=False):
        super(TVOSSimulatorServerProcess,
    ...
    def _start(self):
        ...
        # The tvOS simulator doesn't allow to use stdin stream for packaged apps.
        # Additionally, the stdout from run-test-suite not only includes
        # additional data from the tvOS test infrastructure but also combines
        # stderr and stdout. As a result, when executing content_shell on the
        # tvOS simulator, it becomes impractical to employ stdin for transmitting
        ...
        # Wait for incoming connection from the tvOS content_shell.
