 ...
 * API for compatibility. \brief For latest OpenVINO Provider Options update to
 * the ProviderOptions map. \brief Latest OpenVINO Provider Options are listed
 * in the \htmlonly <a
 * href="https://onnxruntime.ai/docs/execution-providers/OpenVINO-ExecutionProvider.html#summary-of-options">onnxruntime
 * document.</a> \endhtmlonly \see
 ...
 * value will be nullptr \param[out] num_outputs Number of outputs, on error,
 * the value will be zero \param[out] status On error, status will provide
 * details
 */
...
struct OrtApi {
   ...
   * function; all arguments to this function will be ignored. \param[in]
   ...
   * SetUserLoggingFunction API instead. \param[in] logging_function A pointer
   * to a logging function. \param[in] logger_param A pointer to arbitrary data
   * passed as the ::OrtLoggingFunction `param` parameter to `logging_function`.
   * This parameter is optional. \param[in] log_severity_level The log severity
   * level. \param[in] logid The log identifier. \param[out] out Returned newly
   ...
   * for an in-depth explanation \param[in,out] options The session options
   * object \param[in] graph_optimization_level The optimization level
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(CreateCustomOpDomain,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(CreateIoBinding,
                  _Inout_ OrtSession* session,
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(RegisterAllocator,
                  _Inout_ OrtEnv* env,
                  _In_ OrtAllocator* allocator);
   ...
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(UnregisterAllocator,
                  _Inout_ OrtEnv* env,
                  _In_ const OrtMemoryInfo* mem_info);
   ...
   * pointer will be returned and no error message will be printed if the
   * training api is not supported with this build. A NULL pointer will be
   * returned and an error message will be printed if the provided version is
   ...
   * CreateAllocator() or GetAllocatorWithDefaultOptions(), the specified
   * allocator will be used to allocate continuous buffers for output strings
   * and lengths. \param[out] ptr is a UTF-8 null terminated string allocated
   * using 'allocator'. The caller is responsible for using the same allocator
   * to free it.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   * argument in the call to the registration function. \param[in]
   * registration_func_name Name of registration function to use.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   * \since Version 1.14
   */
   ...
   * status if out-of-bounds. \param[out] out Memory location into which to
   * write the UTF-8 null-terminated string representing the input's name.
   ...
   * status if out-of-bounds. \param[out] out Memory location into which to
   * write the UTF-8 null-terminated string representing the output's name.
   * \param[in,out] size Pointer to the size of the `out` buffer. See above
   * comments for details.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   ...
   * name. \param[in] allocator Allocator used to allocate the internal tensor
   * state. \param[out] out Returns newly created ::OrtValue. Must be freed with
   ...
   * configuration key. \param[out] out Pointer set to 1 if the entry exists and
   * 0 otherwise.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   * \since Version 1.14
   ...
   * \param[in] options The session options.
   * \param[in] config_key A null-terminated UTF-8 string representation of the
   * config key. \param[in] config_value Pointer to memory where the
   * null-terminated UTF-8 string value will be stored. \param[in,out] size
   * Pointer to the size of the `config_value` buffer. See above comments for
   * details.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   * \since Version 1.14
   ...
   * null-terminated string representing the name. \param[in,out] size Pointer
   * to the size of the `out` buffer. See above comments for details.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   * \since Version 1.15
   */
   ...
   * logged. Usually the value of ORT_FILE. \param[in] line_number The file line
   * number in which the message is logged. Usually the value of __LINE__.
   ...
   * \since Version 1.15
   */
  ORT_API2_STATUS(Logger_LogMessage,
   ...
   * null terminator) \param[inout] buffer - address of return value
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * is already registered. \param[in] env OrtEnv instance \param[in]
   * provider_type ExecutionProvider type \param[in] mem_info OrtMemoryInfo
   * instance \param[in] arena_cfg Arena configuration \param[in]
   * provider_options_keys key of the provider options map \param[in]
   * provider_options_values value of the provider options map \param[in]
   * num_keys Length of the provider options map
   */
   ...
   * \param[in] run_options If nullptr, will use a default ::OrtRunOptions
   * \param[in] input_names Array of null terminated UTF8 encoded strings of the
   * input names \param[in] input Array of ::OrtValue%s of the input values
   * \param[in] input_len Number of elements in the input_names and inputs
   * arrays \param[in] output_names Array of null terminated UTF8 encoded
   * strings of the output names \param[in] output_names_len Number of elements
   * in the output_names and outputs array \param[out] output OrtValue* array of
   * size output_names_len. On calling RunAsync, output[i] could either be a
   * null or a pointer to a preallocated OrtValue. Later, the output array will
   * be passed to run_async_callback with all null(s) filled with valid OrtValue
   ...
   * customer. \param[in] run_async_callback Callback function on model run
   * completion \param[in] user_data User data that pass back to
   * run_async_callback
   */
  ORT_API2_STATUS(RunAsync,
                  _Inout_ OrtSession* session,
                  _In_opt_ const OrtRunOptions* run_options,
                  _In_reads_(input_len) const char* const* input_names,
   ...
   * divided in maximum. When zero, there is no limit \param[in] usr_data User
   * data to be passed back to fn
   *
   * \since Version 1.17.
   */
   ...
   * this scratch buffer from the corresponding allocator \param[in] context
   * OrtKernelContext instance \param[in] mem_info OrtMemoryInfo instance
   ...
   * the length of file content \param[in] num_external_initializer_files Number
   * of external files
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
   ...
   * device if required by the model at inference time. \param[out] out A
   * pointer to a newly created OrtLoraAdapter instance. Must be released with
   ...
   * option keys \param[in] values Array of null terminated UTF8 encoded string
   * of EP dynamic option values \param[in] kv_len Number of elements in the
   * keys and values arrays
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   */
  ORT_API2_STATUS(SetEpDynamicOptions,
