...
export class BidiServerRunner {
    ...
    static run(onOpen: (bidiServer: IServer) => Promise<any>, onClose: (stateObject: any) => void) {
        const self = this;
        ...
        wsServer.on('request', async function (request) {
            ...
            connection.on('message', function (message) {
                ...
                if (message.type !== 'utf8') {
                    self._respondWithError(connection, {}, "invalid argument", `not supported type (${message.type})`);
                    return;
                }
    ...
    private static _sendClientMessageStr(messageStr: string, connection: websocket.connection): Promise<void> {
        debugSend(messageStr);
        connection.sendUTF(messageStr);
        return Promise.resolve();
    }
    private static _sendClientMessage(messageObj: any, connection: websocket.connection): Promise<void> {
        const messageStr = JSON.stringify(messageObj);
        return this._sendClientMessageStr(messageStr, connection);
    }

    private static _respondWithError(connection: websocket.connection, plainCommandData: any, errorCode: string, errorMessage: string) {
        const errorResponse = this._getErrorResponse(plainCommandData, errorCode, errorMessage);
        this._sendClientMessage(errorResponse, connection);
    }

    private static _getErrorResponse(plainCommandData: any, errorCode: string, errorMessage: string): any {
