...
@pytest_asyncio.fixture
async def target_context_id(target_user_context_id, context_id, create_context):
...
@pytest_asyncio.fixture
async def assert_success_download_events(
    ...
    async def assert_success_download_events(context_id, user_context_id=ANY_STR):
        ...
        download_id = event["params"]["download"]
...
@pytest_asyncio.fixture
async def assert_denied_download_events(
...
@pytest.mark.asyncio
async def test_browsing_context_download_navigation(
    websocket,
    context_id,
    html,
    url_download,
    ...
    download_url = url_download(FILE_NAME, content)
    page_url = html(f"""<a id="download_link" href="{download_url}">Download</a>""")
    ...
    nav_event = await wait_for_event(websocket, "browsingContext.navigationStarted")
    navigation_id = nav_event["params"]["navigation"]

    will_begin_event = await wait_for_event(
        websocket, "browsingContext.downloadWillBegin"
    )
    assert will_begin_event == {
        "method": "browsingContext.downloadWillBegin",
        "params": {
            "context": context_id,
            "download": ANY_UUID,
    ...
    download_id = will_begin_event["params"]["download"]

    end_event = await wait_for_event(websocket, "browsingContext.downloadEnd")
    assert end_event == {
        "method": "browsingContext.downloadEnd",
        "params": {
            "context": context_id,
...
async def test_browsing_context_download_behavior_deny(
    websocket, target_context_id, target_user_context_id, assert_denied_download_events
):
    await execute_command(
        websocket,
        {
...
async def test_browsing_context_download_behavior_allow_global(
    ...
    download_path = await assert_success_download_events(context_in_some_user_context)
    assert download_path.startswith(str(tmp_path))
    ...
    download_path = await assert_success_download_events(
        await create_context(await create_user_context())
    )
    assert download_path.startswith(str(tmp_path))
...
async def test_browsing_context_download_behavior_global_and_per_user_context(
    ...
    await execute_command(
        ...
        {
            "method": "browser.setDownloadBehavior",
            "params": {"downloadBehavior": None, "userContexts": ["default"]},
        },
    )
