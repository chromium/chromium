...
async def test_cookies_delete_params_empty(websocket, context_id):
    ...
    not_partitioned_cookie = get_bidi_cookie(
        SOME_COOKIE_NAME, SOME_COOKIE_VALUE, SOME_DOMAIN
    )
    await set_cookie(websocket, context_id, not_partitioned_cookie)
    partitioned_cookie = get_bidi_cookie(
        ANOTHER_COOKIE_NAME, ANOTHER_COOKIE_VALUE, SOME_DOMAIN
    )
    await set_cookie(
        websocket, context_id, partitioned_cookie, partition=source_origin_partition
    )

    # Delete all cookies.
    resp = await execute_command(
        websocket, {"method": "storage.deleteCookies", "params": {}}
    )
...
@pytest.mark.asyncio
async def test_cookies_delete_partition_has_cross_site_ancestor(websocket, context_id):
    same_site_cookie = get_bidi_cookie(SOME_COOKIE_NAME, SOME_COOKIE_VALUE, SOME_DOMAIN)
    await set_cookie(
        websocket,
        context_id,
        same_site_cookie,
    ...
    cross_site_cookie = get_bidi_cookie(
        ANOTHER_COOKIE_NAME, ANOTHER_COOKIE_VALUE, SOME_DOMAIN
    )
    await set_cookie(
        websocket,
    ...
    await assert_only_cookies_present(websocket, [same_site_cookie])


@pytest.mark.asyncio
async def test_cookies_delete_partition_user_context(
    ...
    new_user_context_cookie = get_bidi_cookie(
        SOME_COOKIE_NAME, SOME_COOKIE_VALUE, SOME_DOMAIN
    )
    # Set `cookie` to the new user context.
    await set_cookie(
        websocket, context_id, new_user_context_cookie, user_context_partition
    )

    default_user_context_cookie = get_bidi_cookie(
        ANOTHER_COOKIE_NAME, ANOTHER_COOKIE_VALUE, SOME_DOMAIN
    )
...
@pytest.mark.asyncio
async def test_cookies_delete_partition_user_context_unknown(websocket, context_id):
    user_context_partition = {
        "type": "storageKey",
...
@pytest.mark.asyncio
async def test_cookies_delete_partition_browsing_context(websocket, context_id):
    cookie = get_bidi_cookie(SOME_COOKIE_NAME, SOME_COOKIE_VALUE, SOME_DOMAIN)
    await set_cookie(websocket, context_id, cookie)
    ...
    resp = await execute_command(
        ...
        {
            "method": "storage.deleteCookies",
            "params": {"partition": {"type": "context", "context": context_id}},
        },
    )
...
@pytest.mark.asyncio
async def test_cookies_delete_params_filter(websocket, context_id, cookie_filter):
    some_cookie = get_bidi_cookie(SOME_COOKIE_NAME, SOME_COOKIE_VALUE, SOME_DOMAIN)
    await set_cookie(websocket, context_id, some_cookie)

    another_cookie = get_bidi_cookie(
        ANOTHER_COOKIE_NAME, ANOTHER_COOKIE_VALUE, ANOTHER_DOMAIN
    )
    ...
    resp = await execute_command(
        websocket,
        {"method": "storage.deleteCookies", "params": {"filter": cookie_filter}},
    )
    assert resp == {"partitionKey": {"userContext": "default"}}
...
async def assert_only_cookies_present(websocket, expected_cookies):
    ...
    assert resp == {
        "cookies": list(map(lambda cookie: AnyExtending(cookie), expected_cookies)),
