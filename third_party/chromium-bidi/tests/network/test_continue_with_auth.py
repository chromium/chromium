...
async def test_continue_with_auth_invalid_phase(
    websocket, context_id, url_example, phase
):
    network_id = await create_blocked_request(
        websocket, context_id, url=url_example, phase=phase
    )
...
@pytest.mark.parametrize(
    "credentials",
    [{}, {"type": "notapassword", "username": "user", "password": "pass"}],
...
async def test_continue_with_auth_invalid_credentials(
    ...
    network_id = await create_blocked_request(
        websocket, context_id, url_auth_required, phase="authRequired"
    )

    with pytest.raises(
        Exception,
        match=str({"error": "invalid argument", "message": "Invalid input in ."}),
    ):
        await execute_command(
            websocket,
            {
...
@pytest.mark.asyncio
async def test_continue_with_auth_completes(websocket, context_id, url_auth_required):
    await subscribe(
        websocket, ["network.authRequired", "network.responseCompleted"], [context_id]
    )
...
@pytest.mark.asyncio
async def test_continue_with_auth_twice(websocket, context_id, url_auth_required):
    await subscribe(
        websocket, ["network.authRequired", "network.responseCompleted"], [context_id]
    )
