...
if ! command -v gclient &> /dev/null; then
  echo "Error: 'gclient' is required but not installed or not in PATH." >&2
  echo "Please ensure depot_tools is installed and in your PATH." >&2
  exit 1
fi

if ! command -v vpython3 &> /dev/null; then
  echo "Error: 'vpython3' is required but not installed or not in PATH." >&2
  echo "Please ensure depot_tools is installed and in your PATH." >&2
  exit 1
fi
...
if [ -n "$COMMIT_MSG" ] && [ -n "$(git status --porcelain)" ]; then
  ...
  echo "Updating node_modules and DEPS..."
  node tools/update_node_modules.mjs --force

  echo "Running code formatting..."
  npm run format || npm run format

  echo "Committing changes: '$COMMIT_MSG'..."
  git add .browser package.json package-lock.json DEPS
