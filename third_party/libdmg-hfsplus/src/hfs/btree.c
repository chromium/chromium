...
/**
 * Splits a node in half by descriptor count. The caller is responsible for
...
/*
 * doAddRecord inserts a B-tree record into a specific node. This is a low-level
 * operation and callers are responsible for determining that:
 * - this is the correct node for this key
 * - the key is not already in the node
 * - the node has enough space for the new record and key
 *
 * If any of these constraints are violated, doAddRecord may silently corrupt
 * `tree`.
 *
 * If `doAddRecord` detects a problem, it prints a terse diagnostic to stderr
 * and returns FALSE. If it inserts the record, it returns TRUE. If it
 * encounters an I/O error, it halts the program (with an assertion failure).
 */
static int doAddRecord(BTree* tree, uint32_t root, BTKey* searchKey, size_t length, unsigned char* content) {
  ...
  if(descriptor == NULL) {
    fprintf(stderr, "cannot read node descriptor for node %u\n", root);
    return FALSE;
  }

  // HFS+ B-tree records consist of:
  // 1. A 2-byte key length field.
  // 2. The key data itself (searchKey->keyLength bytes).
  // 3. The record data (length bytes).
  // 4. A 2-byte entry in the node's offset table.
  //
  // If the node doesn't have enough space for the new record, fail.
  //
  // If the B-tree has already been corrupted, available space may be negative,
  // so this calculation must use signed arithmetic.
  intmax_t requiredSpace = sizeof(searchKey->keyLength) + searchKey->keyLength + length + sizeof(uint16_t);
  ASSERT(requiredSpace > 0, "record size calculation overflow in doAddRecord");
  intmax_t availableSpace = (intmax_t)getFreeSpace(root, descriptor, tree);
  if (availableSpace < requiredSpace) {
    fprintf(stderr, "cannot fit %jd-byte complete record into into B-tree node %u with %jd free bytes\n", requiredSpace, root, availableSpace);
    free(descriptor);
    return FALSE;
  }
  ...
  for(i = 0; i < descriptor->numRecords; i++) {
    recordOffset = getRecordOffset(i, root, tree);
    BTKey* key = READ_KEY(tree, recordOffset, tree->io);
    recordDataOffset = recordOffset + key->keyLength + sizeof(key->keyLength);

    res = COMPARE(tree, key, searchKey);
    if(res == 0) {
      fprintf(stderr, "doAddRecord: key already exists in node %u", root);
      free(key);
      free(descriptor);
      return FALSE;
    } else if(res > 0) {
  ...
  if(i != descriptor->numRecords) {
    ...
    FLIPENDIAN(offset);
    ASSERT(WRITE(tree->io, (root * tree->headerRec->nodeSize) + tree->headerRec->nodeSize - (sizeof(uint16_t) * (i + 1)), sizeof(uint16_t), &offset), "WRITE");
  } else {
...
/**
 * addRecord recursively traverses the B-tree to insert a new record.
 *
 * If the insertion causes a node split, this function returns the node number
 * of the new (right-hand) sibling node. The caller (the parent node's recursive
 * call) is then responsible for inserting a key and pointer to this new node
 * into its own records. If no split occurred, it returns 0.
 *
 * If the operation cannot be completed in a single pass, callAgain is set to
 * TRUE and the node is split. The top-level caller (addToBTree) must then
 * restart the entire operation after the index is updated for new node splits.
 */
static uint32_t addRecord(BTree* tree, uint32_t root, BTKey* searchKey, size_t length, unsigned char* content, int* callAgain) {
  BTNodeDescriptor* descriptor;
  BTKey* key;
  off_t recordOffset;
  off_t recordDataOffset;
  ...
  if(descriptor->kind == kBTLeafNode) {
    // Leaf Node insertion:
    // If there's enough space, just add the record.
    // Otherwise, split the node and add the record to the correct sibling.
    if(freeSpace < (sizeof(searchKey->keyLength) + searchKey->keyLength + length + sizeof(uint16_t))) {
      newNode = splitNode(root, descriptor, tree);
      if(i < descriptor->numRecords) {
        ASSERT(doAddRecord(tree, root, searchKey, length, content), "addRecord: doAddRecord failed in leaf node after split");
      } else {
        ASSERT(doAddRecord(tree, newNode, searchKey, length, content), "addRecord: doAddRecord failed in new leaf node after split");
      }

      free(descriptor);
      return newNode;
    } else {
      ASSERT(doAddRecord(tree, root, searchKey, length, content), "addRecord: doAddRecord failed in leaf node");

      free(descriptor);
      return 0;
    }
  } else {
    // Index Node traversal:
    // Find the child node that should contain the key.
    if(lastRecordDataOffset == 0) {
      // The new record needs to become the new first record of a subtree we are
      // traversing. Update this node's record to report its new lower bound.
      ...
      if(searchKey->keyLength != key->keyLength) {
        // Move everything else to fit the new record size, since the key length
        // is changing.
        ...
        moveRecordsDown(tree, descriptor, 1, root, (int)searchKey->keyLength - (int)key->keyLength, 0);
      }
    ...
    if(newNode == 0) {
      ...
      if(freeSpace < (sizeof(key->keyLength) + key->keyLength + sizeof(newNodeBigEndian) + sizeof(uint16_t))) {
        ...
        if(i < descriptor->numRecords) {
          ASSERT(doAddRecord(tree, root, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)), "addRecord: doAddRecord failed in index node after split");
        } else {
          ASSERT(doAddRecord(tree, newNode, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)), "addRecord: doAddRecord failed in new index node after split");
        }
        ...
        ASSERT(doAddRecord(tree, root, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)), "addRecord: doAddRecord failed in index node");
...
int addToBTree(BTree* tree, BTKey* searchKey, size_t length, unsigned char* content) {
  ...
  if(tree->headerRec->rootNode != 0) {
    ...
    // initial offset: skip the 14-byte BTNodeDescriptor
    uint16_t offset = 14;
    ...
    freeOffset = (uint16_t)bigFreeOffset;
...
/**
 * removeRecord recursively traverses the B-tree to find and remove a record.
 *
 * If this node was split as a result of the operation, this returns the index
 * number of the new sibling node, which the caller must integrate into the
 * parent index.
 *
 * If a node becomes empty after removal, gone is set to TRUE and the node
 * itself has been removed; the caller must update the parent index accordingly.
 *
 * If the operation could not be completed in a single pass, callAgain is set
 * to TRUE and the caller must retry the deletion after adjusting the index
 * for the split node. The tree is in an inconsistent state until this additonal
 * pass is performed.
 */
static uint32_t removeRecord(BTree* tree, uint32_t root, BTKey* searchKey, int* callAgain, int* gone) {
  ...
  for(i = 0; i < descriptor->numRecords; i++) {
    ...
    if(res == 0) {
      ...
      if(descriptor->kind == kBTLeafNode) {
        // Record found in a leaf node. Remove it and potentially remove the
        // entire node if it becomes empty.
        if(i != (descriptor->numRecords - 1)) {
        ...
        // This is not a leaf node, but we are about to remove this node's
        // zeroth child, so plan to update this node's key on the way back up.
        nodeToTraverse = getNodeNumberFromPointerRecord(recordDataOffset, tree->io);
        checkForChangedKey = TRUE;
        break;
      }
    } else if(res > 0) {
      // The target is somewhere before the record we are inspecting.
      free(key);
  ...
  if(childGone) {
    // A child node was removed because it became empty. We must now
    // remove the pointer to that child from this (the parent) node.
    if(i != (descriptor->numRecords - 1)) {
      length = getRecordOffset(i + 1, root, tree) - recordOffset;
      moveRecordsDown(tree, descriptor, i + 1, root, -length, -1);
    } // don't need to do that if we're the last record, because our old offset pointer will be the new free space pointer
  ...
  // If the child call to removeRecord (or our own internal record insertion)
  // resulted in a split or a node deletion, handle the consequences here.
  if(newNode == 0) {
    if(descriptor->numRecords == 0) {
      removeNode(tree, root);
      (*gone) = TRUE;
    ...
    if(freeSpace < (sizeof(key->keyLength) + key->keyLength + sizeof(newNodeBigEndian) + sizeof(uint16_t))) {
      ...
      if(i < descriptor->numRecords) {
        ASSERT(doAddRecord(tree, root, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)),
               "removeRecord: doAddRecord failed in index node after split");
      } else {
        ASSERT(doAddRecord(tree, newNode, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)),
               "removeRecord: doAddRecord failed in new index node after split");
      }
      ...
      ASSERT(doAddRecord(tree, root, key, sizeof(newNodeBigEndian), (unsigned char*)(&newNodeBigEndian)),
             "removeRecord: doAddRecord failed in index node");
      free(key);
      free(descriptor);
      return 0;
    }
