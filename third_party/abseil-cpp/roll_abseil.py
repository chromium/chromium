...
def _UpdateAbseilInChromium(abseil_dir, chromium_dir):
    ...
    exclude = [
        ...
        'convert_bazel_to_gn.py',
        '*.def',
    ]
    params = [
        'rsync', '-aP', abseil_dir,
        os.path.join(chromium_dir, 'third_party'), '--delete'
    ]
    for e in exclude:
...
def _PatchAbseil(abseil_in_chromium_dir):
    ...
    for patch in os.listdir(os.path.join(abseil_in_chromium_dir, 'patches')):
        subprocess.check_call([
            'patch', '--strip', '1', '-i',
            os.path.join(abseil_in_chromium_dir, 'patches', patch)
        ])

    os.remove(
        os.path.join(abseil_in_chromium_dir, 'absl', 'base', 'internal',
...
Bug: None""".format(hash_diff)
    ...
    subprocess.check_call(['git', 'add', 'third_party/abseil-cpp'],
                          cwd=chromium_dir)
    proc = subprocess.run(
        ['git', 'diff', '--staged', '--quiet', 'third_party/abseil-cpp'],
        cwd=chromium_dir)
...
def _Roll(should_branch, should_pull, should_upload, revision):
    ...
    if should_branch:
        ...
        subprocess.check_call(['git', 'new-branch', branch_name],
                              cwd=chromium_dir)
    ...
    with tempfile.TemporaryDirectory() as abseil_root:
        ...
        hash_diff = _UpdateChromiumReadme(
            os.path.join(abseil_in_chromium_dir, 'README.chromium'),
            abseil_dir)
