...
/// Shared state between `ChromiumParallelRunner` and the tasks it spawns on
/// the Chromium thread pool.
...
fn run_parallel_task(context: *mut c_void, task_index: usize) {
    ...
    // `ChromiumParallelRunner::run_ordered`, which outlives the (synchronous)
