...
//! lines to your Cargo.toml. `strum_macros` contains the macros needed to
//! derive all the traits in Strum.
//!
//! ```toml
//! [dependencies]
//! strum = "0.28"
...
/// macro of the same name in the `strum_macros` crate. Currently, the string,
/// integer and bool literals are supported in attributes.
///
/// # Example
///
/// ```rust
...
/// A trait for retrieving the enum generated by [`EnumDiscriminants`] from an
/// associated Type on the original enumeration. This trait can be autoderived
/// by `strum_macros`.
...
/// A trait for retrieving a static array containing all the variants in an
/// Enum. This trait can be autoderived by `strum_macros`. For derived usage,
...
/// using it alongside [`EnumDiscriminants`] if you require inner data but still
/// want to have an static array of variants.
...
// and then reference it through strum. The renaming feature should be
// deprecated now that 2018 edition is almost 2 years old, but we'll need to
// give people some time to do that.
