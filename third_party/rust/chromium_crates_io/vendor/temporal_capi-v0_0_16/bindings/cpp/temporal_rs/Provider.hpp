...
namespace capi {
    ...
    temporal_rs_Provider_new_zoneinfo64_result temporal_rs_Provider_new_zoneinfo64(diplomat::capi::DiplomatU32View data);
...
inline diplomat::result<std::unique_ptr<temporal_rs::Provider>, std::monostate> temporal_rs::Provider::new_zoneinfo64(diplomat::span<const uint32_t> data) {
  auto result = temporal_rs::capi::temporal_rs_Provider_new_zoneinfo64({data.data(), data.size()});
  return result.is_ok ? diplomat::result<std::unique_ptr<temporal_rs::Provider>, std::monostate>(diplomat::Ok<std::unique_ptr<temporal_rs::Provider>>(std::unique_ptr<temporal_rs::Provider>(temporal_rs::Provider::FromFFI(result.ok)))) : diplomat::result<std::unique_ptr<temporal_rs::Provider>, std::monostate>(diplomat::Err<std::monostate>());
}
