...
impl<'a> LinesIter<'a> {
    fn new(s: &'a str) -> Self {
        let mut this = Self { iter: s.lines().peekable() };
...
// --downgrade-cff2,--desubroutinize, --iftb-requirements, --retian-num-glyphs,
// --gid-map
        ...
        match split_line {
            ...
            Some((option_str, list)) => {
                ...
                match option {
                    ...
                    "--gids" => {
                        parse_list(list, gids, action, |s| GlyphId::from(s.parse::<u32>().unwrap()))
                    }
                    "--unicodes" => {
                        parse_list(list, unicodes, action, |s| u32::from_str_radix(s, 16).unwrap())
                    }
...
fn parse_tag_list(list: &str, tag_set: &mut IntSet<Tag>, action: Option<bool>) {
    parse_list::<Tag>(list, tag_set, action, |s| Tag::new_checked(s.as_bytes()).unwrap());
}
...
impl SubsetTestCase {
    ...
    fn gen_expected_output(&self) {
        ...
        let expected_dir = Path::new(TEST_DATA_DIR).join("expected").join(&self.expected_dir);
...
fn compare_with_expected(output_dir: &Path, output_file: &Path, expected_file: &Path) {
    ...
    if expected != output {
        // uncomment to overwrite expected file with output for updating integration
        // tests fs::write(expected_file, &output).unwrap(); return;
        assert_has_ttx_exec();
...
fn parse_test() {
    ...
    assert_eq!(subset_test.profiles[2], String::from("drop-hints-retain-gids.txt"));
    ...
    assert_eq!(subset_test.profiles[10], String::from("keep-all-layout-features.txt"));
    ...
    assert_eq!(subset_test.profiles[13], String::from("retain-gids-glyph-names.txt"));
    subset_flag = SubsetFlags::SUBSET_FLAGS_DEFAULT;
