...
// reference: subset() for CBLC in fonttools, Harfbuzz implementation is
// suboptimal <https://github.com/fonttools/fonttools/blob/7854669acd63be43e1ad41d0486297d8d6da325d/Lib/fontTools/subset/__init__.py#L1799>
impl Subset for Cblc<'_> {
    fn subset(
        ...
        let cbdt = font.cbdt().or(Err(SubsetError::SubsetTableError(Cbdt::TAG)))?;

        s.embed(self.major_version()).map_err(|_| SubsetError::SubsetTableError(Cblc::TAG))?;
        s.embed(self.minor_version()).map_err(|_| SubsetError::SubsetTableError(Cblc::TAG))?;

        let mut num_sizes: u32 = 0;
        let num_sizes_pos =
            s.embed(num_sizes).map_err(|_| SubsetError::SubsetTableError(Cblc::TAG))?;

        let bitmapsize_records = self.bitmap_sizes();
        let bitmapsize_bytes =
            self.offset_data().as_bytes().get(self.bitmap_sizes_byte_range()).unwrap();
        ...
        for (idx, bitmap_size_table) in bitmapsize_records.iter().enumerate() {
            ...
            let src_bytes = bitmapsize_bytes.get(start..start + BitmapSize::RAW_BYTE_LEN).unwrap();
...
impl SubsetTable<'_> for IndexSubtableRecord {
    ...
    fn subset(
        ...
        let _ = s.add_link(offset_pos..offset_pos + 4, args.0, OffsetWhence::Head, 0, false);
        Ok(())
    }
}
...
impl<'a> SubsetTable<'a> for IndexSubtable1<'a> {
    ...
    fn subset(
        ...
        for (new_gid, old_gid) in
            plan.new_to_old_gid_list.iter().filter(|&(_, g)| retained_glyphs.contains(*g))
        {
...
mod test {
    ...
    fn test_subset_cbdt_noop() {
        let font =
            FontRef::new(include_bytes!("../test-data/fonts/NotoColorEmoji.subset.ttf")).unwrap();

        let cblc = font.cblc().unwrap();
        let mut builder = FontBuilder::new();

        let mut plan = Plan::default();
        plan.glyphset.insert_range(GlyphId::NOTDEF..=GlyphId::from(5_u32));

        plan.glyph_map.insert(GlyphId::NOTDEF, GlyphId::NOTDEF);
        plan.glyph_map.insert(GlyphId::from(1_u32), GlyphId::from(1_u32));
        plan.glyph_map.insert(GlyphId::from(2_u32), GlyphId::from(2_u32));
        plan.glyph_map.insert(GlyphId::from(3_u32), GlyphId::from(3_u32));
        plan.glyph_map.insert(GlyphId::from(4_u32), GlyphId::from(4_u32));
        plan.glyph_map.insert(GlyphId::from(5_u32), GlyphId::from(5_u32));

        plan.new_to_old_gid_list.push((GlyphId::NOTDEF, GlyphId::NOTDEF));
        plan.new_to_old_gid_list.push((GlyphId::from(1_u32), GlyphId::from(1_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(2_u32), GlyphId::from(2_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(3_u32), GlyphId::from(3_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(4_u32), GlyphId::from(4_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(5_u32), GlyphId::from(5_u32)));
    ...
    fn test_subset_cbdt_keep_one() {
        ...
        plan.glyph_map.insert(GlyphId::NOTDEF, GlyphId::NOTDEF);
        plan.glyph_map.insert(GlyphId::from(2_u32), GlyphId::from(1_u32));

        plan.new_to_old_gid_list.push((GlyphId::NOTDEF, GlyphId::NOTDEF));
        plan.new_to_old_gid_list.push((GlyphId::from(1_u32), GlyphId::from(2_u32)));
    ...
    fn test_subset_cbdt_keep_one_last_subtable() {
        ...
        plan.glyph_map.insert(GlyphId::NOTDEF, GlyphId::NOTDEF);
        plan.glyph_map.insert(GlyphId::from(4_u32), GlyphId::from(1_u32));

        plan.new_to_old_gid_list.push((GlyphId::NOTDEF, GlyphId::NOTDEF));
        plan.new_to_old_gid_list.push((GlyphId::from(1_u32), GlyphId::from(4_u32)));
    ...
    fn test_subset_cbdt_keep_multiple_subtables() {
        ...
        plan.glyph_map.insert(GlyphId::NOTDEF, GlyphId::NOTDEF);
        plan.glyph_map.insert(GlyphId::from(1_u32), GlyphId::from(1_u32));
        plan.glyph_map.insert(GlyphId::from(3_u32), GlyphId::from(2_u32));
        plan.glyph_map.insert(GlyphId::from(4_u32), GlyphId::from(3_u32));

        plan.new_to_old_gid_list.push((GlyphId::NOTDEF, GlyphId::NOTDEF));
        plan.new_to_old_gid_list.push((GlyphId::from(1_u32), GlyphId::from(1_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(2_u32), GlyphId::from(3_u32)));
        plan.new_to_old_gid_list.push((GlyphId::from(3_u32), GlyphId::from(4_u32)));
