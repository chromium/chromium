...
impl SubsetFlags {
    ...
    /// subroutines from the CFF glyphs. This flag is UNIMPLEMENTED yet
    ...
    /// deltas. This flag is UNIMPLEMENTED yet
    ...
    /// Returns `true` if all of the flags in `other` are contained within
    /// `self`.
    #[inline]
...
pub struct Plan {
    ...
    //active features(with duplicates) old->new feature index map, used by
    // Script/FeatureVariations
...
impl Plan {
    ...
    pub fn new(
        ...
        this.no_subset_tables.extend(default_no_subset_tables.iter().copied());
    ...
    fn populate_unicodes_to_retain(
        ...
        let unicodes = unicode_closure(
            input_unicodes,
            !self.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_BIDI_CLOSURE),
        );
        ...
        if input_gids.is_empty() && unicodes.len() < (self.font_num_glyphs as u64) {
            ...
            let vec_cap: usize = vec_cap.min(cmap_unicodes.len()).try_into().unwrap_or(usize::MAX);
            ...
            for range in input_gids.iter_ranges() {
                ...
                self.glyphset_gsub.insert_range(*range.start()..=GlyphId::from(last as u32));
            }
        }
        self.unicode_to_new_gid_list.sort();
        self.glyphset_gsub.extend(self.unicode_to_new_gid_list.iter().map(|t| t.1));
        self.unicodes.extend(self.unicode_to_new_gid_list.iter().map(|t| t.0));
        ...
        if let Ok(cmap) = font.cmap() {
            ...
            if let Ok(i) = encoding_records.binary_search_by(|r| {
                ...
                if let Ok(CmapSubtable::Format14(cmap14)) =
                    encoding_records.get(i).unwrap().subtable(cmap.offset_data())
                {
    ...
    fn populate_gids_to_retain(&mut self, font: &FontRef) {
        ...
        /* Populate a full set of glyphs to retain by adding all referenced composite
         * glyphs. */
    ...
    fn create_old_gid_to_new_gid_map(&mut self) {
        ...
        if !self.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_RETAIN_GIDS) {
            self.new_to_old_gid_list
                .extend(self.glyphset.iter().zip(0u16..).map(|x| (GlyphId::from(x.1), x.0)));
            self.num_output_glyphs = self.new_to_old_gid_list.len();
        } else {
            self.new_to_old_gid_list.extend(self.glyphset.iter().map(|x| (x, x)));
    ...
    fn colr_closure(&mut self, font: &FontRef) {
        if let Ok(colr) = font.colr() {
            ...
            let _ =
                std::mem::replace(&mut self.colr_palettes, remap_palette_indices(palette_indices));
            ...
            // varIdx,delta). Otherwise, it's a mapping from old delta set idx->
            // (new delta set idx, delta). Mapping delta set indices is the same
            // as gid mapping.
            //
            ...
            if let Some(Ok(var_store)) = colr.item_variation_store() {
                ...
                // when a DeltaSetIndexMap is included, collected variation indices are actually
                // delta set indices, we need to map them into variation indices
    ...
    fn collect_layout_var_indices(&mut self, font: &FontRef) {
        ...
        let vardata_count = var_store.item_variation_data_count() as u32;
        remap_variation_indices(vardata_count, &varidx_set, &mut self.layout_varidx_delta_map);
...
// TODO: when instancing, calculate delta value and set new varidx to
// NO_VARIATIONS_IDX if all axes are pinned
...
pub(crate) fn remap_indices<T: Domain + std::cmp::Eq + std::hash::Hash + From<u16>>(
    ...
    indices.iter().enumerate().map(|x| (x.1, T::from(x.0 as u16))).collect()
}
...
fn remap_palette_indices(indices: IntSet<u16>) -> FnvHashMap<u16, u16> {
    indices
        .iter()
        .enumerate()
        .map(|x| if x.1 == 0xFFFF { (0xFFFF, 0xFFFF) } else { (x.1, x.0 as u16) })
        .collect()
}
...
pub(crate) trait LayoutClosure {
    ...
    //remove unreferenced langsys and return (script->langsys mapping, retained
    // feature indices)
    fn prune_langsys(
        &self,
...
pub trait Subset {
    /// Subset this table, if successful a subset version of this table will be
    /// added to builder
    fn subset(
        &self,
        _plan: &Plan,
    ...
    /// This is needed when some tables have dependencies on other table's
    /// subset output
...
// A helper trait providing a 'subset' method for various subtables that have no
// associated tag
pub(crate) trait SubsetTable<'a> {
    type ArgsForSubset: 'a;
    type Output: 'a;
    /// Subset this table and write a subset version of this table into
    /// serializer
    fn subset(
        &self,
        plan: &Plan,
...
fn should_drop_table(tag: Tag, plan: &Plan) -> bool {
    ...
    let no_hinting = plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_HINTING);
...
fn try_subset<'a>(
    ...
    s.start_serialize().map_err(|_| SubsetError::SubsetTableError(table_tag))?;
...
fn subset_table<'a>(
    ...
    match tag {
        ...
        _ if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_PASSTHROUGH_UNRECOGNIZED) => {
...
fn passthrough_table(tag: Tag, font: &FontRef<'_>, s: &mut Serializer) -> Result<(), SubsetError> {
    if let Some(data) = font.data_for_tag(tag) {
        s.embed_bytes(data.as_bytes()).map_err(|_| SubsetError::SubsetTableError(tag))?;
    }
    Ok(())
}
...
pub fn estimate_subset_table_size(font: &FontRef, table_tag: Tag, plan: &Plan) -> usize {
    ...
    // ported from HB: Tables that we want to allocate same space as the source
    // table. For GSUB/GPOS it's because those are expensive to subset, so
    // giving them more room is fine.
    let same_size: bool =
        table_tag == Gsub::TAG || table_tag == Gpos::TAG || table_tag == Name::TAG;

    if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_RETAIN_GIDS) {
...
mod test {
    ...
    fn populate_unicodes_wo_input_gid() {
        ...
        assert_eq!(plan.codepoint_to_glyph.get(&0x2c_u32), Some(GlyphId::new(2)).as_ref());
        assert_eq!(plan.codepoint_to_glyph.get(&0x31_u32), Some(GlyphId::new(4)).as_ref());
    }
