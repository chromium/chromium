...
impl<'a> SubsetTable<'a> for MarkLigPosFormat1<'_> {
    ...
    fn subset(
        ...
        // lig glyphs might have no anchor points defined for retained class of mark
        // glyphs
...
mod test {
    ...
    fn test_collect_variation_indices_markligpos() {
        ...
        let font = FontRef::new(include_bytes!("../../test-data/fonts/Comfortaa-Regular-new.ttf"))
            .unwrap();
    ...
    fn test_subset_marklig_pos() {
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(67_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(68_u32), GlyphId::from(2_u32));
        ...
        markligpos_table.subset(&plan, &mut s, (&subset_state, &font, &plan.gpos_lookups)).unwrap();
        assert!(!s.in_error());
        s.end_serialize();
    ...
    fn test_subset_marklig_pos_remove_empty_ligattach() {
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(30_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(987_u32), GlyphId::from(3_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(216_u32), GlyphId::from(2_u32));
