...
impl<'a> SubsetTable<'a> for PairValueRecord {
    ...
    fn subset(
        ...
        //value records
        self.value_record1().subset(plan, s, (new_format1, font_data))?;
        self.value_record2().subset(plan, s, (new_format2, font_data))
    }
}
...
impl<'a> SubsetTable<'a> for PairPosFormat1<'_> {
    ...
    fn subset(
        ...
        // value_formats
        let (new_format1, new_format2) =
            if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_HINTING) {
                // do not strip hints for VF unless it has no GDEF varstore after subsetting
                let strip_hints =
                    if font.fvar().is_ok() { !subset_state.has_gdef_varstore } else { true };
...
impl<'a> SubsetTable<'a> for PairPosFormat2<'_> {
    ...
    fn subset(
        ...
        // value formats
        let (subset_state, font) = args;
        let class1_idxes: Vec<u16> =
            (0..self.class1_count()).filter(|i| class1_map.contains_key(i)).collect();
        let class2_idxes: Vec<u16> =
            (0..self.class2_count()).filter(|i| class2_map.contains_key(i)).collect();
        ...
        for i in class1_idxes {
            ...
            for j in &class2_idxes {
                ...
                class2_rec.value_record1().subset(plan, s, (new_format1, font_data))?;
                class2_rec.value_record2().subset(plan, s, (new_format2, font_data))?;
            }
        }

        // this can be moved, put it at last so we have the same binary data with
        // Harfbuzz subsetter
...
mod test {
    ...
    fn test_subset_pairpos_format1() {
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(6292_u32), GlyphId::from(3_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(6298_u32), GlyphId::from(4_u32));
        ...
        pairpos_table.subset(&plan, &mut s, (&subset_state, &font, &plan.gpos_lookups)).unwrap();
        assert!(!s.in_error());
        s.end_serialize();
    ...
    fn test_subset_pairpos_format2() {
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(40_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(72_u32), GlyphId::from(2_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(168_u32), GlyphId::from(3_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(6736_u32), GlyphId::from(4_u32));
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(72_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(168_u32), GlyphId::from(2_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(6736_u32), GlyphId::from(3_u32));
    ...
    fn test_collect_variation_indices_pairpos_format1() {
        ...
        let font =
            FontRef::new(include_bytes!("../../test-data/fonts/RobotoFlex-Variable.ttf")).unwrap();
        let gpos_lookups = font.gpos().unwrap().lookup_list().unwrap();
