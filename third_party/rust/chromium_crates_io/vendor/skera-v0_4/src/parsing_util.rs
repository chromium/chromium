...
pub fn populate_gids(gid_str: &str) -> Result<IntSet<GlyphId>, SubsetError> {
    ...
    for gid in gid_str.split(',') {
        if let Some((start, end)) = gid.split_once('-') {
            let start: u32 =
                start.parse::<u32>().map_err(|_| SubsetError::InvalidGid(start.to_owned()))?;
            let end: u32 =
                end.parse::<u32>().map_err(|_| SubsetError::InvalidGid(end.to_owned()))?;
            if start > end {
            ...
            let glyph_id: u32 =
                gid.parse::<u32>().map_err(|_| SubsetError::InvalidGid(gid.to_owned()))?;
...
/// parse input unicodes string, which is a comma/whitespace-separated list of
/// Unicode codepoints or ranges as hex numbers, optionally prefixed with 'U+',
/// 'u', etc. For example: --unicodes=41-5a,61-7a adds ASCII letters, so does
/// the more verbose --unicodes=U+0041-005A,U+0061-007A. The special strings '*'
...
fn parse_list<T: Domain>(
    ...
    input_str.split(&[',', ' ']).filter(|raw| !raw.is_empty()).map(parse_one).collect()
}

//parse input tag list string, which is a comma/whitespace-separated list of
// tags(layout script or feature or table name)
...
//parse input name_IDs string, which is a comma/whitespace-separated list of
// nameIDs that will be retained
pub fn parse_name_ids(input_str: &str) -> Result<IntSet<NameId>, SubsetError> {
    parse_list(input_str, |raw| {
        raw.parse::<u16>().map(NameId::from).map_err(|_| SubsetError::InvalidId(raw.to_owned()))
    })
}

//parse input name_languages string, which is a comma/whitespace-separated list
// of langIDs that will be retained
pub fn parse_name_languages(input_str: &str) -> Result<IntSet<u16>, SubsetError> {
    parse_list(input_str, |raw| {
        raw.parse::<u16>().map_err(|_| SubsetError::InvalidId(raw.to_owned()))
    })
}
