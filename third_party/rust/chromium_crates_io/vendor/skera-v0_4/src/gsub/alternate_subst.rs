...
mod test {
    ...
    fn test_subset_alternate_subst_format1() {
        ...
        let font =
            FontRef::new(include_bytes!("../../test-data/fonts/gsub_alternate_substitution.otf"))
                .unwrap();
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(3_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(10_u32), GlyphId::from(2_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(18_u32), GlyphId::from(3_u32));
        ...
        alt_subst_table.subset(&plan, &mut s, (&subset_state, &font, &plan.gsub_lookups)).unwrap();
        assert!(!s.in_error());
        s.end_serialize();
