...
impl<'a> SubsetTable<'a> for LigatureSubstFormat1<'_> {
    ...
    fn subset(
        ...
        // we need to serialize retained coverage glyphs first so it'll be packed after
        // the LigatureSet and Ligature tables ref: <https://github.com/harfbuzz/harfbuzz/blob/0a257b0188ce8b002b51d9955713cd7136ca4769/src/OT/Layout/GSUB/LigatureSubstFormat1.hh#L155>
        ...
        // to ensure that the repacker always orders the coverage table after the
        // LigatureSet and LigatureSubtable's they will be linked to the
        // Coverage table via a virtual link the coverage table object idx is
        // passed down to facilitate this.
...
impl SubsetTable<'_> for Ligature<'_> {
    ...
    fn subset(
        ...
        // ensure that the repacker always orders the coverage table after this Ligature
        // subtable
        s.add_virtual_link(coverage_idx)
    }
}
...
mod test {
    ...
    fn test_subset_ligature_subst() {
        ...
        plan.glyph_map_gsub.insert(GlyphId::from(51_u32), GlyphId::from(1_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(169_u32), GlyphId::from(4_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(170_u32), GlyphId::from(5_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(657_u32), GlyphId::from(9_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(659_u32), GlyphId::from(10_u32));
        plan.glyph_map_gsub.insert(GlyphId::from(1208_u32), GlyphId::from(13_u32));
        ...
        ligsubst_table.subset(&plan, &mut s, (&subset_state, &font, &plan.gsub_lookups)).unwrap();
        assert!(!s.in_error());
        s.end_serialize();
