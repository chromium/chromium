...
impl NameIdClosure for Fvar<'_> {
    ...
    fn collect_name_ids(&self, plan: &mut Plan) {
        ...
        plan.name_ids.extend_unsorted(axis_instance_array.axes().iter().map(|x| x.axis_name_id()));

        for instance_record in axis_instance_array.instances().iter().filter_map(|x| x.ok()) {
