...
impl<'a> Serialize<'a> for Hvar<'_> {
    ...
    fn serialize(s: &mut Serializer, args: Self::Args) -> Result<(), SerializeErrorFlags> {
        ...
        if hvar.item_variation_store_offset().is_null() {
            return s.allocate_size(5 * Offset32::RAW_BYTE_LEN, true).map(|_| ());
        }
        ...
        serialize_index_maps(s, plan, &index_maps, hvar_subset_plan.index_map_subset_plans())
    }
}
...
impl IndexMapSubsetPlan {
    ...
    fn remap(
        ...
        for (max_inner, inner_map) in self.max_inners.iter().zip(inner_maps) {
            ...
            let bit_count =
                32 - inner_map.get(*max_inner as u32).unwrap_or(&0).leading_zeros() as u8;
        ...
        for (new_gid, old_gid) in plan.new_to_old_gid_list.iter() {
            ...
            self.output_map.insert(new_gid.to_u32(), (*new_outer << 16) | *new_inner);
        }
    }
...
impl HvarVvarSubsetPlan {
    pub(crate) fn new(
        ...
        let retain_adv_map = index_maps[0].is_none()
            && plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_RETAIN_GIDS);

        if retain_adv_map {
            let inner_map =
                plan.new_to_old_gid_list.iter().map(|(_, old_gid)| old_gid.to_u32()).collect();
        ...
        this.index_map_subset_plans.iter_mut().zip(index_maps).for_each(
            |(index_map_subset_plan, index_map)| {
...
mod test {
    ...
    fn test_subset_hvar_noop() {
        ...
        plan.glyphset.insert_range(GlyphId::NOTDEF..=GlyphId::from(3_u32));
