...
impl Subset for Gvar<'_> {
    fn subset(
        ...
        s.embed(num_glyphs).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?;
        ...
        let subset_data_size: usize = plan
            ...
            .filter_map(|x| {
                if x.0 == GlyphId::NOTDEF
                    && !plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NOTDEF_OUTLINE)
                {
                    return None;
                }
                self.data_for_gid(x.0).ok().flatten().map(|data| data.len() + data.len() % 2)
            })
            .sum();

        // According to the spec: If the short format (Offset16) is used for offsets,
        // the value stored is the offset divided by 2. So the maximum subset
        // data size that could use short format should be 2 * 0xFFFFu, which is 0x1FFFE
        let long_offset = if subset_data_size > 0x1FFFE_usize { 1_u16 } else { 0_u16 };
        // flags
        s.embed(long_offset).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?;
...
fn subset_with_offset_type<OffsetType: GvarOffset>(
    ...
    // shared tuples array follow the GlyphVariationData offsets array at the end of
    // the 'gvar' header.
    ...
    //update sharedTuplesOffset, which is of Offset32 type and byte position in
    // gvar is 8..12
    s.copy_assign(gvar.shared_tuples_offset_byte_range().start, shared_tuples_offset);

    // calculate glyphVariationDataArrayOffset: put the glyphVariationData at last
    // in the table
    ...
    s.embed(glyph_var_data_offset).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?;
    ...
    if shared_tuples_offset > 0 {
        ...
        s.embed_bytes(shared_tuples_data).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?;
    }
    ...
    for (new_gid, old_gid) in plan.new_to_old_gid_list.iter().filter(|x| {
        ...
        if let Some(glyph_var_data) =
            gvar.data_for_gid(*old_gid).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?
        {
            ...
            if off_size == 2 && len % 2 != 0 {
                s.embed(1_u8).map_err(|_| SubsetError::SubsetTableError(Gvar::TAG))?;
                glyph_offset += 1;
            }
        };
