...
// glyf data is written into the serializer, returning loca data to be added by
// FontBuilder
fn write_glyf_loca(
    font: &FontRef,
    plan: &Plan,
    ...
    if loca_format == 0 {
        ...
        for ((new_gid, _), i) in plan.new_to_old_gid_list.iter().zip(0u16..) {
            ...
            loca_out.extend_from_slice(&value);
            s.embed_bytes(g).map_err(|_| SubsetError::SubsetTableError(Glyf::TAG))?;
            if padded_len > g.len() {
                s.embed_bytes(&[0]).map_err(|_| SubsetError::SubsetTableError(Glyf::TAG))?;
            }
    ...
    // As a special case when all glyph in the font are empty, add a zero byte to
    // the table, so that OTS doesn’t reject it, and to make the table work on
    // Windows as well. See https://github.com/khaledhosny/ots/issues/52
...
fn subset_simple_glyph(g: &SimpleGlyph, plan: &Plan) -> Vec<u8> {
    ...
    if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_SET_OVERLAPS_FLAG) {
...
fn subset_composite_glyph(g: &CompositeGlyph, plan: &Plan) -> Vec<u8> {
    ...
    while more {
        ...
        if flags.contains(CompositeGlyphFlags::WE_HAVE_INSTRUCTIONS) {
            ...
            if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_HINTING) {
                ...
                out.get_mut(i..i + 2).unwrap().copy_from_slice(&flags.bits().to_be_bytes());
            }
        }

        // only set overlaps flag on the first component
        if plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_SET_OVERLAPS_FLAG) && i == 10 {
    ...
    if we_have_instructions && !plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_HINTING) {
        if i + 1 >= len {
            return Vec::new();
        }
...
// trim padding bytes for simple glyphs, return trimmed length of the raw data
// for flags & x/y coordinates
...
fn subset_head(head: &Head, loca_format: u8) -> Vec<u8> {
    ...
    out.get_mut(50..52).unwrap().copy_from_slice(&[0, loca_format]);
    out
}
...
mod test {
    ...
    fn test_subset_composite_glyph_trim_padding() {
        ...
        plan.glyph_map.insert(GlyphId::from(1_u16), GlyphId::from(2_u16));
