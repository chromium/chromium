//! binary subset tool
//!
//! Takes a font file and a subset input which describes the desired subset, and
//! output is a new font file containing only the data specified in the input.
...
fn main() {
    ...
    std::fs::write(&args.output_file, output_bytes).unwrap_or_else(|err| {
        panic!("Failed to write output to {path:?}.\n{err}", path = &args.output_file)
    });
}
