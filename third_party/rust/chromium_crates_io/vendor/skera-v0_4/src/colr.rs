...
impl Subset for Colr<'_> {
    fn subset(
        ...
        if let Some(layer_list) =
            self.layer_list().transpose().map_err(|_| SubsetError::SubsetTableError(Colr::TAG))?
        {
        ...
        if let Some(clip_list) =
            self.clip_list().transpose().map_err(|_| SubsetError::SubsetTableError(Colr::TAG))?
        {
...
impl SubsetTable<'_> for BaseGlyphList<'_> {
    ...
    fn subset(
        ...
        for paint_record in self.base_glyph_paint_records() {
            ...
            if !plan.glyphset_colred.contains(GlyphId::from(paint_record.glyph_id())) {
                continue;
            }
...
fn serialize_clips(
    ...
    for g in gids_set.iter().skip(1) {
        let offset = gids_offset_map.get(&g).ok_or(SerializeErrorFlags::SERIALIZE_ERROR_OTHER)?;
...
impl SubsetTable<'_> for ClipBoxFormat2<'_> {
    ...
    fn subset(
        ...
        if varidx_base != NO_VARIATION_INDEX {
            ...
            // update VarIdxBase
            s.copy_assign(start_pos + self.var_index_base_byte_range().start, *new_varidx);
        }
        Ok(())
    }
}
...
mod test {
    ...
    fn test_subset_colr_retain_all() {
        ...
        plan.glyphset_colred.insert_range(GlyphId::NOTDEF..=GlyphId::from(6_u32));
        ...
        plan.glyph_map.insert(GlyphId::from(6_u32), GlyphId::from(6_u32));
    ...
    fn test_subset_colr_keep_one_colr_glyph() {
        ...
        plan.glyph_map.insert(GlyphId::from(4_u32), GlyphId::from(2_u32));
        plan.glyph_map.insert(GlyphId::from(5_u32), GlyphId::from(3_u32));
    ...
    fn test_subset_colr_keep_mixed_glyph() {
        ...
        plan.glyph_map.insert(GlyphId::from(6_u32), GlyphId::from(4_u32));
