...
impl Subset for Base<'_> {
    fn subset(
        ...
        s.embed(self.version()).map_err(|_| SubsetError::SubsetTableError(Base::TAG))?;

        //hAxis offset
        let haxis_offset_pos =
            s.embed(0_u16).map_err(|_| SubsetError::SubsetTableError(Base::TAG))?;

        if let Some(h_axis) =
            self.horiz_axis().transpose().map_err(|_| SubsetError::SubsetTableError(Base::TAG))?
        {
        ...
        //vertAxis offset
        let vaxis_offset_pos =
            s.embed(0_u16).map_err(|_| SubsetError::SubsetTableError(Base::TAG))?;

        if let Some(v_axis) =
            self.vert_axis().transpose().map_err(|_| SubsetError::SubsetTableError(Base::TAG))?
        {
        ...
        {
            let varstore_offset_pos =
                s.embed(0_u32).map_err(|_| SubsetError::SubsetTableError(Base::TAG))?;
...
impl SubsetTable<'_> for Axis<'_> {
    ...
    fn subset(
        ...
        let base_scriptlist =
            self.base_script_list().map_err(|_| SerializeErrorFlags::SERIALIZE_ERROR_READ_ERROR)?;
...
impl<'a> SubsetTable<'a> for BaseScriptRecord {
    ...
    fn subset(
        ...
        let base_script =
            self.base_script(data).map_err(|_| SerializeErrorFlags::SERIALIZE_ERROR_READ_ERROR)?;
