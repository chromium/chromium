...
impl<'a> SubsetTable<'a> for ItemVariationStore<'a> {
    ...
    fn subset(
        ...
        Offset32::serialize_subset(&var_region_list, s, plan, &region_map, region_list_offset_pos)?;
...
impl<'a> SubsetTable<'a> for VariationRegionList<'a> {
    ...
    fn subset(
        ...
        let Some(src_var_regions_bytes) =
            self.offset_data().as_bytes().get(self.variation_regions_byte_range())
        else {
            return Err(s.set_err(SerializeErrorFlags::SERIALIZE_ERROR_READ_ERROR));
        };
...
impl<'a> SubsetTable<'a> for ItemVariationData<'_> {
    ...
    fn subset(
        ...
        let new_word_delta_count = if has_long { new_word_count | 0x8000 } else { new_word_count };
...
impl<'a> DeltaSetIndexMapSerializePlan<'a> {
    pub(crate) fn new(
        ...
        Self { outer_bit_count, inner_bit_count, output_map, map_count }
    }
...
mod test {
    ...
    fn test_subset_item_varstore() {
        ...
        //layout_features
        plan.layout_features.extend(DEFAULT_LAYOUT_FEATURES.iter().copied());
