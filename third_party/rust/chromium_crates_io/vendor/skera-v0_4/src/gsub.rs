...
impl LayoutClosure for Gsub<'_> {
    fn prune_features(
        ...
        prune_features(&feature_list, &alternate_features, lookup_indices, feature_indices)
    }
    ...
    fn closure_glyphs_lookups_features(&self, plan: &mut Plan) {
        ...
        if !plan.subset_flags.contains(SubsetFlags::SUBSET_FLAGS_NO_LAYOUT_CLOSURE)
            && self.closure_glyphs(&lookup_indices, &mut plan.glyphset_gsub).is_err()
        {
            return;
        };
        if self.closure_lookups(&plan.glyphset_gsub, &mut lookup_indices).is_err() {
            return;
        };
...
mod test {
    ...
    fn test_nameid_closure() {
        ...
        let font =
            FontRef::new(include_bytes!("../test-data/fonts/NotoSansOriya-subset.ttf")).unwrap();
    ...
    fn test_prune_features_w_variations() {
        ...
        // feature indexed at 0 has an alternate version that intersects lookup indexed
        // at 1
        let retained_features = gsub.prune_features(&lookup_indices, feature_indices);
        assert_eq!(retained_features.len(), 1);
