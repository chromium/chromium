...
pub struct Cpal {
    ...
    ///
    /// This is an array of 32-bit flag fields that describe properties of each
    /// palette.
    ///
    ...
    /// name ID range) that specify user interface strings associated with
    /// each palette. Use 0xFFFF if no name ID is provided for a palette.
    ///
    ...
    /// Offset from the beginning of CPAL table to the [Palette Entry Labels
    /// Array][].
    ///
    /// This is an array of 'name' table IDs (typically in the font-specific
    /// name ID range) that specify user interface strings associated with
    /// each palette entry, e.g. “Outline”, “Fill”. This set of palette
    /// entry labels applies to all palettes in the font. Use  0xFFFF if no
    /// name ID is provided for a palette entry.
    ///
...
impl FontWrite for Cpal {
    ...
    fn write_into(&self, writer: &mut TableWriter) {
        ...
        version.compatible(1u16).then(|| self.palette_types_array.write_into(writer));
        version.compatible(1u16).then(|| self.palette_labels_array.write_into(writer));
        version.compatible(1u16).then(|| self.palette_entry_labels_array.write_into(writer));
    }
...
impl FromObjRef<read_fonts::tables::cpal::ColorRecord> for ColorRecord {
    fn from_obj_ref(obj: &read_fonts::tables::cpal::ColorRecord, _: FontData) -> Self {
        ColorRecord { blue: obj.blue(), green: obj.green(), red: obj.red(), alpha: obj.alpha() }
    }
}
