...
impl Sbix {
    ...
    pub fn new(flags: HeaderFlags, strikes: Vec<Strike>) -> Self {
        Self { flags, strikes: strikes.into_iter().map(Into::into).collect() }
    }
}
...
impl<'a> FromObjRef<read_fonts::tables::sbix::Sbix<'a>> for Sbix {
    fn from_obj_ref(obj: &read_fonts::tables::sbix::Sbix<'a>, _: FontData) -> Self {
        Sbix { flags: obj.flags(), strikes: obj.strikes().to_owned_table() }
    }
}
...
pub struct Strike {
    ...
    /// (E.g., 96 PPI, 192 PPI.)
    pub ppi: u16,
    /// Offset from the beginning of the strike data header to bitmap data for
    /// an individual glyph ID.
...
impl Strike {
    ...
    pub fn new(ppem: u16, ppi: u16, glyph_data_offsets: Vec<u32>) -> Self {
        Self { ppem, ppi, glyph_data_offsets }
    }
}
...
pub struct GlyphData {
    /// The horizontal (x-axis) position of the left edge of the bitmap graphic
    ...
    /// sequential entries in the glyphDataOffsets array and the fixed size (8
    /// bytes) of the preceding fields.
    pub data: Vec<u8>,
}
...
impl GlyphData {
    ...
    pub fn new(
        ...
        Self { origin_offset_x, origin_offset_y, graphic_type, data }
    }
}
