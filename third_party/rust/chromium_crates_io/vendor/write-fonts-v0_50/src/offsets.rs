...
// useful than the Option::as_ref and Option::as_mut methods available through
// deref
...
impl<const N: usize, T> NullableOffsetMarker<T, N> {
    ...
    pub fn new(obj: Option<T>) -> Self {
        NullableOffsetMarker { obj: obj.map(|t| Box::new(t)) }
    }
...
//impl<T: std::fmt::Debug, const N: usize> std::fmt::Debug for OffsetMarker<T,
// N> { fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
//write!(f, "OffsetMarker({}, {:?})", N * 8, self.obj.as_ref(),)
//}

//impl<const N: usize, T: std::fmt::Debug> std::fmt::Debug for
// NullableOffsetMarker<T, N> { fn fmt(&self, f: &mut std::fmt::Formatter) ->
// std::fmt::Result { write!(
//f,
