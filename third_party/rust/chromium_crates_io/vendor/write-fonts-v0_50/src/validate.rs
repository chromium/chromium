...
pub trait Validate {
    ...
    ///
    /// This is an auto-generated method that calls to
    /// [validate_impl][Self::validate_impl] and collects any errors.
...
/// via appropriate calls to methods like [in_table][Self::in_table] and
...
impl ValidationCtx {
    ...
    pub fn report(&mut self, msg: impl Display) {
        self.errors
            .push(ValidationError { location: self.cur_location.clone(), error: msg.to_string() });
    }
...
mod tests {
    ...
    fn sanity_check_array_validation() {
        ...
        let my_derps =
            DerpStore { derps: [1i16, 0, 3, 4, 12, 7, 6].into_iter().map(Derp).collect() };
