...
/// A trait for types that can resolve themselves when provided data to resolve
/// offsets.
///
/// It is possible that the generated object is malformed; for instance offsets
/// may be null where it is not allowed. This can be checked by calling
/// [`validate`][] on the generated object.
///
...
pub trait FromObjRef<T: ?Sized>: Sized {
    /// Convert `from` to an instance of `Self`, using the provided data to
    /// resolve offsets.
...
/// [`std::convert::Into`], it is provided as a blanket impl when you implement
/// [`FromObjRef<T>`].
pub trait ToOwnedObj<T> {
    /// Convert this type into `T`, using the provided data to resolve any
    /// offsets.
    fn to_owned_obj(&self, data: FontData) -> T;
}

/// A conversion from a fully resolvable parsed font table to its owned
/// equivalent.
///
    ...
    O: Scalar + Offset,
{
    fn from_obj_ref(from: &ArrayOfOffsets<'a, U, O>, data: FontData) -> Self {
        from.iter().map(|x| OffsetMarker::from_obj_ref(&x, data)).collect()
    }
}
    ...
    O: Scalar + Offset,
{
    fn from_obj_ref(from: &ArrayOfNullableOffsets<'a, U, O>, data: FontData) -> Self {
        from.iter().map(|x| NullableOffsetMarker::from_obj_ref(&x, data)).collect()
    }
}
