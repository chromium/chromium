...
impl<T: SplitTable + FontWrite> TableSplitter<T> {
    ...
    fn finish_current(&mut self) {
        if !self.current_coverage.is_empty() {
            ...
            self.finished.push(T::instantiate(coverage, std::mem::take(&mut self.current_items)));
            self.current_size = 0;
        }
    }
...
impl SingleSubBuilder {
    ...
    pub fn promote_to_multi_sub(self) -> MultipleSubBuilder {
        MultipleSubBuilder {
            items: self.items.into_iter().map(|(key, gid)| (key, vec![gid])).collect(),
        }
    }
    ...
    pub fn promote_to_ligature_sub(self) -> LigatureSubBuilder {
        ...
        for (from, to) in self.items.into_iter() {
            items.entry(from).or_insert(Vec::new()).push((Vec::new(), to));
        }
...
/// A builder for [`MultipleSubstFormat1`](super::MultipleSubstFormat1)
/// subtables.
#[derive(Clone, Debug, Default)]
...
/// A builder for [`AlternateSubstFormat1`](super::AlternateSubstFormat1)
/// subtables
#[derive(Clone, Debug, Default)]
...
impl AlternateSubBuilder {
    ...
    pub fn iter_pairs(&self) -> impl Iterator<Item = (GlyphId16, GlyphId16)> + '_ {
        self.items.iter().flat_map(|(target, alt)| alt.iter().map(|alt| (*target, *alt)))
    }
}
...
impl Builder for AlternateSubBuilder {
    ...
    fn build(self, _: &mut VariationStoreBuilder) -> Self::Output {
        let coverage = self.items.keys().copied().collect();
        let seq_tables = self.items.into_values().map(super::AlternateSet::new).collect();
        vec![super::AlternateSubstFormat1::new(coverage, seq_tables)]
    }
}

/// A builder for [`LigatureSubstFormat1`](super::LigatureSubstFormat1)
/// subtables.
#[derive(Clone, Debug, Default)]
...
impl LigatureSubBuilder {
    ...
    pub fn insert(&mut self, target: Vec<GlyphId16>, replacement: GlyphId16) {
        ...
        // skip duplicates
        if !entry.iter().any(|existing| existing.0 == rest && existing.1 == replacement) {
    ...
    pub fn can_add(&self, target: &[GlyphId16], replacement: GlyphId16) -> bool {
        ...
        match self.items.get(first) {
            Some(ligs) => !ligs.iter().any(|(seq, target)| seq == rest && *target != replacement),
            None => true,
        }
    }
