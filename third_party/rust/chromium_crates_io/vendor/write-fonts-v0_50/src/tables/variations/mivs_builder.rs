...
impl MultiItemVariationStoreBuilder {
    ...
    /// # Arguments
    ///
    /// * `deltas` - Vec of (SparseRegion, delta_tuple) pairs. Each delta_tuple
    ...
    pub fn add_deltas<T: Into<i32>>(
        ...
        let tuple_len = deltas.iter().map(|(_, tuple)| tuple.len()).next().unwrap_or(0);
        ...
        for (region, tuple) in deltas {
            assert_eq!(tuple.len(), tuple_len, "all delta tuples must have the same length");
            if region.is_empty() {
                continue;
            }
    ...
    pub fn build(self) -> (MultiItemVariationStore, MultiVariationIndexRemapping) {
        ...
        for (delta_set, temp_id) in &self.delta_sets {
            ...
            var_data_groups.entry(region_indices).or_default().push((delta_set, *temp_id));
        }
    ...
    fn build_region_list(&self) -> SparseVariationRegionList {
        ...
        let sparse_regions: Vec<SparseVariationRegion> =
            regions.into_iter().map(|(region, _)| region.to_sparse_variation_region()).collect();
    ...
    fn flatten_deltas(&self, delta_set: &MultiDeltaSet, region_indices: &[u16]) -> Vec<i32> {
        ...
        let delta_map: HashMap<u16, &Vec<i32>> =
            delta_set.deltas.iter().map(|(idx, tuple)| (*idx, tuple)).collect();
...
mod tests {
    ...
    fn deduplication() {
        ...
        let id1 = builder.add_deltas(vec![(region.clone(), vec![10, 20])]).unwrap();
    ...
    fn multiple_regions() {
        ...
        let temp_id =
            builder.add_deltas(vec![(region1, vec![10, 20]), (region2, vec![30, 40])]).unwrap();
