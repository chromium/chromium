...
mod tests {
    ...
    fn read_metric_delta(mvar: &read_mvar::Mvar, tag: &[u8; 4], coords: &[f32]) -> f64 {
        let coords = coords.iter().map(|c| F2Dot14::from_f32(*c)).collect::<Vec<_>>();
    ...
    fn assert_value_record(actual: &read_mvar::ValueRecord, expected: ValueRecord) {
        ...
        assert_eq!(actual.delta_set_outer_index(), expected.delta_set_outer_index);
        assert_eq!(actual.delta_set_inner_index(), expected.delta_set_inner_index);
    }
    ...
    fn simple_smoke_test() {
        ...
        assert_value_record(&read.value_records()[0], ValueRecord::new(Tag::new(b"hasc"), 0, 1));
        ...
        assert_value_record(&read.value_records()[1], ValueRecord::new(Tag::new(b"hdsc"), 0, 0));
