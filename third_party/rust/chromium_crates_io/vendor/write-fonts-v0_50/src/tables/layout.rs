...
impl<T: LookupSubtable + FontWrite> FontWrite for Lookup<T> {
    fn write_into(&self, writer: &mut TableWriter) {
        ...
        u16::try_from(self.subtables.len()).unwrap().write_into(writer);
...
impl Lookup<SequenceContext> {
    /// Convert this untyped SequenceContext into its GSUB or GPOS specific
    /// version
    pub fn into_concrete<T: From<SequenceContext>>(self) -> Lookup<T> {
        let Lookup { lookup_flag, subtables, mark_filtering_set } = self;
        let subtables = subtables
            .into_iter()
            .map(|offset| OffsetMarker::new(offset.into_inner().into()))
            .collect();
        Lookup { lookup_flag, subtables, mark_filtering_set }
    }
}
...
///
/// This allows us to attach the numerical lookup type to the appropriate
/// concrete types, so that we can write it as needed without passing it around.
...
impl ClassDefFormat1 {
    fn iter(&self) -> impl Iterator<Item = (GlyphId16, u16)> + '_ {
        self.class_value_array.iter().enumerate().map(|(i, cls)| {
            (GlyphId16::new(self.start_glyph_id.to_u16().saturating_add(i as u16)), *cls)
        })
    }
}
...
impl ClassDef {
    ...
    /// Returns `true` if no glyphs are explicitly assigned to a class in this
    /// table
...
impl CoverageFormat2 {
    fn iter(&self) -> impl Iterator<Item = GlyphId16> + '_ {
        self.range_records.iter().flat_map(|rcd| iter_gids(rcd.start_glyph_id, rcd.end_glyph_id))
    }
    ...
    fn len(&self) -> usize {
        self.range_records
            .iter()
            .map(|rcd| {
                rcd.end_glyph_id.to_u16().saturating_sub(rcd.start_glyph_id.to_u16()) as usize + 1
            })
            .sum()
    }
}
...
impl RangeRecord {
    ...
    pub fn iter_for_glyphs(glyphs: &[GlyphId16]) -> impl Iterator<Item = RangeRecord> + '_ {
        ...
        std::iter::from_fn(move || {
            ...
            cur_range.take().map(|(start_glyph_id, end_glyph_id)| RangeRecord {
                start_glyph_id,
                end_glyph_id,
                start_coverage_index: len,
            })
...
impl Device {
    pub fn new(start_size: u16, end_size: u16, values: &[i8]) -> Self {
        ...
        Device { start_size, end_size, delta_format, delta_value }
    }
}
...
fn encode_delta(format: DeltaFormat, values: &[i8]) -> Vec<u16> {
    ...
    values.chunks(chunk_size).map(|chunk| encode_chunk(chunk, mask, bits)).collect()
}
