...
impl Varc {
    pub fn new_from_composite_glyphs(
        ...
        let var_composite_glyphs = Index2::from_items(
            composites.iter().map(|x| x.to_bytes(&axis_indices_list, &remappings)).collect(),
        );
    ...
    fn create_axis_indices_list(composites: &Vec<VarCompositeGlyph>) -> Vec<Vec<u16>> {
        ...
        let mut axis_indices_list: Vec<(Vec<u16>, u32)> =
            axis_indices_counter.into_iter().collect::<Vec<(Vec<u16>, u32)>>();
        axis_indices_list.sort_by_key(|(_, count)| std::cmp::Reverse(*count));

        axis_indices_list.into_iter().map(|(indices, _)| indices).collect()
    }
}
...
impl FontWrite for RawVarComponent {
    ...
    fn write_into(&self, writer: &mut TableWriter) {
        ...
        if let Some(axis_values) = &self.axis_values {
            let converted_axis_values =
                axis_values.iter().map(|v| F2Dot14::from_f32(*v).to_bits() as i32).collect();
...
mod tests {
    ...
    fn test_varc_with_axis_values_and_transform() {
        ...
        assert_eq!(varc_roundtrip.axis_indices(0).unwrap().iter().collect::<Vec<_>>(), vec![0, 1],);
        let composite = varc_roundtrip.glyph(0).unwrap();
        assert_eq!(composite.components().count(), 1);
        let mut components = composite.components();
        let component = components.next().unwrap().unwrap();
        assert!(component.flags().contains(VarcFlags::RESET_UNSPECIFIED_AXES));
        ...
        let axis_values_vec: Vec<f32> =
            axis_values.iter().map(|b| F2Dot14::from_bits(b as i16).to_f32()).collect();
