    ...
    pub fn format(&self) -> ValueFormat {
        ...
        macro_rules! flag_if_true {
            ($field:expr, $flag:expr) => {
                $field.is_some().then(|| $flag).unwrap_or(ValueFormat::empty())
            };
        }
...
impl FontWrite for ValueRecord {
    fn write_into(&self, writer: &mut TableWriter) {
        ...
        write_field!(self.x_placement_device, ValueFormat::X_PLACEMENT_DEVICE, off);
        write_field!(self.y_placement_device, ValueFormat::Y_PLACEMENT_DEVICE, off);
...
impl std::fmt::Debug for ValueRecord {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        self.x_placement_device.as_ref().map(|x| f.field("x_placement_device", &x));
        self.y_placement_device.as_ref().map(|y| f.field("y_placement_device", &y));
        self.x_advance_device.as_ref().map(|x| f.field("x_advance_device", &x));
        self.y_advance_device.as_ref().map(|y| f.field("y_advance_device", &y));
        f.finish()
    }
}
...
mod tests {
    ...
    fn serialize_explicit_value_record() {
        let mut my_record = ValueRecord { x_advance: Some(5), ..Default::default() };
    ...
    fn roundtrip_preserves_format() {
        ...
        let record = ValueRecord::new().with_x_advance(5).with_explicit_value_format(format);
    ...
    fn roundtrip_in_table() {
        ...
        let a_table = SinglePos::format_2(
            ...
            vec![
                ValueRecord::new().with_explicit_value_format(format).with_x_advance(5),
                ValueRecord::new()
                    .with_x_advance(7)
