...
impl SimpleGlyph {
    ...
    /// calling.
    ///
    /// Returns an error if the input path is malformed; that is, if it is
    /// empty, contains cubic segments, or does not begin with a 'move'
    /// instruction.
    ///
    /// **Context**
    ///
    /// * In the glyf table simple (contour based) glyph paths implicitly close
    ///   when rendering.
    ...
    ///    * This is not the same as ending with some other drawing command
    ///      whose endpoint is the contour startpoint
    /// * In FontTools endPath says I'm done with this subpath, [BezPath] has no
    ///   endPath.
    ///
    ...
    pub fn recompute_bounding_box(&mut self) {
        let mut points = self.contours.iter().flat_map(|c| c.iter()).map(|p| (p.x, p.y));
...
impl FromObjRef<read_fonts::tables::glyf::SimpleGlyph<'_>> for SimpleGlyph {
    fn from_obj_ref(
        ...
        Self { bbox, contours, instructions: from.instructions().to_owned() }
    }
}
...
impl FontWrite for RepeatableFlag {
    fn write_into(&self, writer: &mut crate::TableWriter) {
        debug_assert_eq!(self.flag.contains(SimpleGlyphFlags::REPEAT_FLAG), self.repeat > 0);
...
impl RepeatableFlag {
    /// given an iterator over raw flags, return an iterator over flags + repeat
    /// values
...
///
/// Similar to read_fonts::tables::glyf::CurvePoint, but uses kurbo::Point
/// directly thus it does not require (x, y) coordinates to be rounded to
/// integers.
...
impl InterpolatableContourBuilder {
    /// Create new set of interpolatable contours beginning at the provided
    /// points
    fn new(move_pts: &[kurbo::Point]) -> Self {
        assert!(!move_pts.is_empty());
        Self(move_pts.iter().map(|pt| vec![ContourPoint::on_curve(*pt)]).collect())
    }
    ...
    fn is_implicit_on_curve(&self, idx: usize) -> bool {
        self.0.iter().all(|points| is_implicit_on_curve(points, idx))
    }

    /// Build the contours, dropping any on-curve points that can be implied in
    /// all contours
    fn build(self) -> Vec<Contour> {
        ...
        for point_idx in (0..num_points).filter(|point_idx| !self.is_implicit_on_curve(*point_idx))
        {
            for (contour_idx, contour) in contours.iter_mut().enumerate() {
                contour.0.push(CurvePoint::from(self.0[contour_idx][point_idx]));
            }
        }
        contours
    }
...
///
/// We check both before and after rounding float coordinates to integer to
/// avoid false negatives due to rounding.
#[inline]
...
fn simple_glyphs_from_kurbo(paths: &[BezPath]) -> Result<Vec<SimpleGlyph>, MalformedPath> {
    // check that all paths have the same number of elements so we can zip them
    // together
    ...
    for (i, elements) in path_iters.enumerate() {
        ...
        // elements is never empty (if it were, MultiZip would have stopped), hence the
        // unwrap
        ...
        match first_el {
            ...
            kurbo::PathEl::LineTo(_) => {
                ...
                current.as_mut().ok_or(MalformedPath::MissingMove)?.line_to(&pts)
            }
            kurbo::PathEl::QuadTo(_, _) => {
                ...
                current.as_mut().ok_or(MalformedPath::MissingMove)?.quad_to(&quad_pts)
            }
            ...
            kurbo::PathEl::ClosePath => {
                ...
                // FontTools has an else case to support UFO glif's choice to not include 'move'
                // for closed paths that does not apply here.
...
mod tests {
    ...
    // For `indexToLocFormat == 0` (short version), offset divided by 2 is stored,
    // so add a padding byte if the length is not even to ensure our computed
    // bytes match those of our test glyphs.
    ...
    fn round_trip_multi_contour() {
        ...
        let our_points = ours.points().collect::<Vec<_>>();
        let their_points = orig.points().collect::<Vec<_>>();
        dbg!(SimpleGlyphFlags::from_bits(1), SimpleGlyphFlags::from_bits(9));
        assert_eq!(our_points, their_points);
        assert_eq!(orig.glyph_data(), ours.glyph_data());
        assert_eq!(orig_bytes.len(), bytes.len());
        assert_eq!(orig_bytes.as_ref(), bytes);
    ...
    fn simple_glyph_open_path() {
        ...
        // even if the last point is on top of the first, the path was not deliberately
        // closed hence there is going to be an extra point (6, not 5 in total)
        path.line_to((20., -100.));
    ...
    fn compile_repeatable_flags() {
        ...
        let glyph = SimpleGlyph::from_bezpath(&path).unwrap();
        let flags = glyph.compute_point_deltas().map(|x| x.0).collect::<Vec<_>>();
    ...
    fn simple_glyphs_from_kurbo_inconsistent_path_elements() {
        ...
        assert_eq!(format!("{:?}", err), "InconsistentPathElements(1, [\"L\", \"Q\"])");
    }

    /// Create a number of interpolatable BezPaths with the given element types.
    /// The paths will be identical except for the point coordinates of the
    /// elements, which will be (0.0, 0.0), (1.0, 1.0), (2.0, 2.0), etc. for
    /// each subsequent element. If `last_pt_equal_move` is true, the last
    /// point of each sub-path will be equal to the first (M) point of that
    /// sub-path. E.g.:
    /// ```
    ...
    fn simple_glyphs_from_kurbo_2_quads_closed() {
        ...
        // where the respective off-curves are placed at equal distance from the
        // on-curve point joining them; the paths are closed and the last quad
        // point is the same as the move point.
    ...
    fn simple_glyphs_from_kurbo_multiple_contours_mixed_segments() {
        // four paths, each containing two sub-paths, with a mix of line and quad
        // segments
    ...
    fn simple_glyphs_from_kurbo_keep_on_curve_unless_impliable_for_all() {
        ...
        // However, when making interpolatable SimpleGlyphs from both paths, the
        // on-curve can no longer be implied/dropped (for it is not impliable in
        // the second path).
        assert_contour_points(
            &glyphs[0],
            vec![vec![
    ...
    fn simple_glyphs_from_kurbo_3_lines_open_duplicate_last_pt() {
        ...
        // point overlapping the first point; the last point gets duplicated (and not
        // fused). The special treatment for the last point is only applied to
        // Z-ending contours, not to open contours.
    ...
    fn repeatable_flags_basic() {
        ...
        let expected =
            flags.into_iter().map(|flag| RepeatableFlag { flag, repeat: 0 }).collect::<Vec<_>>();
    ...
    fn repeatable_flags_repeats() {
        ...
        assert_eq!(
            repeatable[2],
            RepeatableFlag { flag: SimpleGlyphFlags::Y_SHORT_VECTOR, repeat: 0 }
        )
    }
