...
impl FromObjRef<read_fonts::tables::glyf::CompositeGlyph<'_>> for CompositeGlyph {
    fn from_obj_ref(
        ...
        Self {
            bbox,
            components,
            _instructions: from.instructions().map(|v| v.to_owned()).unwrap_or_default(),
        }
    }
}
...
impl Component {
    ...
    pub fn new(
        ...
        Component { glyph, anchor, flags: flags.into(), transform }
    }
...
/// An error that occurs if a `CompositeGlyph` is constructed with no
/// components.
...
impl CompositeGlyph {
    ...
    pub fn new(component: Component, bbox: impl Into<Bbox>) -> Self {
        Self { bbox: bbox.into(), components: vec![component], _instructions: Default::default() }
    }
    ...
    /// Construct a `CompositeGlyph` from an iterator of `Component` and
    /// `Bbox`es.
    ///
    /// This returns an error if the iterator is empty; a CompositeGlyph must
    /// always contain at least one component.
...
impl FontWrite for CompositeGlyph {
    fn write_into(&self, writer: &mut crate::TableWriter) {
        ...
        let (last, rest) =
            self.components.split_last().expect("empty composites checked in validation");
        for comp in rest {
...
impl FontWrite for Anchor {
    fn write_into(&self, writer: &mut crate::TableWriter) {
        let two_bytes = self.compute_flags().contains(CompositeGlyphFlags::ARG_1_AND_2_ARE_WORDS);
        match self {
