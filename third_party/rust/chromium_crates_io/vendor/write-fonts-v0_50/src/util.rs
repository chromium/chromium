...
impl FloatComparator {
    /// Create a new FloatComparator with the specified relative and absolute
    /// tolerances.
    ...
    pub fn isclose(self, a: f64, b: f64) -> bool {
        ...
        //   diff <= f64::max(self.rel_tol * f64::max(f64::abs(a), f64::abs(b)),
        // self.abs_tol) However below I use || instead of max(), since the
        // logic works out the same and should be a bit faster. In the PEP it's
        // referred to as Boost's 'weak' formulation.
        let diff = (a - b).abs();
