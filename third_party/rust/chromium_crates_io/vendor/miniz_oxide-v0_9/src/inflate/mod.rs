...
pub enum TINFLStatus {
    /// More input data was expected, but the caller indicated that there was no
    /// more data, so the input stream is likely truncated.
    ///
    ...
    /// decompression.  By setting that flag, you indicate more input exists but
    /// is not provided, and so reaching the end of the input data without
    /// finding the end of the compressed stream would instead return a
    /// [`NeedsMoreInput`][Self::NeedsMoreInput] status.
    ...
    /// The decompression went fine, but the adler32 checksum did not match the
    /// one provided in the header.
    ...
    ///
    /// This occurs when there's no more consumable input, but the end of the
    /// stream hasn't been reached, and you have supplied the
    /// [`TINFL_FLAG_HAS_MORE_INPUT`][core::inflate_flags::TINFL_FLAG_HAS_MORE_INPUT] flag to the
    /// decompressor.  Had you not supplied that flag (which would mean you were
    /// asserting that you believed all the data was available) you would
    /// have gotten a
    ...
    ///
    /// At this point, you can suspend decompression and later resume with a new
    /// `DecompressorOxide`. The only state that must be preserved is
    /// [`DecompressorOxide::block_boundary_state()`], plus the last 32KiB
    /// of the output buffer (or less if you know the stream was compressed with
...
///
/// NOTE: This function will not bound the output, so if the output is large
/// enough it can result in an out of memory error. It is therefore suggested to
/// not use this for anything other than test programs, use the functions with a
/// specified limit, or ideally streaming decompression via the [flate2](https://github.com/alexcrichton/flate2-rs) library instead.
///
/// Returns a [`Result`] containing the [`Vec`] of decompressed data on success,
/// and a [struct][DecompressError] containing the status and so far
/// decompressed data if any on failure.
#[inline]
#[cfg(feature = "with-alloc")]
...
/// Decompress the deflate-encoded data in `input` to a vector.
///
/// The vector is grown to at most `max_size` bytes; if the data does not fit in
/// that size, the error [struct][DecompressError] will contain the status
/// [`TINFLStatus::HasMoreOutput`] and the data that was decompressed on
/// failure.
///
/// As this function tries to decompress everything in one go, it's not ideal
/// for general use outside of tests or where the output size is expected to be
/// small. It is suggested to use streaming decompression via the [flate2](https://github.com/alexcrichton/flate2-rs) library instead.
///
/// Returns a [`Result`] containing the [`Vec`] of decompressed data on success,
/// and a [struct][DecompressError] on failure.
#[inline]
#[cfg(feature = "with-alloc")]
...
/// Decompress the deflate-encoded data (with a zlib wrapper) in `input` to a
/// vector. The vector is grown to at most `max_size` bytes; if the data does
/// not fit in that size, the error [struct][DecompressError] will contain the
/// status [`TINFLStatus::HasMoreOutput`] and the data that was decompressed on
/// failure.
///
/// As this function tries to decompress everything in one go, it's not ideal
/// for general use outside of tests or where the output size is expected to be
...
///
/// Returns [`Vec`] of decompressed data on success and the [error
/// struct][DecompressError] with details on failure.
#[cfg(feature = "with-alloc")]
...
/// * `ignore_adler32` if the adler32 checksum should be validated in case of of
///   zlib data. (Set this to true if it should be ignored)
///
/// # Examples
/// ```
/// use core::iter;
