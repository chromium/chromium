...
//!
//! There is no DeflateState as the needed state is contained in the compressor
//! struct itself.
...
/// progress was made due to lack of expected input data, or if called without
/// [`MZFlush::Finish`] after the compression was already finished.
///
...
mod test {
    ...
    fn test_state() {
        ...
        let decomp = result.map_err(|err| panic!("Decompress failed with {:}", err)).unwrap();
