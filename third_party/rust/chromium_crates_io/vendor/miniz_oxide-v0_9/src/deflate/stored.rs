...
/// Compression function for stored blocks, split out from the main compression
/// function.
pub(crate) fn compress_stored(d: &mut CompressorOxide, callback: &mut CallbackOxide) -> bool {
    ...
    // Right now there isn't any code for re-adding previous data to the hash chain
    // if compression is switched from stored mode to level 1 or higher after
    // first starting compression at level 0 which causes a slight deviation
    // from oroginal miniz behaviour but much faster stored compression since
    // original miniz does this by being super inefficient and adds data to hash
    // table when doing stored and rle compression. Ideally we would handle it
    // like zlib which keeps track of previous data and only adds to hash table
    // when switching over to actual compression but that would need a bunch of
    // more work.
    ...
    // TODO: This mostly copied from the existing miniz code that was part of the
    // main compression function but could be much simplified and optimized
    // further to a simple copy.
