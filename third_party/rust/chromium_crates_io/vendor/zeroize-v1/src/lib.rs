...
//! in doing so they love to "optimize away" unnecessary zeroing calls. There
//! are many documented "tricks" to attempt to avoid these optimizations and
//! ensure that a zeroing routine is performed reliably.
//!
...
//!
//! In the future, we reserve the right to change MSRV (i.e. MSRV is
//! out-of-scope for this crate's SemVer guarantees), however when we do it will
...
//! impl'd for `Vec<T>` for the above types as well as `String`, where it
//! provides [`Vec::clear`] / [`String::clear`]-like behavior (truncating to
//! zero-length) but ensures the backing memory is securely zeroed with some
//! caveats.
//!
//! With the `std` feature enabled (which it is **not** by default), [`Zeroize`]
//! is also implemented for [`CString`]. After calling `zeroize()` on a
//! `CString`, its internal buffer will contain exactly one nul byte. The
//! backing memory is zeroed by converting it to a `Vec<u8>` and back into a
//! `CString`. (NOTE: see "Stack/Heap Zeroing Notes" for important
...
//! access). In as much as there may be merit to these approaches, there are
//! also many other crates that already implement more sophisticated memory
//! protections. Such protections are explicitly out-of-scope for this crate.
//!
//! Zeroing memory is [good cryptographic hygiene] and this crate seeks to
//! promote it in the most unobtrusive manner possible. This includes omitting
//! complex `unsafe` memory protection systems and just trying to make the best
//! memory zeroing crate available.
//!
...
/// Marker trait signifying that this type will [`Zeroize::zeroize`] itself on
/// [`Drop`].
    ...
    Z: Zeroize,
{
    fn zeroize(&mut self) {
        if let Some(value) = self {
            ...
            // Ensures self is None and that the value was dropped. Without the take, the
            // drop of the (zeroized) value isn't called, which might lead to a
            // leak or other unexpected behavior. For example, if this were
            // Option<Vec<T>>, the above call to zeroize would not free the
            // allocated memory, but the the `take` call will.
            self.take();
        }

        // Ensure that if the `Option` were previously `Some` but a value was
        // copied/moved out that the remaining space in the `Option` is
        // zeroized.
        //
        // Safety:
        //
        ...
        // Ensures self is overwritten with the `None` bit pattern. volatile_write can't
        // be used because Option<Z> is not copy.
        //
        // Safety:
        //
        // self is safe to replace with `None`, which the take() call above should have
        // already done semantically. Any value which needed to be dropped will have
        // been done so by take().
...
impl<Z> Zeroize for [MaybeUninit<Z>] {
    fn zeroize(&mut self) {
        ...
        // and it is backed by a single allocated object for at least `self.len() *
        // size_pf::<Z>()` bytes. and 0 is a valid value for `MaybeUninit<Z>`
    ...
    Z: DefaultIsZeroes,
{
    fn zeroize(&mut self) {
        ...
        // Safety:
        //
        // This is safe, because the slice is well aligned and is backed by a single
        // allocated object for at least `self.len()` elements of type `Z`.
        // `self.len()` is also not larger than an `isize`, because of the assertion
        // above. The memory of the slice should not wrap around the address
        // space.
...
/// [`PhantomData`] is always zero sized so provide a [`Zeroize`]
/// implementation.
impl<Z> Zeroize for PhantomData<Z> {
    fn zeroize(&mut self) {}
}

/// [`PhantomData` is always zero sized so provide a ZeroizeOnDrop
/// implementation.
...
where
    Z: Zeroize,
{
    /// Unlike `Vec`, `Box<[Z]>` cannot reallocate, so we can be sure that we
    /// are not leaving values on the heap.
    fn zeroize(&mut self) {
        self.iter_mut().zeroize();
    }
}
...
impl Zeroize for CString {
    fn zeroize(&mut self) {
        ...
        // Unfortunately this results in an allocation for a Box::new(&[0]) as CString
        // must contain a trailing zero byte
        let this = mem::take(self);

        // - CString::into_bytes_with_nul calls ::into_vec which takes ownership of the
        //   heap pointer
        // as a Vec<u8>
...
///
/// Safety:
/// The memory pointed to by `dst` must be a single allocated object that is
/// valid for `count` contiguous elements of `T`.
/// `count` must not be larger than an `isize`.
/// `dst` being offset by `size_of::<T> * count` bytes must not wrap around the
/// address space. Also `dst` must be properly aligned.
#[inline(always)]
unsafe fn volatile_set<T: Copy + Sized>(dst: *mut T, src: T, count: usize) {
    ...
    for i in 0..count {
        // Safety:
        //
        // This is safe because there is room for at least `count` objects of type `T`
        // in the allocation pointed to by `dst`, because `count <= isize::MAX`
        // and because `dst.add(count)` must not wrap around the address space.
        let ptr = dst.add(i);

        // Safety:
        //
        // This is safe, because the pointer is valid and because `dst` is well aligned
        // for `T` and `ptr` is an offset of `dst` by a multiple of
        // `size_of::<T>()` bytes.
        ptr::write_volatile(ptr, src);
    }
}

/// Zeroizes a flat type/struct. Only zeroizes the values that it owns, and it
/// does not work on dynamically sized values or trait objects. It would be
/// inefficient to use this function on a type that already implements
/// `ZeroizeOnDrop`.
///
/// # Safety
/// - The type must not contain references to outside data or dynamically sized
///   data, such as `Vec<T>` or `String`.
/// - Values stored in the type must not have `Drop` impls.
/// - This function can invalidate the type if it is used after this function is
///   called on it. It is advisable to call this function only in `impl Drop`.
/// - The bit pattern of all zeroes must be valid for the data being zeroized.
///   This may not be true for enums and pointers.
///
...
/// - Smart pointers and collections: `Arc<T>`, `Box<T>`, `Vec<T>`, `HashMap<K,
///   V>`, `String`, etc.
///
/// # Examples
...
pub unsafe fn zeroize_flat_type<F: Sized>(data: *mut F) {
    ...
    // Safety:
    //
    // This is safe because `size_of<T>()` returns the exact size of the object in
    // memory, and `data_ptr` points directly to the first byte of the data.
