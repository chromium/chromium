...
if (not os.path.isfile("../encoding/encodings.json")) or (
        not os.path.isfile("../encoding/indexes.json")):
    sys.stderr.write(
        "This script needs a clone of https://github.com/whatwg/encoding/ (preferably at revision 1d519bf8e5555cef64cf3a712485f41cd1a6a990 ) next to the encoding_rs directory.\n"
    )
    sys.exit(-1)

if not os.path.isfile("../encoding_c/src/lib.rs"):
    sys.stderr.write(
        "This script also writes the generated parts of the encoding_c crate and needs a clone of https://github.com/hsivonen/encoding_c next to the encoding_rs directory.\n"
    )
    sys.exit(-1)
...
def to_camel_name(name):
    ...
    return name.title().replace(u"X-", u"").replace(u"-",
                                                    u"").replace(u"_", u"")
...
start_of_longest_run_in_single_byte = {
    "IBM866":
    96,  # 0 would be longest, but we don't want to start in the first quadrant
    "windows-874": 33,
    "windows-1250": 92,
    "windows-1251": 64,
    "windows-1252": 32,
...
for name in preferred:
    ...
    if is_single_byte(name):
        ...
        variant = "SingleByte(&data::SINGLE_BYTE_DATA.%s, 0x%04X, %d, %d)" % (
            to_snake_name(u"iso-8859-8" if name == u"ISO-8859-8-I" else name),
            run_bmp_offset, run_byte_offset, run_length)
    else:
...
''' % (to_dom_name(name), to_constant_name(name), to_constant_name(name),
       to_dom_name(name), variant, to_dom_name(name), doctext,
       to_constant_name(name), to_constant_name(name)))
...
static_u16_table_from_indexable("BIG5_LEVEL1_HANZI_CODE_POINTS",
                                level1_hanzi_pairs, 0, "big5-hanzi-encode")
static_u8_pair_table_from_indexable("BIG5_LEVEL1_HANZI_BYTES",
                                    level1_hanzi_pairs, 1, "big5-hanzi-encode")
    ...
    for column in xrange(157):
        ...
        if code_point and code_point >= 0x4E00 and code_point <= 0x9FCB:
            ...
            if code_point == 0x5341 or code_point == 0x5345 or not big5_unified_ideograph_bytes[
                    unified_offset]:
                big5_unified_ideograph_bytes[unified_offset] = (unified_lead,
                                                                unified_trail)

static_u8_pair_table("BIG5_UNIFIED_IDEOGRAPH_BYTES",
                     big5_unified_ideograph_bytes, "fast-big5-hanzi-encode")
...
static_u16_table_from_indexable("JIS0208_LEVEL1_KANJI_CODE_POINTS",
                                level1_kanji_pairs, 0, "kanji-encode")
static_u8_pair_table_from_indexable("JIS0208_LEVEL1_KANJI_SHIFT_JIS_BYTES",
                                    level1_kanji_pairs, 1, "kanji-encode")
...
for row in xrange(0x20):
    for column in xrange(190):
        i = column + (row * 190)
        # Skip the gaps
        if (column >= 0x1A and column < 0x20) or (column >= 0x3A
                                                  and column < 0x40):
            continue
        code_point = index[i]
...
static_u8_pair_table("KSX1001_UNIFIED_HANJA_BYTES", hanja_unified_bytes,
                     "fast-hanja-encode")
static_u8_pair_table("KSX1001_COMPATIBILITY_HANJA_BYTES",
                     hanja_compatibility_bytes, "fast-hanja-encode")
...
static_u16_table_from_indexable("GB2312_LEVEL1_HANZI_CODE_POINTS",
                                level1_hanzi_pairs, 0, "gb-hanzi-encode")
static_u8_pair_table_from_indexable("GB2312_LEVEL1_HANZI_BYTES",
                                    level1_hanzi_pairs, 1, "gb-hanzi-encode")
...
def write_variant_method(name, mut, arg_list, ret, variants, excludes, kind):
    ...
    for variant in variants:
        variant_file.write('''Variant%s::%s(ref ''' %
...
write_variant_method("max_utf16_buffer_length", False,
                     [("byte_length", "usize")], "Option<usize>",
...
write_variant_method("max_utf8_buffer_length", False,
                     [("byte_length", "usize")], "Option<usize>",
                     decoder_variants, [], "Decoder")
...
write_variant_method("decode_to_utf16_raw", True, [("src", "&[u8]"),
                                                   ("dst", "&mut [u16]"),
                                                   ("last", "bool")],
                     "(DecoderResult, usize, usize)", decoder_variants, [],
                     "Decoder")
...
write_variant_method("max_buffer_length_from_utf16_without_replacement", False,
                     [("u16_length", "usize")], "Option<usize>",
                     encoder_variants, [], "Encoder")
...
write_variant_method("encode_from_utf16_raw", True, [("src", "&[u16]"),
                                                     ("dst", "&mut [u8]"),
                                                     ("last", "bool")],
                     "(EncoderResult, usize, usize)", encoder_variants, [],
                     "Encoder")
...
(single_byte_rs_begin,
 single_byte_rs_end) = read_non_generated("src/single_byte.rs")
...
for name in preferred:
    ...
    if is_single_byte(name):
        single_byte_file.write("""
        decode_single_byte(%s, &data::SINGLE_BYTE_DATA.%s);""" %
                               (to_constant_name(name), to_snake_name(name)))
        idx += 1
        if idx == 2:
            single_byte_file.write("""
        if cfg!(miri) {
...
for name in preferred:
    ...
    if is_single_byte(name):
        single_byte_file.write("""
        encode_single_byte(%s, &data::SINGLE_BYTE_DATA.%s);""" %
                               (to_constant_name(name), to_snake_name(name)))
        idx += 1
        if idx == 2:
            single_byte_file.write("""
...
for pointer in range(0, 94 * 94):
    ...
    if code_point:
        ...
        if revised_pointer == 8644 or (revised_pointer >= 1207
                                       and revised_pointer < 1220):
...
for pointer in range(0, len(index)):
    code_point = 0xE000 - 8836 + pointer if pointer >= 8836 and pointer <= 10715 else index[
        pointer]
    if code_point:
