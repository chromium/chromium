...
struct Args {
    ...
    /// Comma separated list of Unicode codepoint values (base 10) to extend the
    /// font to cover.
    ///
    ...
    /// Comma separate list of open type layout feature tags to extend the font
    /// to cover.
    ///
    ...
    /// Comma separate list of design space ranges of the form tag@point or
    /// tag@start:end to extend the font to cover.
    ///
    ...
    /// The maximum number of simulated network round trips allowed during
    /// extension.
    ///
...
fn main() {
    ...
    let mut font_bytes = std::fs::read(&args.font).unwrap_or_else(|e| {
        panic!("Unable to read input font file ({}): {:?}", args.font.display(), e)
    });
    ...
    // For this a roundtrip is defined as an iteration of the below loop where at
    // least one new patch URL is needed.
    ...
    loop {
        ...
        for url in next_patches.urls() {
            patch_data.entry(url.clone()).or_insert_with_key(|key| {
                ...
                let patch_bytes = match std::fs::read(url_path.clone()) {
                    ...
                    Result::Err(e) => {
                        panic!("Unable to read patch file ({}): {:?}", url_path.display(), e)
                    }
                };
        ...
        font_bytes =
            next_patches.apply_next_patches(&mut patch_data).expect("Patch application failed.");
    }
    ...
    println!("    Glyphs added (count = {}): {:?}", loaded_glyphs.len(), loaded_glyphs);
}
...
impl std::fmt::Display for ParsingError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ParsingError::DesignSpaceParsingFailed { flag_value, message } => {
                write!(f, "Failed parsing design_space flag ({}): {}", flag_value, message)
            }
