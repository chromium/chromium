//! IFT Graph
//!
//! This command inspects an IFT font an generates a representation of the
//! extension graph formed by invalidating patches.
...
fn get_node_name(font: &FontRef<'_>) -> Result<String, ReadError> {
    let chars: BTreeSet<char> =
        font.charmap().mappings().map(|(cp, _)| char::from_u32(cp).unwrap()).collect();
...
fn to_graph(
    ...
    for patch in patches {
        if !matches!(patch.format(), PatchFormat::TableKeyed { .. }) {
            // This graph only considers invalidating (that is table keyed patches), so skip
            // all other types.
            continue;
        }
...

        {
            ...
            e.insert(Edge { name: NodeName(next_node_name), url: url_string });
        }
...
fn main() {
    ...
    for (key, values) in graph {
        ...
        let values: Vec<_> = if !args.include_patch_paths {
            ...
            values.into_iter().map(|edge| format!("{}|{}", edge.name.0, edge.url)).collect()
        };
