...
mod testdata {
        ...
        F: Fn(&mut Vec<u32>),
    {
        ...
        let maxp = Maxp { num_glyphs: 15, ..Default::default() };
        font_builder.add_table(&maxp).unwrap();

        let head =
            Head { index_to_loc_format: if short_loca { 0 } else { 1 }, ..Default::default() };
        ...
        let glyf = if !short_loca {
            // Since we want a long loca artificially inflate glyf table to ensure long
            // offsets are needed.
