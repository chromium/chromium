...
type PatchResult = (Vec<u8>, HashMap<PatchUrl, UrlStatus>, HashMap<PatchUrl, Vec<u8>>);
...
const ROBOTO: FontEntry = FontEntry { dir: "roboto", file: "Roboto-IFT.woff2" };

const NOTO_SC_HIGH: FontEntry =
    FontEntry { dir: "notosanshigh", file: "NotoSansSC-HighFreq-IFT.woff2" };
...
impl FontEntry {
    ...
    /// Applies all patches needed for `subset`, starting from `start_font` with
    /// existing `patch_data`. Returns `(final_font, updated_patch_data,
    /// newly_loaded_patches)`.
    ...
    /// Applies all patches needed for `chars`, starting from `start_font` with
    /// existing `patch_data`. Returns `(final_font, updated_patch_data,
    /// newly_loaded_patches)`.
    ...
    /// Pre-loads all patch bytes needed to fully extend the font for
...
/// Applies all pending patches to `current_font` for the given subset until no
/// more are needed. Returns the number of patch rounds applied.
...
fn bench_apply_patches_incremental(c: &mut Criterion) {
    ...
    let (_, _, patches_gc) = ROBOTO.apply_patches_for_chars(
        ...
        pd_after_viet,
        TEXT_LATIN.chars().chain(TEXT_VIETNAMESE.chars()).chain(TEXT_GREEK_CYRILLIC.chars()),
    );
    ...
    group.bench_with_input(
        ...
        &(font_after_latin.clone(), patches_latin.clone(), patches_viet.clone()),
