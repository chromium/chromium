...
/// Attributes to copy from the main enum's variants to the discriminant enum's
/// variants.
///
/// Attributes not in this list may be for other `proc_macro`s on the main enum,
/// and may cause compilation problems when copied across.
...
pub fn enum_discriminants_inner(ast: &DeriveInput) -> syn::Result<TokenStream> {
    ...
    for variant in variants {
        ...
        // Don't copy across the "strum" meta attribute. Only passthrough the
        // whitelisted attributes and proxy `#[strum_discriminants(...)]`
        // attributes
        ...
        for attr in &variant.attrs {
            ...
            if !ATTRIBUTES_TO_COPY.iter().any(|whitelisted| attr.path().is_ident(whitelisted)) {
                continue;
            }
            ...
            if attr.path().is_ident("strum_discriminants") {
                ...
                let passthrough_group =
                    ts.next().ok_or_else(|| strum_discriminants_passthrough_error(attr))?;
    ...
    let mut pass_through_attributes: Vec<_> =
        pass_through_attributes.into_iter().map(ToTokens::into_token_stream).collect();
    if !has_doc {
    ...
    // * For `!Copy` types, we `impl<'enum> From<&'enum TheEnum> for
    //   TheEnumDiscriminants`
    //
    // That way we ensure users are not able to pass a `Copy` type by reference.
    // However, the `#[derive(..)]` attributes are not in the parsed tokens, so
    // we are not able to check if a type is `Copy`, so we just implement both.
    //
    ...
    let from_fn_body = if variants.is_empty() {
        //this method on empty enum is impossible to be called. it is therefor left
        // empty
        quote! { unreachable!()}
    } else {
    ...
    // For now, only implement IntoDiscriminant if the user has not overriden the
    // visibility.
