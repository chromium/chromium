...
pub enum EnumMeta {
    SerializeAll { kw: kw::serialize_all, case_style: CaseStyle },
    AsciiCaseInsensitive(kw::ascii_case_insensitive),
    Crate { kw: kw::Crate, crate_module_path: Path },
    UsePhf(kw::use_phf),
    Prefix { kw: kw::prefix, prefix: LitStr },
    Suffix { kw: kw::suffix, suffix: LitStr },
    ParseErrTy { kw: kw::parse_err_ty, path: Path },
    ParseErrFn { kw: kw::parse_err_fn, path: Path },
...
impl Parse for EnumMeta {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        ...
        if lookahead.peek(kw::serialize_all) {
            ...
            Ok(EnumMeta::Crate { kw, crate_module_path })
...
impl Parse for EnumDiscriminantsMeta {
    fn parse(input: ParseStream) -> syn::Result<Self> {
        if input.peek(kw::derive) {
            ...
            Ok(EnumDiscriminantsMeta::Derive { _kw, paths: paths.into_iter().collect() })
...
pub enum VariantMeta {
    Message { kw: kw::message, value: LitStr },
    DetailedMessage { kw: kw::detailed_message, value: LitStr },
    Serialize { _kw: kw::serialize, value: LitStr },
    Documentation { value: LitStr },
    ToString { kw: kw::to_string, value: LitStr },
    Transparent(kw::transparent),
    Disabled(kw::disabled),
    Default(kw::default),
    DefaultWith { kw: kw::default_with, value: LitStr },
    AsciiCaseInsensitive { kw: kw::ascii_case_insensitive, value: bool },
    Props { _kw: kw::props, props: Vec<(LitStr, Lit)> },
}
...
impl VariantExt for Variant {
    fn get_metadata(&self) -> syn::Result<Vec<VariantMeta>> {
        ...
        self.attrs.iter().filter(|attr| attr.meta.path().is_ident("doc")).try_fold(
            result,
            |mut vec, attr| {
                ...
                {
                    vec.push(VariantMeta::Documentation { value: value.clone() })
                }
                Ok(vec)
            },
        )
