...
///
/// auto-derives `std::str::FromStr` on the enum. Each variant of the enum will
/// match on its own name. This can be overridden using
/// `serialize="DifferentName"` or `to_string="DifferentName"` on the attribute
/// as shown below. Multiple deserializations can be added to the same variant.
/// If the variant contains additional data, they will be set to their default
/// values upon deserialization.
///
/// The `default` attribute can be applied to a tuple variant with a single data
/// parameter. When a match isn't found, the given variant will be returned and
/// the input string will be captured in the parameter.
///
/// Note that the implementation of `FromStr` by default only matches on the
/// name of the variant. There is an option to match on different case
/// conversions through the `#[strum(serialize_all = "snake_case")]` type
/// attribute.
///
/// See the [Additional Attributes](https://docs.rs/strum/latest/strum/additional_attributes/index.html)
/// Section for more information on using this feature.
///
/// If you have a large enum, you may want to consider using the `use_phf`
/// attribute here. PHF (Perfect Hash Functions) use a hash lookup instead of a
/// linear search that may perform faster for large enums. Note: as with all
/// optimizations, you should test this for your specific usecase rather than
/// just assume it will be faster. With SIMD + pipelining, linear string search
/// (aka memcmp) can be very fast for enums with a surprisingly large number of
/// enum variants.
///
/// # Infallible Parsing
///
/// If the enum has a `#[strum(default)]` variant and no `parse_err_ty` is set,
/// parsing is infallible: `From<&str>` is derived instead of `TryFrom<&str>`,
/// which allows calling `MyEnum::from("string")` directly.
///
/// # Custom Error Types
///
/// The default error type is `strum::ParseError`. This can be overridden by
/// applying both the `parse_err_ty` and `parse_err_fn` attributes at the type
/// level. `parse_err_fn` should be a function that accepts an `&str` and
/// returns the type `parse_err_ty`. See [this test case](https://github.com/Peternator7/strum/blob/9db3c4dc9b6f585aeb9f5f15f9cc18b6cf4fd780/strum_tests/tests/from_str.rs#L233)
/// for an example. When `parse_err_ty` is set, `TryFrom<&str>` is always
/// derived, even if the enum has a `#[strum(default)]` variant.
///
...
/// Converts enum variants to `&'a str`, where `'a` is the lifetime of the input
/// enum reference.
///
/// Implements `AsRef<str>` on your enum using the same rules as
/// `Display` for determining what string is returned. The difference is that
/// `as_ref()` returns a `&str` instead of a `String` so you don't allocate any
/// additional memory with each call.
///
...
/// `VARIANTS` which is a `'static` slice of discriminant names.
///
/// Adds an `impl` block for the `enum` that adds a static `VARIANTS` array of
/// `&'static str` that are the discriminant names. This will respect the
/// `serialize_all` attribute on the `enum` (like `#[strum(serialize_all =
/// "snake_case")]`.
///
/// ```
...
///
/// Implements `strum::VariantArray` which adds an associated constant
/// `VARIANTS`. This constant contains an array with all the variants of the
/// enumerator.
///
/// This trait can only be autoderived if the enumerator is composed only of
/// unit-type variants, meaning that the variants must not have any data.
///
/// ```
...
///
/// Implements `From<YourEnum>` and `From<&'a YourEnum>` for `&'static str`.
/// This is useful for turning an enum variant into a static string.
/// The Rust `std` provides a blanket impl of the reverse direction - i.e. `impl
/// Into<&'static str> for YourEnum`.
///
/// ```
...
///
/// Deriving `Display` on an enum prints out the given enum. This enables you to
/// perform round trip style conversions from enum into string and back again
/// for unit style variants. `Display` choose which serialization to used based
/// on the following criteria:
///
/// 1. If there is a `to_string` property, this value will be used. There can
///    only be one per variant.
/// 2. Of the various `serialize` properties, the value with the longest length
///    is chosen. If that behavior isn't desired, you should use `to_string`.
/// 3. The name of the variant will be used if there are no `serialize` or
///    `to_string` attributes.
/// 4. If the enum has a `strum(prefix = "some_value_")`, every variant will
///    have that prefix prepended to the serialization.
/// 5. If the enum has a `strum(suffix = "_another_value")`, every variant will
///    have that suffix appended to the serialization.
/// 6. Enums with fields support string interpolation. Note this means the
///    variant will not "round trip" if you then deserialize the string.
///
///    ```rust
...
///
/// Iterate over the variants of an Enum. Any additional data on your variants
/// will be set to `Default::default()`. The macro implements [`strum::IntoEnumIterator`](https://docs.rs/strum/latest/strum/trait.IntoEnumIterator.html) on your enum and creates a new type called `YourEnumIter` that is the iterator object.
/// You cannot derive `EnumIter` on any type with a lifetime bound (`<'a>`)
/// because the iterator would surely create [unbounded lifetimes](https://doc.rust-lang.org/nightly/nomicon/unbounded-lifetimes.html).
///
/// ```
/// 
...
///
/// These methods will only be generated for tuple-style variants, not for named
/// or unit variants.
///
/// ```
...
/// Creates a new type that maps all the variants of an enum to another generic
/// value.
///
/// This macro only supports enums with unit type variants.A new type called
/// `YourEnumTable<T>`. Essentially, it's a wrapper `[T; YourEnum::Count]` where
/// gets/sets are infallible. Some important caveats to note:
///
/// * The size of `YourEnumTable<T>` increases with the number of variants, not
///   the number of values because it's always fully populated. This means it
///   may not be a good choice for sparsely populated maps.
///
/// * Lookups are basically constant time since it's functionally an array
///   index.
///
/// * Your variants cannot have associated data. You can use `EnumDiscriminants`
///   to generate an Enum with the same names to work around this.
///
/// # Stability
///
/// Several people expressed interest in a data structure like this and pushed
/// the PR through to completion, but the api seems incomplete, and I reserve
/// the right to deprecate it in the future if it becomes clear the design is
/// flawed.
///
/// # Example
/// ```rust
...
///
/// This macro adds a standalone function to obtain an enum variant by its
/// discriminant. The macro adds `from_repr(discriminant: usize) ->
/// Option<YourEnum>` as a standalone function on the enum. For variants with
/// additional data, the returned variant will use the `Default` trait to fill
/// the data. The discriminant follows the same rules as `rustc`. The first
/// discriminant is zero and each successive variant has a discriminant of one
/// greater than the previous variant, except where an explicit discriminant is
/// specified. The type of the discriminant will match the `repr` type if
/// it is specified.
///
/// When the macro is applied using rustc >= 1.46 and when there is no
/// additional data on any of the variants, the `from_repr` function is marked
/// `const`. rustc >= 1.46 is required to allow `match` statements in `const
/// fn`. The no additional data requirement is due to the inability to use
/// `Default::default()` in a `const fn`.
///
/// You cannot derive `FromRepr` on any type with a lifetime bound (`<'a>`)
/// because the function would surely create [unbounded lifetimes](https://doc.rust-lang.org/nightly/nomicon/unbounded-lifetimes.html).
///
/// ```
/// 
...
///
/// Encode strings into the enum itself. The `strum_macros::EmumMessage` macro
/// implements the `strum::EnumMessage` trait. `EnumMessage` looks for
/// `#[strum(message="...")]` attributes on your variants. You can also provided
/// a `detailed_message="..."` attribute to create a separate more detailed
/// message than the first.
///
/// `EnumMessage` also exposes the variants doc comments through
/// `get_documentation()`. This is useful in some scenarios, but `get_message`
/// should generally be preferred. Rust doc comments are intended for developer
/// facing documentation, not end user messaging.
///
/// ```
/// // You need to bring the trait into scope to use it
...
/// currently only supports adding additional string values. Other types of
/// literals are still experimental in the rustc compiler. The generated code
/// works by nesting match statements. The first match statement matches on the
/// type of the enum, and the inner match statement matches on the name of the
/// property requested. This design works well for enums with a small
/// number of variants and properties, but scales linearly with the number of
/// variants so may not be the best choice in all situations.
///
/// ```
/// 
/// use strum_macros;
...
///
/// Given an enum named `MyEnum`, generates another enum called
/// `MyEnumDiscriminants` with the same variants but without any data fields.
/// This is useful when you wish to determine the variant of an `enum` but one
/// or more of the variants contains a non-`Default` field. `From`
/// implementations are generated so that you can easily convert from `MyEnum`
/// to `MyEnumDiscriminants`.
///
/// By default, the generated enum has the following derives: `Clone, Copy,
/// Debug, PartialEq, Eq`. If your enum derives `Default` and has a `#[default]`
/// variant, that will also be copied onto the discriminant enum. You can add
/// additional derives using the
/// `#[strum_discriminants(derive(AdditionalDerive))]` attribute.
///
/// Note, the variant attributes passed to the discriminant enum are filtered to
/// avoid compilation errors due to the derives mismatches, thus only `#[doc]`,
/// `#[cfg]`, `#[allow]`, and `#[deny]` are passed through by default. If you
/// want to specify a custom attribute on the discriminant variant, wrap it with
/// `#[strum_discriminants(...)]` attribute.
///
/// ```
...
/// which adds a static property `COUNT` of type usize that holds the number of
/// variants.
///
/// ```
